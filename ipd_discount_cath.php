...
$query3 = mysqli_query($db,"select * from cath_charge where id='$updateid'");
...
//$hos_doc_dis1=$hos_doc_dis+$eqty22;
			...
			if($eqty22>0){
			//$strSQL = "update cath_charge set discount='$eqty22'";
            ...
            ('$dname','$eqty22','$proce','$refund_time','$user','$pmrn','$eid','$ot_id','$edate','Cathlab')";
  ...
  $ins_query3="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$pmrn','DR','617410','$date','$eqty22','IPD_CATH_DISCOUNT')";
    ...
    values ('$pmrn','CR','111999','$date','$eqty22','IPD_CATH_DISCOUNT')";
...
$query5555 = mysqli_query($db,"select SUM(discount) from doc_dis where pmrn='$pmrn' and eid='$eid' and location='Cathlab'");
...
$hos_doc_dis=$data['hos_doc_dis'];
      ...
      $sum_bill555=$data5555['SUM(discount)']+$hos_doc_dis;
      
  
  
      $strSQL11 = "update inpatient set hos_doc_dis='$sum_bill555' where pmrn='$pmrn' and eid='$eid'";
...
$count=1;
$sel_query="Select SUM(charge),sname,date1,plevel,id from cath_charge where pmrn= '$pmrn' and ieid='$eid' and c_status='' group by `sname` asc;";
...
<td align="center" colspan="1"><?php echo $count; ?></td>
      <td align="center"colspan="1"><?php echo $row["sname"]; ?></td>
      <td align="center"colspan="10"><?php echo $row["date1"]; ?></td>
	  <td align="center"colspan="2"><?php echo $row["plevel"]; ?></td>  
      
	      
	  <td align="center"colspan="3"><?php echo $row['SUM(charge)']; ?>
  <input type="text" name="total_sum" value="<?php echo $row['SUM(charge)'];?>" />
  </td>  
<?php
$id = $row["id"];
$dd=$row['sname'];
...
$query555 = mysqli_query($db,"select SUM(discount) from doc_dis where pmrn='$pmrn' and eid='$eid' and dname='$dd' and location='Cathlab'");
$data555= mysqli_fetch_assoc($query555);
$sum_bill55=$data555['SUM(discount)'];
?>	  
<td align="center"colspan="3"><input name="eqty1_<?= $id ?>" type="number" value="" max="<?php echo $row['SUM(charge)']-$sum_bill55;?>" id="eqty1" required ></td>                            
...
$count=1;
$sel_query="Select * from doc_dis where pmrn= '$pmrn' and eid='$eid' and location='Cathlab';";
