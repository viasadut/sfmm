...
<p align="center" class="style1">Pending Reports </p> 
...
<table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;">
    ...
    <tr>
      ...
      <th width="15%"><strong>Request BY</strong>
	  <th width="15%"><strong>Performing Consultant </strong>
...
$date= date('Y-m-d');
$date2= date('2022-04-08');
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from alltest where rstatus='RECEIVED' and status in('RECEIVED','SEEN') and date1>='$date2' and type in ('spd','spd1') and per_doc='$full' order by id desc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	  ...
	  $lid='O'.$row['id'];
	  if($row['report']!=''){echo'
	  <a target="_blank" href="ecg_pdf1_lid?lid='.$lid.'">Report</a>';
	  }
	  else{echo "Report Not Uploaded";}
	  ?>
	  ...
	  if($row['report']!=''){echo'
	  <a target="_blank" href="spd_edit_doc2?pmrn='.$row['pmrn'].'&eid='.$row['eid'].'&id='.$row['id'].'">Edit</a>';
	  }
	  else{echo "Report Not Uploaded";}
	  ?>
	 ...
	 <?php
$lid='O'.$row['id'];
 $query2 = "SELECT * from ecg_test where lid='$lid'"; 
...
?>
	  ...
	  <td align="center">
	  <a target='_blank' class="thumbnail fancybox" rel="ligthbox" href="spdpic/<?php echo $upload ?>">
                        
                        <div class='text-center'>
                            <small class='text-muted'><?php echo $upload ?></small>
                        </div> <!-- text-center / end -->
                    </a></td>
...
//$id =$_GET['id'];
//$count=1;
$sel_query="Select * from iinves where rstatus='RECEIVED' and status='RECEIVED' and report='' and ndate>'2022-04-08' and type in ('spd','spd1') and per_doc='$full' order by id desc;";
...
<td align="center"><?php echo $row["infusion"]; ?></td>
	  ...
	  if($row['report']!=''){echo'
	  <a target="_blank" href="spd_edit_doc2_in?pmrn='.$row['pmrn'].'&eid='.$row['eid'].'&id='.$row['id'].'">Edit</a>';
	  }
	  else{echo "Report Not Uploaded";}
	  ?>
...
//$id =$_GET['id'];
//$count=1;
$sel_query="Select * from einves where rstatus='RECEIVED' and status='RECEIVED' and report='' and ndate>'2022-04-08' and type in ('spd','spd1') and per_doc='$full' order by id desc;";
...
<td align="center"><?php echo $row["infusion"]; ?></td>
	  ...
	  if($row['report']!=''){echo'
	  <a target="_blank" href="spd_edit_doc2_ae?pmrn='.$row['pmrn'].'&eid='.$row['eid'].'&id='.$row['id'].'">Edit</a>';
	  }
	  else{echo "Report Not Uploaded";}
	  ?>
