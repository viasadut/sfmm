...
else if($ftype=='Corporate'){
  $cor_name=$corporate_name;
}
...
$receive_remarks=$_REQUEST['receive_remarks'];
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
			...
			$strSQL118 = "select COUNT(id) from inpatient where pmrn='$pmrn' and eid='$eid' and billno>'0'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery118 = mysqli_query($objConnect,$strSQL118);
			$result118 = mysqli_fetch_array($objQuery118);
			
			
			$strSQL188 = "select COUNT(billno) from pms_bill where pmrn='$pmrn' and eid='$eid' and dname='IPD'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery188 = mysqli_query($objConnect,$strSQL188);
			$result188 = mysqli_fetch_array($objQuery188);


if($result118['COUNT(id)']<=0 and $result188['COUNT(billno)']<=0){
...
$sql = "insert into pms_bill(pmrn,eid,location,amount,date,time,user,remarks,dname,s_no,p_mode,p_remarks,receive_amount, outstanding,cor_name) VALUES
('$pmrn', '$eid', '$ipd', '$payment', '$appdate', '$apptime', '$user', '$ipd', '$ipd','$mno', '$vehicle1','$due_remarks','$receive_amount','$outstanding','$cor_name')";
...
implant=?, extra=?, endo=?, opdpro=?, payment_status=?, dis_medi=?, emer_bill=?, cath_bill=?, msuite_bill=?,receive_amount=?,gtotal=?,service_charge=?, receive_remarks=? WHERE pmrn=? and eid=?");
$sh->execute([$last_id, $payment, $room_charge, $inves_charge, $disposable_charge, $doc_charge, $pharmacy_charge,
$ot_hos_charge, $ot_doc_charge, $ot_phar_charge, $implant, $extra, $endo, $opdpro, $payment_status, $dis_medi, $emer_all_bill, $cath_bill, $msuite_bill, $receive_amount, $gtotal, $service_charge, $receive_remarks, $pmrn, $eid]);
...
<h1 align="center"style="background-color:lightgreen;">PATIENT'S FINAL BILL SUMMARY 
...
$query198j_doc = "SELECT SUM(room) FROM otivisitendo where pmrn= '$pmrn' and ieid='$eid' and c_status='0'"; 
...
$payable=$test1c+$test1+$test1as+$test1a1+$test1al+$test1al_rad+$test1ai+$test1am+$test1c_bed+$test1c_doc+$test1c_dis+$test1c_medi+$test1c_amedi+$test1c_ainfu+$implant+$extra+$endo_summary+$opd_pro_summary+$test1_dis+$emer_all_bill-$test1al_rad_dis-$total_bed_dis-$data['hos1_dis']-$data['hos_doc_dis']-$data['advance']-$data['hos_doc_dis_ot'];
...
//$in_ipd_charge1=$test1c+$test1+$test1as+$test1a1+$test1al+$test1al_rad+$test1ai+$test1am+$bed_charge_new+$implant+$extra+$endo_summary+$opd_pro_summary+$opd_cath_summary+$opd_msuite_summary;
$in_ipd_charge2=$test1c+$test1+$test1as+$test1a1+$test1al+$test1al_rad+$test1ai+$test1am+$implant+$extra+$endo_summary+$opd_pro_summary+$opd_cath_summary+$opd_msuite_summary;
...
<td colspan="20" align="right" bgcolor="lightgreen">
<font size="6" color="#FF0000"><strong>Remarks

<textarea name="receive_remarks" style="text-transform:uppercase;text-align:right;" required></textarea>
...
<tr>
<td colspan="20" style="text-align:right; font-size:18px; font-weight:bold;" hidden>
...
<?php if($data['payment_status']!='PAID' and $data['disstatus']=='Discharge Requested'){echo"
...
<tr>
<td>
<form action="ipd_discount_lab1"method="post" name="dis_lab">
...
<form action="ipd_discount_lab_new1"method="post" name="dis_lab">
<input type="hidden" name="pmrn" value="<?php echo $pmrn5;?>">
<input type="hidden" name="eid" value="<?php echo $eid5;?>">
...
<form action="ipd_discount_ot1"method="post" name="dis_lab">
<input type="hidden" name="pmrn" value="<?php echo $pmrn5;?>">
<input type="hidden" name="eid" value="<?php echo $eid5;?>">
...
<form action="ipd_discount_cath1"method="post" name="dis_cath">
<input type="hidden" name="pmrn" value="<?php echo $pmrn5;?>">
<input type="hidden" name="eid" value="<?php echo $eid5;?>">
 ...
 <div id="add_data_Modal" class="modal fade">  
      <div class="modal-dialog">  
           <div class="modal-content">  
              ...
              <select name="pa_type" id="dname1" value="" required class="country" style="font-size:20px; font-weight:bold;color:green;width:570px;">
...
<?php
	...
	{
		?>
        <option value="<?php echo $row['code']; ?>"><?php echo $row['c_name']; ?></option>
        <?php
	} 
?>
</select>
