...
echo date("Y-m-d 10:00:00", strtotime('tomorrow'));
?>
...
$ins_query="insert into pappnew (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`user`,`yage`,`bdate`,`dis`,`aatime`,`adate1`,`z_link`,`email`) values ('$name', '$pmrn','$pphone','$padd','$dname','$date10','$slot','NOT SEEN','$diff1','$psex','$fullname','$diff2','$date91','$dis','$aatime','$adate1','$z_link','$email')";
mysqli_query($con,$ins_query) or die(mysql_error());

$update="update test set status='Booked' where `dname`='$dname' and `ddate`='$date10' and `dslot`='$slot'";
mysqli_query($con,$update) or die(mysql_error());

$update1="update patient set bdate='$date91', dis='$dis', pname='$name', padd='$padd', psex='$psex' where `pmrn`='$pmrn'";
...
<p>Hi ".$name.",</p>
			<p>Please be informed that Your Online Appointment with ".$dname." on  ".$zd1." at ".$slot." is confirmed.</p>
			
			<p>Please use the following link for joining online conversation- ".$z_link."<br /></p>
			<p>Best Regards<br />SFMMKPJSH<br />Powered By KPJ_IT_DHAKA</p>
			";
			...
			$mail->Username = 'kpjitdhaka02@gmail.com';					//Sets SMTP username
			$mail->Password = 'KPJIT@dhaka';					//Sets SMTP password
			$mail->SMTPSecure = 'tls';							//Sets connection prefix. Options are "", "ssl" or "tls"
			$mail->From = 'kpjitdhaka02@gmail.com';			//Sets the From email address for the message
			$mail->FromName = 'SFMMKPJSH ONLINE APPOINTMENT SYSTEM';					//Sets the From name of the message
			$mail->AddAddress($email);		//Adds a "To" address			
			$mail->WordWrap = 50;							//Sets word wrapping on the body of the message to a given number of characters
			$mail->IsHTML(true);							//Sets message type to HTML				
			$mail->Subject = 'Online Appointment Confirmmation';			//Sets the Subject of the message
			$mail->Body = $mail_body;							//An HTML or plain text message body
			if($mail->Send())								//Send an Email. Return true on success or false on error
			{
				$message = '<label class="text-success">Appointment Done, Please check your mail.</label>';
			}
...
<!-- Form Title -->
      ...
      <input name="padd" type="text" size="40" value="<?php echo $data["padd"]; ?>"required>
	  
	  <select name="dis" class="style1" placeholder="District" required> 
		
		<option value='<?php echo $data["dis"]; 
...
<option value='Sylhet'>Sylhet</option> 
	  ...
	  <label for="age"><strong>Patient's Details :</strong></label>
            ...
            <input name="psex" type="text" size="15" value="<?php echo $data["psex"]; ?>"required>
...
?>" size ="57" readonly>
	   ...
	   $doc1= $_REQUEST['doc'];
			...
			?>
									...
									<label for="age"><strong>Zoom Link:</strong></label>
			
			<select name="z_link"> <option value=''>--Select--</option>
	   <?php 
	   
			$sql = "select * from `zoom_link` where `date`='$zd'order by id asc";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->z_link."'>".$row->z_link."</option>";
				}
