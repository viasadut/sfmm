...
//$total = 0;
               ...
               $g_discount=$_REQUEST['g_discount'];
...
$new_price=$new_qty*$uprice;
					...
					if($eqty2>0 and $recevied==0 and $count_qtyw==0 and $balanced>=$eqty2){
					$ins_query1="update po_table1 set status='Updated',r_qty='$eqty2',check_by='$check_by' where id='$updateid'";
mysqli_query($con,$ins_query1) or die(mysql_error());

$ins_query2="insert into purchase_stock3 (`code`,`location`,`g_name`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`sno`,`u_price`,`t_price`,`add_by`,`add_time`,`re_date`,`req_qty`,`grn`,`invoice_no`,`p_id`,`check_by`,`po_id`)
values('$code','Store','$g_name','$b_name','$eqty2','$expiry','$batchno','$runningTime','$pono','$uprice','$tprice','$user','$add_time','$re_date','$eqty2','$grn','$invoice_no','$ono','$check_by','$po_id')";
mysqli_query($con,$ins_query2) or die(mysql_error());

if($bonus>0){

 
  $ins_query22="insert into purchase_stock3 (`code`,`location`,`g_name`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`sno`,`u_price`,`t_price`,`add_by`,`add_time`,`re_date`,`req_qty`,`grn`,`invoice_no`,`p_id`,`b_remarks`,`check_by`,`po_id`)
  values('$code','Store','$g_name','$b_name','$bonus','$expiry','$batchno','$runningTime1','$pono','$uprice','$bprice','$user','$add_time','$re_date','$eqty2','$grn','$invoice_no','$ono','Bonus Item','$check_by','$po_id')";
...
$total += $tprice;
					...
					else if($eqty2>0 and $recevied>0 and $count_qtyw==0 and $balanced>=$eqty2){
...
if($total>0)
  ...
  $ins_queryr="insert into acct_ap (`grn`,`creditor_code`,`vat`,`tax`,`amount`,`payable`,`cheque_no`,`bank`,`user`,`add_time`,`date`,`pono`,`grn_time`,`grn_by`,`invoice_no`)
        values('$grn','$creditor_code','','','$total','$total','','','','','','$po_ono','$add_time','$user','$invoice_no')";
		...
		values ('$last_id','CR','311190','$date','$total','PO_PUR')";
		mysqli_query($con,$ins_query) or die(mysql_error());

		
		$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
		values ('$last_id','DR','713085','$date','$total','PO_PUR')";
		mysqli_query($con,$ins_query2) or die(mysql_error());


    $ins_query_dis="update po_table set amount_discount='$g_discount' where id='$po_id'";
        mysqli_query($con,$ins_query_dis) or die(mysql_error());
...
<table id="itemTable" width="95%" height ="100%" border="0" align="center" bgcolor="lightblue" style="border-collapse:collapse;">	
    ...
    <a target="_blank" href="grn_upload?ono=<?php echo $ono;?>&sno=<?php echo $grn;?>"><img src="upload.png" title="Upload GRN Documents" width="50" height="50" />
   </a>  </td>
