...
$query431 = "SELECT * FROM medicine where mname= '$mmn';"; 
	 
$result431 = mysqli_query($con, $query431) or die(mysqli_error());
$row11 = mysqli_fetch_assoc($result431);
...
$ins_query1="update medicineedit set `mname`='$mname',`brand1`='$bname',`brand2`='$cname',`pre`='$form',`pcat`='$cat',`eby`='$user',`etime`='$adate',`frequency`='$frequency',`frelation`='$frelation',`pcategory`='$pcategory',`duration`='$duration',
`contrain`='$contrain',`meffect`='$meffect',`uprice`='$uprice',`status`='WAITING FOR Chairman APPROVAL',`mid`='$mid',`phar_remarks`='$remarks',`code`='$code',`reuse`='$reuse',`uprice1`='$uprice1',`cprice`='$cprice',`fby`='338',`cfo`='cfo',`ceo`='ceo',`it`='322',`ittime1`='$ittime1',`md`='md',`chairman`='1175',mtest='$stest' where id='$id'";
...
<!-- Form Title -->
      ...
      <input name="code" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["code"];?>"required>
      ...
      <input name="cname" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row1["brand2"];?>"required>
	  <label for="age"><strong>Form :</strong></label>
      <input name="form" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["pre"];?>"required>
	  <label for="age"><strong>Category :</strong></label>
      <input name="cat" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["pcat"];?>"required>
	  <label for="age"><strong>Frequency :</strong></label>
      <input name="frequency" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["frequency"];?>"required>
	  <label for="age"><strong>Food Relation :</strong></label>
      <input name="frelation" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["frelation"];?>"required>
	  <label for="age"><strong>Duration :</strong></label>
      <input name="duration" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["duration"];?>"required>
	  <label for="age"><strong>Pregnancy Category :</strong></label>
      <input name="pcategory" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["pcategory"];?>"required>
	  <label for="age"><strong>Contraindications :</strong></label>
      <input name="contrain" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["contrain"];?>"required>
	  <label for="age"><strong>Major Side Effects :</strong></label>
      <input name="meffect" type="text" size="70" style="text-transform:uppercase" value="<?php echo $row11["meffect"];?>"required>
...
<label for="age"><strong>Reusable:</strong></label>
      
	  <select name="reuse" >
        
						
						<option value=''>-Select-</option>
						<option value='Reuse'>YES</option>
	  ...
	  <label for="age"><strong>Skin Test Requirement:</strong></label>
      
	  <select name="stest" required>
        
						
						<option value='<?php echo $row1['mtype'];?>'><?php echo $row1['mtype'];?></option>
