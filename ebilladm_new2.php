...
<?php
	...
	$stmt_dept_prepare = $dbh->prepare($staff_dept_query);
	$stmt_dept_prepare->execute();
	$row_dept=$stmt_dept_prepare->fetch(PDO::FETCH_ASSOC);
	echo $transaction_by_dept = $row_dept['dept'];
	...
	$query4 = mysqli_query($con,"select * from patient where pmrn='$pmrn'");
	...
	$query444 = mysqli_query($con,"select * from preadm where id='$id'");
	...
	$query4444 = mysqli_query($con,"select * from preadm where pmrn='$pmrn' and eid='$count'");
...
if(isset($_POST['Submit'])){
	...
	$vcard1=$_REQUEST['visitor_rfid'];
	$acard=$_REQUEST['attendant_rfid'];
	...
	$sel_patient_rfid       = "SELECT COUNT(`id`) FROM `rfid` WHERE  `rfid`='$patient_rfid' AND `status`='0' AND `destroyed`= '0' AND `use_for`='Inpatient Tag' ";
	...
	$sel_attendant_rfid     = "SELECT COUNT(`id`) FROM `rfid` WHERE  `rfid`='$attendant_rfid' AND `status`='0' AND `destroyed`= '0' AND `use_for`='Attendant Card' ";
	...
	$sel_visitor_rfid       = "SELECT COUNT(`id`) FROM `rfid` WHERE  `rfid`='$visitor_rfid' AND `status`='0' AND `destroyed`= '0' AND `use_for`='Visitor Card' ";
	...
	// }
		...
		if ($patient_rfid_count==1) {
			$query159 = mysqli_query($con,"select * from doctor where covid='yes'");
			...
			try {
				...
				$dbh->query($update212);
				$dbh->query($ins_query33);
				$dbh->query($ins_query111);
				$dbh->query($update);
				$dbh->query($ins_query99);
				$dbh->query($update111);
				$dbh->query($update112);
				$dbh->query("UPDATE `rfid` SET `status` = '1' WHERE `rfid`='$patient_rfid'");
			...
			if ($attendant_rfid_count==1) {
				try {
					$dbh->beginTransaction();
					$dbh->query("UPDATE `rfid` SET `status` = '1' WHERE `rfid`='$attendant_rfid'");
					$dbh->query("UPDATE `inpatient` SET `rfid_acard_status` = '1' WHERE `rfid`='$patient_rfid' AND `rfid_status`='1' AND `acard`='$attendant_rfid'");
					$dbh->query("INSERT INTO `rfid_transaction` (
							`rfid`,
								`transaction_type`,
									`transaction_by`,
			...
			if ($visitor_rfid_count==1) {
				try {
					$dbh->beginTransaction();
					$dbh->query("UPDATE `rfid` SET `status` = '1' WHERE `rfid`='$visitor_rfid'");
					$dbh->query("UPDATE `inpatient` SET `rfid_acard_status` = '1' WHERE `rfid`='$patient_rfid' AND `rfid_status`='1' AND `card2`='$visitor_rfid'");
					$dbh->query("INSERT INTO `rfid_transaction` (
							`rfid`,
								`transaction_type`,
									`transaction_by`,
...
<!-- Form Title -->
		...
		<input name="dd" type="text" maxlength="2" size="1" value="<?php 
																		if($ttr == 0000-00-00){
																			echo '';
																		} else {
																			echo $te;
		...
		<label for="name"><strong>Select Ward :</strong></label>
			...
			<select name="btype1" class="country" value=''required/>
				...
				<?php
					$stmt = $dbh->prepare("SELECT distinct type FROM bed where status!='deactivate'");
					$stmt->execute();
					while($row=$stmt->fetch(PDO::FETCH_ASSOC))
					{
				?>
	...
	<?php
		...
		$query49 = mysqli_query($con,"select * from inpatient where pmrn='$pmrn' and eid='$count1' and discharge=''");
	...
	<table>
		<tr>
			...
			<td colspan="10"><a target='_blank' href="admnew?pmrn=<?php echo "$pmrn"; ?>&adoc=<?php echo $data49["adoc"]; ?>&adate=<?php echo $data49["adate"]; ?>&eid=<?php echo $count1; ?>&id=<?php echo $id; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>
		</tr>
	</table>
</form>
</body>
