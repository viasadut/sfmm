...
$sel90="SELECT * FROM dietfollow WHERE `pmrn`='$pmrn' and `eid`='$eid' and`infusion`='$infu' and odate='$odate1' and `time`='$infu1' and `status`='Active';";
...
echo '<script language="javascript">';
    echo 'alert("This Order is Already Added in Todays Chart  !!"); ';
...
$ins_query6="insert into dietfollow (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`) values ('$pmrn','$dname','$eid','$infu','$infu1','$ddate','$alert','$user','$date1','Active','$root','Rupdated','Ordered','$ortime')";
...
{
return confirm("Are you Sure to Stop The Diet Order ?");
}
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Dietician Follow Up Note</strong></label></td> </tr>
<tr>
<td colspan="3" align="center"><label><strong>Date</strong></label></td>
<td colspan="2" align="center"><label><strong>Time</strong></label></td> 
<td colspan="3" align="center"><label><strong>Feeding Route</strong></label></td> 
<td colspan="5" align="center"><label><strong>Feed Information</strong></label></td>
<td colspan="4" align="center"><label><strong>Calorie</strong></label></td>
<td colspan="3" align="center"><label><strong>Protine</strong></label></td>
</tr>
...
<td colspan="2" align="center"><input list="rr1" name="infu1" class="form-control" required>
  <datalist id="rr1">
...
<td colspan="3" align="center"><input list="rr10" name="root" class="form-control" required autocomplete ="off">
  <datalist id="rr10">
						...
						<option value='Feeding Gastrostomy'>Feeding Gastrostomy</option>
						<option value='Feeding Jejunostomy'>Feeding Jejunostomy</option>
						
			  </datalist></td>
<td colspan="5" align="center"><textarea name="infu"  value="" required></textarea></td></td>


			  <td colspan="4" align="center"><textarea name="ddate"  value="" required></textarea></td>

			   <td colspan="3" align="center"><textarea name="alert"  value="" required></textarea></td>
...
<tr>
		<td colspan="20"align="right"><font size="4.5" color="#FF0000"><b><label><strong><a href="datewisemediimodiet?pmrn=<?php echo $pmrn; ?>&eid=<?php echo $eid; ?>"><font size="4.5" color="#FF0000"><b>(See Datewise Diet Order)<b></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<tr>
	  ...
	  <td colspan="1" align="center"><strong>System Time</strong></td>
        
      <td colspan="5" align="center"><strong>Feed Information</strong></td>   
	  <td colspan="1" align="center"><strong>Route</strong></td>
	  <td colspan="2" align="center"><strong>Calorie</strong></td>
      
      <td colspan="1" align="center"><strong>User Done</strong></td>
	  <td colspan="2" align="center"><strong>Done time</strong></td>
	  <td colspan="1" align="center"><strong>Protine</strong></td>
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from dietfollow where pmrn= '$pmrn' and eid='$eid' and status !='Cancel' order by `time` and `infusion` asc;";
...
<td align="center" colspan="1"><a onclick="return confirm_click();" href="imediupdate1diet?id=<?php echo $row["id"]; ?>&eid=<?php echo $row["eid"]; ?>&pmrn=<?php echo $row["pmrn"]; ?>&user=<?php echo $user; ?>">Stop</a></td>
<td align="center" colspan="1"><a onclick="return confirm_click1();" href="imoinmeditomorrowdiet?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&infusion=<?php echo $row["infusion"]; ?>&time=<?php echo $row["time"]; ?>&instruc=<?php echo $row["instruc"]; ?>&orderby=<?php echo $user; ?>&root=<?php echo $row["root"]; ?>&alert=<?php echo $row["alert"]; ?>">ADD For Tomorrow</a></td>

      </tr>
    <?php $count++; } ?>
	 <tr><td colspan="10"><a target='_blank' href="testpdfdiet.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>
