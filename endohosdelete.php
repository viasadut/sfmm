...
$url = "endochargenurse1.php?pmrn=$pmrn&eid=$eid&ID=$id1";


$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM endohoscharge WHERE `id`='$id';");
$result_p = mysqli_fetch_assoc($sel_p);
$del_id=$result_p['COUNT(id)'];
if($del_id>0){

$query = "DELETE FROM endohoscharge WHERE id=$id"; 
...
header("Location:endochargenurse1.php?pmrn=$pmrn&eid=$eid&ID=$id1");
