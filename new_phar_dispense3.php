...
$query="update medi_stock set status='Served',`add_qty`='$t_g_qty',`given_qty`='$t_g_qty',`batch_no`='$batch_no',`exdate`='$exdate',add_time='$rr',s_qty='$t_g_qty', s_date='$date' where id='$id'";
...
if(mysqli_query($con,$query)==true){			
			...
			$ins_query3="update medi_stock set `add_qty`='$nqty' where rfid='$rfid' and location='Pharmacy'";
...
if(mysqli_query($con,$strSQL3)==true){			
	...
	$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`) values
...
$query="update medi_stock set status='Partially Served',`add_qty`='$t_g_qty',`given_qty`='$t_g_qty',`batch_no`='$batch_no',`exdate`='$exdate' where id='$id'";
