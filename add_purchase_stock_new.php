...
$sel95="SELECT COUNT(id),add_qty FROM purchase_stock3 WHERE `code`='$code' and location='Store';";
$result95 = mysqli_query($con,$sel95);
$b_chk=mysqli_fetch_assoc($result95);
$count_qty=$b_chk['COUNT(id)'];
$add_n_qty=$b_chk['add_qty'] + $add_qty;
...
$ins_query1="insert into purchase_stock_new (`code`,`g_name`,`b_name`,`add_qty`,`given_qty`,`exdate`,`u_price`,`t_price`,`batch_no`,`rfid`,`remarks`,`add_by`,`add_time`,`location`,`location1`)
 values ('$code','$b_name','$b_name','$add_qty','$add_qty','$exdate','$u_price','$t_price','$batch_no','$rfid','$remarks','$user','$ittime1','Store','$location')";
mysqli_query($con,$ins_query1) or die(mysql_error());


$ins_query2="update purchase_stock3 set add_qty='$add_n_qty' where code='$code' and location='Store'";
mysqli_query($con,$ins_query2) or die(mysql_error());

//header("Location: add_medi_stock");
header("Location: barcode_purchase_receive?id=$code");
...
$ins_query2="insert into purchase_stock3 (`code`,`g_name`,`b_name`,`add_qty`,`given_qty`,`exdate`,`u_price`,`t_price`,`batch_no`,`rfid`,`remarks`,`add_by`,`add_time`,`location`,`location1`)
 values ('$code','$b_name','$b_name','$add_qty','$add_qty','$exdate','$u_price','$t_price','$batch_no','$rfid','$remarks','$user','$ittime1','Store','$location')";
mysqli_query($con,$ins_query2) or die(mysql_error());
...
<!-- Form Title -->
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `purchase_stock3` where location='Store'";
...
<td colspan="3" ><input type="text" name="add_qty" id="" required value="" style="font-weight: bold;font-size:22px;color:green"></td>
						...
						<tr>
						<td colspan="3" ><input type="number" name="u_price" id="uprice" required value="" style="font-weight: bold;font-size:22px;color:green" step="any"></td>
	  </tr>
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `purchase_stock` where location='Store' and add_qty>0";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "stock_purchase.php?pmrn=" + str, true);
