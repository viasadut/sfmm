...
$objDB1 = mysqli_select_db($objConnect1,"sfmmkpjnew");
	...
	for($i=0;$i<count($_POST["chkDel"]);$i++)
	{
			...
			
		{
			...
			$sid_n=$dd['sid'];
			$cfleave_db=$dd['cfleave'];
			$new_leave=$b_leave1 - $dd['cfleave'];
			
			
			
			
			if($b_leave1>$cfleave_db){
			
			$strSQL = "update staff3 set cfleave='0',c_forward='$cfleave_db', ataken='$new_leave'";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect1,$strSQL);
			
			}
			
			else if($b_leave1<=$cfleave_db){
			
			$strSQL = "update staff3 set cfleave='0',c_forward='$cfleave_db', ataken='0'";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect1,$strSQL);
...
<td align="center"><input type="text" name="b_leave[]" id="b_leave<?php echo $i;?>" value="<?php echo $row39['ataken'];?>"readonly></td>
