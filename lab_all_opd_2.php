...
$laba = "SELECT COUNT(id) from alltest where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Confirmed By Consultant' and rdate between '$test' and '$apdate'"; 
$lab_resulta= mysqli_query($con, $laba) or die(mysqli_error());
$lab_rowa = mysqli_fetch_array($lab_resulta);
$lab_ra=$lab_rowa['COUNT(id)'];

$lab1a = "SELECT COUNT(id) from iinves where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Confirmed By Consultant' and rdate between '$test' and '$apdate'"; 
...
$lab2a = "SELECT COUNT(id) from einves where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Confirmed By Consultant' and rdate between '$test' and '$apdate'"; 
...
$lab2a4 = "SELECT COUNT(id) from alltest where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate'and subtype in('BIOCHEMISTRY','HAEMATOLOGY','PROFILE','FLUIDS & EXCREATIONS','BLOOD BANK') and rejectby!=''"; 
...
<body>
    ...
    <br>
    <h1 align="center" style="background-color:lightgreen;text-align:left"><a href='lab_all_opd_2' style="color:red">OPD (<?php echo $lab_ra;?>)</a>&nbsp;&nbsp;&nbsp;<a href='lab_all_ipd_2'>IPD(<?php echo $lab_r1a;?>)</a>&nbsp;&nbsp;&nbsp;<a href='lab_all_ae_2'>AE(<?php echo $lab_r2a;?>)</a></h1>
            ...
            <tbody>
                <?php
                    if($ugroup=='lab' && $status='active'){	
                        ...
                        $test=date('Y-m-d', strtotime('-3 days') );
                        $count=1;
                        $sel_query="Select * from alltest where type in ('lab','LAB','Lab') and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Confirmed By Consultant' and rdate between '$test' and '$apdate' and subtype NOT IN ('HISTOLOGY','CYTOLOGY') and cby !='' order by `resulttime` ASC;";
...
*/
			...
			?>
                    ...
                    <td align="center" colspan="1"><a onclick="return confirm_click();"
                        href="labreportconfirmopd_2?id=<?php echo $row["id"]; ?>">Confirm</a></td>
...
<?php
                    if($ugroup=='lab' && $status='active'){	
                        ...
                        $test=date('Y-m-d', strtotime('-3 days') );
                        $count=1;
                        $sel_query="Select * from alltest where type in ('lab','LAB','Lab') and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Confirmed By Consultant' and rdate between '$test' and '$apdate' and subtype IN('HISTOLOGY','CYTOLOGY') order by `resulttime` ASC;";
                        $result = mysqli_query($con,$sel_query);
                        while($row = mysqli_fetch_assoc($result)){
                ?>
                <tr>
...
*/
			...
			?>
                    ...
                    <td align="center" colspan="1"><a onclick="return confirm_click();"
                        href="labreportconfirmopd_2?id=<?php echo $row["id"]; ?>">Confirm</a></td>
                    <td align="center" colspan="1"><a target='_blank'
                    href="<?php echo $row['linkv']?>?id=<?php echo $row['id']; ?>&pmrn=<?php echo $row['pmrn']; ?>&eid=<?php echo $row['eid']; ?>">Edit</a>
                    </td>
                    <td align="center" colspan="1"><a target='_blank'
                        href="<?php echo $row['report']?>?id=<?php echo $row['id']; ?>&pmrn=<?php echo $row['pmrn']; ?>&eid=<?php echo $row['eid']; ?>&sno=<?php echo 'O'.$row['id']; ?>">Report</a>
                    </td>
