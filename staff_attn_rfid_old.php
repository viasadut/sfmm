<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('attn','mng')"; 
...
$queryd = "SELECT * FROM iclock_transaction where punch_time between '$s' and '$e' order by id DESC limit 1"; 
	 
$resultd = mysqli_query($con2, $queryd) or die(mysqli_error());

// Print out result
$rowd = mysqli_fetch_array($resultd);
$attn_id=$rowd['emp_code'];
$attn_time=$rowd['punch_time'];
...
<h1 style="color:green;font-size:60px;font-weight:bold">Attendance Confirmed...<h1>
...
<tr><td colspan="20"align="center"bgcolor="lightgreen"><img  src="staff_pic/'.$row3["pic"].'" width="180"  height="180" align="center"></td>
...
<h1 style="color:green;font-size:60px;font-weight:bold">Please Confirm Your Attendance...<h1>
<tr><td colspan="20"align="center"bgcolor="lightblue"class="style1" border="0" style="color:red;font-size:60px;font-weight:bold">Date: '.$cdate.'</td>
...
<td colspan="20" align="center"bgcolor="lightblue"><input type="hidden" name="rfid_card" required value="" autofocus placeholder='RFID'></td>
</tr>
...
<table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;">
    ...
    <tr>
      ...
      <th width="14%"><strong>Punch Date & time</strong>   
...
$sel_query="SELECT * FROM iclock_transaction where punch_time between '$s' and '$e' order by id DESC";
 
$count=1;
//$sel_query="Select * from presnew where dname='$bt' and date BETWEEN '$start' and '$end'";


$result = mysqli_query($con2,$sel_query);
...
{ ?>    <tr>
	...
	<?php  
	  $attn_id=$row['emp_code'];
$attn_time=$row['punch_time'];

$query3 = "SELECT * FROM staff3 where sid= '$attn_id' and status='Active'"; 
...
$query3i = "SELECT * FROM personnel_employee where emp_code= '$attn_id'"; 
	 
$result3i = mysqli_query($con2, $query3i) or die(mysqli_error());

// Print out result
$row3i = mysqli_fetch_array($result3i);
//$dept=$row3['dept'];
$ggi=$row3i['first_name'];
	  ?>
	  <?php if($gg==''){
		  echo'
      <td align="center">'.$ggi.'</td>
      <td align="center">'.$row["emp_code"].'</td>
      <td align="center">Intern</td>
      <td align="center">'.$row["punch_time"].'</td>';}
	
	  else {
		  echo'
      <td align="center">'.$row3["sname"].'</td>
      <td align="center">'.$row3["sid1"].'</td>
      <td align="center">'.$row3["dept"].'</td>
      <td align="center">'.$row["punch_time"].'</td>';}
