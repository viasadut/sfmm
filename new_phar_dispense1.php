...
<input name="eqty1_<?= $id ?>" type="text" value="<?php echo $row["add_qty"]; ?>" id="eqty1" required  <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>></td>
 ...
 <select onkeyup="GetDetail(this.value)" id="pmrn"class="form-control action" name="dname1" required  >
						<option value=''>-Select-</option>
						
						<?php 
						
						$cc1=$row['code'];
			$sql1 = 'select * from `medi_stock` where code='.$cc1.' and add_qty>0 and location="Pharmacy"';
			$res1 = mysqli_query($con, $sql1);
			if(mysqli_num_rows($res1) > 0) {
				while($row1 = mysqli_fetch_object($res1)) {
					echo "<option value='".$row1->rfid."'>".$row1->rfid."</option>";
				}
			}
			?>
