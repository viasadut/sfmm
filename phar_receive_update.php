...
$u_price=$eqty2*$p_price;
			...
			else if($eqty5 == 0 and $mqty=='')
	{
			...
			$strSQL1 = "update phar_sale set status='Received',qty='$eqty2',receive_by='$user',receive_time='$r_time' where id='".$updateid."'";
...
<tr>
	  ...
	  <td colspan="7" align="center"><strong>Instruction</strong></td>
	    ...
	    <?php 
                    $query = "select * from phar_sale where sno='$sno' and s_type='Transfer' and pmrn='$pmrn' and status='Served'";
...
<input name="eqty1_<?= $id ?>" type="text" value="<?php echo $row["qty"]; ?>" id="eqty1" required  <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?> readonly></td>
