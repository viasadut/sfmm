...
$url = "otchargenurse1.php?pmrn=$pmrn&id=$eid";	 

$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM othoscharge WHERE `id`='$id';");
$result_p = mysqli_fetch_assoc($sel_p);
$del_id=$result_p['COUNT(id)'];
if($del_id>0){

$query = "DELETE FROM othoscharge WHERE id=$id"; 
...
values ('$last_id','DR','$tb_data','$date','$price','OT_HOS_DEL')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','CR','615100','$date','$price','OT_HOS_DEL')";
...
header("Location: otchargenurse1.php?pmrn=$pmrn&id=$eid"); 
}
