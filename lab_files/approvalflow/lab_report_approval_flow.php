...
if(!is_dir(__DIR__.'/'.$SIG_DIR)){ @mkdir(__DIR__.'/'.$SIG_DIR, 0777, true); }
...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['save_flow'])){
    ...
    if($subtype===''){
        $msg = 'Please select a Category.'; $msgType='err';
    } else {
        ...
        $msg = 'Approval flow saved for category: '.htmlspecialchars($subtype); $msgType='ok';
...
$overviewCount = 0;
foreach($overview as $g){ foreach($g as $rows){ $overviewCount += count($rows); } }
?>
<!DOCTYPE html>
<html>
<head>
...
.wrap { width:92%; margin:15px auto; }
.card { background:#fff; border:1px solid #ccc; border-radius:6px; padding:15px 20px; margin-bottom:18px; box-shadow:0 1px 3px rgba(0,0,0,.1); }
.card h3 { margin:0 0 12px; color:#5a3e8c; border-bottom:2px solid #A085C6; padding-bottom:6px; }
.people { width:100%; border-collapse:collapse; }
.people th, .people td { border:1px solid #ddd; padding:7px 9px; text-align:left; font-size:13px; }
.people th { background:#f0ebf7; }
.sig-thumb { height:34px; max-width:130px; border:1px solid #ccc; background:#fff; vertical-align:middle; }
.no-sig { color:#999; font-style:italic; font-size:12px; }
select.cat { padding:8px; font-size:14px; min-width:320px; }
.btn { padding:12px 30px; color:#fff; background:#A085C6; font-size:14px; border:1px solid #8265B0; border-radius:5px; cursor:pointer; }
.btn:hover { background:#8265B0; }
.flash { padding:12px 16px; border-radius:5px; margin:12px auto; width:92%; font-weight:bold; }
.flash.ok  { background:#e3f7e3; border:1px solid #4bc970; color:#1c6b1c; }
.flash.err { background:#fde5e5; border:1px solid #e77; color:#a11; }
.hint { color:#666; font-size:12px; margin:2px 0 10px; }
.count-badge{ display:inline-block; background:#A085C6; color:#fff; border-radius:10px; padding:1px 9px; font-size:12px; margin-left:6px;}
</style>
</head>
<body>
<div id='cssmenu'>
    ...
    <table class="people">
      ...
      <?php foreach($overview as $subtype=>$group):
            ...
            foreach($roles as $rk=>$rlabel){
                if(!empty($group[$rk])){ foreach($group[$rk] as $p){ $p['_rlabel']=$rlabel; $catRows[]=$p; } }
            }
            $rowspan = count($catRows); $first = true;
        ...
        <tr>
          ...
          <td><?php echo htmlspecialchars($p['fullname']); ?>
              <small style="color:#999">(<?php echo htmlspecialchars($p['uname']); ?>)</small></td>
...
<?php if($selSubtype!==''): ?>
  <form action="lab_report_approval_flow.php?subtype=<?php echo urlencode($selSubtype); ?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="card">
      ...
      <table class="people">
        <tr><th width="6%">Select</th><th width="26%">Name</th><th width="24%">Designation</th><th width="22%">Current Signature</th><th width="22%">Upload / Replace Signature</th></tr>
