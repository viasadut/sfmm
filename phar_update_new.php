...
$sel96=mysqli_query($db,"SELECT * FROM medi_stock WHERE `rfid`='$rfid';");
$b_chk_m=mysqli_fetch_assoc($sel96);
...
$m_qty1=$b_chk_m['add_qty']-$eqty2;
			...
			else if($mqty=='' and $mm_qty<$eqty2)
	{
			...
			$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`pname`,`pmrn`) values('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','$ins','$pname','$pmrn')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery2 = mysqli_query($objConnect,$strSQL2);

			
			$ins_query3="update medi_stock set `add_qty`='0' where rfid='$rfid'";
...
//header("Location:$url");
				...
				else if($mqty=='' and $mm_qty>=$eqty2)
	{
			...
			$ins_query3="update medi_stock set `add_qty`='$m_qty1' where rfid='$rfid'";
mysqli_query($con,$ins_query3) or die(mysql_error());
...
$query1 = "select * from medicine where mname='$medi' and status='Active'";
     ...
     <td align="center" colspan="1" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $count; ?></td>
      ...
      <td align="center"colspan="11" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["medi"]; ?></td>
	        
						<td align="center"colspan="2" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["eqty1"]; ?></td>
			
		
<td align="center"colspan="2"><input name="eqty2_<?= $id ?>" type="text" value="<?php echo $row1['tqty'];?>" id="eqty2" readonly <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?> ></td>
...
<input name="rfid_<?= $id ?>" type="text" value="" id="rfid" required value=""></td>
