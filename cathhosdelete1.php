...
//$id1=$_REQUEST['ID'];
$url = "cathhoscharge.php?pmrn=$pmrn&eid=$eid&dname=$dname&id=$id&ieid=$ieid&type=$type";

$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM cathhoscharge WHERE `id`='$rid';");
...
$query = "DELETE FROM cathhoscharge WHERE id=$rid"; 
...
values ('$last_id','DR','$tb_data','$date','$price','CATHLAB_DEL')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','CR','111999','$date','$price','CATHLAB_DEL')";
...
header("Location: cathhoscharge.php?pmrn=$pmrn&eid=$eid&dname=$dname&id=$id&ieid=$ieid&type=$type"); 
