<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('doctor','staff','mng','ot','endo','imo','mofficer','nurse','emergency','moopd','lab','rad','physio')"; 
...
$query39 = "SELECT * FROM staff3 where sid= '$fullname' and status in ('Active','active')"; 
...
$date= date('m/d/Y');
$count=1;
$sel_query="Select * from dleave where hstatus='Forwarded to Incharge' and incharge='$full' and uname !=''and sdate>'2024-12-21'";
//$start=$row["aadate"];
...
while($row = mysqli_fetch_assoc($row1)) { ?>
	 ...
	 <?php
	  $sname=$row['uname'];
	  $query3 = "SELECT * FROM staff3 where sid= '$sname' and status in ('Active','active')"; 
...
echo "<td align='center' style='background-color:#FF2400; font-weight:bold; color:white'>".$fulln."</td>";
	  ...
	  $ttl=$row["tleave"];
	  $query39 = "SELECT * FROM staff3 where sid= '$sname' and status in ('Active','active')"; 
...
$date= date('m/d/Y');
$count=1;
$sel_query="Select * from dleave where hstatus='Approval Pending' and hos in('$full') and recomby='' and uname !='' and sdate>'2024-12-21'";
//$start=$row["aadate"];
...
$fday9= round($datediff78 / (60 * 60 * 24)*.0274,2) ;
	  ...
	  <td align="center"><?php echo $row["hos"].'<br>'.$row['iatime']; ?>
...
$date= date('m/d/Y');
$count=1;
$sel_query="Select * from dleave where hstatus='Approval Pending' and hos in('$fullname') and recomby!='' and uname !='' and sdate>'2023-12-21'";
//$start=$row["aadate"];
...
<td align="center"><a onclick="return confirm_click1();" href="leaveapprovere?id=<?php echo $row["id"]; ?>&user=<?php echo "$user"; ?>&uname=<?php echo $row['uname']; ?>"><strong>Reject</strong></a></td>
