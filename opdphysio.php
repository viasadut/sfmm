...
$query43 = "SELECT COUNT(dname) FROM presnew where dname in('Al-Amin','Mr. K. Utshab Zaman','Devjanee Sheel')  and date1 BETWEEN '$start' and '$end'and dstatus='Seen';"; 
...
$query44 = "SELECT COUNT(infusion) FROM icnote where user in('Al-Amin','Mr. K. Utshab Zaman','Devjanee Sheel') and daten BETWEEN '$start' and '$end';"; 
...
<h1 align="center">Monthly OPD Stats</h1>
...
$user=$_SESSION["sess_username"];
$start=date('Y-m-d',strtotime($_REQUEST["stdate"]));
$end=date('Y-m-d',strtotime($_REQUEST["endate"]));//$bt=$_REQUEST["bt"];
//$id=$_REQUEST["id"];
...
echo " (IPD- Referral Visit)  ";
echo " ,  From  ";
echo $start;
echo "  To  ";
echo $end;
	 $sel_query="Select * from presnew where dname in('Al-Amin','Mr. K. Utshab Zaman','Ms. Nipa Roy') and date1 BETWEEN '$start' and '$end' and dstatus='Seen' order by dname asc";
...
{ ?>    <tr>
	  ...
	  <td align="center"><?php echo $row["cdetails"]; ?>  
...
$query4 = mysqli_query($db,"select * from opd_referral where pmrn='$pp' and ref_name='$full' and rdate between '$start' and '$end'");
...
//$id=$_REQUEST["id"];
	 ...
	 $sel_query="Select * from icnote where user in('Al-Amin','Mr. K. Utshab Zaman','Devjanee Sheel') and daten BETWEEN '$start' and '$end'";
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row["pphone"]; ?>  
	  <td align="center"><?php echo $row["pnote"]; ?>  
	  <td align="center"><?php echo $row["inves"]; ?>  
		...
		$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$query4 = mysqli_query($db,"select * from user where uname='$pp'");
$data = mysqli_fetch_assoc($query4);
$dd=$data['fullname'];
?>
    ...
    <?php $count++; } }?>
      ...
      <td colspan="10" align="right"><a target='_blank' href="dietostat?dname=<?php echo "$full";?>&date=<?php echo "$start"; ?>&date1=<?php echo "$end"; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>	
  </tbody>
</table>
