...
$pdf->Cell('182' , 5, $data1['dname'],0,1,'L');
if($data1['dname1'] !='')
{
$pdf->Cell('182' , 5, $data1['dname1'],0,1,'L');

}
if($data1['dname2'] !='')
{

$pdf->Cell('182' , 5,$data1['dname2'],0,1,'L');
}
if($data1['dname3'] !='')
{
$pdf->Cell('182' , 5,$data1['dname3'],0,1,'L');
...
$pdf->Cell('182' , 5,$data1['dname4'],0,1,'L');
...
$pdf->ln(2);
$query1 = mysqli_query($db,"Select * from otivisitendo where pmrn= '$pmrn' and eid='$id'  order by `infusion` DESC;");
$count=1;
while($data1 = mysqli_fetch_array($query1))
{
...
$pdf->MultiCell('182' , 5,$count.') '.$data1['infusion'].' - '.$data1['vtype'].' -Charge Confirmed-'.$data1['odate'],0,1);
...
while($data1 = mysqli_fetch_array($query1))
{
$date44=strtotime('d/m/Y', $data1['time']);

//$pdf->MultiCell('182', 5,$data1['medi'],1,1);
$pdf->SetFont('Arial' , '' , 10);
$pdf->MultiCell('182' , 5,$count.') '.$data1['medi'].' - '.$data1['pdos'].' -'.$data1['code'].' -'.$date44.' '.$data1['mtime'] ,0,1);
$count++;
$pdf->ln(3);
}
$pdf->ln(3);
