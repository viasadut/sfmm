...
?>
 ...
 {  
	  ...
	  $pbp1 = mysqli_real_escape_string($connect, $_POST["pbp8"]);
	  	  $bt2 = mysqli_real_escape_string($connect, $_POST["bt2"]);
        $anes = mysqli_real_escape_string($connect, $_POST["anes"]);
...
//$pbp1= implode(",",$pbp2);
	 //$temp = mysqli_real_escape_string($connect, $_POST["temp"]);	  
      ...
      if($_POST["employee_id"] != '' and $user!='')  
        
      {  
           $query = "update ot set p_anes='$anes',p_time='$pbp1',p_ot='$bt2',p_user='$user' WHERE id = '".$_POST["employee_id"]."'";  
