...
$sel_query="Select * from purchase_stock where fstatus in('1','2') and hod='$sid1' group by rfid ORDER BY id asc;";
...
<a href="purchase_approve_mng?id=<?php echo $row['rfid']; ?>">View/Edit</a>
	  ...
	  <td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><a onclick="return confirm_click();" href="prf_forward_mng?sno=<?php echo $row["rfid"]; ?>&user=<?php echo "$fullname";?>">Approve</a> </td>
...
$sel_query="Select * from purchase_stock where fstatus in('2') and '$user'='cfo' and ptype='Stock Items' group by rfid ORDER BY id asc;";
...
<a href="purchase_approve_mng?id=<?php echo $row['rfid']; ?>">View/Edit</a>
	  ...
	  else if(($row['fstatus']!=2) and $row['ptype']=='Stock Items'){
...
//$sel_query="Select * from purchase_stock where fstatus in('2') and '$user'='ceo' group by rfid ORDER BY id asc;";
$sel_query="Select * from purchase_stock where fstatus in('3') and '$user'='ceo' and ptype='Stock Items' group by rfid ORDER BY id asc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	  ...
	  <?php if($row['fstatus']==3 and $row['ptype']=='Stock Items'){?>
...
<a href="purchase_approve_mng?id=<?php echo $row['rfid']; ?>">View/Edit</a>
	  ...
	  else if(($row['fstatus']!=3) and $row['ptype']=='Stock Items'){
