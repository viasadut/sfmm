<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','doctor','imo','nurse','mofficer','emergency')"; 
...
<?php 
$dd= date('m/d/Y',strtotime("-3 days")); 
$pmrn=$_REQUEST["pmrn"];
$eid=$_REQUEST["eid"];
?>
...
{
return confirm("Are you Sure to Add The Medicine ?");
}
...
<h1 align="center">2 Day Before's Medicine</h1>
...
$count=1;
$sel_query="Select * from imedi3 where pmrn= '$pmrn' and eid='$eid' and odate ='$dd';";
...
{ ?>    <tr>
	  ...
	  <td align="center" colspan="1"><a onclick="return confirm_click();" href="inmediadd2?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&infusion=<?php echo $row["infusion"]; ?>&time=<?php echo $row["time"]; ?>&instruc=<?php echo $row["instruc"]; ?>&orderby=<?php echo $user; ?>&root=<?php echo $row["root"]; ?>&alert=<?php echo $row["alert"]; ?>&uprice=<?php echo $row["uprice"]; ?>">ADD</a></td>
      </tr>
