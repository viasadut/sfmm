...
$ins_query="insert into pappnew (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`user`,`yage`,`bdate`,`dis`,`aatime`,`adate1`,`ptype`,`page1`) values ('$name', '$pmrn','$pphone','$padd','$dname','$date23','$slot','NOT SEEN','$diff1','$psex','$user','$diff2','$date91','$dis','$aatime','$adate1','$ptype','ccgg1new_test1')";
...
if($l_date1=='')
	
	{
		...
		echo "Need Billing Confrmation";
	}

else if($diff1_b>5 and $l_date1!='')
	...
	
	{
		...
		echo "No Need Billing Confrmation";
	}
  ...
  $("input").on("change", function() {
   // var ret = parseInt($("#field1").val()) - parseInt($("#field2").val())
    ...
    $("#date4").val(ret1);
			...
			<select name="slot" required> <option value=''>--Select--</option>
	   <?php 
				$ct=date('H:i:s');
...
<label for="age"><strong>Patient's Type :</strong></label>
	  	...
	  	<select name="type" id="ptype"class="style1" placeholder="Patient Type"  required readonly> 
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "gfg1_test.php?pmrn=" + str, true);
