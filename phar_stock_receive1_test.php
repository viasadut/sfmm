...
$select = $_REQUEST['select'];
		...
		if($code_1 !='' and $b_no !='' and $select=='Returned')
		{

$sel96="SELECT *, COUNT(id) FROM medi_stock WHERE `code`='$code_1' and batch_no='$b_no' and location='Pharmacy' order by id desc limit 1;";
...
if($b_chk_m['COUNT(id)']>0)
						{				
			$ins_query3="update medi_stock set `add_qty`='$m_qty1' where id='$m_id'";
mysqli_query($con,$ins_query3) or die(mysql_error());
                        }

                        else if($b_chk_m['COUNT(id)']<=0)
						{				
                            $ins_query1="insert into medi_stock (`code`,`g_name`,`b_name`,`add_qty`,`req_loc`,`status`,`location`,`rfid`,`sno`,`batch_no`,`add_by`,`add_time`)
                            values ('$code_1','$medi_1','$brand_1','$add_1','$loc','Served','Pharmacy','$sno_1','$code_1','$batch_1','$user','$add_time')";
