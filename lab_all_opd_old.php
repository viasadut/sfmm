...
$laba = "SELECT COUNT(id) from alltest where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate' and subtype in('BIOCHEMISTRY','HAEMATOLOGY','PROFILE','FLUIDS & EXCREATIONS','BLOOD BANK') and rejectby=''"; 
$lab_resulta= mysqli_query($con, $laba) or die(mysqli_error());
$lab_rowa = mysqli_fetch_array($lab_resulta);
$lab_ra=$lab_rowa['COUNT(id)'];

$lab1a = "SELECT COUNT(id) from iinves where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate' and subtype in('BIOCHEMISTRY','HAEMATOLOGY','PROFILE','FLUIDS & EXCREATIONS','BLOOD BANK') and rejectby=''"; 
...
$lab2a = "SELECT COUNT(id) from einves where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate'and subtype in('BIOCHEMISTRY','HAEMATOLOGY','PROFILE','FLUIDS & EXCREATIONS','BLOOD BANK') and rejectby=''"; 
...
<body>
    ...
    <br>
    <h1 align="center" style="background-color:lightgreen;text-align:left"><a href='lab_all_opd' style="color:red">OPD (<?php echo $lab_ra;?>)</a>&nbsp;&nbsp;&nbsp;<a href='lab_all_ipd'>IPD(<?php echo $lab_r1a;?>)</a>&nbsp;&nbsp;&nbsp;<a href='lab_all_ae'>AE(<?php echo $lab_r2a;?>)</a></h1>
            ...
            <tbody>
                <?php
                    if($ugroup=='lab' && $status='active'){	
                        ...
                        $test=date('Y-m-d', strtotime('-30 days') );
                        $count=1;
                        $sel_query="Select * from alltest where type in ('lab','LAB','Lab') and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate' and subtype in('BIOCHEMISTRY','HAEMATOLOGY','PROFILE','FLUIDS & EXCREATIONS','BLOOD BANK') and rejectby='' order by `resulttime` ASC;";
...
*/
                    ...
                    <td align="left" colspan="4"><?php echo nl2br($row["result"]); ?></td>
