...
$update="update opd_appoint1 set status='AVAILABLE',remarks='$reason' where dslot='$select' and date1='$dd' and dname='$name' and status!='Booked'";
mysqli_query($con,$update);
}
			
echo '<script language="javascript">';
    echo 'alert("Appointment Unblocked Successfully"); ';
    echo '</script>';
...
<!-- Form Title -->
		<h2 align="center">UNBLOCK DOCTOR'S AVAILABLE DATE &amp; TIME </h2>
		...
		<fieldset>
			...
			<select name="dname" id="dname" value="" required readonly>
...
<input type="text" name="date" id="datepicker" placeholder="Select Date" value="<?php 
			  if(isset($_POST['load'])==1){
				echo $date_1=$_REQUEST['date'];
				}?>" readonly required>
<!-- Password Input --><!-- Age Dropdown -->

<input name="load" type="submit" id="load" value="Check Slots">
			<label for="age"><strong>Available Slot :</strong></label>
			
			
			<select id='select' name = 'select[]' multiple size=60>   
			
			  <?php 
			  if(isset($_POST['load'])==1){
				
				$date11=date('Y-m-d',strtotime($_REQUEST['date']));
				
				$tday=date('Y-m-d');
				$ct=date('H:i:s');
				
				$ct1=$date11.' '.$ct;
				$ct2=date('Y-m-d H:i:s');
				
				if($date11==$tday)
				{
			$sql = "select * from opd_appoint1 where dslot>='$ct' and dname='$dname2' and date1='$date11'and status='NOT AVAILABLE' order by dslot";
...
//echo "<option value='".$row->dslot."'>".$row->dslot.'-'.$row1->status."</option>";
				...
				{
			$sql = "select * from opd_appoint1 where dname='$dname2' and date1='$date11'and status='NOT AVAILABLE' order by dslot";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
