...
$sel3="Select * from inpatient where '$ad3' between alert1 and alert2 and discharge='' and room1 like '5A%' order by id desc limit 1";
...
$sel4="Select * from inpatient where '$ad3' between alert1 and alert2 and discharge='' and room1 like '5B%' order by id desc limit 1";
...
<tr>
		...
		<strong>5th Floor A Block</strong>&nbsp;&nbsp;
...
$bed_log="Select COUNT(id) from bed where ncall='1' and bno='$bed' and block= '5A';";
$bed_log_data = mysqli_query($con, $bed_log) or die(mysqli_error());
$bed_log_result = mysqli_fetch_assoc($bed_log_data);
$bed_log_count =$bed_log_result['COUNT(id)'];
...
$rw = mysqli_fetch_assoc($resu);
  ...
  if($rw['COUNT(id)']>0 and $bed_log_count>0){

    echo "<td colspan='3' style='background-color:violet;' class='invalid'>"; 
...
";
 ...
 else if($rw['COUNT(id)']>0 and $bed_log_count==0){
...
if($bed_log_count>0){
  ...
  echo "<td colspan='1' style='background-color:violet;' class='invalid'>"; 
...
echo "</td>";
  ...
  if($rows_ot>0 and $m_c!='')
	{ 
echo "<td colspan='1' >C. Done</td>";
	}
	
	else if($rows_ot>0 and $m_c=='')
	{ 
echo "<td colspan='1' >C. Pending</td>";
	}
...
<tr>
		...
		<strong>5th Floor B Block</strong>&nbsp;&nbsp;
...
$bed_log="Select COUNT(id) from bed where ncall='1' and bno='$bed' and block= '5B';";
$bed_log_data = mysqli_query($con, $bed_log) or die(mysqli_error());
$bed_log_result = mysqli_fetch_assoc($bed_log_data);
$bed_log_count =$bed_log_result['COUNT(id)'];
...
$txt='One Patient is Admitted in Block - 5A, Patient Name is-'.$tt4.' And Patient- '.'MRN'.$tt3;
...
if($rw4==true){
							$txt='One Patient is Admitted in Block - 5B, Patient Name is-'.$tt66.' And Patient- '.'MRN'.$tt55;
  $txt1=htmlspecialchars($txt);
  $txt2=rawurlencode($txt1);
  $html=file_get_contents('https://translate.google.com/translate_tts?ie=UTF-8&client=gtx&q='.$txt2.'&tl=en-US');
