<?php 
   session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('doctor','msuite')"; 
...
$sel_query="Select * from m_suite where pphone= '$pmrn' and ustatus='paid' order by id desc ";}
 else{
	 $sel_query="Select * from m_suite where pmrn= '$pmrn' and ustatus='paid' order by id desc;";
 } 
$count=1;	 
...
{ ?>    <tr>
	  ...
	  <td align="Left"><?php echo $row["dname"]; ?><td colspan="5"><a target='_blank' href="proreport_ms.php?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>						
      </tr>
