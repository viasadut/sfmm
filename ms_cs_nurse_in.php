<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('nurse','msuite')"; 
...
$query5 = mysqli_query($db,"select * from m_suite where pmrn='$pmrn' and eid='$eid'");
...
$query159 = mysqli_query($db,"select * from radio1 where iname='$medi'");
...
$ins_query="insert into iinves (`pmrn`,`eid`,`pname`,`page`,`pgender`,`odate`,`infusion`,`user`,`status`,`ordate`,`type`,`code`,`price`,`subtype`,`rstatus`,`link`,`report`,`ndate`,`linkv`,`reportv`,`unit`,`dname`) values 
( '$pmrn','$eid','$pname','$page','$pgender','$date','$medi','$user','Data Updated','$ordate','$type','$code','$price','$type1','Ordered','$link','$report','$ndate','$linkv','$reportv','$unit','$dname')";
