...
$mcode = $row1['code'];
	  $sum = "SELECT SUM(add_qty) FROM medi_stock where code='$code_medi' and location='Pharmacy_opd'" ;
...
if(myObj[8]>30){
				...
				xmlhttp.open("GET", "stock_medi_test_opd.php?pmrn=" + str, true);
  ...
  $('#add_data_Modal').on('shown.bs.modal', function() {
    $('input[name="code"]').focus();
	  ...
	  $('#add_data_Modal').modal({backdrop: 'static', keyboard: false})  
           ...
           $.ajax({  
                url:"medi_dispense_phar_opd_new_opd.php",  
                method:"POST",  
...
$('#lrfid').val(data.sno);					 
	  ...
	  //var tr = $('#sqty').val() + $('#gqty').val();
           ...
           {  
                $.ajax({  
                     url:"new_phar_dispense3_opd_new_opd.php",  
                     method:"POST",  
