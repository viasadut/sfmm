...
$eid=$_REQUEST['eid'];
$full = $row39['fullname'];
$query43 = "SELECT COUNT(pmrn) FROM deathn where pmrn= '$pmrn';"; 
...
$query4 = mysqli_query($db,"select * from deathn where pmrn='$pmrn'");
$data = mysqli_fetch_assoc($query4);

$iiby=$data['iby'];

/*$dd=date('d',strtotime($data['rdate']));
$mm=date('m',strtotime($data['rdate']));
$yy=date('Y',,strtotime($data['rdate']));
 */
require('db1.php');
//include("auth.php");
...
if($fullname!=''){
$update3="update death1 set `fname`='$fname',`padd`='$padd',`ppadd`='$ppadd',`rdoc`='$rdoc',`rdate`='$rdate',`rdate1`='$rdate1',`rtime`='$rtime',`pphone`='$pphone',`diag`='$diag',`adm`='$adm',`cdeath`='$cdeath',`iby`='$iiby',`idate`='$idate',`year`='$year' where pmrn='$pmrn'";
mysqli_query($con,$update3) or die(mysqli_error(oo));

$ins_query="update deathn set `fname`='$fname',`padd`='$padd',`ppadd`='$ppadd',`rdoc`='$rdoc',`rdate`='$rdate',`rdate1`='$rdate1',`rtime`='$rtime',`pphone`='$pphone',`diag`='$diag',`adm`='$adm',`cdeath`='$cdeath',`iby`='$iiby',`idate`='$idate',`year`='$year' where pmrn='$pmrn'";
...
<!-- Form Title -->
      ...
      <input name="name" type="text" size="80" value="<?php echo $data["name"]; ?>" readonly>
      ...
      <input name="fname" type="text" size="80" value="<?php echo $data["fname"]; ?>" required>
 	  <label for="age"><strong>Patient's Present ADDRESS :</strong></label>
      <input name="padd" type="text" size="80" value="<?php echo $data["padd"]; ?>"required>
	  <label for="age"><strong>Patient's Permanet ADDRESS :</strong></label>
      <input name="ppadd" type="text" size="80" value="<?php echo $data["ppadd"]; ?>" required>

<label for="age"><strong>Ward / Bed / Admission Date:</strong></label>
	  	
            <input name="ward" type="text" size="15" value="<?php echo $data["ward"]; ?>"readonly>
            <input name="bed" type="text" size="15" value="<?php echo $data["bed"]; ?>"readonly>
	   <input name="adm" type="text" size="15" value="<?php echo $data["adm"]; ?>"readonly>	  
      
			<label for="name"><strong>Received Doctor's Name :</strong></label>
			
			<select name="rdoc" value=""required>
			        <option value='<?php echo $data["rdoc"]; ?>'><?php echo $data["rdoc"]; ?></option>
				<?php 
      ...
      <input name="diag" type="text" size="80" value="<?php echo $data["diag"]; ?>" required>
	  
	  <label for="mail"><strong>Date and Time of Death :(Month/Day/Year)</strong></label>
									<p>
									  <input name="rdate" type="date" size="80" value="<?php echo $data["rdate"]; ?>" required min="2023-01-01" max="2050-12-31">
									  <input name="rtime" type="text" size="80" value="<?php echo $data["rtime"]; ?>" required>
  			
			
	  
	  <label for="age"><strong>Cause Of Death :</strong></label>
      <textarea size="100" name="cdeath" required><?php echo $data["cdeath"]; ?></textarea>
  </fieldset>
