...
/*$f1=date('Y-m-d', strtotime('-1 days') );
...
$pac_f = "SELECT * FROM pac_request where pmrn='$pmrn' and status='Pending' order by r_s_d DESC LIMIT 1 "; 
...
$date11=date_create($f2);
	...
	$pac_fr = "SELECT COUNT(id) FROM pac_request where pmrn='$pmrn' and status='Pending'"; 
$pac1_fr = mysqli_query($con, $pac_fr) or die(mysqli_error());

$pac2_fr = mysqli_fetch_array($pac1_fr);
$f1r=$pac2_fr['COUNT(id)'];
...
$query4 = mysqli_query($db,"select * from package_screening where pmrn='$pmrn' and status='Active' and id='$id'");
...
$query5 = mysqli_query($db,"select * from package_screening where pmrn='$pmrn' and status='Active'");
$data1 = mysqli_fetch_assoc($query5);

$query59 = mysqli_query($db,"select * from package_screening where pmrn='$pmrn' and status='Active'");
...
$query3 = "SELECT COUNT(pmrn) FROM alltest where pmrn='$pmrn' and eid='$eid' and status='Rupdated' "; 
...
$query26 = "SELECT COUNT(pmrn) FROM package_screening where pmrn='$pmrn' and eid='$eid' and status='Active' "; 
...
{
$url = "imoidocinves_pack?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
...
if(isset($_POST['pac_req']))
{
$update="Insert pac_request (`pmrn`,`pname`,`dname`,`r_s_by`,`r_s_t`,`r_s_d`,`status`,`page`,`psex`,`pphone`) values('$pmrn','$pname','$dname','$full','$req_time','$req_date','Pending','$page','$psex','$pphone')";
mysqli_query($con,$update) or die(mysql_error());
$url = "imoidetails?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
...
{
$url = "ot1nurse9_new.php?pmrn=$pmrn";
header("Location: $url");
}
?>
...
{
$url = "all_consent_form.php?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
?>
...
{
$url = "imoedocvitals_pack?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
?>
<?php
...
{
$url = "imoidocnote_pack?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
?>
...
{
$url = "inassessment1_pack?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
?>
...
{
$url = "death_summary?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
?>
...
{
$url = "mother_baby?pmrn=$pmrn&dname=$full&eid=$eid";
header("Location: $url");
}
?>
...
$url = "imoidoccondis_pack?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
?>
...
{
$update="update package_screening set medi_legal='1' where `pmrn`='$pmrn' and `eid`='$eid'";
mysqli_query($con,$update) or die(mysql_error());
$url = "imoidetails?pmrn=$pmrn&eid=$eid";
header("Location: $url");
}
...
<head>
  <meta charset="UTF-8">
  <title>package_screening Panel</title>
...
{
return confirm("Are you Sure to Add package_screening Visit ?");
}
...
<h1 align="center"style="background-color:lightgreen;">DETAILS package_screening RECORD</h1>
<!-- Form Title -->
...
$n1=$data_baby1['beid'];
 ...
 echo'
		  <a target="_blank" href="imoidetails?pmrn='.$n.'&eid='.$n1.'"><strong><img src="baby1.png" title="Baby Details" width="50" height="50" /></strong></a>
	  ...
	  <tr><td align="Left"style="font-size:20px;color:green;">Patient's Age :<b><?php echo $data["page"]; ?></b></td>  </tr>
	  <tr><td align="Left"style="font-size:20px;color:green;">Patient's Gender :<b><?php echo $data["psex"]; ?></b></td>  </tr>
	  <tr><td align="Left"style="font-size:20px;color:green;">Patient's Phone :<b><?php echo $data["pphone"]; ?></b></td>  </tr>
	  <tr><td align="Left"style="font-size:20px;color:green;">Admission Date:<b><?php echo $data["sdate"]; ?></b></td>  </tr>
	   <tr><td align="Left" style="font-size:20px;color:red;">Blood Group:<b><?php echo $b_group; ?></b></td>  </tr>
...
$count=1;
$sel_query="Select * from alltest where pmrn= '$pmrn' and eid='$eid'  order by `id` DESC limit 8;";
...
{ ?>    <tr>
	  ...
	  <td align="center"><a target="_blank" href="compareinvesimo.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid";?>&infu=<?php echo $row['medi'];?>"><?php echo $row["medi"]; ?></a></td>  
...
$count=1;
$sel_query="Select * from vitals_package where pmrn= '$pmrn' and eid='$eid' order by `id` DESC limit 4;";
...
{ ?>    <tr>
	  ...
	  <td align="center"><?php echo $row["sbp"].'/'.$row["dbp"]; ?></td>  
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row["stime"]; ?></td>
	  <td align="center"><?php echo $row["pulse"]; ?></td>  
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row["stime"]; ?></td>
	  <td align="center"><?php echo $row["temp"]; ?></td>  
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row["spo2"]; ?></td>  
	  <td align="center"><?php echo $row["time"]; ?></td>
...
<a target="_blank" href="indmdoc.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid";?>">RR Chart:</a><a target="_blank" href="rr_chart.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid";?>&odate1=<?php echo "$odate1";?>"><span style="font-size:14px;color:red;background:lightgreen;"><b>(Graph)</b></span></a>
				<table class="table table-bordered" id="dynamic_field">
				<?php
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row["rr"]; ?></td>  
	  <td align="center"><?php echo $row["time"]; ?></td>
...
<a target="_blank" href="indmdoc.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid";?>">Pain Score Chart:</a><a target="_blank" href="pain_score_chart.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid";?>&odate1=<?php echo "$odate1";?>"><span style="font-size:14px;color:red;background:lightgreen;"><b>(Graph)</b></span></a>
				<table class="table table-bordered" id="dynamic_field">
				<?php
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row["pain_score"]; ?></td>  
	  <td align="center"><?php echo $row["time"]; ?></td>
...
<tr>
		<td colspan="4"align="center"><button type="submit" name="Submit123">Doctor's Assessment</button></td>
	
		<td colspan="4" align="center"><button type="submit" name="Submit12">MO's Note</button></td>
    <td colspan="3" align="center"><button type="submit" name="Submit6">MO Order</button><font size="4.5" color="#FF0000"><b>(<?php echo  $row19['COUNT(pmrn)']; ?>)<b></td>
