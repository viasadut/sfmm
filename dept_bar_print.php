...
<table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;">
    ...
    <th width="4%"style="font-size:18px;font-weight:bold;color:red;"><strong>S.No</strong></th>
	  ...
	  <th width="10%"style="font-size:18px;font-weight:bold;color:red;"><strong>Charge Code</strong></th>
	  ...
	  <th width="14%"style="font-size:18px;font-weight:bold;color:red;"><strong>Barcode</strong> 
      <th width="15%"style="font-size:18px;font-weight:bold;color:red;"><strong>Expire date</strong>
...
$sel_query="Select * from medi_stock where add_qty>0 and location='$start'  and status in('Served','Partially Served') ORDER BY g_name asc ;";
...
$queryq = "SELECT SUM(qty) from phar_sale where rfid='".$rrid."' and reuse='' and status!='Cancel' and adate between '$sd' and '$ed'"; 
...
$rrid=$row['sno'];
$cc=$row['code'];
/*$queryq = "SELECT SUM(pdos) from othoscharge1 where rfid='".$rrid."' and reuse='' and ndate between '$sd' and '$ed'"; 
$resultq = mysqli_query($con, $queryq) or die ( mysqli_error());
$rowq = mysqli_fetch_assoc($resultq);
*/
$queryc = "SELECT * from medicine where code='".$cc."' and status='Active'"; 
$resultc = mysqli_query($con, $queryc) or die ( mysqli_error());
$rowc = mysqli_fetch_assoc($resultc);


$queryq1 = "SELECT SUM(pdos) from othoscharge1 where rfid='".$rrid."' and remarks='Finished' and ndate between '$sd' and '$ed'"; 
$resultq1 = mysqli_query($con, $queryq1) or die ( mysqli_error());
$rowq1 = mysqli_fetch_assoc($resultq1);

$cc=$rowc["c_code"];

?>
<td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $rowc["c_code"]; ?></td>	  
	  
<td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $row["given_qty"]; ?></td>
	  <td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $row["add_qty"]; ?></td>
<td align="center"style="font-size:18px;font-weight:bold;color:red;"><?php if($cc==''){ echo $rowq["SUM(qty)"];} else{echo $rowq["SUM(qty)"];} ?></td>

<td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $row["sno"]; ?></td>
      <td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $row["exdate"]; ?></td>
 ...
 <td align="left" ><a target='_blank' href="edit_medi_stock0000.php?id=<?php echo $row['id']; ?>"></a></td>
