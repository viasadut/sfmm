...
/*function count_digit($number1) {
	return strlen((string) $number1);
	}

	$number_of_digits1 = count_digit($user); //this is call :)
	$number_of_digits1;
*/	
...
//$one=			
			...
			*/
			...
			$new_bar1=$host.date('ymd').'00'.$mno;
			$new_bar2=$host.date('ymd').'0'.$mno;
			$new_bar3=$host.date('ymd').$mno;
			...
			*/
			...
			$a1=$ns.date('sd').'000'.$mno;
			$a2=$ns.date('sd').'00'.$mno;
			$a3=$ns.date('sd').'0'.$mno;
			$a4=$ns.date('sd').$mno;
			...
			$strSQL5 = "select COUNT(barcode1) from alltest where rdate='$cdate' and barcode1 in ('$new_bar1','$new_bar2','$new_bar3','$new_bar4')";
...
function showUser() {
  ...
  xmlhttp.open("GET","lab_serial1.php",true);
  xmlhttp.send();
}
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from alltest where pmrn= '$pmrn' and billstatus='Billed' and type='lab' and rstatus ='' and billdate between '$test' and '$dd' and billno!=''";
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from alltest where pmrn= '$pmrn' and billstatus='Billed' and type in ('lab','LAB','Lab') and rstatus ='RECEIVED' and billdate between '$test' and '$dd'";
$objQuery = mysqli_query($objConnect,$strSQL) or die ("Error Query [".$strSQL."]");
?>
