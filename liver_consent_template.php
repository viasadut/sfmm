<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','lab','outdoc','staff','nurse')"; 
...
{
$ins_query_format="insert into vitals_report_format (`dname`,`dcode`,`report`,`add_by`,`add_time`,`status`,`type`,`iname`) 
values ('$full', '$fullname','$cdetails','$fullname','$add_time','Waiting','$type','$ptemp')";
mysqli_query($con,$ins_query_format);


$url = "liver_format_view?type=$type&dname=$full";
header("Location: $url");
...
$ins_query_format="update vitals_report_format set `report`='$cdetails',`status`='Waiting',`add_time`='$add_time' where type='$type'";
mysqli_query($con,$ins_query_format);
...
//var variables = "pmrn=str&string=$pd";
				...
				xmlhttp.open("GET", "liver_data_template.php?pmrn=" + str + "&porder=<?php echo $pd;?>", true);
