...
$test2=date('Y-m-d', strtotime('+7 days') );
?>
...
$sel_query="Select * from ot where status='' and date5 between '$test1' and '$test2' ORDER BY date5 asc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
      ...
      <?php
$ppmrn=$row["pmrn"];
$p_receive = "SELECT COUNT(id) FROM inpatient where discharge='' and pmrn='$ppmrn'"; 
$p_result = mysqli_query($con, $p_receive) or die(mysqli_error());
...
?>
      ...
      <?php if($p_r>0){echo "<td align='center' style='background-color:lightgreen;'>".$row["pname"]."</td>";}

else {echo "<td align='center' style='background-color:red;'>".$row["pname"]."</td>";}

?>
      <td align="center"><?php echo $row["pmrn"]; ?></td>
	  <td align="center"><?php echo date('d/m/Y', strtotime($row["date5"]) ); ?></td>
      <td align="center"><?php echo $row["stime"].' To '.$row["etime"]; ?></td>
...
echo "<td align='center' style='background-color:lightgreen;'>".$tr."</td>";
	  ...
	  ?>
	  	  ...
	  	  <td align="Left"><a target='_blank' href="billotbill_bill?pmrn=<?php echo $row['pmrn']; ?>&id=<?php echo $row['id']; ?>&proce=<?php echo $row['proce']; ?>"><?php echo $row["proce"]; ?></a> 
		...
		$rid=$row['id'];
		$pp=$row['pmrn'];
		$url = "m_clear?pmrn=$pp&id=$rid"; 
		   
		   
		
	if($m_c=='')
	{ 
    echo "<td align='center' style='background-color:lightblue;'><a href='$url'>Confirm Clearance</a></td>";
	}
	
	else 
	{ 
echo "<td align='center' style='background-color:lightgreen;'>Clearance Already Confirmed</td>";
	}
...
<table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;">
    ...
    <tr>
      ...
      <th width="14%"><strong>Procedure</strong>  
			...
			<th width="14%"><strong>All Charges(PDF)</strong>
			<th width="14%"><strong>Anaes Document</strong>
			<th width="14%"><strong>PAC</strong>
    ...
    <tr>
	  ...
	  <td align="Left"><?php echo $row["duration1"]; ?>  </td>
	       ...
	       <td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><?php echo $row["ptype"];?></td>
				   ...
				   <td align="center" <?php if($row['ot_charge_status']=='Charge Confirmed'): ?> style="background-color:RED;"<?php else: ?> style="background-color:lightgreen;" <?php endif ; ?>><a target='_blank' href="b_ot_dis.php?pmrn=<?php echo $row['pmrn']; ?>&id=<?php echo $row['id']; ?>"><?php if($row['ot_charge_status']=='Charge Confirmed'){echo 'Charge Confirmed';} else {echo 'OT Charge';}?></a>
				   
				   
				   
				   </td>
				   <td><a target='_blank' href="otuse.php?pmrn=<?php echo $row['pmrn']; ?>&id=<?php echo $row['id']; ?>">All Charge (PDF)</a></td>
				   <td><a target='_blank' href="otanaesprint.php?pmrn=<?php echo $row['pmrn']; ?>&id=<?php echo $row['id']; ?>">Anaes Document</a></td>
				    <td><a target='_blank' href="preanaesprintdoc.php?pmrn=<?php echo $row['pmrn']; ?>&id=<?php echo $row['id']; ?>">PAC</a></td>
