...
$query159 ="select * from radio1 where iname='$iname'";
$result59 = mysqli_query($con, $query159) or die(mysqli_error());
$data159 = mysqli_fetch_array($result59);
$type=$data159["type"];
$code=$data159["code"];
$price=$data159["price"];
...
$opd_last= mysqli_query($db,"select * from alltest where pmrn='$pmrn' and medi='$iname' and billdate NOT IN('0000-00-00','NULL') and cby!='' order by id desc limit 1");
$data_opd = mysqli_fetch_assoc($opd_last);
$result_opd=$data_opd['result'];
$opd_bill_date=$data_opd['billdate'];


$ipd_last= mysqli_query($db,"select * from iinves where pmrn='$pmrn' and infusion='$iname' and rdate NOT IN('0000-00-00','NULL') and conby!='' order by id desc limit 1");
$data_ipd = mysqli_fetch_assoc($ipd_last);
$result_ipd=$data_ipd['result'];
$ipd_bill_date=$data_ipd['rdate'];


$ae_last= mysqli_query($db,"select * from einves where pmrn='$pmrn' and infusion='$iname' and rdate NOT IN('0000-00-00','NULL') and conby!='' order by id desc limit 1");
...
$result1 = $_REQUEST['result'].' '.$unit;

$update="update alltest set resultstatus='Updated By Technologist',resulttime='$rtime',resultby='$user',result='$result',result1='$result1' where id='$id'";
mysqli_query($con,$update) or die(mysql_error());


//$update1="update alllab set resultstatus='Updated By Technologist',result='$result',resulttime='$rtime',resultby='$user' where `sno`='$sid'";
//mysqli_query($con,$update1);


    echo '<script language="javascript">';
    echo 'alert("Result Successfully Updated"); ';
    echo '</script>';
...
<!-- Form Title -->
		<h1>INVESTIGATION RESULT UPDATE PANEL</h1>
        ...
        <fieldset>
			...
			if($opd_bill_date!='' and $ipd_bill_date!='' and $ae_bill_date!='' and $opd_bill_date>$ipd_bill_date and $opd_bill_date>$ae_bill_date)
				{echo"<p style='color:red;font-size:20px;font-weight:bold'>Last Result(OPD)- ".$result_opd." (Result Date: ".$opd_bill_date.")</p>";}		
			...
			{echo"<p style='color:red;font-size:20px;font-weight:bold'>Last Result(IPD)- ".$result_ipd." (Result Date: ".$ipd_bill_date.")</p>";}		
			...
			{echo"<p style='color:red;font-size:20px;font-weight:bold'>Last Result(AE)- ".$result_ae." (Result Date: ".$ae_bill_date.")</p>";}		
      ...
      <input name="padd" type="text" size="70" style="text-transform:uppercase" value="<?php echo $data['pmrn'];?>"required>
	  
	  <label for="age"><strong>Investigation Name :</strong></label>
      <input name="padd" type="text" size="70" style="text-transform:uppercase" value="<?php echo $data['medi'];?>"required readonly>

<label><strong>Result</strong></label>  
                          <textarea rows="25" cols="50" name="result"  style="text-transform:uppercase"><?php 
						  
						  if($data['result']==''){
						  
						  echo $data_SG['machine_result'];} else {
