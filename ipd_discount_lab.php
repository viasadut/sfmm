...
$query4 = mysqli_query($db,"select COUNT(id) from iinves where pmrn='$pmrn' and eid='$eid' and rstatus='RECEIVED'");
$data4= mysqli_fetch_assoc($query4);
$inves_num=$data4['COUNT(id)'];



$query5 = mysqli_query($db,"select SUM(price) from iinves where pmrn='$pmrn' and eid='$eid' and rstatus='RECEIVED'");
$data5= mysqli_fetch_assoc($query5);
$sum_bill=$data5['SUM(price)'];
...
$eqty2 = $_POST['eqty1_'.$updateid] - $disco;
			...
			$strSQL = "update iinves set dprice='$eqty2', discount='$disco',discount_type='$discount_type',o_dis='$percentage1' where id='".$updateid."'";
...
$strSQL1 = "update inpatient set lab_dis='$sum_discount' where pmrn='$pmrn1' and eid='$eid1'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
$ins_query3="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$pmrn','DR','617410','$date','$sum_discount','IPD_LAB_DISCOUNT')";
  ...
  values ('$pmrn','CR','111999','$date','$sum_discount','IPD_LAB_DISCOUNT')";
...
$refund_time=date('Y-m-d H:i:s');
$disco=$_POST['eqty1_'.$updateid] * $percentage;
$eqty2 = $_POST['eqty1_'.$updateid] - $taka;
			...
			$strSQL1 = "update iinves set lab_dis='$eqty2', discount='$taka', discount_type='$discount_type',o_dis='$taka1' where id='".$updateid."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery1 = mysqli_query($objConnect,$strSQL);
	...
	$strSQL1 = "update inpatient set lab_dis='$taka1' where pmrn='$pmrn1' and eid='$eid1'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery1 = mysqli_query($objConnect,$strSQL1);
      ...
      $date=date('Y-m-d');
$ins_query3="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$pmrn','DR','617410','$date','$taka1','IPD_LAB_DISCOUNT')";
  mysqli_query($con,$ins_query3) or die(mysql_error());


  $ins_query4="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$pmrn','CR','111999','$date','$taka1','IPD_LAB_DISCOUNT')";
  mysqli_query($con,$ins_query4) or die(mysql_error());
...
$count=1;
$sel_query="Select * from iinves where pmrn= '$pmrn' and eid='$eid' and rstatus='RECEIVED' and type in('LAB','Lab','lab','RAD','Rad','rad') order by `id` DESC;";
...
$id = $row['id'];
?>	  
<td align="center"colspan="2"><input name="eqty1_<?= $id ?>" type="text" value="<?php echo $row['price'];?>" id="eqty1" required ></td>                            
...
<tr>
		...
		<td colspan="20" align="left" style="color:red; font-weight:bold;font-size:18px"><label><strong>Type </strong></label>
<select name="discount_type" value="" class="style1" id="pmrn" onchange="GetDetail(this.value)">
   ...
   <input name="taka" type="number" class="style1" id="sdate12" placeholder="Discount In Taka" max="10000">
   
						  
						  
					

<input type="number" name="percentage" id="sdate1" class="style1" hidden placeholder="Discount In Percentage" max="30">
