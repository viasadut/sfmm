...
$sel96="SELECT * FROM purchase_stock WHERE `code`='$code_1' and add_qty>0 and location='Store'  order by exdate asc limit 1;";
...
<tr>
	  ...
	  <td colspan="7" align="center"><strong>Code</strong></td>
	    ...
	    <?php 
                    $query = "select * from purchase_stock where rfid='$sno' and req_loc='$req_loc' and status in ('Pending','Served','Partially Served')";
...
<td align="center"colspan="1"style="font-weight: bold;font-size:22px;color:red"><?php echo $new_qty_r;?></td>
 ...
 <td align="center" colspan="1" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row['add_qty']; ?></td>
	 ...
	 <a target="_blank" href="department_bar_transfer_new_purchase_new?sno='.$rfid.'"><img src="phar_pic/barcode.png" title="Print Instruction" width="20" height="20" /></a>';}
	 ?>
	 </td>						
...
<p id="demo"></p>
 ...
 <div id="add_data_Modal" class="modal fade" tabindex="-1" >  
	...
	<option value=''>-Select-</option>
            ...
            while($row = mysqli_fetch_array($result)) {
        ?>
            <option value="<?php echo $row['code']; ?>"><?php echo $row['code'].'('.$row['g_name'].')'; ?></option>
        <?php } ?>
  ...
  $('#add_data_Modal').on('shown.bs.modal', function() {
    $('input[name="code"]').focus();
	  ...
	  $('#add_data_Modal').modal({backdrop: 'static', keyboard: false})  
           ...
           $.ajax({  
                url:"medi_dispense_purchase.php",  
                method:"POST",  
...
$('#lrfid').val(data.sno);					 
	  ...
	  //var tr = $('#sqty').val() + $('#gqty').val();
           ...
           {  
                $.ajax({  
                     url:"new_purchase_dispense3_new.php",  
                     method:"POST",  
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "stock_purchase_test_new.php?pmrn=" + str, true);
