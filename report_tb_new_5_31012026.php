  ...
  echo "<tr><td colspan='20' style='font-size:22px; font-weight:bold;color:red;text-align:center'>From $start To $end </td></tr>";
                    ...
                    while($row = mysqli_fetch_array($result) ){
                       ...
                       $acct_group=$row['acct_code'];
                        ...
                        $query2 = "select SUM(amount) FROM pms_tb WHERE trans_type='CR' and acct_code='$acct_group' and date between '$start' and '$end'"; 
                        $result2 = mysqli_query($con,$query2) or die ( mysqli_error());
                        
                        $data_cr = mysqli_fetch_array($result2);	   


                        $query1 = "select SUM(amount) FROM pms_tb WHERE trans_type='DR' and acct_code='$acct_group' and date between '$start' and '$end'"; 
                        $result1 = mysqli_query($con,$query1) or die ( mysqli_error());
                        
                        $data_dr = mysqli_fetch_array($result1);	

                       // $query3 = "select SUM(amount) FROM pms_tb WHERE trans_type='DR' and acct_code='0' and date='2026-01-24'"; 
                        //$result3 = mysqli_query($con,$query3) or die ( mysqli_error());
                        
                        //$data_0 = mysqli_fetch_array($result3);	
                        ...
                        $querydr = "select COUNT(id) FROM pms_tb WHERE acct_code='$acct_group' and trans_type='DR' and date between '$start' and '$end'"; 
                        $resultdr = mysqli_query($con,$querydr) or die ( mysqli_error());
                        
                        $datadr = mysqli_fetch_array($resultdr);	   
                        $dr=$datadr['COUNT(id)'];

                        $querycr = "select COUNT(id) FROM pms_tb WHERE acct_code='$acct_group' and trans_type='CR' and date between '$start' and '$end'"; 
                        ...
                        if($acct_group=='0')
{
                          $value4=$data_cr['SUM(amount)']-$data_dr['SUM(amount)'];
                        }

                        else if($data22['acct_type']=='R')
                        {

                          $value=$data_cr['SUM(amount)']-$data_dr['SUM(amount)'];
                        }

                        else if($data22['acct_type']=='A')
                        {

                          $value1=$data_dr['SUM(amount)']-$data_cr['SUM(amount)'];
                        }

                        else if($data22['acct_type']=='A' || $data22['acct_type']=='R' || $data22['acct_type']=='E' and $dr=='0' and $cr>0)
                        {

                          $value22=$data2['SUM(amount)'];
                        }

                        else if($data22['acct_type']=='L')
{
                          $value33=$data_cr['SUM(amount)']-$data_dr['SUM(amount)'];
                        }
