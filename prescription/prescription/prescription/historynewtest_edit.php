...
$query = "SELECT * from pappnew where pmrn='$pmrn'and adate='$date1' and dname='$dname';" ;
...
$query1 = "SELECT * from pappnew where pmrn='$pmrn'and eid='$eid1';" ;
...
$sel="SELECT * FROM presnew WHERE `pmrn`='$pmrn' and dname='$pd' and date='$date1';";
...
if($res=mysqli_num_rows($result)>0)
{
    ...
    $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $date1, $pg, $pdiet, $ps, $eid, $seen, $date4, $fdate, $user, $fav, $surgery]);

	$sh = $db->prepare("UPDATE pappnew SET eid=?,status=?,stime=?,adate1=?,pbmi=? WHERE ID=?");
    $sh->execute([$eid, $seen, $stime, $date4, $pbmi, $id]);

    $db->commit();
	$url = "historynewview?pmrn=$pmrn&eid=$eid&date=$date1&dname=$dname" ;
header("Location:$url");
    ...
    $db->rollBack();
            ...
            $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $date1, $pg, $pdiet, $ps, $eid, $seen, $date4, $fdate, $user, $fav]);
        ...
        header("Location:$url");
            ...
            $db->rollBack();
                    ...
                    $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $date1, $pg, $pdiet, $ps, $eid, $seen, $date4, $fdate, $user, $surgery]);
        ...
        /*else{
            ...
            $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $date1, $pg, $pdiet, $ps, $eid, $seen, $date4, $fdate, $user]);
...
<option value=''>-Select-</option>
    ...
    ?>
                                         ...
                                         <textarea id="editor90" name="cdetails" class="form-control" placeholder="Enter Product's Comparison"><?php echo $row43['cdetails'];?></textarea>
      ...
      disableNativeSpellChecker: false
                                ...
                                <div class="form-group row">
                                   <label for="inputEmail3" class="col-sm-12 col-form-label">Patient's Diagnosis:</label> 
                                    <div class="col-sm-12">
                                        <textarea id="editor1" name="diagnosis" class="form-control" placeholder="Enter Product's Comparison"><?php echo $row43['diagnosis'];?></textarea>
                                    </div>
                                </div>
<script>
  ...
  extraPlugins : 'filebrowser',
	...
	removePlugins: 'scayt',
      disableNativeSpellChecker: false
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-12">
                                         <input list=diet1 name="pdiet" placeholder="Select Diet" class="form-control" value="<?php echo $row43['pdiet'];?>">
					<datalist id="diet1">	
			...
			?>	
                                ...
                                <div class="form-group row">
                                   ...
                                   <div class="col-sm-12">
                                         <input type="text" id="editor91" onchange="GetDetail2(this.value)" class="form-control action" list="categoryname8" autocomplete="off" name='medi1' placeholder='Use Template'>
	...
	<option value=''>-Select-</option>
			...
			?>
										...
										<textarea id="editor1" name="other" class="form-control" placeholder="Enter Product's Comparison"><?php echo $row43['other'];?></textarea>
                                    </div>
  ...
  extraPlugins : 'filebrowser',
	...
	<option value=''>-Select-</option>
			...
			?>
                                        ...
                                        <textarea class="form-control" name="other_b" id="other_b" rows="3" placeholder="Other Instructions (In Bangla)"><?php echo $row43['other_b'];?></textarea>
                                    </div>
								...
								<div class="form-group row">
                                    ...
                                    <div class="col-sm-12">
										...
										<input type="date" class="form-control" name="fdate" placeholder="Next Follow Up Date" value="">
                                    </div>
								</div>
                ...
                <div class="col-md-3">
                    <div class="card card-success">
                        ...
                        <div class="card-body">
                            <button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-pills float-left fa-2x"></i> <a target='_blank' href="newtest2test2?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$dname"?>&eid=<?php echo "$eid"?>&eido=<?php echo "$eid1"?>&pname=<?php echo "$pname"?>"style="color:white">Medication</a></button>
                            <button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-vials float-left fa-2x"></i> <a target='_blank' href="newtest2_edit?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$dname"?>&eid=<?php echo "$eid"?>&eido=<?php echo "$eid1"?>&pname=<?php echo "$pname"?>"style="color:white">Investigation</a></button>
							...
							<button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-id-card-alt float-left fa-2x"></i> <a target='_blank' href="opd_referral_test?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$eid"?>&eido=<?php echo "$eid1"?>"style="color:white">Referral</a></button>
