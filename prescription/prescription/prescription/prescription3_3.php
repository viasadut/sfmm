...
$c1=$rowc['COUNT(utype)'];
    ...
    $_SESSION['user_id']=$_SESSION["sess_username"];
    $_SESSION['user_name']=$_SESSION['sess_fullname'];
    $_SESSION['sess_userrole']=$_SESSION['sess_userrole'];
    //$_SESSION['user_photo']=
?>
...
/*$sel_fav="SELECT * FROM presnew WHERE `pmrn`='$pmrn' and dname='$pd';";
$result_fav = mysqli_query($con,$sel_fav);  
$data_fav=mysql_fetch_assoc($result_fav);
*/
$query_an = "SELECT COUNT(id) from antenatal_program_patient where `mrn`='$pmrn' and status='1' and delete_status='1'"; 
$result_an = mysqli_query($con, $query_an) or die ( mysqli_error());
$row_an = mysqli_fetch_assoc($result_an);
$register=$row_an['COUNT(id)'];
...
$other_b = str_replace("'", "''",$_REQUEST['other_b']);
...
try {
    ...
    $sh = $db->prepare("insert into presnew (dname,pname,pmrn,pphone,cdetails,diagnosis,other,other_b,date,page,pdiet,psex,eid,dstatus,date1,fdate,did,fav,surgery) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $pdate, $pg, $pdiet, $ps, $count1, $seen, $date4, $fdate, $user, $fav, $surgery]);

    $sh = $db->prepare("UPDATE pappnew SET eid=?,status=?,stime=?,adate1=?,pbmi=? WHERE ID=?");
    $sh->execute([$count1, $seen, $stime, $date4, $pbmi, $id]);
...
header("Location:$url");
...
else if($fav!='' and $surgery==''){
		
    try {
        $db->beginTransaction();
    
        $sh = $db->prepare("insert into presnew (dname,pname,pmrn,pphone,cdetails,diagnosis,other,other_b,date,page,pdiet,psex,eid,dstatus,date1,fdate,did,fav) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $pdate, $pg, $pdiet, $ps, $count1, $seen, $date4, $fdate, $user, $fav]);
    ...
    header("Location:$url");
        ...
        try {
            ...
            $sh = $db->prepare("insert into presnew (dname,pname,pmrn,pphone,cdetails,diagnosis,other,other_b,date,page,pdiet,psex,eid,dstatus,date1,fdate,did,surgery) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $pdate, $pg, $pdiet, $ps, $count1, $seen, $date4, $fdate, $user, $surgery]);
    ...
    try {
        ...
        $sh = $db->prepare("insert into presnew (dname,pname,pmrn,pphone,cdetails,diagnosis,other,other_b,date,page,pdiet,psex,eid,dstatus,date1,fdate,did) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $sh->execute([$dd, $pn, $pm, $pp, $cdetails, $diagnosis, $other, $other_b, $pdate, $pg, $pdiet, $ps, $count1, $seen, $date4, $fdate, $user]);
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <b>Phone:</b> <?php echo $row['pphone'];?> <?php echo $count1;?>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <b>Drug History:</b> <?php if ($row['pdrug']=='YES'){echo '<strong class="text-danger h5 font-weight-bold">YES</strong>';} else {echo $row['pdrug'];}?><br>
                                </div>
                                <div class="col-md-2">
                ...
                <div class="col-md-3">
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            ...
                            <button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-notes-medical float-left fa-2x"></i><a href="view3newtesttest?pmrn=<?php echo $pmrn; ?>&dname=<?php echo $full?>&eid=<?php echo $count1?>&ID=<?php echo "$id"?>"style="color:white"> Template Of Previous Visits</a></button>
                            ...
                            <button type="button" class="btn btn-block btn-info font-weight-bold" style="background-color:#ED6572"><i class="fas fa-microscope float-left fa-2x" ></i><a target='_blank' href="../../deathstatdetailsmng?pmrn=<?php echo "$pmrn"; ?>"style="color:white">ALL RECORDS</a></button>
                            <button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fa fa-user-md float-left fa-2x"></i><a target='_blank' href="../../surnotemng?pmrn=<?php echo $pmrn; ?>" style="color:white"> SURGERY NOTE</a></button>
...
else 
{
	...
	<form target="_blank" action="https://182.160.124.36:443/PACSAPI/Launch_Viewer?" method="post" id="tt" >
<input type="hidden" name="PatientID" value="'.$pmrn.'"></input>
<input type="hidden" name="Username" value="hisuser"></input>
<input type="hidden" name="Password" value="hisuser"></input>
<input type="submit" name="Submit90" value="PACS VIEW"align="left"></input>
...
<button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-id-card-alt float-left fa-2x"></i> <a target='_blank' href="../../TB_FORM.pdf"style="color:white">TB FORM</a></button>
<button type="button" class="btn btn-block btn-info font-weight-bold"  style="background-color:#ED6572"><i class="fas fa-id-card-alt float-left fa-2x"></i> <a target='_blank' href="../../idocinvesstat?pmrn=<?php echo $pmrn; ?>"style="color:white">Outside Investigation</a></button>
                    ...
                    <div class="card card-success">
						...
						<form  class="form-horizontal" action="" method="post" onsubmit='return confirm("Do You Want To Proceed??");'id="prescrip" name="prescrip" />
                            ...
                            <div class="col-sm-12">
                                        ...
                                        <input type="text" id="pmrn" onchange="GetDetail1(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='medi1' placeholder='Use Template' >
...
<option value=''>-Select-</option>
        ...
        <?php 
    $sql = "select * from `cdetails` where user='$user'";
    ...
    ?>
                                           ...
                                           <textarea id="editor90" name="cdetails" class="form-control" placeholder="Enter Product's Comparison"></textarea>
      ...
      disableNativeSpellChecker: false
			...
			?>	
                                ...
                                <div class="form-group row">
                                   ...
                                   <div class="col-sm-12">
                                         <input type="text" id="pmrn" onchange="GetDetail2(this.value)" class="form-control action" list="categoryname8" autocomplete="off" name='medi1' placeholder='Use Template'>

    <datalist id="categoryname8">
	<option value=''>-Select-</option>
			...
			?>
                                       ...
                                       <textarea id="editor91" name="other" class="form-control" placeholder="Enter Product's Comparison"></textarea>
	...
	removePlugins: 'scayt',
      disableNativeSpellChecker: false
								...
								<div class="form-group row">
                                    ...
                                    <div class="col-sm-12">
                                                                 <input type="text" id="pmrn" onchange="GetDetail3(this.value)" class="form-control action" list="categoryname88" autocomplete="off" name='medi1' placeholder='Use Template'>

    <datalist id="categoryname88">
	<option value=''>-Select-</option>
				...
				<?php 
			$sql = "select * from `other_ins_b` where user='$user'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->cname."'>".$row->cname."</option>";
			...
			?>
                                        ...
                                        <textarea class="form-control" name="other_b" id="other_b" rows="3" placeholder="Other Instructions (In Bangla)"></textarea>
                                    </div>
							...
							<div class="form-group row">
                                    ...
                                    <span style="color:green;font-size:20px;font-weight:bold">Add To Favourite:</span> <input type="checkbox" id="critical" name="fav" value="1" style="height:20px; width:20px; color:red;">
                                    <span style="color:green;font-size:20px;font-weight:bold">&nbsp;&nbsp;&nbsp;Plan For Surgery:</span> <input type="checkbox" id="surgery" name="surgery" value="1" style="height:20px; width:20px; color:red;">
                                    </div>
								</div>
                            </div>
                ...
                <div class="col-md-3">
                    <div class="card card-success">
                        <div class="card-header">
                            <h3 class="card-title">Medication & Investigation</h
                            <i class="fas fa-notes-medical float-right fa-lg"></i>
                        </div>
                        <div class="card-body">
                            <button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-pills float-left fa-2x"></i> <a target='_blank' href="newtest5_1?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Medication</a></button>
							<button type="button" class="btn btn-block btn-warning font-weight-bold"><i class="fa fa-pills float-left fa-2x"></i><a target='_blank' href="newtest2test2?pmrn=<?php echo "$pm"; ?>&dname=<?php echo "$pd"?>&eid=<?php echo "$count1"?>&eido=<?php echo "$oeid"?>&pname=<?php echo "$pn"?>"style="color:white">Load Last Medicine</a></button>
                            <button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-vials float-left fa-2x"></i> <a target='_blank' href="newtest2?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>&eido=<?php echo "$oeid"?>"style="color:white">Investigation</a></button>
							<button type="button" class="btn btn-block btn-success font-weight-bold"><i class="fas fa-head-side-cough float-left fa-2x"></i> <a target='_blank' href="../../comorimo?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Comorbidities</a></button>
							<button type="button" class="btn btn-block btn-success font-weight-bold"><i class="fas fa-allergies float-left fa-2x"></i> <a target='_blank' href="../../allergyimo?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Allergies</a></button>
							<button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-id-card-alt float-left fa-2x"></i> <a target='_blank' href="opd_referral?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Referral</a></button>
							<button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-store float-left fa-2x"></i> <a target='_blank' href="care_shop?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Care Shop Item</a></button>
							<button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-eye float-left fa-2x"></i> <a target='_blank' href="eye_01?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Spectacles Power</a></button>
							<button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-bell float-left fa-2x"></i> <a target='_blank' href="../../con_work_self?pmrn=<?php echo "$pmrn"; ?>"style="color:white">Set Work List</a></button>
...
$urla = "../../AP/NewPatientRegistration.php?mrn=$pmrn"; 
$urlt = "../../AP/NewPatientRegistration.php?mrn=$pmrn"; 

if($ps=='F' and $register==0)
	
	{
        ...
        <button type='button' class='btn btn-block btn-info font-weight-bold' style='background-color:#ED6572'><i class='fas fa-venus float-left fa-2x'></i><a target='_blank' href='$urla' style='color:white'>Register under antenatal program</a></button>
	...
	
	{
		...
		"<button type='button' class='btn btn-block btn-info font-weight-bold'><i class='fas fa-venus float-left fa-2x'></i><a target='_blank' href='$url' style='color:white'>Obstetrical History</a></button>
        <button type='button' class='btn btn-block btn-info font-weight-bold' style='background-color:green'><i class='fas fa-venus float-left fa-2x'></i><a target='_blank' style='color:white'>Alreday Registered in antenatal program</a></button>
...
<button type="button" class="btn btn-block btn-info font-weight-bold" style="background-color:#ED6572"><i class="fas fa-store float-left fa-2x"></i> <a target='_blank' href="register_tumor?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Tumor Registration</a></button>

							
							<button type="button" class="btn btn-block btn-info font-weight-bold"style="background-color:#ED6572"><i class="fas fa-id-card-alt float-left fa-2x"></i> <a target='_blank' href="docadm?pmrn=<?php echo "$pm"; ?>&dname=<?php echo "$pd"; ?>&eid=<?php echo "$count1"; ?>"style="color:white">Admission Advise</a></button>
							<button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-id-card-alt float-left fa-2x"></i> <a target='_blank' href="../../cam_test/test_cam?pmrn=<?php echo "$pm"; ?>"style="color:white">Upload Patient Image</a></button>
							<button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-thin fa-child float-left fa-2x"></i> <a target='_blank' href="set_pic_attach?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Upload Body Image</a></button>
                            <button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fas fa-solid fa-notes-medical float-left fa-2x"></i> <a target='_blank' href="../../medicalcertificate_opd?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">Issue Medical Certificates</a></button>
                            <button type="button" class="btn btn-block btn-info font-weight-bold"><i class="fa fa-user-md float-left fa-2x"></i> <a target='_blank' href="../../ot1nurse9_new_doc?pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$pd"?>&ID=<?php echo "$id"?>&eid=<?php echo "$count1"?>"style="color:white">OT Booking</a></button>
...
//						document.getElementById(
	//						"page").value = myObj[1];
							...
							document.getElementById(
							"editor90").value = myObj[0];
                                  CKEDITOR.instances["editor90"].setData(myObj[0]);
				...
				xmlhttp.open("GET", "cdetails_data.php?pmrn=" + str, true);
...
//						document.getElementById(
	//						"page").value = myObj[1];
							...
							document.getElementById(
							"editor91").value = myObj[0];
                                  CKEDITOR.instances["editor91"].setData(myObj[0]);
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				// xhttp.open("GET", "filename", true);
				xmlhttp.open("GET", "other_data_b.php?pmrn=" + str, true);
