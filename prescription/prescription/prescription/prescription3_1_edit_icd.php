...
$ins_query="update presnew set cdetails='$cdetails',diagnosis='$diagnosis',pdiet='$pdiet',other='$other',other_b='$other_b',fdate='$fdate',did='$user' where pmrn='$pmrn' and eid='$eid'";
...
$ins_delete="delete from icd_diagnosis where eid='$eid' and pmrn='$pmrn' and dname='$pd'";
mysqli_query($con,$ins_delete) or die(mysql_error());


/*foreach($icd as $group_member){
$update6="insert into icd_diagnosis (dname,pmrn,eid,diagnosis) values('$pd', '$pm', '$eid', '$group_member')";
mysqli_query($con,$update6) or die(mysql_error());
}*/

foreach($icd as $group_member){

     $doc_q = "SELECT * FROM icd_code where code= '$group_member';"; 
     $doc_q_r = mysqli_query($con, $doc_q) or die(mysqli_error());
     $doc_q_d = mysqli_fetch_assoc($doc_q_r);
     $diagnosis =$doc_q_d['name'];
     $code =$doc_q_d['code'];
     
     /*$sh = $db->prepare("insert into icd_diagnosis (dname,pmrn,eid,diagnosis,code) 
     VALUES (?, ?, ?, ?, ?)");
     $sh->execute([$pd, $pm, $eid, $diagnosis, $code]);
*/
     $update6="insert into icd_diagnosis (dname,pmrn,eid,diagnosis,code) values
     ('$pd', '$pm', '$eid', '$diagnosis','$code')";
mysqli_query($con,$update6) or die(mysql_error());
                    ...
                    <div class="card card-success">
						...
						<form  class="form-horizontal" action="" method="post" onsubmit='return confirm("Do You Want To Proceed??");'id="prescrip" name="prescrip" />
						    ...
						    <label for="inputEmail3" class="col-sm-12 col-form-label">Patient's Clinical Details:</label>
                                    ...
                                    <div class="col-sm-12">
                                           <textarea id="editor1" name="cdetails" class="form-control" placeholder="Enter Product's Comparison"><?php echo $row431['cdetails'];?></textarea>
...
<div class="form-group row">
                                    <label for="inputEmail3" class="col-sm-12 col-form-label">ICD Diagnosis:</label>
                                    <div class="col-sm-12">
	...
	$stmt5 = $DB_con->prepare("select * from `icd_diagnosis` where eid='$eid' and pmrn='$pmrn' and dname='$pd'");
	$stmt5->execute();
	while($row5=$stmt5->fetch(PDO::FETCH_ASSOC))
	{
		?>
        <option value="<?php echo $row5['code']; ?>"selected><?php echo $row5['diagnosis']; ?></option>
        <?php
	} 
?>
			...
			?>	
                                ...
                                <div class="form-group row">
                                   <label for="inputEmail3" class="col-sm-12 col-form-label">Other Instructions (In English):</label>
                                    <div class="col-sm-12">
                                        <textarea id="editor1" name="other" class="form-control" placeholder="Enter Product's Comparison"><?php echo $row431['other'];?></textarea>
	...
	removePlugins: 'scayt',
      disableNativeSpellChecker: false
								...
								<div class="form-group row">
                                    ...
                                    <div class="col-sm-12">
                                        <textarea class="form-control" name="other_b" id="ha" rows="3" placeholder="Other Instructions (In Bangla)"><?php echo $row431['other_b'];?></textarea>
                                    </div>
