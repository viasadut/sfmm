...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('doctor','diet','physio','moopd')"; 
...
$query40 = "SELECT COUNT(id) FROM user where uname= '$user' and utype in ('doctor','physio','diet','moopd')"; 
...
$inves = str_replace("'", "''",$_REQUEST['inves']);
$charge = $_REQUEST['charge'];
$price = (int)$_REQUEST['charge'];
//$pnote = $_REQUEST['pnote'];
$pnote = str_replace("'", "''",$_REQUEST['pnote']);
...
//$sel95="SELECT * FROM privilege WHERE `pname`='$pp' and dname in('$full','common') and status in ('Approved','Waiting For CFO Approval');";
$sel95="SELECT * FROM privilege WHERE `pname`='$pp' and dname in('$full') and status in ('Approved','Waiting For CFO Approval');";
$result95 = mysqli_query($con,$sel95);


$sel96="SELECT COUNT(id) FROM icnote WHERE `pmrn`='$pmrn' and user in('$full') and infusion='$infu' and daten='$daten';";
$result96 = mysqli_query($con,$sel96);
$row96 = mysqli_fetch_array($result96);
$visit=$row96['COUNT(id)'];
...
if($res95=mysqli_num_rows($result95)==0)
{
...
else if($full =='$adoc' and $charge!='' and $type_count>0 and $visit<=0){
  ...
  values ( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$adate1','$inves','$infu','$pnote','$full','Data Updated','Doctor','$charge','$vtype','$daten','$entry_time','$dcode','$code','$ip','$op','$app_con','$ccentre')";
  ...
  ('Consultant Note','$pmrn','$eid','$full','$daten1','1','imoidocnotedoc','all')";
  //mysqli_query($con,$ins_query2);
  $result_p = mysqli_query($con, $ins_query2) or die(mysqli_error());
  ...
  if($decoded_response['invoice_no']!='' and $decoded_response['invoice_id']!=''){
    ...
    //header("Location:$url");
     ...
     $api_query = "update icnote set api_status='1', invoice_no='$invoice_no' where id='".$last_id."'"; 
  ...
  header("Location:idocnote?pmrn=$pmrn&eid=$eid");
...
else if($full !='' and $rtype=='Continuous' and $charge!='' and $type_count>0 and $visit<=0){
...
$sql = "insert into icnote (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`inves`,`infusion`,`pnote`,`user`,`status`,`ugroup`,`charge`,`vtype`,`daten`,`entry_time`,`dcode`,`ccode`,`ip`,`op`,`app_con`,`ccentre`) values 
( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$adate1','$inves','$infu','$pnote','$full','Data Updated','Doctor','$charge','$vtype','$daten','$entry_time','$dcode','$code','$ip','$op','$app_con','$ccentre')";
...
//$ins_query="insert into icnote (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`inves`,`infusion`,`pnote`,`user`,`status`,`ugroup`,`charge`,`vtype`,`daten`,`entry_time`) values ( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$adate1','$inves','$infu','$pnote','$full','Data Updated','Doctor','$charge','$vtype','$daten','$entry_time')";
...
<td colspan="2" align="center"><select name="infu"  value="" required class="js-example-basic-single">
...
<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='vtype' required>
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
           /* $query = "select * from `privilege` where dname='common' and status in ('Approved','Waiting For CFO Approval')";
        ...
        <?php*/
            require('db1.php');
            $uname = '';
            $query = "select * from `privilege` where dname='$full' and status in ('Approved','Waiting For CFO Approval')";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
            <option value="<?php echo $row['pname'].','.$row['id']; ?>"><?php echo $row['pname']; ?></option>
...
let idleTime = 0;
const idleMax = 5; // Logout after 10 minutes of inactivity
