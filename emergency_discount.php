...
<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('doctor','staff','mng','ot','endo','imo','mofficer','nurse','emergency','moopd','call','bill','billin','diet','physio','mrd','adminmng','outdoc')"; 
...
$query4 = mysqli_query($db,"select COUNT(id) from einves where pmrn='$pmrn' and eid='$eid' and rstatus='RECEIVED'");
$data4= mysqli_fetch_assoc($query4);
echo $inves_num=$data4['COUNT(id)'];
...
$taka=$_REQUEST['taka']/$inves_num;
$percentage=$_REQUEST['percentage']/100;
...
$eqty2 = $_POST['eqty1_'.$updateid];
			...
			$strSQL = "update ecnote set discount='$eqty2' where id='".$updateid."'";
...
$selected = mysqli_select_db($dbhandle,"sfmmkpjnew") 
  or die("Could not select examples");
	...
	$opd_procedure = "SELECT SUM(discount) FROM ecnote where pmrn='$pmrn1' and eid='$eid1'"; 
...
$doc_dis=	$opd_procedure_data['SUM(discount)'];
			...
			$strSQL1 = "update emergency set doc_dis='$doc_dis',hos_discount='$hos_dis',lab_dis='$lab_dis',rad_dis='$rad_dis' where pmrn='$pmrn1' and eid='$eid1'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery1 = mysqli_query($objConnect,$strSQL1);
			
						$strSQL4 = "update einves set discount='$taka' where pmrn='$pmrn1' and eid='$eid1' and rstatus='RECEIVED'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery4 = mysqli_query($objConnect,$strSQL4);
			
$url="billpall?pmrn=$pmrn&id=$id&eid=$eid";
...
<!-- Form Title -->
		...
		<input name="mname" type="text"  size="70" value="LAB DISCOUNT"readonly  style="font-size:20px; color:green;font-weight:bold;">
<?php

$tt=$charge-($charge*($data59["lab_dis"]/100));
 if($data59['lab_dis']>0){
	
	
		echo '<label for="age"><strong>Charge After Discount :</strong></label>
<input name="peradd" type="text" size="70" value="'.$tt.'"readonly  style="font-size:20px; color:green;font-weight:bold;">';}
...
else
	{
		...
		echo '<label for="age"><strong>Original Price:</strong></label>
<input name="peradd" type="text" size="70" value="'.$tt.'"readonly  style="font-size:20px; color:green;font-weight:bold;">';}
		?>
		
		<label for="age"><strong>LAB Discount :</strong></label>
		<input type="number"name="lab_dis" size="70" value="<?php echo $data['lab_dis'];?>"required  style="font-size:20px; color:red;font-weight:bold;">
		
		<label for="age"><strong>RAD Discount :</strong></label>
		<input type="number"name="rad_dis" size="70" value="<?php echo $data['rad_dis'];?>"required  style="font-size:20px; color:red;font-weight:bold;">
		
		<label for="age"><strong>HOSPITAL Discount :</strong></label>
		<input type="number"name="hos_dis" size="70" value="<?php echo $data['hos_discount'];?>"required  style="font-size:20px; color:red;font-weight:bold;">
		
		<td colspan="4" align="center" style="color:red; font-weight:bold;font-size:18px"><label><strong>Frequency </strong></label>
<select name="discount_type" value="" class="style1" id="pmrn" onchange="GetDetail(this.value)">
...
<td  colspan="4"align="center">
   ...
   <input name="taka" type="number" class="style1" id="sdate12" placeholder="Discount In Taka" max="1000">
   
						  
						  
					

<input type="number" name="percentage" id="sdate1" class="style1" hidden placeholder="Discount In Percentage" max="3">
...
{ ?>    <tr>
	  ...
	  <td align="center"colspan="1"><?php echo $row["visit"]; ?></td>  
<?php
$id = $row['id'];
?>	  
<td align="center"colspan="2"><input name="eqty1_<?= $id ?>" type="text" value="<?php echo $row['discount'];?>" id="eqty1" required ></td>                            
