...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('opdpro','pharmacy')"; 
...
$sel96="SELECT * FROM medi_stock WHERE `code`='$code' and add_qty!='0' order by asc limit 1;";
	...
	else if($tqty<$tprice)
{
    ...
    echo '</script>';
			...
			$ins_query2="update medicine set `tqty`='$m_qty' where code='$code'";
...
$sel97="SELECT * FROM medi_stock WHERE `code`='$code' and add_qty!='0' order by asc limit 1;";
...
<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='code' required style="font-weight: bold;font-size:22px;color:green">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `medi_stock` where add_qty!='0' and location='OPD PROCEDURE ROOM'";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
            <option value="<?php echo $row['g_name'].','.$row['rfid']; ?>"><?php echo $row['g_name'].','.$row['rfid']; ?></option>
        <?php } ?>
