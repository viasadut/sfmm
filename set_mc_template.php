...
$q = "SELECT COUNT(id) FROM mc_report_format where type= '$type'"; 
...
{
$ins_query_format="insert into mc_report_format (`dname`,`dcode`,`inves`,`add_by`,`add_time`,`status`,`type`,`iname`) 
values ('$full', '$fullname','$cdetails','$fullname','$add_time','Waiting','$type','$ptemp')";
mysqli_query($con,$ins_query_format);


$url = "mc_format_view?type=$type&dname=$full";
header("Location: $url");
...
$ins_query_format="update mc_report_format set `inves`='$cdetails',`status`='Waiting',`add_time`='$add_time' where type='$type'";
mysqli_query($con,$ins_query_format);
...
<td colspan="20">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query_1 = "select * from `mc_report_format` where status in ('Active','Waiting','Approved')";
...
//var variables = "pmrn=str&string=$pd";
				...
				xmlhttp.open("GET", "pro_mc_for.php?pmrn=" + str + "&porder=<?php echo $pd;?>", true);
