...
$url = "endochargenurse2_new.php?pmrn=$pmrn&eid=$eid&ID=$id1";

$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM endohoscharge1 WHERE `id`='$id';");
...
$query = "DELETE FROM endohoscharge1 WHERE id=$id"; 
...
values ('$id','DR','$tb_data','$date','$tprice','ENDOSCOPY')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$id','CR','615100','$date','$tprice','ENDOSCOPY')";
mysqli_query($con,$ins_query2) or die(mysql_error());



header("Location:endochargenurse2_new.php?pmrn=$pmrn&eid=$eid&ID=$id1");
