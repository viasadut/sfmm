...
$sel95="SELECT COUNT(id) FROM medi_stock WHERE `code`='$code' and location='2nd Fl Pharmacy' and add_qty>0;";
$result95 = mysqli_query($con,$sel95);
$b_chk=mysqli_fetch_assoc($result95);
$count_qty=$b_chk['COUNT(id)'];


$sel95w="SELECT COUNT(id) FROM medi_stock WHERE `code`='$code' and location='2nd Fl Pharmacy' and batch_no='$batch_no' and add_qty>0;";
$result95w = mysqli_query($con,$sel95w);
$b_chkw=mysqli_fetch_assoc($result95w);
$count_qtyw=$b_chkw['COUNT(id)'];


$sel95wz="SELECT * FROM medi_stock WHERE `code`='$code' and location='2nd Fl Pharmacy' and batch_no='$batch_no' order by id desc limit 1;";
...
if($g_name=='')
    ...
    echo 'alert("Generic Name Is Empty"); ';
    echo '</script>';
...
else if($count_qty<=7 and $count_qtyw<=0 and $add_qty>0)
	
	{
		
$ins_query1="insert into medi_stock (`code`,`g_name`,`b_name`,`add_qty`,`given_qty`,`exdate`,`u_price`,`t_price`,`batch_no`,`rfid`,`remarks`,`add_by`,`add_time`,`location`,`location1`,`sno`)
 values ('$code','$g_name','$b_name','$add_qty','$add_qty','$exdate','$u_price','$t_price','$batch_no','$rfid','$remarks','$user','$ittime1','2nd Fl Pharmacy','$location','$rfid')";
...
header("Location: medi_bar_new?g_name=$g_name&rfid=$rfid");
...
$ins_query1="update medi_stock set `add_qty`='$new_qty',`given_qty`='$new_given',`exdate`='$exdate',`u_price`='$u_price',`t_price`='$t_price',`remarks`='$remarks',`add_by`='$user',`add_time`='$ittime1' where id='$row_id' and location='2nd Fl Pharmacy'";
mysqli_query($con,$ins_query1) or die(mysql_error());
...
<head>
  <meta charset="UTF-8">
  <title>2nd Floor Pharmacy Add</title>
...
<!-- Form Title -->
		<h1>Medicine Top Up Panel (2nd Fl Pharmacy)</h1>
...
<td colspan="3" ><input type="number" name="add_qty" id="" required value="" style="font-weight: bold;font-size:22px;color:green" min="1" ></td>
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `medi_stock` where location='2nd Fl Pharmacy' and add_qty>0";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
      ...
      <input name="rfid" type="text" size="70" style="text-transform:uppercase" value="<?php echo $runningTime.'13';?>"required readonly>
	      </tr>  
	  
	  <tr>
	  <label for="age"><strong>Expire Date :</strong></label>
      <input type="date" name="exdate" placeholder="Select Date" size="15" required min="<?php echo date('Y-m-d');?>">
	  <tr>
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "stock_medi_2.php?pmrn=" + str, true);
