...
$query = mysqli_query($db,"select * from birth where id='$id'");
...
$this->ln(2);
$this->SetFont('Arial','B',8);
$this->Cell(273,10,'Contact Numbers:  Ambulance:  +880244077029, +8801791987466, Appointments: +880244077030, +8801703788561 (SFMMKPJSH/NSG/MR-30)',0,0,'C');
...
$pdf->Cell('48',5,$idate1,0,0,'L');

$pdf->ln(14);
$pdf->SetFont('Arial' , 'BI' , 16);
$pdf->Cell('253',5,'THIS IS TO CERTIFY THAT THE BABY',0,1,'C');
$pdf->ln(7);
$pdf->Cell('253',5,$data['bname'],0,1,'C');
$pdf->ln(7);

$pdf->Cell('253',5,'Of  Mrs. '.'  '.$data['mname'].' (Mother name)',0,1,'C');
$pdf->ln(7);
$pdf->Cell('253',5,'and Mr. '.'  '.$data['fname'].' (Father name)',0,1,'C');

$pdf->ln(7);
$pdf->Cell('253',12,'Sex: '.'  '.$data['sex'].' , BIRTH WEIGHT: ' .$data['weight'].' KG',0,1,'C');
$pdf->Cell('253',12,'WAS BORN IN THIS HOSPITAL AT  '.$data['btime'].' ON '.$bdate1,0,1,'C');
$pdf->Cell('253',12,'UNDER OBSTETRICIAN- '.$data['dname'],0,1,'C');
$pdf->Cell('253',12,'AND PEDIATRICIAN- '.$data['dname1'],0,1,'C');
...
$pdf->Cell('203');
$pdf->Cell('53',5,$data1['fullname'],0,1,'C');
