...
if(empty($_REQUEST['update']))
{
    ...
    echo '</script>';
			...
			$qq = mysqli_query($db,"select * from medi_stock1 where id='".$updateid."'");
    ...
    $qq1 = mysqli_query($db,"select * from medi_stock where batch_no='".$b_no."' and code='$code_1' and location='Pharmacy'");
...
$select = $_REQUEST['select'];
		...
		if($code_1 !='' and $b_no !='' and $select=='Returned' and $b_no1!='')
		{

$sel96="SELECT * FROM medi_stock WHERE `code`='$code_1' and batch_no='$b_no' and location='Pharmacy' order by id asc limit 1;";
...
$m_id=$b_chk_m['id'];
			...
			$ins_query3="update medi_stock set `add_qty`='$m_qty1' where id='$m_id'";
...
$strSQL1 = "update medi_stock1 set status='Received' where id='".$updateid."'";
    ...
    if($code_1 !='' and $b_no !='' and $select=='Returned' and  $b_no1=='')
      ...
      $strSQL2 = "update medi_stock1 set status='Received' where id='".$updateid."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
$strSQL1 = "insert into medi_stock (code,location,g_name,b_name,add_qty,batch_no,rfid,add_time,add_by,sno,remarks)
 values 
('$code_1','".$dd["location"]."','".$dd["g_name"]."','".$dd["b_name"]."','".$dd["add_qty"]."','".$dd["batch_no"]."','".$dd["rfid"]."','$add_time','$user','".$dd["sno"]."','Return')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
		$objQuery1 = mysqli_query($objConnect,$strSQL1);
		}
		
		else if($code_1 !='' and $b_no !='' and $select=='Expired')
...
<tr>
	  ...
	  <td colspan="7" align="center"><strong>Instruction</strong></td>
	    ...
	    <?php 
                    $query = "select * from medi_stock1 where rfid='$sno' and req_loc='$req_loc' and status='Returned'";
...
<input name="eqty1_<?= $id ?>" type="text" value="<?php echo $row["add_qty"]; ?>" id="eqty1" required  readonly <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>></td>
