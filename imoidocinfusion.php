...
$sel90="SELECT * FROM medicine WHERE `mname`='$infu' and status='Active';";
$result90 = mysqli_query($con,$sel90);
$row5 = mysqli_fetch_assoc($result90);
$u=$row5['uprice'];
$i_code=$row5['code'];
...
$sel93="SELECT * FROM medicine WHERE `mname`='$add' and status='Active';";
$result93 = mysqli_query($con,$sel93);
$row6 = mysqli_fetch_assoc($result93);
$u1=$row6['uprice'];
$a_c_1=$row6['code'];

$sel94="SELECT * FROM medicine WHERE `mname`='$add1' and status='Active';";
$result94 = mysqli_query($con,$sel94);
$row7 = mysqli_fetch_assoc($result94);
$u1=$row7['uprice'];
$a_c_2=$row7['code'];
...
$ins_query="insert into iinfusion (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`status`,`alert`,`status1`,`odate1`,`otime`,`addi`,`otime1`,`infu1`,`add1`,`qty1`,`qty2`,`pstatus`,`ortime`,`uprice`,`code`,`a_d_c_1`,`a_d_c_2`) 
values ( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$date','$infu','$user','Data Updated','$alert','Active','$adate','$time',' $add','$otime1','$infu1',' $add1','$qty1','$qty2','Ordered','$ortime','$uf','$i_code','$a_c_1','$a_c_2')";
...
<option value=''>-Select Infusion-</option>
				<?php 
			$sql = "select * from `medicine` where pre='infusion' and status='Active'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
...
<option value=''>-Select Additive-</option>
				<?php 
			$sql = "select * from `medicine` where pre='injection' and status='Active'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->mname."'>".$row->mname."</option>";
...
<tr>
<td colspan="20"align="right"><font size="4.5" color="#FF0000"><strong><a href="addpharmedidoctorinfuimo?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>">View Tomorrow's Infusion</a></strong>&nbsp;&nbsp;<a href="idocinfusiondateimo?pmrn=<?php echo $pmrn; ?>&eid=<?php echo $eid; ?>"><b>(See Datewise Infusion List)<b></a>&nbsp;<button type="submit" name="Submit">Confirm</button></td>
