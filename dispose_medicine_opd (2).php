...
<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('rad','staff','lab')"; 
...
$sel96="SELECT * FROM medi_stock WHERE `sno`='$dilu' and location='Out Patient Services';";
...
$sel91="SELECT * FROM medi_stock WHERE `sno`='$dilu' and add_qty!='0' and location='Out Patient Services';";
...
$query1="update medi_stock set `add_qty`='$m_qty1' where `sno`='$dilu' and location in ('Out Patient Services')";
...
$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`rfid`,`status`,`location`,`code`) values
			('$g_name','1','$u_price','$u_price','$user','$adate','$bb_name','$dilu','Finished','Out Patient Services','$code')";
...
<!-- Form Title -->
  ...
  <datalist id="stock">
				...
				<?php 
			$sql76 = "select * from `medi_stock` where add_qty>0 and location in ('RAD')";
