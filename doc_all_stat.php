...
$sqlRole = "SELECT COUNT(utype) AS c FROM user WHERE '$role' IN ('mng','ddf','doctor')";
...
======================== */
$sqlname = "SELECT * FROM user WHERE uname='$user'";
$resName = mysqli_query($con, $sqlname) or die(mysqli_error($con));
...
$isFirstLoad = ($_SERVER['REQUEST_METHOD'] === 'GET');
$isSearch = isset($_POST['bsearch']) || $isFirstLoad;

$bt = trim((string)($_POST['bt'] ?? ($rowName['fullname'] ?? '')));
$stdate_raw = $_POST['stdate'] ?? date('d-M-Y', strtotime($firstDayOfMonth));
$endate_raw = $_POST['endate'] ?? date('d-M-Y', strtotime($today));
...
/* =========================================================
   LOAD DATA ON SEARCH / FIRST PAGE LOAD
========================================================= */
if ($isSearch && !$errors) {
    ...
    $glance_counts['IPD_EMER'] = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM inpatient WHERE anew BETWEEN ? AND ? AND adoc=? AND emerid!='0'",
        [$stdate, $endate, $bt],
        "sss"
    ...
    $glance_counts['Anaes_OT'] = (int)(one(
        $con,
    ...
    $glance_counts['Endoscopy'] = (int)(one(
        $con,
    ...
    $glance_counts['Anaes_Endo'] = (int)(one(
        $con,
    ...
    $glance_counts['ProcedureRoom'] = (int)(one(
        $con,
    ...
    $glance_counts['RadiologyReport'] = (int)(one(
        $con,
    ...
    $lab1 = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM alltest WHERE date1 BETWEEN ? AND ? AND cby=? AND type='lab'",
        [$stdate, $endate, $dd],
        "sss"
    )['c'] ?? 0);
    ...
    $lab2 = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM iinves WHERE dis_date BETWEEN ? AND ? AND conby=? AND type='lab'",
        [$stdate, $endate, $dd],
        "sss"
    )['c'] ?? 0);
    ...
    $lab3 = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM einves WHERE ndate BETWEEN ? AND ? AND conby=? AND type='lab'",
        [$stdate, $endate, $dd],
        "sss"
    )['c'] ?? 0);
    ...
    $glance_counts['ECG'] = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM ecg WHERE datenew BETWEEN ? AND ? AND dname1=? AND status1='Confirmed'",
        [$stdate, $endate, $bt],
        "sss"
    )['c'] ?? 0);

    $glance_counts['ECHO'] = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM echo WHERE datenew BETWEEN ? AND ? AND dname=? AND status1='Confirmed'",
        [$stdate, $endate, $bt],
        "sss"
    )['c'] ?? 0);

    $glance_counts['ETT'] = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM ett WHERE date2 BETWEEN ? AND ? AND dname=? AND status1='Confirmed'",
        [$stdate, $endate, $bt],
        "sss"
    )['c'] ?? 0);

    $glance_counts['HistoDone'] = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM histo WHERE date1 BETWEEN ? AND ? AND dname1=? AND status='REPORT DONE'",
        [$stdate, $endate, $bt],
        "sss"
    )['c'] ?? 0);

    $glance_counts['FNACDone'] = (int)(one(
        $con,
        "SELECT COUNT(pmrn) AS c FROM fnacreport WHERE date5 BETWEEN ? AND ? AND dname=? AND status='SEEN'",
        [$stdate, $endate, $bt],
        "sss"
    )['c'] ?? 0);

    /* ---------- INCOME PIECES ---------- */
    $income_ipd = (float)(one(
        $con,
    ...
    $manual_charge = (float)(one(
        $con,
        "SELECT COALESCE(SUM(pdos),0) AS s
         FROM ipd_extra_charge
         WHERE date1 BETWEEN ? AND ?
           AND delete_status!='1'
           AND medi1 IN ('ANAESTHETIST','PROCEDURE','WARD REVIEW','SURGERY','CONSULTATION')
           AND doc_code=?",
        [$stdate, $endate, $new_doc_code],
        "sss"
    )['s'] ?? 0);

    $manual_discount = (float)(one(
        $con,
        "SELECT COALESCE(SUM(pdos),0) AS s
         FROM ipd_extra_charge
         WHERE date1 BETWEEN ? AND ?
           AND delete_status!='1'
           AND medi1 IN ('DISCOUNT')
           AND doc_code=?",
        [$stdate, $endate, $new_doc_code],
        "sss"
    )['s'] ?? 0);

    $income_endo_ipd = (float)(one(
        $con,
    ...
    $spd_ecg = (int)(one(
        $con,
        "SELECT COUNT(*) AS c FROM ecg_test WHERE datenew BETWEEN ? AND ? AND con_by=? AND ron='ECG'",
        [$stdate, $endate, $dd],
        "sss"
    )['c'] ?? 0);
    ...
    $spd_e2d = (int)(one(
        $con,
        "SELECT COUNT(*) AS c FROM ecg_test WHERE datenew BETWEEN ? AND ? AND con_by=? AND ron='ECHO-2D'",
        [$stdate, $endate, $dd],
        "sss"
    )['c'] ?? 0);
    ...
    $spd_col = (int)(one(
        $con,
        "SELECT COUNT(*) AS c FROM ecg_test WHERE datenew BETWEEN ? AND ? AND con_by=? AND ron='ECHO-COLOR DOPPLER'",
        [$stdate, $endate, $dd],
        "sss"
    )['c'] ?? 0);
    ...
    $discount_parts = [
        'Doctor Discount' => $doc_discount,
        'Manual Discount Entry' => $manual_discount,
    ];

    $total_income = array_sum($income_parts);
    $total_discount = array_sum($discount_parts);
    ...
    $rows_doc_discount = all(
        $con,
        "SELECT edate, discount,pmrn
         FROM doc_dis
         WHERE edate BETWEEN ? AND ? AND dname=?
         ORDER BY edate ASC",
        [$stdate, $endate, $bt],
        "sss"
    );

    $rows_manual_discount = all(
        $con,
        "SELECT date1, medi1, pdos, pmrn
         FROM ipd_extra_charge
         WHERE date1 BETWEEN ? AND ?
           AND delete_status!='1'
           AND medi1='DISCOUNT'
...
<script>
    $(function(){
        var minAllowedDate = new Date(2026, 2, 31); // 31 March 2026
...
<div class="wrap">
    <h2 class="page-title text-center">MONTH WISE CONSULTANT STATEMENT</h2>
        ...
        <div class="card-body">
            ...
            <form method="POST" class="row" style="margin:0;">
                ...
                <div class="col-md-4">
                    ...
                    <input list="doctor_list" class="form-control" name="bt" value="<?= h($bt) ?>" placeholder="Type or select consultant" readonly>
                    <datalist id="doctor_list">
                        <option value="">-Select-</option>
                        <?php foreach($doctorList as $doc): ?>
                            <option value="<?= h($doc['dname']) ?>"></option>
                        <?php endforeach; ?>
                    </datalist>
                </div>
    ...
    <?php if ($isSearch && !$errors): ?>
        <div class="card">
            <div class="card-header">
                Summary
                <span class="badge-soft"><?= h($bt) ?></span>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <h4 class="section-title">Discount Breakdown (BDT)</h4>
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>Head</th>
                            ...
                            <tbody>
                                <?php foreach($discount_parts as $k => $v): ?>
                                    <tr>
                                        <td><?= h($k) ?></td>
                                        <td><?= money($v) ?></td>
                                    </tr>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if ($rows_opd): foreach($rows_opd as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if ($rows_ipd): foreach($rows_ipd as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['dis_date'] ?? '') ?></td>
                                <td><?= money($r['charge'] ?? 0) ?></td>
                            </tr>
                        <?php endforeach; else: ?>
                            <tr><td colspan="5" class="text-center">No data found</td></tr>
                        <?php endif; ?>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if ($rows_ae): foreach($rows_ae as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['daten'] ?? '') ?></td>
                                <td><?= money($r['visit'] ?? 0) ?></td>
                            </tr>
                        <?php endforeach; else: ?>
                            <tr><td colspan="5" class="text-center">No data found</td></tr>
                        <?php endif; ?>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if ($rows_ot): foreach($rows_ot as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['dis_date'] ?? '') ?></td>
                ...
                <h4 class="section-title">Procedure (IPD)</h4>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        <?php if ($rows_proc_ipd): foreach($rows_proc_ipd as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['date1'] ?? '') ?></td>
                                <td><?= money($r['procharge'] ?? 0) ?></td>
                            </tr>
                        <?php endforeach; else: ?>
                            <tr><td colspan="5" class="text-center">No data found</td></tr>
                        <?php endif; ?>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if ($rows_proc_opd): foreach($rows_proc_opd as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if ($rows_endo): foreach($rows_endo as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                ...
                <h4 class="section-title">Cathlab</h4>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        <?php if ($rows_cath): foreach($rows_cath as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['dis_date'] ?? '') ?></td>
                ...
                <h4 class="section-title">SPD (ECG / ECHO)</h4>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        <?php if ($rows_spd): foreach($rows_spd as $i => $r): ?>
                            <?php
                                $spdAmount = 0;
                                if (($r['ron'] ?? '') === 'ECG') $spdAmount = 100;
                                elseif (($r['ron'] ?? '') === 'ECHO-2D') $spdAmount = 1000;
                                elseif (($r['ron'] ?? '') === 'ECHO-COLOR DOPPLER') $spdAmount = 1500;
                            ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pname'] ?? '') ?></td>
                ...
                <h4 class="section-title">Doctor Discount Details</h4>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        <?php if ($rows_doc_discount): foreach($rows_doc_discount as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['edate'] ?? '') ?></td>
                                <td><?= money($r['discount'] ?? 0) ?></td>
                            </tr>
                        <?php endforeach; else: ?>
                    ...
                    <tfoot>
                        <tr>
                        <th></th>    
                        <th colspan="2" class="text-right">Total Doctor Discount</th>
                            <th><?= money($discount_parts['Doctor Discount'] ?? 0) ?></th>
                        </tr>
                    </tfoot>
                </table>

                <h4 class="section-title">Manual Discount Entry Details</h4>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        <?php if ($rows_manual_discount): foreach($rows_manual_discount as $i => $r): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= h($r['pmrn'] ?? '') ?></td>
                                <td><?= h($r['date1'] ?? '') ?></td>
                                <td><?= h($r['medi1'] ?? '') ?></td>
                                <td><?= money($r['pdos'] ?? 0) ?></td>
                            </tr>
                        <?php endforeach; else: ?>
                            <tr><td colspan="4" class="text-center">No data found</td></tr>
                        <?php endif; ?>
                    ...
                    <tfoot>
                        <tr>
                        <th></th>    
                            <th colspan="3" class="text-right">Total Manual Discount</th>
                            <th><?= money($discount_parts['Manual Discount Entry'] ?? 0) ?></th>
                        </tr>
                    </tfoot>
                </table>
