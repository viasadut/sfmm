...
$nn=date('Y-m-d',strtotime("+1 days"));


$dd= date('m/d/Y',strtotime("+1 days")); 
$dd1= date('Y-m-d',strtotime("+1 days")); 
$retime=date('Y-m-d');
...
$sel96="SELECT * FROM imedi3 WHERE `pmrn`='$pmrn' and eid='$eid' and pstatus='Served' and status !='Cancel' and ndate='$nn' order by pdate desc limit 1;";
...
$pno=$b_chk_m['pno'];
		 ...
		 $mpdf = new \Mpdf\Mpdf([
                ...
                $query1 = mysqli_query($con,"select * from imedi3 where pmrn='$pmrn' and eid='$eid' and pstatus ='Served' and status !='Cancel' and pno='$pno' and odate='$dd' order by infusion asc");
                //$count=1;
