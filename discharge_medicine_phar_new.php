...
<?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row['qty']*$row1['uprice'];?>
	 ...
	 <td align="center" colspan="1"><input type='checkbox' name='update[]' value='<?= $id ?>' checked hidden></td>						
  	   ...
  	   <td align="center"colspan="1"><input type="button" name="edit" value="Update List" id="<?php echo $id; ?>" class="btn btn-info btn-xs edit_data" /></td>  		  	  
...
<label for="age"><strong>Brand Name:</strong></label>						
		...
		<label for="age"><strong>U.Price:</strong></label>	
...
<input type="number" name="tqty" id="tqty" required value="" readonly style="font-weight: bold;font-size:16px;color:green;width:80px">
  ...
  $('#add_data_Modal').on('shown.bs.modal', function() {
    $('input[name="code"]').focus();
	  ...
	  $('#add_data_Modal').modal({backdrop: 'static', keyboard: false})  
           ...
           $.ajax({  
                url:"medi_dispense_phar_discharge.php",  
                method:"POST",  
                ...
                success:function(data){  
					 ...
					 //$('#ins').val(data.duration); 
					 $('#add2').val(data.code); 
					 //$('#uuprice').val(data.u_price); 
...
$('#lrfid').val(data.sno);					 
	  ...
	  //var tr = $('#sqty').val() + $('#gqty').val();
           ...
           {  
                $.ajax({  
                     url:"new_phar_dispense3_discharge.php",  
                     method:"POST",  
