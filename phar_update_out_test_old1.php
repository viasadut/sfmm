...
if($eqty5 >= $eqty2 and $mqty=='' and $mm_qty>=$eqty2 and $eqty2>0)
	
	{
			...
			$strSQL1 = "update medi_stock set status='Served',add_qty='$eqty2',given_qty='$eqty2',exdate='$exdate',u_price='$p_price',t_price='$u_price',batch_no='$bb_no' where id='".$updateid."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery1 = mysqli_query($objConnect,$strSQL1);
	
	$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`,`pmrn`,`pname`,`rfid`) values
			('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','','OPD','$code_1','$p_mrn','$p_name','$rf')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
//header("Location:$url");
	...
	$strSQL2 = "update phar_sale set `qty`='$fqty',`tprice`='$charge_f',`aby`='$user',`adate`='$adate',`brand`='$brand' where id='$r_id'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
<tr>
      ...
      <td colspan="10" align="center"><strong>Medicine Name</strong></td>
     	  ...
     	  <td colspan="3" align="center"><strong>Request_QTY</strong></td>
		  <td colspan="3" align="center"><strong>Available_QTY</strong></td>
      	  <td colspan="3" align="center"><strong>Issue_Qty</strong></td>
...
$new_qty=$sumr['SUM(add_qty)'];					 
     ...
     <td align="center" colspan="1" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $count; ?></td>
      ...
      <td align="center"colspan="10" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["g_name"]; ?></td>
	  
	  
	        
						<td align="center"colspan="3" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>readonly><?php echo $row["add_qty"]; ?></td>
			
		
<td align="center"colspan="3"><input name="eqty2_<?= $id ?>" type="text" value="<?php echo $new_qty;?>" id="eqty2" readonly <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>></td>
