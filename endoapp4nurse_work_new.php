...
$query40 = "SELECT * FROM patient where pmrn= '$pmrn'"; 
...
$sel43="SELECT * FROM endoapp WHERE `dname`='$dname2' and `ndate`='$date22' and dslot='$slot' and status in ('Booked') and machine='$machine';";
$result43 = mysqli_query($con,$sel43);


$db = mysqli_connect('localhost','root','Godiloveu16');
	mysqli_select_db($db,'sfmmkpjnew');
$query444d = mysqli_query($db,"select * from doctor where dname='$tname23'");
...
else
{
	...
	$sql = "insert into endopapp (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`dreffer`,`machine`) values 
	('$name', '$pmrn','$pphone','$padd','$dname','$date22','$slot1','NOT SEEN','$diff1','$psex','$tname23','$machine')";
	



//$ins_query="insert into endopapp (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`dreffer`,`machine`) values 
//('$name', '$pmrn','$pphone','$padd','$dname','$date22','$slot1','NOT SEEN','$diff1','$psex','$tname23','$machine')";
...
foreach($slot as $group_member){
//$update="update endoapp set status='Booked' where `ndate`='$date22' and `dslot`='$group_member' and machine='$machine'";
$update="update endoapp set status='Booked' where `ddate`='$date22' and `dslot`='$group_member'";
...
$data = array(
  ...
  "IN_INITIAL_BED_NO_FK"=> "Endoscopy-01",
  "IN_CURR_BED_NO_FK"=> "Endoscopy-01",
  "IN_CURR_WARD_NO_FK"=> "Endoscopy",
...
$adm_id=$decoded_response['OUT_ADMISSION_NO_PK'];
 if($decoded_response['OUT_ADMISSION_NO_PK']!='' and $decoded_response['OUT_ADMISSION_UID']!=''){
	 ...
	 $update_in="update endopapp set api_status='1', OUT_ADMISSION_NO_PK='$adm_id' where `ID`='$last_id'";
...
<!-- Form Title -->
        ...
        <fieldset>
			...
			<select class="js-example-basic-single" name="tname23"   id="dname" style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:350px" required readonly>
			
						<option value='<?php echo $row39['dname'];?>'><?php echo $row39['dname'];?></option>
...
<input name="pmrn" id="pmrn"onkeyup="GetDetail(this.value)" type="text" placeholder="MRN" value="<?php echo $row40['pmrn'];?>" style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:300px" readonly>
      
	  

			<label for="age"><strong>Name :</strong></label>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="name" id="pname" type="text" value="<?php echo $row40['pname'];?>" required readonly style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:300px">
 	  <label for="age"><strong>ADDRESS :</strong></label>
      <input name="padd" id="padd" type="text" size="85" value="<?php echo $row40['padd'];?>" required readonly style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:330px">


<label for="age"><strong>District :</strong></label>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select name="dis" id="dis" class="style1" placeholder="District"  style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:200px"> 
		
		
		<option value="<?php echo $row40['dis'];?>"><?php echo $row40['dis'];?></option>
...
<option value='Sylhet'>Sylhet</option> 
	  ...
	  <label for="age"><strong>Gender :</strong></label>
		...
		<option value="<?php echo $row40['psex'];?>"><?php echo $row40['psex'];?></option>
	 ...
	 <input name="pphone" type="text" id="pphone" placeholder="Phone No"value="<?php echo $row40['pphone'];?>" required readonly style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:200px">	  
...
<input name="dd" id="dd" type="text" maxlength="2" size="1" value="<?php echo $te;?>"  readonly required placeholder="DD" style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:60px">	/

<input name="mm" id="mm" type="text" maxlength="2" size="1" value="<?php echo $te1;?>"  readonly required placeholder="MM" style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:60px"> /	

<input name="yy" id="yy" type="text" maxlength="4" size="1" value="<?php echo $te2;?>"   readonly required placeholder="YYYY" style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:90px">		  
...
<label for="age"><strong>Patient's Type :</strong></label>
		...
		<option value="<?php echo $row40['type'];?>"><?php echo $row40['type'];?></option>
	  ...
	  &nbsp;&nbsp;&nbsp;<label for="age"><strong>Machine Name :</strong></label>
	  	...
	  	<select name="machine" id="machine" onchange="showUser()" class="style1" placeholder="Machine Type"  required style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:100px"> 
...
<label for="mail"><strong>Appointment Date :</strong></label>
									...
									<input type='date' id="daten" name="daten" onchange="showUser()"size="20" style='background-color:lightgreen;font-size:22px;font-weight:bold;color:red;width:200px' min="<?= date('Y-m-d'); ?>" max="<?= date('Y-m-d', strtotime('45 days') ); ?>">  
...
function showUser() {
  ...
  var q = document.getElementById('daten').value;
...
//if (q=="" || g=="") {
  ...
  xmlhttp.open("GET","endo_slot_api.php?q="+q+"&dname2="+g, true);
 // xmlhttp.open("GET","endo_slot_api.php?q="+str+ "&dname2=<?php echo $dname2;?>", true);
