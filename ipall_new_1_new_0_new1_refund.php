...
$sql = "update pms_bill set amount=$payment,receive_amount=$receive_amount, outstanding=$outstanding where billno='$billno'";
...
//$sh = $db1->prepare("UPDATE iinves SET billno=? WHERE pmrn=? and eid=? and rstatus=?");
  //$sh->execute([$billno, $pmrn, $eid, $rstatus]);
...
$sh = $db1->prepare("UPDATE inpatient SET payment=?, room_charge=?, inves_charge=?,
disposable_charge=?, doc_charge=?, pharmacy_charge=?, ot_hos_charge=?, ot_doc_charge=?, ot_phar_charge=?, 
implant=?, extra=?, endo=?, opdpro=?, payment_status=?, dis_medi=?, emer_bill=?, cath_bill=?, msuite_bill=?,receive_amount=?,gtotal=?,service_charge=? WHERE pmrn=? and eid=?");
$sh->execute([$payment, $room_charge, $inves_charge, $disposable_charge, $doc_charge, $pharmacy_charge,
$ot_hos_charge, $ot_doc_charge, $ot_phar_charge, $implant, $extra, $endo, $opdpro, $payment_status, $dis_medi, $emer_all_bill, $cath_bill, $msuite_bill, $receive_amount, $gtotal, $service_charge, $pmrn, $eid]);
...
header("Location:new_bill/new_ipd_payment.php?id=$id&pmrn=$pmrn&dname=$dname&billno=$billno&eid=$eid");  
...
<?php if($data['payment_status']!='PAID' || $data['payment_status']=='PAID'){echo"
