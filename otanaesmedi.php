...
$data59 = mysqli_fetch_assoc($query4);
  $date=date('d/m/Y');
	...
	$ot_charge=$data59['ot_charge_status'];    
?>
...
$sel90="SELECT * FROM otanaesmedi WHERE `pmrn`='$pmrn' and `eid`='$id' and`infusion`='$infu' and odate='$odate1' and `status`='Active';";
$result90 = mysqli_query($con,$sel90);


$sel990="SELECT * FROM medicine WHERE `mname`='$infu';";
$result990 = mysqli_query($con,$sel990);
$result1 = mysqli_fetch_assoc($result990);
$dcode=$result1["code"];
...
$ins_query6="insert into otanaesmedi (`pmrn`,`dname`,`eid`,`infusion`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`price`,`code`) values 
('$pmrn','$dname','$id','$infu','$ddate','$alert','$user','$date1','Active','$root','Rupdated','Ordered','$ortime','$price','$dcode')";
...
<td colspan="2" align="left"><input type="text" class="style" name="alert"value="" ></td>
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from otanaesmedi where pmrn= '$pmrn' and eid='$id'and status !='Cancel' order by `infusion` and `time` asc;";
...
{ echo'
				 ...
				 <td align="center" colspan="2"><a href="otanaesdelete?id3='.$row["id"].'&pmrn='.$pmrn.'&id='.$id.'">DELETE</a></td>';
