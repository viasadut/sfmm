...
<tr>
	  ...
	  <td colspan="7" align="center"><strong>Code</strong></td>
	    ...
	    <?php 
                    $query = "select * from purchase_stock3 where rfid='$sno' and req_loc='$req_loc' and status in ('Pending','Served','Partially Served')";
	  ...
	  $sum = "SELECT SUM(add_qty) FROM purchase_stock3 where code='$ccode' and location='Store'" ;
...
$sum_r = "SELECT SUM(add_qty) FROM purchase_stock3 where code='$ccode' and location='$req_loc' and add_qty>0 and status='Served'" ;
...
<td align="center"colspan="1"style="font-weight: bold;font-size:22px;color:red"><?php echo $new_qty_r;?></td>
 ...
 <td align="center" colspan="1" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row['add_qty']; ?></td>
	 ...
	 <?php
						...
						$pno = $row['sno'];
	                $queryg = "select COUNT(id) from purchase_stock2 where sno='$pno' and code='$medi'";
