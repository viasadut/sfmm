...
$query40 = "SELECT * FROM discharge1 where pmrn= '$pmrn' and eid='$eid'"; 
...
{
$ins_query="update discharge1 set `dname`='$dname',
`discharge`='$discharge',
`ddia`='$ddia',
`surgery`='$surgery',
`ill`='$ill',
...
$url = "edischarge1view?pmrn=$pmrn&eid=$eid" ;
header("Location:$url");
...
$ins_query="insert into discharge1 (`dname`,`pname`,`pphone`,`pmrn`,`page`,`psex`,`discharge`,`ddia`,`surgery`,`ill`,`other`,`plan`, `ddate`,`eid`,`emo`,`dinves`,`date`)
 values ('$dname', '$pname','$pphone','$pmrn','$page','$psex','$discharge','$ddia','$surgery','$ill','$other','$plan','$date1','$eid','$full','$dinves','$ddate1')";
mysqli_query($con,$ins_query) or die("Problem in presnew");
...
<!-- Form Title -->
		...
		&nbsp;&nbsp;<a target='_blank' href="opdradreport?pmrn=<?php echo "$pmrn"; ?>"><b>Record of Radiology Report<b></a>&nbsp;&nbsp;<a target='_blank' href="https://medex.com.bd"><b>Reference Drug Index of Bangladesh(medex.com.bd)<b></a>&nbsp&nbsp<a target='_blank' href="http://192.168.100.254?pmrn=<?php echo "$pmrn"; ?>"><b>LAB REPORT<b></a></td></tr>
				...
				<tr>	  
				 <tr><td colspan="20"><select name="xl[]" multiple="multiple" class="3col active" placeholder="Select Investigations"required>
...
<tr><td colspan="20"><textarea class="form-control" id="exampleTextarea" name="ddia" rows="5" required><?php echo $row40['ddia'];?></textarea></td>  </tr>
<tr><td colspan="20"><label><strong>History Of present Illness (Optional) :</strong></label></td></tr>
<tr><td colspan="20"><textarea class="form-control" id="exampleTextarea1" name="ill" rows="5"><?php 
if($row40!=''){
echo $row40['ill'];} else {echo $row2['pill'].','.$row2['gexam'];}?></textarea></td>  </tr>
