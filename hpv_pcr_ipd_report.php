...
$query3 = $db->query("select * from inpatient where pmrn='$pmrn'");
...
$pdf->SetFont('Times', 'b', 12);
$pdf->Cell(32,5,'Type Of Sample:',0,0,'L');
$pdf->SetFont('Times', '', 12);
$pdf->Cell(100,5,$data->pdos,0,1);
$pdf->ln(5);


$pdf->SetFont('Times', 'b', 12);
$pdf->Cell(100,5,'Test Result:',0,1);
...
if($data->result!=''){
	$pdf->SetFont('Times', '', 12);
	$pdf->Cell(81,5,$data->infusion.':',0,0);
	$pdf->SetFont('Times', 'b', 12);
	$pdf->Cell(100,5,$data->result,0,1);
	$pdf->ln(5);
	}
	
	if($data->hpv16!=''){
		$pdf->SetFont('Times', '', 12);
		$pdf->Cell(21,5,'HPV 16 :',0,0);
		$pdf->SetFont('Times', 'b', 12);
		$pdf->Cell(100,5,$data->hpv16,0,1);
		$pdf->ln(5);
		}
	
		if($data->hpv18!=''){
			$pdf->SetFont('Times', '', 12);
			$pdf->Cell(21,5,'HPV 18 :',0,0);
			$pdf->SetFont('Times', 'b', 12);
			$pdf->Cell(100,5,$data->hpv18,0,1);
			$pdf->ln(5);
			}
	
			if($data->others!=''){
				$pdf->SetFont('Times', '', 12);
				$pdf->Cell(21,5,'Others :',0,0);
				$pdf->SetFont('Times', 'b', 12);
				$pdf->Cell(100,5,$data->others,0,1);
				$pdf->ln(5);
				}

$pdf->SetFont('Times', 'B', 12);
$pdf->Cell(32,5,'Result Interpretation:',0,1);
$pdf->SetFont('Times', '', 12);
$pdf->MultiCell(185,5,$data->ins);
$pdf->ln(5);


$pdf->SetFont('Times', 'B', 12);
$pdf->Cell(32,5,'Test Platform:',0,1);
$pdf->SetFont('Times', '', 12);
$pdf->MultiCell(185,5,$data->ins1);
...
lab_render_approval_footer($pdf, $db, 'BACTERIOLOGY', (isset($data->resultby)?$data->resultby:''), (isset($data->checked_by)?$data->checked_by:''), (isset($data->conby)?$data->conby:''));
$pdf->Ln(10);
