...
$query43r = "SELECT SUM(price) FROM alltest where billdate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('RAD','Rad','rad');"; 
...
$sql ="Select * from iinves where ndate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') and discount>0;";
$re = mysqli_query($con,$sql);
$total=0;
while($rowt=mysqli_fetch_assoc($re)) { 
    $discount    =   $rowt['discount'];
    $price = $rowt['price']- ($rowt['price']*$discount/100);
    $total = $total + $price;
...
$sql_ipd ="Select * from iinves where ndate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') and discount='0';";
$re_ipd = mysqli_query($con,$sql_ipd);
$total_ipd=0;
while($rowt_ipd=mysqli_fetch_assoc($re_ipd)) { 

    $price_ipd = $rowt_ipd['price'];
    $total_ipd = $total_ipd + $price_ipd;
...
$sql_ae ="Select * from einves where ndate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') and discount>0;";
$re_ae = mysqli_query($con,$sql_ae);
$total_ae=0;
while($rowt_ae=mysqli_fetch_assoc($re_ae)) { 
    $discount_ae    =   $rowt_ae['discount'];
    $price_ae = $rowt_ae['price']- ($rowt_ae['price']*$discount_ae/100);
    $total_ae = $total_ae + $price_ae;
...
$sql_ae1 ="Select * from einves where ndate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') and discount='0';";
$re_ae1 = mysqli_query($con,$sql_ae1);
$total_ae1=0;
while($rowt_ae1=mysqli_fetch_assoc($re_ae1)) { 

    $price_ae1 = $rowt_ae1['price'];
    $total_ae1 = $total_ae1 + $price_ae1;
...
$sql_opd ="Select * from alltest where billdate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') and discount>0;";
$re_opd = mysqli_query($con,$sql_opd);
$total_opd=0;
while($rowt_opd=mysqli_fetch_assoc($re_opd)) { 
    $discount_opd    =   $rowt_opd['discount'];
    $price_opd = $rowt_opd['price']- ($rowt_opd['price']*$discount_opd/100);
    $total_opd = $total_opd + $price_opd;
...
$sql_opd1 ="Select * from alltest where billdate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') and discount='0';";
$re_opd1 = mysqli_query($con,$sql_opd1);
$total_opd1=0;
while($rowt_opd1=mysqli_fetch_assoc($re_opd1)) { 

    $price_opd1 = $rowt_opd1['price'];
    $total_opd1 = $total_opd1 + $price_opd1;

    
  // echo "<p> All Sumtotals should display here </p> ";

}
$query44 = "SELECT SUM(price) FROM iinves where ndate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad');"; 
...
$query44r = "SELECT COUNT(infusion) FROM iinves where ndate BETWEEN '$start' and '$end' and type in('lab','LAB','Lab','RAD','Rad','rad') and rstatus='RECEIVED';"; 
...
<!-- Form Title -->
        <table align="center" class="table table-bordered" id="dynamic_field">  
					...
					<td colspan="3"><input list="test" name="bt" id="test1" size='50'>
...
$pay=$income*.05;
//$new_income=$total+$total_ipd+$row43['SUM(price)']+ $row45['SUM(price)'];
$new_income=$total+$total_ipd+$total_ae+$total_ae1+$total_opd+$total_opd1;
$new_pay=$new_income*.05;
...
echo "<br>";
echo $total+$total_ipd;

echo " ,  A&E-  ";	 
echo $row45['SUM(price)'];
echo "<br>";
echo $total_ae+$total_ae1;
...
{ ?>    <tr>
  ...
  <tr> <td colspan="20"></td> </tr>
	...
	$sel_query2="Select * from iinves where ndate BETWEEN '$start' and '$end' and status in ('RECEIVED','DONE') and dname='$bt' and type in('lab','LAB','Lab','RAD','Rad','rad') order by type asc";
...
{ ?>    <tr>
	  ...
	  $tt=$row2['type'];
	  $dis=$row2['discount'];
	  $inq = "SELECT * FROM inpatient where pmrn='$pp' and eid='$ee';"; 
	 
$inr = mysqli_query($con, $inq) or die(mysqli_error());
$ina = mysqli_fetch_assoc($inr);

$lab=$row2["price"]-($row2['price']*($ina['lab_dis']/100));
$rad=$row2["price"]-($row2['price']*($ina['rad_dis']/100));
	  ?>
	  <?php
	  if($tt=='lab' || $tt=='Lab' || $tt=='LAB')
	  {echo'
	  <td align="center">'.$lab.'';}
	  else if($tt=='rad' || $tt=='Rad' || $tt=='RAD') {echo'
	  <td align="center">'.$rad.'';}
