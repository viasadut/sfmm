...
$page=$row3['page'];
$bdate=$row3['bdate'];
$bb=date('d-m-Y', strtotime($bdate));
$bdate1=date_create($bb);

$rr= date('d-m-Y');
$rr1=date_create($rr);

$diff=date_diff($rr1,$bdate1);
...
<?php
$full = $row39['fullname'];
$sel453="SELECT * FROM endopapp WHERE `pmrn`='$pmrn' and eid='$eid' and status='Received' ;";
$result453 = mysqli_query($con, $sel453) or die(mysqli_error());
$row53 = mysqli_fetch_assoc($result453);
$anes=$row53['anes'];
//$page=$row3['page'];
...
$sel90="SELECT * FROM consent2 WHERE `pmrn`='$pmrn' and `eid`='$eid' and `pro`='$pro';";
...
$ins_query="insert into consent2 (`pmrn`,`eid`,`pname`,`dname`,`padd`,`psex`,`tdate`,`pro`,`page`,`user`,`tdate1`,`ward`,`bed`) values 
('$pmrn','$eid','$ppname','$anes','$padd','$psex','$idate','$pro','$diff1','$user','$idate1','Endoscopy','Daycare')";
...
<!-- Form Title -->
		<h1>Consent For Moderate Seadation</h1>
...
<label for="age"><strong>Patient's Name :</strong></label>
	  ...
	  <label for="age"><strong>Anaesthetis Name :</strong></label>
      <input name="dname" type="text" size="80" value="<?php echo $row53['anes']; ?>" readonly>          
...
<td><a target='_blank' href="consentprint2.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>&pro=<?php echo "$pro"; ?>&dname=<?php echo "$dname"; ?>&anes=<?php echo "$anes"; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>  
</form>
