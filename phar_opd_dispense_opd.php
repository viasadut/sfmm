...
$date= date('Y-m-d');
echo $previous_date = date('Y-m-d', strtotime($date . ' -4 day'));
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from phar_sale where adate between '$previous_date' and '$date' and status_opd='' and location in ('OPD','OTC','OPD_DIS') and billno!=''  group by billno desc ;";
...
$row1 = mysqli_fetch_assoc($re);
     ...
     <td align="center"><?php echo $row["billno"]; ?></td>
	  ...
	  <td align="center"><a href="phar_update_new222_new_opd?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&billno=<?php echo $row['billno'];?>">UPDATE</a></td>
