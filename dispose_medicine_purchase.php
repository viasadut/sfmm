...
<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','nurse','pharmacy','ot','endo','opdpro','emergency')"; 
...
$sel96="SELECT * FROM purchase_stock WHERE `sno`='$dilu';";
...
$sel91="SELECT * FROM purchase_stock WHERE `sno`='$dilu' and add_qty!='0';";
...
$query1="update purchase_stock set `add_qty`='$m_qty1' where `sno`='$dilu'";
...
$strSQL2 = "insert into purchase_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`rfid`,`status`,`location`,`code`) values
			('$g_name','1','$u_price','$u_price','$user','$adate','$bb_name','$dilu','Finished','OT','$code')";
...
$ins_query1="insert into othoscharge1 (`medi`,`brand`,`date`,`rfid`,`code`,`ndate`,`location`,`remarks`,`pdos`) values 
('$g_name','$bb_name','$date1','$dilu','$code','$adate','OT','Finished','1')";
...
<!-- Form Title -->
  ...
  <datalist id="stock">
				...
				<?php 
			$sql76 = "select * from `purchase_stock` where add_qty>0 and location='OT Medicine Store'";
