...
$strSQL = "Select * from iinfusion where pmrn= '$pmrn'and eid='$eid'and odate='$ad' and status1='Active' and status='Data Updated'order by `id` DESC;";
...
<tr>
	  ...
	  <td colspan="2" align="center"><strong>P.Status</strong></td>
...
<tr>
	  ...
	  $ii=$row['id'];
	  $query44 = "SELECT COUNT(pstatus) FROM iinfusion where id= '$ii' and pstatus='ordered' and status1 !='Cancel';"; 
$result44 = mysqli_query($con, $query44) or die(mysqli_error());
$row44 = mysqli_fetch_assoc($result44);
$count56 =$row44['COUNT(pstatus)'];
?>
      <td align="center"colspan="2"<?php if($count56>0): ?> style="background-color:RED;"<?php else: ?> style="" <?php endif ; ?>><?php echo $row["infusion"]; ?></td>
	  ...
	  <td align="center"colspan="2"<?php if($row['alert']== "H. Alert"): ?> style="background-color:RED;"<?php else: ?> style="background-color:lightblue;" <?php endif ; ?>>
        <?php echo $row['alert'];?></td>
  	  <td align="center"colspan="2"><?php echo $row["pstatus"]; ?></td>
<?php
...
$rf=$row['rfid'];
$mcode=$row['code'];
$url8 = "inurseinfusion?pmrn=$pm&eid=$ed&id=$idd&rf=$rf"; 
?>
...
{echo'<input type="button" name="edit_co" value="Use From Stock" id="'.$row['id'].'" class="btn btn-info btn-xs edit_data_co" />';} ?></a>
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><Strong><b>Implemented Infusion List<b><strong></td></tr>
<tr>
      <td colspan="1" align="center"><strong>S.No</strong></td>
...
$count=1;
$sel_query="Select * from iinfusion where pmrn= '$pmrn' and eid='$eid' and status='implemented' and odate='$odate'order by `id` desc;";
                ...
                <div class="modal-body">  
                     <form method="post" name="insert_form7" id="insert_form7"> 
                         ...
                         <label>Patient MRN</label>  
						  ...
						  <script>
						var yy= document.getElementById("address1").value;
							
														//document.writeln($row0['fullname']);
//						var res="<?php echo $dreffer;?>";

//var tt=document.getElementById("address1").innerHTML;
						</script>
  ...
  <datalist id="stock">
				...
				<?php 
			$sql76 = "select * from `medi_stock` where ('ICU','NICU','5AB Medicine stock','5CD Medicine stock','6AB Medicine stock','6CD Medicine stock','HMD','Cathlab and SPD','5AB emergency trolley','5CD emergency trolley','6th Fl emergency trolley') and add_qty!='0'";
 ...
 $(document).ready(function(){  
      ...
      $('#insert_form7').on("submit", function(event){  
           ...
           {  
          $.ajax({  
                     url:"edit_update_nurse1_infu.php",  
                     method:"POST",  
