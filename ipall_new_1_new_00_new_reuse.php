...
if($reuse=='Reuse'){
            $query4p5 = mysqli_query($db,"
            

            SELECT
            rfid,
            COUNT(*) AS rfid,
            COUNT(*) OVER() AS total_departments_count
        FROM
            imedi3
            WHERE pmrn='$pmrn' and eid='$eid' and infusion='$pp_medi' and udone !='' and reuse='Reuse'
        GROUP BY
            rfid;
            
            
            ");
						$datap5 = mysqli_fetch_assoc($query4p5);
						$tqty=$datap5['total_departments_count'];
						...
						$query4py = mysqli_query($db,"select COUNT(infusion) from imedi3 where pmrn='$pmrn' and eid='$eid' and infusion='$pp_medi' and udone !='' and reuse=''");
						$datapy = mysqli_fetch_assoc($query4py);
						$tqty=$datapy['COUNT(infusion)'];
}
          	
						$query4pc = mysqli_query($db,"select SUM(uprice) from phar_sale where pmrn='$pmrn' and eid='$eid' and medi='$pp_medi' and status='Sale' and location='IPD' and r_by!=''");
  	  ...
  	  <td align="center"colspan="2"><?php echo $tqty; ?></td>
<td align="center"colspan="4"><?php echo $n_uom; ?></td>
...
$query198ad3 = "SELECT SUM(uprice) FROM phar_sale where pmrn= '$pmrn' and eid='$eid' and status='Sale' and location in ('IPD','CCU','ICU') and r_by!=''"; 
	 
$result198ad3 = mysqli_query($dbhandle, $query198ad3) or die(mysql_error());

// Print out result
$row198ad3 = mysqli_fetch_array($result198ad3);
echo $test1am3=	$row198ad3['SUM(uprice)'];
