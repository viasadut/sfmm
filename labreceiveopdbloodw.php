...
$pdf=new PDF_BARCODE();
$test90=$pdf->UPC_A(1,9,$n1,5,0.3,7);

//echo $nn=UPC_A(1,9,$n1,5,0.3,7);

$querymax1 = "SELECT max(barcode1) FROM alltest where rdate='$datenew'"; 
$resultmax1 = mysqli_query($con, $querymax1) or die(mysqli_error());
$rowmax1 = mysqli_fetch_array($resultmax1);
$max1=$rowmax1['max(barcode1)']+1;



$querymax2 = "SELECT count(barcode1) FROM alltest where rdate='$datenew' and barcode1 !='0'"; 
$resultmax2 = mysqli_query($con, $querymax2) or die(mysqli_error());
$rowmax2= mysqli_fetch_array($resultmax2);
$max2=$rowmax2['count(barcode1)'];
...
$query = "UPDATE iinves set reby='$user',retime='$dtime', rstatus='RECEIVED', barcode1='$result5',status='RECEIVED',rdate='$datenew' where id='$id'"; 
...
$url = "labtest2222?pname=$pname&pmrn=$pmrn&rinfusion=$rinfusion&bar=$result5";
header("Location: $url"); 
...
<tr><td colspan="20"><?php echo $data["padd"]; ?></td></tr>
		...
		<tr>
						...
						<tr>				
						<td colspan="5"><?php echo $data45["page"]; ?></td>  
             		<td colspan="3"><?php echo $data45["ndate"]; ?></td>					 	
					 <td colspan="2"><?php echo $data45["pgender"]; ?></td>
...
<tr><td colspan="2" align="center"><input type="text"  name="odate" required value="<?php echo $data45["ndate"]; ?>" readonly/></td>
...
<td colspan="4" align="center"><input type="text" name="result" required readonly value="<?php if($max2=='0'){echo $n1;} else if($max2!='0'){echo $max1;}?>"></td></td>	  
