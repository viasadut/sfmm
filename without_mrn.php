...
$dd=date('d', strtotime($rowc['bdate']));
	$mm=date('m', strtotime($rowc['bdate']));
	$yy=date('Y', strtotime($rowc['bdate']));
...
$payment= (int)$_REQUEST['payment']-100;
...
//$formatted_date = strtoupper(date("d-M-Y", $timestamp));
...
('$pmrn','$opd','$payment1','$appdate','$apptime','$user','$particulars','$dname2','$mno','$vehicle1','$due_remarks','$b_remarks')";
  ...
  try {
    ...
    //$sql = "insert into patient_new (`pname`,`pmrn`,`padd`,`psex`,`page`,`pphone`,`bdate`) values 
//('$pname','$pmrn','$padd','$psex','$diff1','$pphone','$date91')";
  
    $sh = $db->prepare("insert into patient (pname,pmrn,padd,psex,page,pphone,bdate,type) VALUES 
    (?, ?, ?, ?, ?, ?, ?, ?)");
      $sh->execute([$name, $pmrn, $padd, $psex, $diff1, $pphone, $date91, $ptype]);
...
$data1= array(
  ...
  "in_item_count"=>2,
  "in_ITEM_NO_FK"=> [6004010,0],
  "IN_ITEM_BATCH_FK"=>["",""],
  "IN_ITEM_EXPIRY_DT"=>["",""],
  "in_ITEM_NAME"=> ["Registration", "OPD Consultation"],
  "in_ITEMTYPE_NO_FK"=> [1, 1],
  "in_ITEM_QTY"=> [1, 1],
  "in_ITEM_MU"=>["",""],
  //"in_ITEM_RATE"=> ["$integer_value", "$payment"],
  "in_ITEM_RATE"=> [$regi, $payment],
  "in_item_disc_percent"=> [0, 0],
  "in_item_disc_amount"=> [0, 0],
  "in_ITEM_VAT"=> [5, 5],
  "in_URGENT_FEE"=> [0, 0],
  "in_SERVICE_CHARGE"=> [0, 0],
  "in_REPORT_DELIVERY_DATE"=> ["30-07-2025", "30-07-2025"],
  "in_REPORT_DELIVERY_TIME"=> ["30-07-2025", "30-07-2025"],
  "in_DELIVERY_STATUS_LOOKUP_NO_FK"=> [1, 1],
  "in_PACKAGE_ITEM_IND"=> [0, 0],
  "in_item_level_remarks"=> ["", ""],
  "in_provider_no_fk"=> [0, 0]
);
...
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Return response as a string
curl_setopt($ch, CURLOPT_POST, true); // POST request
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data1)); // Send JSON payload
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    'Content-Type: application/json',
...
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Return response as a string
curl_setopt($ch, CURLOPT_POST, true); // POST request
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data2)); // Send JSON payload
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    'Content-Type: application/json',
...
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Return response as a string
curl_setopt($ch, CURLOPT_POST, true); // POST request
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data3)); // Send JSON payload
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    'Content-Type: application/json',
...
<input type="text" id="pmrn3" name="pmrn3" required />
<input type="text" id="type3" name="type3" required />
...
<input value="<?php echo $rowc['pmrn'];?>" name="pmrn" id="pmrn" type="text" placeholder="MRN" style="background-color:lightgreen;font-size:18px;font-weight:bold;color:red;width:300px" required>
...
<label for="mail"><strong>Appointment Date :</strong></label>
									...
									<input type='date' name="daten" size="20" style='background-color:lightgreen;font-size:22px;font-weight:bold;color:red;width:200px' min="<?= date('Y-m-d'); ?>" max="<?= date('Y-m-d', strtotime('0 days') ); ?>" value="<?php echo $rowc['adate1'];?>">  
...
//	dname1.disabled = true;
	...
	//ftype.disabled = true;
			...
			if (str.length == 0) {
				...
				// xhttp.open("GET", "filename", true);
				xmlhttp.open("GET", "visit_type_new.php?type="+q+"&dname2="+q1+"&ftype="+q2+"&pmrn="+q3, true);
