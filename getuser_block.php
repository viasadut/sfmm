...
mysqli_select_db($con,"ajax_demo");
				if($q==$tday){
			$sql = "select * from opd_appoint1 where dslot>='$ct' and dname='$q2' and date1='$q' order by dslot";
...
//echo "<option value='".$row->dslot."'>".$row->dslot.'-'.$row1->status."</option>";
			...
			$sql = "select * from opd_appoint1 where dname='$q2' and date1='$q' order by dslot";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
