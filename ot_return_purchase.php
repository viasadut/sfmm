...
$sel95="SELECT * FROM purchase_stock WHERE `sno`='$code' and location='$loc';";
...
if($tprice>$aqty)
{
    ...
    echo '</script>';
			...
			*/
			...
			$ins_query1="insert into purchase_stock1 (`code`,`g_name`,`b_name`,`add_qty`,`req_loc`,`status`,`location`,`rfid`,`sno`,`batch_no`,`add_by`,`add_time`)
 values ('$cc','$medi','$brand','$tprice','$loc','Returned','Pharmacy','$rfid1','$sno','$batch_no','$user','$add_time')";
mysqli_query($con,$ins_query1) or die(mysql_error());
			
			
			$strSQL = "update purchase_stock set add_qty='$n_qty' where sno='$code' and location='$loc'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			
$url = "ot_return_purchase?sno=$sno";
header("Location: $url"); 
...
<td colspan="1" ><input type="text" name="uprice" id="uprice" required value="" readonly style="font-weight: bold;font-size:22px;color:green"></td>
						...
						<td colspan="1" ><input type="text" name="tprice" id="qty" required value="" style="font-weight: bold;font-size:22px;color:green">
						
						<input type="hidden" name="rfid1" id="rfid1" required value="" style="font-weight: bold;font-size:22px;color:green"></td>
...
$count=1;
//$count=1;
$sel_query="Select * from purchase_stock1 where sno= '$sno' and req_loc='OT medicine store' order by `id` DESC;";
...
<script>
		...
		function GetDetail(str) {
			if (str.length == 0) {
				...
				document.getElementById("rfid1").value = "";
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "purchase_out1_return.php?pmrn=" + str, true);
