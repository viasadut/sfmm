...
$query55 = mysqli_query($db,"select * from alltest where pmrn='$pmrn' and eid='$eid1'");
...
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
	...
	for($i=0;$i<count($_POST["chkDel"]);$i++)
	{
		if($_POST["chkDel"][$i] != "")
		{
			...
			$strSQL = "UPDATE alltest set rstatus='RECEIVED', reby='$user', retime='$pdate' , barcode='$result'";
	...
	$url = "lab_bar_receive?pmrn=$pmrn&eid=$eid";
header("Location: $url"); 
...
values ('$dname', '$pmrn','$pname','$eid1','$medi','$pins','$date','$type','$price','$code','$link','$date77','$linkv','$report','$reportv','OPD','$diff1','$psex','$subtype')";
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from alltest where pmrn= '$pmrn' and eid='$eid' and dname='$dname' and billstatus='Billed' and type='lab'";
...
<tr>
	  ...
	  <td colspan="1" align="center"><strong>Episode</strong></td>
	  <td colspan="4" align="center"><strong>Patient Name</strong></td>
	  
	  <td colspan="4" align="center"><strong>Investigation Name</strong></td>
...
<td align="center"colspan="1"><?php echo $row["date"]; ?></td>
	  ...
	  <td align="center"colspan="1"><?php echo $row["eid"]; ?></td>
	  <td align="center"colspan="4"><?php echo $row["pname"]; ?></td>
	        <td align="center"colspan="4"><?php echo $row["medi"]; ?></td>
...
?>
    ...
    <td colspan="4" align="center"><input type="text" name="result[]" id="result<?php echo $i;?>" required readonly value="<?php echo $bar;?>"></td></td>	  
...
<?php
 $count++;}
?>
<tr><td colspan="20" align="right"><button type="submit" name="btnDelete">Receive Sample</button><input type="hidden" name="hdnCount" value="<?php echo $i;?>"></td>
</tr>
</table>
<?php
mysqli_close($objConnect);
