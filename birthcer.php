...
$ins_query="insert into birth (`pmrn`,`idate`,`bname`,`fname`,`mname`,`sex`,`weight`,`bdate`,`btime`,`dname`,`iby`,`year`,`bdate1`,`eid`,`dname1`,`status`) values 
('$pmrn', '$issue_date','$bname','$fname','$mname','$sex','$weight','$bdate2','$btime','$dname','$fullname','$year','$bdate','$count1','$dname1','Waiting For Approval')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_queryb="insert into birthb (`pmrn`,`idate`,`bname`,`fname`,`mname`,`sex`,`weight`,`bdate`,`btime`,`dname`,`iby`,`year`,`bdate1`,`eid`,`dname1`) values 
('$pmrn', '$issue_date','$bname','$fname','$mname','$sex','$weight','$bdate2','$btime','$dname','$fullname','$year','$bdate','$count1','$dname1')";
...
<label for="age"><strong>Baby's Name :</strong></label>
      <input name="bname" type="text" size="80" value="<?php echo $data["pname"]; ?>" readonly/>          
...
<label for="age"><strong>Patient's Details (MRN/Gender/Age) :</strong></label>
      ...
      <input name="weight" type="text" size="80" value=""required>
			...
			<select name="dname" value="" required>
				...
				<?php 
			$sql = "select * from `doctor` where discipline ='gynecologist' and status='Active'";
			...
			<select name="dname1" value="" required>
				...
				<?php 
			$sql = "select * from `doctor` where discipline ='pediatrician' and status='Active'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->dname."'>".$row->dname."</option>";
		...
		<label for="mail"><strong>Date of Birth :</strong></label>
									<p>
									  <input type="date" name="bdate" placeholder="Select Date" size="15" required min="2020-01-01" max="2050-12-31">
