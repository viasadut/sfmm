...
$query3 = "SELECT * FROM staff3 where sid= '$fullname' and status in ('Active','active')"; 
...
$date= date('m/d/Y');
$count=1;
$sel_query="Select * from dleave where hstatus='Approved By HOS' and rejectby =''";
//$start=$row["aadate"];
...
{echo'
<a onclick="return confirm_click();" href="leaveapprove1_test?id='.$row["id"].'&uname='.$row["uname"].'&type='.$row["type"].'&bal='.$row["total"].'"><strong>Confirm</strong></a>';
}
