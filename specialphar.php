...
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
	...
	for($i=0;$i<count($_POST["chkDel"]);$i++)
	{
		if($_POST["chkDel"][$i] != "")
		{
			$strSQL = "UPDATE istret set status='Served', duser='$user', ddate='$pdate' ";
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from istret where pmrn= '$pmrn' and eid='$eid'and status1 !='Cancel' and odate='$dd' and status='Data Updated' order by `id` asc;";
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from istret where pmrn= '$pmrn' and eid='$eid'and status ='Served' and odate='$dd'order by `id` asc;";
