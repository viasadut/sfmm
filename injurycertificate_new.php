...
$query43 = "SELECT COUNT(pmrn) FROM injury where pmrn= '$pmrn';"; 
...
/*$ins_query="insert into injury (`pmrn`,`page`,`eid`,`pname`,`nid`,`nid1`,`ffor`,`fdate`,`fdate1`,`diagnosis`,`user`,`idate`,`ct`,`year`,`m1`,`m2`,`m3`,`staff`,`idate1`,`remark`,`ddate`,`dtime`) values 
('$pmrn','$page', '$eid','$pname','$passno','$passno1','$ifind','$adate','$atime','$diag','$full','$idate','Treatment','$year','MR.','HE','HIS','$athrough','$idate1','$remark','$ddate','$dtime')";
mysqli_query($con,$ins_query) or die(mysql_error());
*/

$ins_query="update injury set `pmrn`='$pmrn',`page`='$page',`eid`='$eid',`pname`='$pname',`nid`='$passno',`nid1`='$passno1',`ffor`='$ifind',`fdate`='$adate',`fdate_new`='$adate1',`fdate1`='$atime',`diagnosis`='$diag',`user`='$full',`idate`='$idate',`ct`='Treatment',`year`='$year',`m1`='MR',`m2`='HE',`m3`='HIS',`staff`='$athrough',`idate1`='$idate1',`remark`='$remark',`ddate`='$ddate',`ddate_new`='$ddate1',`dtime`='$dtime',`status`='Waiting For Confirmation',`idate_new`='$idate_new' where id='$id'";
mysqli_query($con,$ins_query) or die(mysql_error());
...
/*$ins_query="insert into injury (`pmrn`,`page`,`eid`,`pname`,`nid`,`nid1`,`ffor`,`fdate`,`fdate1`,`diagnosis`,`user`,`idate`,`ct`,`year`,`m1`,`m2`,`m3`,`staff`,`idate1`,`remark`,`ddate`,`dtime`) values 
('$pmrn','$page', '$eid','$pname','$passno','$passno1','$ifind','$adate','$atime','$diag','$full','$idate','Treatment','$year','MS.','SHE','HER','$athrough','$idate1','$remark','$ddate','$dtime')";
mysqli_query($con,$ins_query) or die(mysql_error());
*/
$ins_query="update injury set `pmrn`='$pmrn',`page`='$page',`eid`='$eid',`pname`='$pname',`nid`='$passno',`nid1`='$passno1',`ffor`='$ifind',`fdate`='$adate',`fdate_new`='$adate1',`fdate1`='$atime',`diagnosis`='$diag',`user`='$full',`idate`='$idate',`ct`='Treatment',`year`='$year',`m1`='MS',`m2`='SHE',`m3`='HER',`staff`='$athrough',`idate1`='$idate1',`remark`='$remark',`ddate`='$ddate',`ddate_new`='$ddate1',`dtime`='$dtime',`status`='Issued',`idate_new`='$idate_new' where id='$id'";
mysqli_query($con,$ins_query) or die(mysql_error());
...
<label for="name"><strong>Admitted Through:</strong></label>
			...
			<label for="mail"><strong>Visit / Admission Date :</strong></label>
									<p>
									  <input type="date" name="adate"  placeholder="Select Date" size="8" value="" required>
									  
									  <label for="mail"><strong>Admission Time :</strong></label>
									  <input name="atime" type="text" size="7" value=""required>
      ...
      <textarea rows="5"  name="ifind" required></textarea>
      ...
      <textarea rows="5"  name="diag" required></textarea>
		...
		<label for="mail"><strong>Discharge Date:</strong></label>
									<p>
									  <input type="date" name="ddate"  placeholder="Select Date" size="8" value="" required>
									  
									  <label for="mail"><strong>Discharge Time :</strong></label>
									  <input name="dtime" type="text" size="7" value=""required>
      ...
      <textarea rows="5"  name="remark" required></textarea>
...
<td><a target='_blank' href="injuryeditprint.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>  
</form>
