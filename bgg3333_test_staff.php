...
else if($res=mysqli_num_rows($result99)>0)
{
	...
	if ($ptype=='Staff'){
        ...
        $update33="update pappnew set `aslot`='$slot', `bill`='$bill',`billby`='$user',`pmrn`='$pmrn', `pname`='$name', `padd`='$padd',`billtime`='$billtime',`payment`='100',`ptype`='$ptype' where `ID`='$id'";
...
<input name="payment" type="text" size="10" style="color:red;font-size:50px;font-weight:bold" value="<?php 
if ($data1['type']=='Staff'){echo '100';} 
else if ($data1['type']=='Staff Spouse'){echo '100';} 
else if ($data1['type']=='Staff Children'){echo '100';} 
else if ($data1['type']=='Consultant'){echo '100';} 

else if($l_date1==''){echo $v1;}else if($data['pmrn']==0){echo $v1;} else if($diff1_b<=4 and $data['pmrn']!=0){echo "0";} else if($diff1_b >4 and $data['pmrn']!=0){echo $v1;}?>" readonly>
