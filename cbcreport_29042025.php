<?php

session_start();
$user=$_SESSION["sess_username"];

//require('force_justify.php');
//require('fpdf/fpdf.php');

$db1 = new PDO('mysql:host=localhost;dbname=sfmmkpjnew','root','Godiloveu16');
//require('force_justify.php');
require('force_justify1.php');
$pmrn=$_REQUEST['pmrn'];
$id='I'.$_REQUEST['id'];
$id1=$_REQUEST['id'];
//$date=$_REQUEST['date'];
$eid=$_REQUEST['eid'];

$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$query = mysqli_query($db,"select * from cbctbl where pmrn='$pmrn' and eid='$eid' and sno='$id'");
$data = mysqli_fetch_array($query);


//$dname=$data['dname'];
$query2 = mysqli_query($db,"select * from inpatient where pmrn='$pmrn' and eid='$eid'");
$data2 = mysqli_fetch_array($query2);

$query3 = mysqli_query($db,"select * from iinves where pmrn='$pmrn' and eid='$eid' and id='$id1'");
$data3 = mysqli_fetch_array($query3);
$barcode=$data3['barcode'];
$sdate=date('d/m/Y H:i:s',strtotime($data3["rtime"]));


$tt1=$data3['code'];


$queryc = $db1->query("SELECT * FROM radio where code= '$tt1'"); 
	 
$resultc = $queryc->Fetch(PDO::FETCH_OBJ);

//$db = new PDO('mysql:host=localhost;dbname=sfmmkpj','root','');



$pdf=new PDF_Code128();
$pdf->AliasNbPages();
$pdf->AddPage('P','A4',1);
$pdf->SetFont('Arial' , 'b' , 9);
$pdf->SetLeftMargin('17');
//$pdf->headerTable();
//$pdf->viewTable($db);




$pdf->SetXY(150,745);
$pdf->Code128(18,90,$barcode,40,10);
$pdf->SetXY(50,45);




$pdf->ln(1);
$pdf->SetFont('Times', 'bu',14);
$pdf->Cell('182',6,$data['iname'].' Report',0,1,'C');
$pdf->Ln(2);

$pdf->SetFont('Times', 'b',14);
$pdf->Cell('30',5,'_________________________________________________________________________',0,1,'L');	

$pdf->Ln(4);
$pdf->SetFont('Times', 'b',12);

$pdf->Cell('60',5,'Referring Consultant Name: '. $data3['dname'],0,1,'L');

$pdf->Ln(4);
$pdf->SetFont('Times', 'b',10);
$pdf->Cell('110',5,'Patient Name: '. $data3['pname'],0,0,'L');
$pdf->Cell('50',5,'MRN: '.$data3['pmrn'],0,1,'L');

$pdf->Cell('110',5,'Gender: '.$data3['pgender'],0,0,'L');
$pdf->Cell('50',5,'Age: '.$data3['page'],0,1,'L');
$pdf->Cell('110',5,'Sample Date: '.$sdate,0,0,'L');	
$pdf->Cell('50',5,'Result Time: '.$data3['resulttime'],0,1,'L');

$pdf->Cell('110',5,'',0,0,'L');
$pdf->Cell('50',5,'Result Status: '. $data3['resultstatus'],0,1,'L');

$pdf->SetFont('Times', 'b',14);

$pdf->ln(6);



$pdf->Cell('30',5,'_________________________________________________________________________',0,1,'L');	
$pdf->ln(3);


//$pdf->SetFont('Arial' , 'b' , 10);
//$pdf->Cell('40',5,'Referral From:',1,0,'L');
//$pdf->Cell('141', 5,$data['dreffer'],1,1,'L');

$pdf->ln(3);


$pdf->SetFont('Arial' , 'b' , 10);



$pdf->Cell('80',5,'Particulars',1,0,'L');
$pdf->Cell('30',5,'Value',1,0,'C');
$pdf->Cell('31',5,'Unit',1,0,'C');
$pdf->Cell('40',5,'Reference Range',1,1,'C');


if($data['haemo']>=13 and $data['haemo']<=18)
{	
$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Haemoglobin',1,0,'L');
$pdf->Cell('30',5,$data['haemo'],1,0,'C');
$pdf->Cell('31',5,'g/dL',1,0,'C');
$pdf->Cell('40',5,'13.0-18.0',1,1,'C');
}

else 
{	
$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'Haemoglobin',1,0,'L');
$pdf->Cell('30',5,$data['haemo'],1,0,'C');
$pdf->Cell('31',5,'g/dL',1,0,'C');
$pdf->Cell('40',5,'13.0-18.0',1,1,'C');
}

 if($data['red']>=4.5 and $data['red']<=5.9)

{	
$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'RBC Total Count',1,0,'L');
$pdf->Cell('30',5,$data['red'],1,0,'C');
$pdf->Cell('31',5,'10^12/L',1,0,'C');
$pdf->Cell('40',5,'4.5-5.9',1,1,'C');
}
else 
{	
$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'RBC Total Count',1,0,'L');
$pdf->Cell('30',5,$data['red'],1,0,'C');
$pdf->Cell('31',5,'10^12/L',1,0,'C');
$pdf->Cell('40',5,'4.5-5.9',1,1,'C');
}

if($data['pcv']>=41 and $data['pcv']<=53)
{
$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Haematocrit (PCV)',1,0,'L');
$pdf->Cell('30',5,$data['pcv'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'41-53',1,1,'C');
}
else 
{
$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'Haematocrit (PCV)',1,0,'L');
$pdf->Cell('30',5,$data['pcv'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'41-53',1,1,'C');
}
if($data['mcv']>=76 and $data['mcv']<=103)
{
$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'MCV',1,0,'L');
$pdf->Cell('30',5,$data['mcv'],1,0,'C');
$pdf->Cell('31',5,'fL',1,0,'C');
$pdf->Cell('40',5,'76-103',1,1,'C');
}

else 
	
{
$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'MCV',1,0,'L');
$pdf->Cell('30',5,$data['mcv'],1,0,'C');
$pdf->Cell('31',5,'fL',1,0,'C');
$pdf->Cell('40',5,'76-103',1,1,'C');
}

if($data['mch']>=26 and $data['mch']<=34)
{
$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'MCH',1,0,'L');
$pdf->Cell('30',5,$data['mch'],1,0,'C');
$pdf->Cell('31',5,'pg',1,0,'C');
$pdf->Cell('40',5,'26-34',1,1,'C');
}
else
{
$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'MCH',1,0,'L');
$pdf->Cell('30',5,$data['mch'],1,0,'C');
$pdf->Cell('31',5,'pg',1,0,'C');
$pdf->Cell('40',5,'26-34',1,1,'C');
}


if($data['mchc']>=31 and $data['mchc']<=36)

{
	$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'MCHC',1,0,'L');
$pdf->Cell('30',5,$data['mchc'],1,0,'C');
$pdf->Cell('31',5,'g/dL',1,0,'C');
$pdf->Cell('40',5,'31-36',1,1,'C');
}

else 
{
	$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'MCHC',1,0,'L');
$pdf->Cell('30',5,$data['mchc'],1,0,'C');
$pdf->Cell('31',5,'g/dL',1,0,'C');
$pdf->Cell('40',5,'31-36',1,1,'C');
}


if($data['rdw']>=8 and $data['rdw']<=14.6)
{
$pdf->SetTextColor(000,0,0);	
$pdf->Cell('80',5,'RDW',1,0,'L');
$pdf->Cell('30',5,$data['rdw'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'8.0-14.6',1,1,'C');
}

else 
{
$pdf->SetTextColor(194,8,8);	
$pdf->Cell('80',5,'RDW',1,0,'L');
$pdf->Cell('30',5,$data['rdw'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'8.0-14.6',1,1,'C');
}


if($data['pla']>=150 and $data['pla']<=450)
{
$pdf->SetTextColor(000,0,0);	
$pdf->Cell('80',5,'Platelet Count',1,0,'L');
$pdf->Cell('30',5,$data['pla'],1,0,'C');
$pdf->Cell('31',5,'10^3/uL',1,0,'C');
$pdf->Cell('40',5,'150-450',1,1,'C');
}

else 
	
{
$pdf->SetTextColor(194,8,8);	
$pdf->Cell('80',5,'Platelet Count',1,0,'L');
$pdf->Cell('30',5,$data['pla'],1,0,'C');
$pdf->Cell('31',5,'10^3/uL',1,0,'C');
$pdf->Cell('40',5,'150-450',1,1,'C');
}


if($data['mpv']>=5.8 and $data['mpv']<=12)
	
	{
$pdf->SetTextColor(000,0,0);	
	$pdf->Cell('80',5,'MPV',1,0,'L');
$pdf->Cell('30',5,$data['mpv'],1,0,'C');
$pdf->Cell('31',5,'fL',1,0,'C');
$pdf->Cell('40',5,'5.8-12.0',1,1,'C');
	}
else 	
	{
$pdf->SetTextColor(194,8,8);	
	$pdf->Cell('80',5,'MPV',1,0,'L');
$pdf->Cell('30',5,$data['mpv'],1,0,'C');
$pdf->Cell('31',5,'fL',1,0,'C');
$pdf->Cell('40',5,'5.8-12.0',1,1,'C');
	}	
	
	
	
	if($data['wbc']>=4.3 and $data['wbc']<=10.5)
		
		{
$pdf->SetTextColor(000,0,0);				
$pdf->Cell('80',5,'WBC Total Count',1,0,'L');
$pdf->Cell('30',5,$data['wbc'],1,0,'C');
$pdf->Cell('31',5,'10^3/uL',1,0,'C');
$pdf->Cell('40',5,'4.3-10.5',1,1,'C');
		}
	else 	
		
		{
$pdf->SetTextColor(194,8,8);			
$pdf->Cell('80',5,'WBC Total Count',1,0,'L');
$pdf->Cell('30',5,$data['wbc'],1,0,'C');
$pdf->Cell('31',5,'10^3/uL',1,0,'C');
$pdf->Cell('40',5,'4.3-10.5',1,1,'C');
		}
		


		
if($data['neu']!='0.00' and $data['neu']>=40 and $data['neu']<=75)
{
$pdf->SetTextColor(000,0,0);	
$pdf->Cell('80',5,'Neutrophil',1,0,'L');
$pdf->Cell('30',5,$data['neu'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'40-75',1,1,'C');
}

//else if($data['neu']!='0.00')
else	
{
$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'Neutrophil',1,0,'L');
$pdf->Cell('30',5,$data['neu'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'40-75',1,1,'C');
}


if($data['lym']!='0.00' and $data['lym']>=20 and $data['lym']<=45)
	
	
	{
$pdf->SetTextColor(000,0,0);	
$pdf->Cell('80',5,'Lymphocyte',1,0,'L');
$pdf->Cell('30',5,$data['lym'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'20-45',1,1,'C');
	}
	
//else if($data['lym']!='0.00')
else
	{
$pdf->SetTextColor(194,8,8);	
$pdf->Cell('80',5,'Lymphocyte',1,0,'L');
$pdf->Cell('30',5,$data['lym'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'20-45',1,1,'C');
	}
	
	

if($data['mono']!='0.00' and $data['mono']>=1 and $data['mono']<=11)
	
	{

$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Monocyte',1,0,'L');
$pdf->Cell('30',5,$data['mono'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'1-11',1,1,'C');
	}
//else if($data['mono']!='0.00')	
else
{

$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'Monocyte',1,0,'L');
$pdf->Cell('30',5,$data['mono'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'1-11',1,1,'C');
	}	

	
	//if($data['eos']!='0.00' and $data['eos']>=0 and $data['eos']<=6)
	if($data['eos']>=0 and $data['eos']<=6)

{	
	$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Eosinophil',1,0,'L');
$pdf->Cell('30',5,$data['eos'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'0-6.0',1,1,'C');
}

//else if($data['eos']!='0.00') 
else 
{	
	$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'Eosinophil',1,0,'L');
$pdf->Cell('30',5,$data['eos'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'0-6.0',1,1,'C');
}

if($data['bas']!='0.00' and $data['bas']>=0 and $data['bas']<=2)

{	
	$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Basophil',1,0,'L');
$pdf->Cell('30',5,$data['bas'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'0-2',1,1,'C');

}
else if($data['bas']!='0.00')
{	
	$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'Basophil',1,0,'L');
$pdf->Cell('30',5,$data['bas'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'0-2',1,1,'C');

}

	
	
	if($data['promyclocyte']!='0.00')
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Pro-Myelocyte',1,0,'L');
$pdf->Cell('30',5,$data['promyclocyte'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'',1,1,'C');
	}
	
	if($data['metamyclocyte']!='0.00')
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Metamyelocyte',1,0,'L');
$pdf->Cell('30',5,$data['metamyclocyte'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'',1,1,'C');
	}
	
	if($data['lymphoblast']!='0.00')
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Lymphoblast',1,0,'L');
$pdf->Cell('30',5,$data['lymphoblast'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'',1,1,'C');
	}
	
	if($data['myoblast']!='0.00')
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Myeloblast',1,0,'L');
$pdf->Cell('30',5,$data['myoblast'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'',1,1,'C');
	}
	
	if($data['nrbc']!='0.00')
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'NRBC',1,0,'L');
$pdf->Cell('30',5,$data['nrbc'],1,0,'C');
$pdf->Cell('31',5,'10^3/uL',1,0,'C');
$pdf->Cell('40',5,'',1,1,'C');
	}

	if($data['a_cell']!='0.00')
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'Atypical, Cells',1,0,'L');
$pdf->Cell('30',5,$data['a_cell'],1,0,'C');
$pdf->Cell('31',5,'%',1,0,'C');
$pdf->Cell('40',5,'',1,1,'C');
	}

	if($data['esr']!='' and $data['esr']>0 and $data['esr']<20)
	
	{
		$pdf->SetTextColor(000,0,0);
$pdf->Cell('80',5,'ESR',1,0,'L');
$pdf->Cell('30',5,$data['esr'],1,0,'C');
$pdf->Cell('31',5,'mm/h',1,0,'C');
$pdf->Cell('40',5,'0-20',1,1,'C');
	}

else if($data['esr']!='')
	{
		$pdf->SetTextColor(194,8,8);
$pdf->Cell('80',5,'ESR',1,0,'L');
$pdf->Cell('30',5,$data['esr'],1,0,'C');
$pdf->Cell('31',5,'mm/h',1,0,'C');
$pdf->Cell('40',5,'0-20',1,1,'C');
	}


	
	if($data['remarks']!='')
	
	{
		$pdf->ln(5);;
		$pdf->SetTextColor(000,0,0);
$pdf->MultiCell('180',5,'Remarks:'.$data['remarks']);
	}
	
$pdf->Ln(15);



	$pdf->SetTextColor(000,0,0);







if($data3['cby'] !='')
{
$db1 = new PDO('mysql:host=localhost;dbname=sfmmkpjnew','root','Godiloveu16');

$rby=$data3['resultby'];
$query24 = $db1->query("select * from staff3 where sid='$rby'");
$data24 = $query24->Fetch(PDO::FETCH_OBJ);
$rby1=$data24->sname;


$cby=$data3['cby'];
$query25 = $db1->query("select * from user where uname='$cby'");
$data25 = $query25->Fetch(PDO::FETCH_OBJ);
$cby1=$data25->fullname;


$query26 = $db1->query("select * from doctor1 where dname='$cby1'");
$data26 = $query26->Fetch(PDO::FETCH_OBJ);
$cby3=$data26->Discipline;







$pdf->Cell('100',5,'Result Updated By',0,0,'L');

$pdf->Cell('100',5,'Result Confirmed By',0,1,'L');



$pdf->Ln(1);
if($rby1==''){
	
$pdf->Cell('100',5,$cby1,0,0,'L');	
}

else {
	
$pdf->Cell('100',5,$rby1,0,0,'L');	
}


$pdf->Cell('100',5,$cby1,0,1,'L');

$pdf->Ln(1);

if($rby1==''){
$pdf->Cell('100',5,$cby3,0,0,'L');	
	
}

else{
$pdf->Cell('100',5,$data24->desig,0,0,'L');
}
$pdf->Cell('100',5,$cby3,0,1,'L');

}



else 
{

$db1 = new PDO('mysql:host=localhost;dbname=sfmmkpjnew','root','Godiloveu16');
$rby=$data3['resultby'];
$query24 = $db1->query("select * from staff3 where sid='$rby'");
$data24 = $query24->Fetch(PDO::FETCH_OBJ);
$rby1=$data24->sname;


//$cby=$data->cby;
//$query25 = $db->query("select * from user where uname='$cby'");
//$data25 = $query25->Fetch(PDO::FETCH_OBJ);
//$cby1=$data25->fullname;


//$query26 = $db->query("select * from doctor1 where dname='$cby1'");
//$data26 = $query26->Fetch(PDO::FETCH_OBJ);
//$cby3=$data26->Discipline;







$pdf->Cell('100',5,'Result Updated By',0,1,'L');

//$pdf->Cell('100',5,'Result Confirmed By',0,1,'L');



$pdf->Ln(1);

$pdf->Cell('100',5,$rby1,0,1,'L');

//$pdf->Cell('100',5,$cby1,0,1,'L');

$pdf->Ln(1);

$pdf->Cell('100',5,$data24->desig,0,1,'L');

//$pdf->Cell('100',5,$cby3,0,1,'L');

}




$pdf->ln(5);

$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('182',5,'Printed by:'.$user.'                                                                   Computer Generated Report, No Signature Required.',0,1,'R');

//$pdf->SetFont('Arial' , 'b' , 15);
//$pdf->Cell('90',5,'OUT PATIENT RECORD',1,0,'L');


//$pdf->ln(10);
//$pdf->MultiCell('160' , 5,$data['xl'],1,1);
//$pdf->Cell('30' , 5,'Doasge',1,1);
//$pdf->MultiCell('160' , 5,'jashfjh sjfh jsdhfjsdhjfh jsdhjf hjsdhfj dsjhf djsh jfdshjf dsjhf jdsh fdhsf hjsdhf sdhf jdhsf hdsjfhjsdhf sdhf jdshjfhjskdhf jsdh fjhsdjkf hjdsfjd s',1,1);
//$dd=$data['refer']

//$dd = rtrim($dd, ',');
//$string = rtrim($string, ',');

$pdf->Output();