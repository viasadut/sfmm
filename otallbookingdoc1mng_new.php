<?php 
   session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','ddf','doctor','imo','nurse')"; 
...
$count=1;
$sel_query="Select * from ot where date5 between '$test' and '$test1' order by date5 asc";
...
?>
	  ...
	  <td align="center" style="font-weight:bold;color:red;"><?php echo 'Anes- '.$row["p_anes"].'<br /> OT-'.$row['p_ot'].'<br /> Time-'.$row['p_time'];?>
       <br /><input type="button" name="edit" value="Set" id="<?php echo $row['id'];?>" class="btn btn-info btn-xs edit_data">
     </td> 
