...
$ins_query="insert into cath_receive (`dname`,`pname`,`pmrn`,`pphone`,`page`,`psex`,`pdate`,`ptime`,`proname`,`type`,`date1`,`ll`,`ieid`) values 
('$dname', '$pname','$pmrn','$pphone','$page','$psex','$pdate','$ptime','$proname','OPD','$date1','$ll','$ieid')";
...
if(isset($_POST['load'])){
			$sql = "select * from `privilege` where dname='$dname'and status in ('Approved','Waiting For CFO Approval')";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
