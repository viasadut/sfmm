...
if (isset($_POST['but_update'])) {
    ...
    try {
        ...
        foreach ($_POST['update'] as $updateid) {
            ...
            */
            ...
            $con->query("
                ...
                SET status='Updated',
                    r_qty='{$new_r_qty}',
                    check_by='" . $con->real_escape_string($check_by) . "'
                WHERE id={$updateid}
            ");
        ...
        if ($total > 0) {
            ...
            $ins_ap = "
                INSERT INTO acct_ap
                (grn, creditor_code, vat, tax, amount, payable, cheque_no, bank, user, add_time, date, pono, grn_time, grn_by, discount, invoice_no)
                VALUES
                ('" . $con->real_escape_string($grn) . "','" . $con->real_escape_string($creditor_code) . "','','','{$total}','{$total}','','','{$user}','{$add_time}','{$date_tb}','" . $con->real_escape_string($po_ono) . "','{$add_time}','{$user}','{$grn_discount}','" . $con->real_escape_string($invoice_no) . "')
            ";
            $con->query($ins_ap);

            $con->query("INSERT INTO pms_tb (trans_id, trans_type, acct_code, date, amount, location)
                         VALUES ('{$grn}','CR','311190','{$date_tb}','{$total}','PO_PUR')");
            $con->query("INSERT INTO pms_tb (trans_id, trans_type, acct_code, date, amount, location)
                         VALUES ('{$grn}','DR','713085','{$date_tb}','{$total}','PO_PUR')");
