...
if ($pmrn !== "") {
	...
	$query1 = mysqli_query($con, "SELECT * FROM purchase_stock3 WHERE rfid='$pmrn' and location='Store' and add_qty>0");
...
$datef= round($datediff / (60 * 60 * 24));
	...
	$query = mysqli_query($con, "SELECT * FROM medicine WHERE code='$code1' and status='Active'");

	$row = mysqli_fetch_array($query);
	$uuprice = $row1["u_price"];
	
	
//$mcode = $row['code'];
	  $sum = "SELECT SUM(add_qty) FROM purchase_stock3 where rfid='$code1' and location='Store' and add_qty>0" ;
