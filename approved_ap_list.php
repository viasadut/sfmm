...
$sel_query="Select * from pms_bill_payment where approve_status='3' group BY chequeno;";
...
$query4 = "SELECT SUM(gtotal) as gtotal,creditor_name,bankno,date,remarks,chequeno FROM pms_bill_payment WHERE chequeno ='$cheque_no' and approve_status='3'";  
...
<a target='_Blank' href="print_allocate_cheque?cname=<?php echo $row['creditor_name']; ?>&chequeno=<?php echo $row['chequeno']; ?>&id=<?php echo $row['id']; ?>">Print PV</a>
<a target='_Blank' href="fams/PrintCheque_po?id=<?php echo $row['chequeno']; ?>&chequeno=<?php echo $row['chequeno']; ?>&no=<?php echo $row['id']; ?>">Print Cheque</a>
