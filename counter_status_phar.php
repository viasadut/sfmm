...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('staff','pharmacy','imo')"; 
...
$search_counter1="select COUNT(sno) from counter where status='Active' and date1='$cdate'";
$serach_counter_result1= mysqli_query($con, $search_counter1);
$search_counter_row1=mysqli_fetch_assoc($serach_counter_result1);
$available_counter1=$search_counter_row1['COUNT(sno)'];
...
$current_counter = "select * from counter where status='Active' and date1='$cdate' order by id asc LIMIT 1"; 
...
$current_token = "select * from counter_token where status='Active' and date='$cdate' order by id asc LIMIT 1"; 
...
$query42 = "update counter_token set status='Call' where token_no=".$current_token_row['token_no']." and status='Active' and date='$cdate'"; 	
//$trt=mysqli_query($con, $query42);

$query44 = "update counter set status='Engage' where sno=".$current_counter_row['sno']." and status='Active'and date1='$cdate'"; 	
...
$sel3="Select * from counter_history where '$ad3' between date and date1 and counter_no='2' and status='1' order by id desc";

$resu3 = mysqli_query($con,$sel3);
$rw3 = mysqli_fetch_assoc($resu3);

$search_counter="select * from counter where status='Engage' and user='$user' and date1='$cdate'";
$serach_counter_result= mysqli_query($con, $search_counter);
$search_counter_row=mysqli_fetch_assoc($serach_counter_result);
$available_counter=$search_counter_row['sno'];
...
$current_token = "select * from counter_history where user='$user' and cdate='$cdate' and status='1' order by id desc LIMIT 1"; 
$current_token_result= mysqli_query($con, $current_token);
$current_token_row=mysqli_fetch_assoc($current_token_result);
...
*/
	 ...
	 echo "
	 <button type='submit' name='tclose' onclick='myFunction1()' style='height:50px;width:200px;background-color:green;font-size:20px;font-weight:bold;'class='btn btn-primary btn-sm'>
						
                                                Token No- ".$current_token_row['token_no']."<i class='fa fa-clipboard' aria-hidden='true'></i>
                                            </button>
