<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('doctor','mng','moopd')"; 
...
('$sid', '$full','$date1','$adate','$server','Present','$tt','Hospital','Anxious.jpg')";
mysqli_query($con,$ins_query23) or die("Problem in DB");}

else {
	
$ins_query23="insert into attendance (`sid`,`sname`,`stime`,`adate`,`aip`,`status`,`sip`,`location`,`mode`) values 
('$sid', '$full','$date1','$adate','$server','Present','$tt','Outside','Anxious.jpg')";
mysqli_query($con,$ins_query23) or die("Problem in DB");}
