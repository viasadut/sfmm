...
if(empty($_REQUEST['update']))
{
    ...
    echo '</script>';
			...
			$qq = mysqli_query($db,"select * from phar_sale where id='".$updateid."'");
			...
			$qq3 = mysqli_query($db,"select * from medicine where code='".$code_1."'");
			$dd3 = mysqli_fetch_assoc($qq3);
			$medi_qty=$dd3['qty'];
...
$eqty2_new = $_POST['eqty1_'.$updateid]-$p_qty;
$eqty2_new_medi = $medi_qty-$eqty2_new;



$eqty2_new1 =$p_qty - $_POST['eqty1_'.$updateid];
$eqty2_new1_medi = $medi_qty-$eqty2_new1;


$eqty5 = $_POST['eqty2_'.$updateid];
$u_qty=$eqty5-$eqty2;
$u_price=$eqty2*$p_price;
$u_price_1=$eqty2_new1*$p_price;
...
$rf=$b_chk_m['rfid'];
$stock=$b_chk_m['add_qty']-$eqty2_new;

$stock1=$b_chk_m['add_qty']-$eqty2_new1;
	...
	else if($eqty5!= 0 and $eqty2>$p_qty and $mm_qty<$eqty2_new)
	{
			
			
			
			$strSQL = "update medicine set tqty='".$eqty2_new_medi."' where code='".$code_1."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			
			//$strSQL1 = "update pmedi set status='Served',qty='$eqty2_new' where id='".$updateid."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			//$objQuery1 = mysqli_query($objConnect,$strSQL1);

			
			$strSQL2 = "update phar_sale set `qty`='$eqty2',`tprice`='$u_price',`aby`='$user',`adate`='$adate',`brand`='$brand',`ins`='$ins' where id='".$updateid."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
$rfid1=$b_chk_7['rfid'];
$second_qty=$b_chk_7['add_qty']-$eqty2_new;
...
$ins_query4="update medi_stock set `add_qty`='$stock' where rfid='$rf'";
mysqli_query($con,$ins_query4) or die(mysql_error());
	...
	else if($eqty5!= 0 and $eqty2<$p_qty and $mm_qty<$eqty2_new)
	{
			...
			$strSQL = "update medicine set tqty='".$eqty2_new1_medi."' where code='".$code_1."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
...
$b_chk_7=mysqli_fetch_assoc($result97);
$rfid1=$b_chk_7['rfid'];
$second_qty=$b_chk_7['add_qty']-$eqty2_new1;
...
$ins_query4="update medi_stock set `add_qty`='$stock1' where rfid='$rf'";
mysqli_query($con,$ins_query4) or die(mysql_error());
...
$query1 = "select * from medicine where mname='$medi' and status='Active'";
                    $result1 = mysqli_query($con,$query1);
					   ...
					   $query2 = "select * from phar_sale where sno='$sno'";
     ...
     <td align="center" colspan="1" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $count; ?></td>
      ...
      <td align="center"colspan="8" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["medi"]; ?></td>
	  
	  <td align="center"colspan="7" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["pdos"].'<br>'.$row["frelation"].'<br>'.$row["duration"]; ?></td>
	        
						<td align="center"colspan="1" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["eqty1"]; ?></td>
			
		
<td align="center"colspan="1"><input name="eqty2_<?= $id ?>" type="text" value="<?php echo $row1['tqty'];?>" id="eqty2" readonly <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?> ></td>
...
<input name="eqty1_<?= $id ?>" type="text" value="<?php echo $s_qty; ?>" id="eqty1" required <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>></td>
