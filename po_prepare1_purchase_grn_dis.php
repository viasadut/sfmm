...
//$sel95w="SELECT COUNT(id) FROM purchase_stock3 WHERE `code`='$code' and location='Store' and batch_no='$batchno' and add_qty>0 and sno='$pono';";
$sel95w="SELECT COUNT(id) FROM purchase_stock3 WHERE `code`='$code' and location='Store' and batch_no='$batchno' and add_qty>0 and sno='IOUYBUIYUYVI';";
$result95w = mysqli_query($con,$sel95w);
$b_chkw=mysqli_fetch_assoc($result95w);
$count_qtyw=$b_chkw['COUNT(id)'];
	
	
$sel95wz="SELECT * FROM purchase_stock3 WHERE `code`='$code' and location='Store' and batch_no='$batchno' order by id desc limit 1;";
$result95wz = mysqli_query($con,$sel95wz);
$b_chkwz=mysqli_fetch_assoc($result95wz);
$row_id=$b_chkwz['id'];
$new_qty=$b_chkwz['add_qty'] + $eqty2;
$new_price=$new_qty*$uprice;
					...
					$ins_query1="update po_table1 set status='Updated',r_qty='$eqty2',check_by='$check_by' where id='$updateid'";
mysqli_query($con,$ins_query1) or die(mysql_error());

$ins_query2="insert into purchase_stock3 (`code`,`location`,`g_name`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`sno`,`u_price`,`t_price`,`add_by`,`add_time`,`re_date`,`req_qty`,`grn`,`invoice_no`,`p_id`,`check_by`)
values('$code','Store','$g_name','$b_name','$eqty2','$expiry','$batchno','$runningTime','$pono','$uprice','$tprice','$user','$add_time','$re_date','$eqty2','$grn','$invoice_no','$ono','$check_by')";
mysqli_query($con,$ins_query2) or die(mysql_error());

if($bonus>0){

 
  $ins_query22="insert into purchase_stock3 (`code`,`location`,`g_name`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`sno`,`u_price`,`t_price`,`add_by`,`add_time`,`re_date`,`req_qty`,`grn`,`invoice_no`,`p_id`,`b_remarks`,`check_by`)
  values('$code','Store','$g_name','$b_name','$bonus','$expiry','$batchno','$runningTime1','$pono','$uprice','$bprice','$user','$add_time','$re_date','$eqty2','$grn','$invoice_no','$ono','Bonus Item','$check_by')";
...
$ins_query2="update purchase_stock3 set `add_qty`='$new_qty', `req_qty`='$new_qty', `t_price`='$new_price' where id='$row_id' and location='Store'";
mysqli_query($con,$ins_query2) or die(mysql_error());
...
if($bonus>0){
  ...
  mysqli_query($con,$ins_query22) or die(mysql_error());
					...
					$ins_query1="update po_table1 set status='Updated',r_qty='$eqty3',check_by='$check_by' where id='$updateid'";
mysqli_query($con,$ins_query1) or die(mysql_error());


/*$ins_query2="insert into purchase_stock3 (`code`,`location`,`g_name`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`sno`,`u_price`,`t_price`,`add_by`,`add_time`,`re_date`,`req_qty`,`grn`,`invoice_no`,`check_by`)
values('$code','Store','$g_name','$b_name','$eqty2','$expiry','$batchno','$runningTime','$pono','$uprice','$tprice','$user','$add_time','$re_date','$eqty2','$grn','$invoice_no','$check_by')";
mysqli_query($con,$ins_query2) or die(mysql_error());
*/
$ins_query2="insert into purchase_stock3 (`code`,`location`,`g_name`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`sno`,`u_price`,`t_price`,`add_by`,`add_time`,`re_date`,`req_qty`,`grn`,`invoice_no`,`p_id`,`check_by`)
values('$code','Store','$g_name','$b_name','$eqty2','$expiry','$batchno','$runningTime','$pono','$uprice','$tprice','$user','$add_time','$re_date','$eqty2','$grn','$invoice_no','$ono','$check_by')";
...
$ins_query2="update purchase_stock3 set `add_qty`='$new_qty',`req_qty`='$new_qty',`t_price`='$tprice' where id='$row_id' and location='Store'";
mysqli_query($con,$ins_query2) or die(mysql_error());
        ...
        values('$grn','$creditor_code','','','$gtotal','$gtotal','','','','','','$po_ono','$add_time','$user','$invoice_no')";
...
<table id="itemTable" width="95%" height ="100%" border="0" align="center" bgcolor="lightblue" style="border-collapse:collapse;">	
...
echo '<input  type="number" name="" class="qty" min="0" value="0" disabled>';
    ...
    echo '<input  type="number" name="eqty1_'.$id.'" class="qty" min="0" value="0" step="any">';
    }
    ?>
</td>

<td colspan="1">
<input  type="text" class="price" name="price[]" value="<?= $i['uprice'] ?>" readonly>

</td>

<td colspan="1">

<input  type="text" class="price_new" name="price_new[]" value="" >
</td>
...
<td align="center"colspan="1"><input class="bonus" name="bonus_'.$id.'" type="text" step="any"></td>
...
<input type="number" name='gtotal' id="grand_total" value="0" readonly>
...
$count=1;
//$count=1;
$sel_query="Select * from purchase_stock3 where sno='$ono' order by `id` DESC;";
...
<td align="center"colspan="1"><a target='_Blank' href="grn_print_pdf?ono=<?php echo $encryption1; ?>&grn=<?php echo $encryption11; ?>&id=<?php echo $row['re_date']; ?>">Print</a></td>	  
<td align="center"colspan="1"><a target='_Blank' href="purchase_bar?g_name=<?php echo $row['g_name']; ?>&rfid=<?php echo $row['rfid']; ?>">Barcode</a></td>	  
...
function updateRowAndGrand(qtyInput) {
    ...
    let price_new = Number(row.querySelector(".price_new").value);
