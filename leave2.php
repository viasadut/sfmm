...
$query39 = "SELECT * FROM staff3 where sid= '$user' and status in ('Active','active')"; 
...
$q39 = "SELECT * FROM staff3 where sid= '$replacement' and status in ('Active','active')"; 
...
$q9b = "SELECT SUM(total) from dleave where hstatus IN('Approval Pending','Forwarded to Incharge') and uname='$user' and tleave='Annual Leave' and cyear='$cyear'"; 
...
if($r9b['SUM(total)'] >= $aleave90 && $tleave=='Annual Leave')
{
...
else if($tleave=='Paternity Leave' && $gender=='M' && $status1=='Confirm' && $incharge=="" && $r_name!='' and $pa_leave1>0){
...
<tr><td colspan="20">
  ...
  <select class="js-example-basic-single" name="replacement" required>
				...
				<?php 
			$sql76 = "select * from `staff3` where status='Active' and dept in ('$dept1','$dept2') and cat in ('Staff','HOD','Incharge') and sid !='$user' and status in ('Active','active')";
			$res76 = mysqli_query($con, $sql76);
			
			
			$sql764 = "select * from `staff3` where status='Active' and cat in ('HOD','Incharge') and sid !='$user' and status in ('Active','active')";
