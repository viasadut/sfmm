...
$runningTime = date('dYims');

$code = $_REQUEST['code'];
$medi = $_REQUEST['medi'];
$uprice = $_REQUEST['uprice'];
$tprice = $_REQUEST['tprice'];
$rfid_m = $_REQUEST['rfid_m'];
$tqty = $_REQUEST['tqty'];
$ins = $_REQUEST['ins'];
...
$sel95="SELECT * FROM medi_stock WHERE `code`='$code' and location='Pharmacy' and add_qty!='0' and rfid='$rfid_m' ;";
$result95 = mysqli_query($con,$sel95);
$b_chk=mysqli_fetch_assoc($result95);
$brand=$b_chk['b_name'];
$bb_no=$b_chk['batch_no'];
$rfid_medi=$b_chk['rfid'];
$exdate=$b_chk['exdate'];
...
else if($tprice>$tqty)
{
       ...
       echo '<script language="javascript">';
    echo 'alert("Unsuccessful !!! Not Enough Stock"); ';
    echo '</script>';
    }
			...
			*/
			...
			$ins_query1="insert into medi_stock (`code`,`g_name`,`b_name`,`add_qty`,`req_loc`,`status`,`location`,`rfid`,`sno`,`batch_no`,`rfid1`,`exdate`)
 values ('$code','$medi','$brand','$tprice','$p_mrn1','Pending','$p_mrn1','$sno','$runningTime','$bb_no','$rfid_medi',`$exdate`)";
mysqli_query($con,$ins_query1) or die(mysql_error());
			
			
$url = "phar_transfer?sno=$sno";
header("Location: $url"); 
...
<input type="text" id="dept" class="form-control action" list="ddd" autocomplete="off" name='dept' required style="font-weight: bold;font-size:22px;color:green" value="<?php echo $p_mrn;?>">
...
<td colspan="2" align="center"><label><strong>RFID</strong></label></td> 
...
<td colspan="1" align="center"><label><strong>Issue qty</strong></label></td>
...
<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='code' required style="font-weight: bold;font-size:22px;color:green">
	...
	<option value=''>-Select-</option>
        ...
        ?>
            <option value="<?php echo $row['code']; ?>"><?php echo $row['g_name']; ?></option>
        <?php } ?>
...
<td colspan="2" align="center"><input type="text" name="rfid_m" id="rfid_m" required value="" readonly style="font-weight: bold;font-size:22px;color:green"></td>
...
<script>
		...
		function GetDetail(str) {
			if (str.length == 0) {
				...
				document.getElementById("rfid_m").value = "";
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "phar_out1_test.php?pmrn=" + str, true);
