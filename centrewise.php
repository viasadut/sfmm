...
if($bt=="ALL")
{
$query43 = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end' and status='collected'"; 
	 
$result43 = mysqli_query($con, $query43) or die(mysqli_error());
$row43 = mysqli_fetch_assoc($result43);


$query43p = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end' and status='collected' and tresult='P'"; 
	 
$result43p = mysqli_query($con, $query43p) or die(mysqli_error());
$row43p = mysqli_fetch_assoc($result43p);


$query43n = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end' and status='collected' and tresult='N'"; 
	 
$result43n = mysqli_query($con, $query43n) or die(mysqli_error());
$row43n = mysqli_fetch_assoc($result43n);

$query43no = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end' and status='collected' and tresult=''"; 
	 
$result43no = mysqli_query($con, $query43no) or die(mysqli_error());
$row43no = mysqli_fetch_assoc($result43no);
...
$query44 = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end' and sentto='$bt' and status='collected'"; 
...
$query44p = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end' and sentto='$bt' and status='collected' and tresult='P'"; 
	 
$result44p = mysqli_query($con, $query44p) or die(mysqli_error());
$row44p = mysqli_fetch_assoc($result44p);


$query44n = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end'  and sentto='$bt' and status='collected' and tresult='N'"; 
	 
$result44n = mysqli_query($con, $query44n) or die(mysqli_error());
$row44n = mysqli_fetch_assoc($result44n);

$query44no = "SELECT COUNT(sid) FROM covidopd where ssent BETWEEN '$start' and '$end'  and sentto='$bt' and status='collected' and tresult=''"; 
	 
$result44no = mysqli_query($con, $query44no) or die(mysqli_error());
$row44no = mysqli_fetch_assoc($result44no);
...
<table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;">
    ...
    <tr>
      ...
      <th width="7%"><strong>Address</strong>
	  ...
	  <th width="5%"><strong>Status</strong>
...
echo " ,  (  ";
echo   $row43p['COUNT(sid)'];
echo " Positive  ";
echo " ,   ";
echo   $row43n['COUNT(sid)'];
echo " Negative  ";
echo " ,   ";
echo   $row43no['COUNT(sid)'];
echo " Result Pending  ";
...
echo "  To  ";
echo $end;
	$sel_query="Select * from covidopd where ssent BETWEEN '$start' and '$end' and status='collected' order by sid asc;";}
...
echo " ,  (  ";
echo   $row44p['COUNT(sid)'];
echo " Positive  ";
echo " ,   ";
echo   $row44n['COUNT(sid)'];
echo " Negative  ";

echo " ,   ";
echo   $row44no['COUNT(sid)'];
echo " Result Pending  ";
...
echo "  To  ";
echo $end;
	$sel_query="Select * from covidopd where ssent BETWEEN '$start' and '$end' and sentto='$bt' and status='collected' order by sid asc;";}

	
 
$count=1;
//$sel_query="Select * from presnew where dname='$bt' and date BETWEEN '$start' and '$end' and status='collected' order by ssent desc";
...
{ ?>    <tr>
     ...
     <td align="center"><?php echo $count; ?></td>
      ...
      <td align="center"><a target='_blank' href="covidstafd?sid=<?php echo $row["ssid"]; ?>"><?php echo $row["name"]; ?></a></td>
...
<td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><?php if($row['tresult']!='' and $dstatus =='confirmed' ){echo"<a target='_blank' href='$url'><img src='print.png' title='Print Report' width='50' height='25' /></a>";}else{echo'Report Pending';} ?></td>
...
<?php $count++; } }?>
      ...
      <td colspan ="20" align="right"><a target='_blank' href="test_pdf12center?date=<?php echo "$start"; ?>&date1=<?php echo "$end"; ?>&bt=<?php echo "$bt"; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>	
