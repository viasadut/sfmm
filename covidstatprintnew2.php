...
$stmta = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end' and status='collected'");
$dataa = $stmta->Fetch(PDO::FETCH_OBJ);
$stmt1a = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end' and sam='NEW' and status='collected'");
$data1a = $stmt1a->Fetch(PDO::FETCH_OBJ);
$stmt2a = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end'and sam='FollowUp' and status='collected'");
$data2a = $stmt2a->Fetch(PDO::FETCH_OBJ);
$stmt3a = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end'and sam='Death' and status='collected'");
$data3a = $stmt3a->Fetch(PDO::FETCH_OBJ);




$stmt = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end' and status='collected' and tp='$bt'");
$data = $stmt->Fetch(PDO::FETCH_OBJ);
$stmt1 = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end' and sam='NEW' and status='collected'and tp='$bt'");
$data1 = $stmt1->Fetch(PDO::FETCH_OBJ);
$stmt2 = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end'and sam='FollowUp' and status='collected'and tp='$bt'");
$data2 = $stmt2->Fetch(PDO::FETCH_OBJ);
$stmt3 = $db->query("SELECT COUNT(name) as tt FROM covidopd where ssent BETWEEN '$start' and '$end'and sam='Death' and status='collected'and tp='$bt'");
...
{
if($bt=='ALL'){
$this->Cell(300,10,$yya.' Samples Has Been Sent  '.'(New Sample- '. $yy1a.' & FollowUp Sample- '.  $yy2a.' & Death Sample- '.  $yy3a.')'. ' From '.$start1.' TO '.$end1,0,0,'C');
}
else
{
...
if($bt=='ALL')
	...
	{
	$stmt = $db->query("Select * from covidopd where ssent BETWEEN '$start' and '$end' and status='collected' order by sid asc");
	...
	$stmt = $db->query("Select * from covidopd where ssent BETWEEN '$start' and '$end' and tp='$bt' and status='collected' order by sid asc");
