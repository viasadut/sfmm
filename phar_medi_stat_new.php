...
<!-- Form Title -->
  ...
  <datalist id="browsers111">
						...
						<?php 
			$sql = "select distinct(location) from `phar_sale`";
...
$sel_query1="Select * from phar_sale where location='$bt' and adate BETWEEN '$start' and '$end' order by id desc";
...
{ ?>    <tr>
      ...
      <td align="center"><?php echo $row1["location"]; ?></td>
	  ...
	  <td align="center"><?php echo $row1["tprice"]; ?></td>
...
$start=date('Y-m-d',strtotime($_REQUEST["stdate"]));
$end=date('Y-m-d',strtotime($_REQUEST["endate"]));	  
	  $f="select SUM(tprice) from phar_sale where location='$bt' and adate between '$start' and '$end'";
	  $r=mysqli_query($con, $f);
	  $rr=mysqli_fetch_assoc($r);
	  
	  ?>
	  
	  <td align="center" style="font-size:22px; color:red;"><?php echo $rr["SUM(tprice)"]; ?></td>
