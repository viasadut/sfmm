...
$sel91="SELECT * FROM medi_stock WHERE `sno`='$dilu' and add_qty>0 and location in('Vaccine Center') and status='Served';";
...
$query1="update medi_stock set `add_qty`='$m_qty1' where `sno`='$dilu' and location in('Vaccine Center') and status='Served'";
...
$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`rfid`,`status`,`location`,`code`) values
			('$g_name','$d_qty','$u_price','$u_price','$user','$adate','$bb_name','$dilu','Finished','ENDOSCOPY','$code')";
...
<!-- Form Title -->
        ...
        <fieldset>
			...
			<label for="age"><strong>Medicine's Name & QTY:</strong></label>
			
			<input list="stock" name="dilu" id="dilu" class="form-control" autocomplete='Off'size="70" required>
  <datalist id="stock">

						<option value=''>-Select Medicine--</option>
				<?php 
			$sql76 = "select * from `medi_stock` where add_qty>0 and location in ('Vaccine Center') and status='Served'";
			...
			<input type="number" name="d_qty" autocomplete='Off'size="30" required>
