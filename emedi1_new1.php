...
$strSQL = "Select * from estat where pmrn= '$pmrn' and eid='$eid';";
...
$cc=$row['code'];
$rf=$row['rfid'];
$url8 = "emedi2?pmrn=$pm&eid=$ed&id=$idd&user=$user&code=$cc&rf=$rf"; 
?>
...
//var tt=document.getElementById("address1").innerHTML;
  ...
  <datalist id="stock">
				...
				<?php 
			$sql76 = "select * from `medi_stock` where location='AE' and add_qty!='0'";
			$res76 = mysqli_query($con, $sql76);
			if(mysqli_num_rows($res76) > 0) {
				while($row76 = mysqli_fetch_object($res76)) {
					echo "<option value='".$row76->rfid."'>".$row76->g_name.'-'.$row76->rfid."</option>";
				}
			}
			?> 
			  </datalist>
