<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','staff','emergency','lab','imo','mofficer','nurse','ot')"; 
...
if($cat=='HOD')
{
$sel_query="Select * from staff3 where status ='Active' and hos='$in_id' || sid='$in_id1' order by sname asc";



}

else if ($cat=='Incharge')
	
	{
		
		$sel_query="Select * from staff3 where status ='Active' and incharge='$in_id' || sid='$in_id1' order by sname asc";
...
if($cat=='HOD' and $fullname=='805')
{
$sel_query="Select * from staff3 where status ='Active' and desig='Medical Officer' order by sname asc";
...
else if($slv1>0 and $ss_id11=='0000-00-00 00:00:00')
...
$fu = $rowi['sname'];
		  ...
		  */
		  ...
		  <label>Duty Shift</label>  
			...
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->schedule_name."'>".$row->schedule_name."($row->start - $row->end)</option>";
				}
			}
			?>
