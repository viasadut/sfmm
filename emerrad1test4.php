...
$count=1;
$sel_query="Select * from einves where pmrn= '$pmrn' and eid='$eid' and status='Data Updated' and type='rad' order by `id` DESC;";
...
<td colspan="2" align="center"><a onclick="return confirm_click();" href="radresult1emer?id=<?php echo $row["id"]; ?>&pmrn=<?php echo $row["pmrn"]; ?>&eid=<?php echo $row["eid"]; ?>&user=<?php echo "$user"; ?>&infu=<?php echo $row["infusion"]; ?>"><strong>RECEIVE</strong></a></td>
...
$count=1;
$sel_query="Select * from einves where pmrn= '$pmrn' and eid='$eid' and type='rad' and rstatus IN ('RECEIVED','REJECTED') and status IN ('RECEIVED','REJECTED')   order by `id` DESC;";
