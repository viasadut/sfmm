...
$date2=date('Y-m-d',strtotime($_REQUEST["date"]));
...
$dd=date('Y-m-d');
        ...
        { 
            ...
            //print "You selected $select<br/>"; 
			{	

//$ins_query3="update opd_appoint1 set status='NOT AVAILABLE', remarks='OT' where dname='$dname2' and dslot='$select' and date1='$date2'";
//mysqli_query($con,$ins_query3);


$update="update opd_appoint1 set status='NOT AVAILABLE',remarks='$reason' where dslot='$select' and date1='$date2' and dname='$dname2' and status!='Booked'";
...
echo '<script language="javascript">';
    echo 'alert("Slot Block Successfully"); ';
    echo '</script>';
...
<!-- Form Title -->
		<h2 align="center">BLOCK DOCTOR'S TODAYS AVAILABLE SLOT</h2>
		...
		<fieldset>
            ...
            <!-- Name Input -->
			<span class="style1">
			<label for="name">Doctor's Name :<?php echo $dname2;?></label>
			
			</span>
			<label for="name">Date:<?php echo date('d/m/Y', strtotime($date2));?></label>
...
<!-- Password Input --><!-- Age Dropdown -->
			...
			$sql = "select * from opd_appoint1 where dslot>='$ct' and dname='$dname2' and date1='$date2' and status in ('AVAILABLE','Booked')";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					
					if($row->status=='AVAILABLE'){
					echo "<option value='".$row->dslot."'>".$row->dslot.'-'.$row->status."</option>";
					}
	  ...
	  <input  name="reason" type="text" size="70" value=""  />
		...
		<button type="submit" name="submit1">Confirm</button>
