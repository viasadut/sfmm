...
$sel95="SELECT COUNT(id) FROM purchase_stock WHERE `code`='$code' and location='Store' and add_qty>0;";
$result95 = mysqli_query($con,$sel95);
$b_chk=mysqli_fetch_assoc($result95);
$count_qty=$b_chk['COUNT(id)'];


$sel95w="SELECT COUNT(id) FROM purchase_stock WHERE `code`='$code' and location='Store' and batch_no='$batch_no' and add_qty>0;";
$result95w = mysqli_query($con,$sel95w);
$b_chkw=mysqli_fetch_assoc($result95w);
$count_qtyw=$b_chkw['COUNT(id)'];


$sel95wz="SELECT * FROM purchase_stock WHERE `code`='$code' and location='Store' and batch_no='$batch_no' and add_qty>0 order by id desc limit 1;";
...
$ins_query1="insert into purchase_stock (`code`,`g_name`,`b_name`,`add_qty`,`given_qty`,`exdate`,`u_price`,`t_price`,`batch_no`,`rfid`,`remarks`,`add_by`,`add_time`,`location`,`location1`)
...
$ins_query1="update purchase_stock set `add_qty`='$new_qty',`given_qty`='$new_given',`exdate`='$exdate',`u_price`='$u_price',`t_price`='$t_price',`remarks`='$remarks',`add_by`='$user',`add_time`='$ittime1' where id='$row_id' and location='Store'";
mysqli_query($con,$ins_query1) or die(mysql_error());
...
<!-- Form Title -->
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `storenew` where estatus='Active' and etype='Disposable' and elocation='Store'";
...
<label for="age"><strong>Product Name:</strong></label>						
...
<td colspan="3" ><input type="text" name="add_qty" id="" required value="" style="font-weight: bold;font-size:22px;color:green"></td>
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `purchase_stock` where location='purchase' and add_qty>0";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
