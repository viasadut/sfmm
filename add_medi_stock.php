...
$sel95wz="SELECT * FROM medi_stock WHERE `code`='$code' and location='Pharmacy' and batch_no='$batch_no' order by id desc limit 1;";
...
else if($count_qty>10 and $count_qtyw<=0)
	
	{
		
		echo '<script language="javascript">';
    echo 'alert("Cannot Add More Than Ten Batch"); ';
    echo '</script>';
...
else if($count_qty<=7 and $add_qty>0 and $user!='')

$add_date=date('Y-m-d');	
	{
		
$ins_query1="insert into medi_stock (`code`,`g_name`,`b_name`,`add_qty`,`given_qty`,`exdate`,`u_price`,`t_price`,`batch_no`,`rfid`,`remarks`,`add_by`,`add_time`,`location`,`location1`,`add_date`)
values ('$code','$g_name','$b_name','$add_qty','$add_qty','$exdate','$u_price','$t_price','$batch_no','$rfid','$remarks','$user','$ittime1','Pharmacy','$location','$add_date')";
...
$ins_query1="update medi_stock set `add_qty`='$new_qty',`given_qty`='$new_given',`exdate`='$exdate',`u_price`='$u_price',`t_price`='$t_price',`remarks`='$remarks',`add_by`='$user',`add_time`='$ittime1' where id='$row_id' and location='Pharmacy'";
mysqli_query($con,$ins_query1) or die(mysql_error());
