...
$query40 = "SELECT * FROM user where uname= '$user'"; 
...
$query39 = "SELECT * FROM endopapp where ID= '$ID'"; 
...
if(empty($_REQUEST['anes']))

{
       echo '<script language="javascript">';
    echo 'alert("No Anaes Name is selected !!"); ';
    echo '</script>';
...
{
       echo '<script language="javascript">';
    echo 'alert("No Procedure Name is selected !!"); ';
    echo '</script>';
...
else 
{
	...
	$query43t = "SELECT MAX(eid) FROM endopapp where pmrn= '$pmrn' and status !='Cancel';"; 
$result43t = mysqli_query($con, $query43t) or die(mysqli_error());
$row43t = mysqli_fetch_assoc($result43t);
$countt =$row43t['MAX(eid)'];
$count1t = $countt+1;

$query400 = "SELECT * FROM inpatient where pmrn='$pmrn' and discharge=''"; 
$result400 = mysqli_query($con, $query400) or die(mysqli_error());
$row400 = mysqli_fetch_array($result400);
$ieid=$row400['eid'];


$ins_query="update endopapp set dreffer='$ddname', tname='$tname',anes='$anes',eid='$count1t',status='Received',diagnosis='$diagnosis',ieid='$ieid' where ID='$ID'";
...
<input name="ddname" type="text" size="32"class="style1" value="<?php if(isset($_POST['load'])==1){$ddname=$_REQUEST['dname']; echo $ddname;}?>" readonly>

<br><br>
				<label for="tname45"><strong>Anaesthethist Name:</strong></label>
				
				<select name="anes" value="" class="style1" >
...
<br><br>
		...
		<br>
		<select name="tname" value="" class="style1" >
				       ...
				       <?php 
					   if(isset($_POST['load'])){
			$sql = "select * from `privilege` where dname='$ddname' and status in ('Approved','Waiting For CFO Approval','Edited')";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
    ...
    <label for="age"><strong>Diagnosis:</strong></label>
      <input name="diagnosis" type="text" size="70" class="style1" value="">
