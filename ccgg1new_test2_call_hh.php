...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('sds')"; 
...
$sel43="SELECT * FROM opd_appoint1 WHERE `dname`='$dname' and `date1`='$date_nn' and dslot='$slot' and status='Booked';";
...
$sel="SELECT * FROM pappnew WHERE `pphone`='$pphone' and `pmrn`='$pmrn' and `dname`='$dname' and adate='$date_nn1';";
...
$ins_query="insert into pappnew (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`user`,`yage`,`bdate`,`dis`,`aatime`,`adate1`) values 
('$name', '$pmrn','$pphone','$padd','$dname','$date_nn1','$slot','NOT SEEN','$diff1','$psex','$user','$diff2','$date91','$dis','$aatime','$adate_nn')";
...
<input name="pmrn" id="pmrn"onkeyup="GetDetail(this.value)" type="text" size="85" placeholder="MRN" value="<?php if(isset($_POST['load'])==1)
{ $pmrn2 = $_REQUEST['pmrn'];
echo $pmrn2;
}
?>">
      ...
      <input name="name" id="pname" type="text" size="85" value="<?php if(isset($_POST['load'])==1)
{ $pname2 = $_REQUEST['name'];
echo $pname2;
}
?>" required >
 	  <label for="age"><strong>Patient's ADDRESS :</strong></label>
      <input name="padd" id="padd" type="text" size="85" value="<?php if(isset($_POST['load'])==1)
{ $padd2 = $_REQUEST['padd'];
echo $padd2;
}
?>" required>
...
<option value='Sylhet'>Sylhet</option> 
	  ...
	  <label for="age"><strong>Patient's Gender :</strong></label>
	  	...
	  	<select name="psex" id="psex"class="style1" placeholder="Gender"  required> 
...
?></option>
	 ...
	 <input name="pphone" type="text" id="pphone"size="85" placeholder="Phone No"value="<?php if(isset($_POST['load'])==1)
{ $pphone2 = $_REQUEST['pphone'];
echo $pphone2;
}
?>" required>	  
...
<input name="dd" id="dd" type="text" maxlength="2" size="1" value="<?php if(isset($_POST['load'])==1)
{ $dd2 = $_REQUEST['dd'];
echo $dd2;
}
?>" required placeholder="DD">	/

<input name="mm" id="mm" type="text" maxlength="2" size="1" value="<?php if(isset($_POST['load'])==1)
{ $mm2 = $_REQUEST['mm'];
echo $mm2;
}
?>" required placeholder="MM"> /	

<input name="yy" id="yy" type="text" maxlength="4" size="1" value="<?php if(isset($_POST['load'])==1)
{ $yy2 = $_REQUEST['yy'];
echo $yy2;
}
?>" required placeholder="YYYY">		  
	  ...
	  <br><br> 
		...
		<label for="mail"><strong>Appointment Date :</strong></label>
									<p>
									  <input type="text" name="date_nn" id="datepicker1" placeholder="Select Date" size="85" value='<?php if(isset($_POST['load'])==1)
{ $date_new = $_REQUEST['date_nn'];
echo $date_new1=date('Y-m-d', strtotime($date_new));
}
?>' readonly>
	  ...
	  $doc1= $_REQUEST['doc'];
			...
			$sql = "select * from opd_appoint1 where dname='$dname2' and date1='$date_new1' and status='AVAILABLE'";
