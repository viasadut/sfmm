...
<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','nurse','pharmacy','ot','endo','msuite','emergency')"; 
...
$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`rfid`,`status`,`location`,`code`) values
			('$g_name','1','$u_price','$u_price','$user','$adate','$bb_name','$dilu','Finished','OPD Procedure','$code')";
...
<!-- Form Title -->
        ...
        <fieldset>
			...
			<label for="age"><strong>Medicine's Name:</strong></label>
  ...
  <datalist id="stock">
				...
				<?php 
			$sql76 = "select * from `medi_stock` where add_qty>0 and location in ('Maternity Suite')";
