...
$aa=mysqli_query($db,"SELECT * FROM patient WHERE `pmrn`='$pmrn'");
$dataa = mysqli_fetch_assoc($aa);
$ppname=$dataa['pname'];  
...
$date3=$result9['date1'];
$page=$result9['page'];
$pgender=$result9['psex'];
$pphone=$result9['pphone'];
//$pname=$data1['pname'];
...
$query160 = mysqli_query($db,"select * from radio1 where iname='$medi'");
$data160 = mysqli_fetch_assoc($query160);
$type_in=$data160["type"];
$code_in=$data160["code"];
$price_in=$data160["price"];
$type1_in=$data160["subtype"];
//$result=$data159["result"];
//$reference=$data159["reference"];
//$unit=$data159["unit"];
$link_in=$data160["link"];
$report_in=$data160["report"];
$linkv_in=$data160["linkv"];
$reportv_in=$data160["reportv"];





$query10 = mysqli_query($db,"select * from m_suite where pmrn='$pmrn' and eid='$eid'");
...
if($type_p=='OPD')
{
$ins_query="insert into alltest (`dname`,`pmrn`,`pname`,`eid`,`medi`,`ins`,`date`,`type`,`price`,`code`,`link`,`date1`,`linkv`,`report`,`reportv`,`location`,`page`,`pgender`,`subtype`,`pphone`) 
values 
('$dname', '$pmrn','$ppname','$eid1','$medi','$pins','$date','$type','$price','$code','$link','$ndate','$linkv','$report','$reportv','Maternity','$page','$pgender','$type1','$pphone')";
mysqli_query($con,$ins_query) or die(mysql_error());
}

else {
	
$ins_query="insert into iinves (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`room`,`status`,`ordate`,`type`,`code`,`price`,`subtype`,`rstatus`,`link`,`report`,`ndate`,`linkv`,`reportv`,`unit`) values ( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$odate','$infu','$user','$remarks','Data Updated','$ordate','$type_in','$code_in','$price_in','$subtype_in','Ordered','$link_in','$report_in','$ndate','$linkv_in','$reportv_in','$unit')";
mysqli_query($con,$ins_query) or die(mysql_error());
...
<tr>
<td colspan="10" align="center"><input list="browsers1" name="medi"  class="form-control" required/>
...
<td colspan="10" align="center"><textarea rows="10" name="pins" required></textarea></td>
...
$count=1;
$sel_query="Select * from alltest where pmrn= '$pmrn' and date1='$date_endo' and package='' order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["medi"]; ?></td>
				  ...
				  <td align="center" colspan="2"><a href="endodelete_ms?eid=<?php echo $eid; ?>&pmrn=<?php echo $pmrn; ?>&id=<?php echo $row['id'];?>">DELETE</a></td>
