...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('pharmacy')"; 
...
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from medi_stock where add_qty>0 and location='Pharmacy_opd'  and status in('Served','Partially Served') ORDER BY g_name asc;";
...
$sd=date('Y-m-01');
$ed=date('Y-m-31');
$queryq = "SELECT SUM(qty) from phar_sale where rfid='".$rrid."' and adate between '$sd' and '$ed' and location in ('OPD','OPD_DIS')"; 
$resultq = mysqli_query($con, $queryq) or die ( mysqli_error());
$rowq = mysqli_fetch_assoc($resultq);

$queryq5 = "SELECT * from medicine where code='".$cc."' and status='Active'"; 
...
<td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $row["sno"]; ?></td>
 ...
 <td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $rowq5["cprice"]* $row['add_qty']; ?></td>
