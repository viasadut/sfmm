...
$query43 = "SELECT * FROM phar_sale where billno='$pmrn'"; 
	 
$result43 = mysqli_query($con, $query43) or die(mysqli_error());
$row43 = mysqli_fetch_assoc($result43);
$location=$row43['location'];

if($select=='id' and $location!='Discharge')
{
//	$runningTime = date('Ymdis');
$url = "phar_opd_return_test1_opd?sno=$pmrn";
header("Location: $url"); 
}


else if($select=='id' and $location=='Discharge')
{
//	$runningTime = date('Ymdis');
$url = "phar_opd_return_test1_dis_opd?sno=$pmrn";
header("Location: $url"); 
}
...
{
	$url = "phar_opd_return_list?pmrn=$pmrn"; 
header("Location: $url"); 
