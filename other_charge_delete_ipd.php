...
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM ipd_extra_charge WHERE `id`='$id3';");
$result_p = mysqli_fetch_assoc($sel_p);
$del_id=$result_p['COUNT(id)'];
if($del_id>0){


$query = "update ipd_extra_charge set delete_status='1', delete_by='$user' WHERE id=$id3"; 
...
values ('$pmrn','DR','$tb_data','$date','$price','IPD_EXTRA_DEL')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$pmrn','CR','111999','$date','$price','IPD_EXTRA_DEL')";
...
header("Location:ipd_extra_charge1_new.php?pmrn=$pmrn&eid=$eid");
