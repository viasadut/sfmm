...
$pdf->Cell('183',6,'List Of Medicine Used',1,1,'C');
...
$pdf->MultiCell('182' , 5,$data1['medi'].' - '.$data1['pdos'].' -'.$data1['code'] ,0,1);
...
$pdf->Cell('40',5,'Infusion Used:',0,1,'L');


$pdf->ln(2);
$query1 = mysqli_query($db,"Select * from otendoinfusion where pmrn= '$pmrn' and eid='$id' and status='given' order by `id` DESC;");
...
//$pdf->MultiCell('182', 5,$data1['medi'],1,1);
$pdf->SetFont('Arial' , '' , 10);
$pdf->MultiCell('182' , 5,$data1['infusion'].' - '.$data1['addi'].' -'.$data1['qty1'].' - '.$data1['add1'].' -'.$data1['qty2'] ,0,1);
