...
<p align="center" class="style1">KPJ ONLINE ROASTER (<?php echo $month.'-'.$year;?>)</p> 
<p align="right"><div2><input style="background-color: lightblue;" type="text" id="myInput" onkeyup="myFunction()" placeholder="Search By Department Name.." title="Type in a Discipline">
</div2></p>
...
while($row = mysqli_fetch_assoc($rown)) { ?>
	  ...
	  <td align="center" style="display:none;"><?php echo $row["c_location"]; ?></td>
      <td align="center"><?php echo $row["sname"]; ?></a>
 ...
 <div id="add_data_Modal1" class="modal fade">  
      <div class="modal-dialog">  
		   ...
		   <label>Duty Location</label>  
			...
			<?php 
			$sql = "Select * from roaster_location where ;";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->loc."'>".$row->loc."</option>";
