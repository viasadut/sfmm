...
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
	...
	for($i=0;$i<count($_POST["chkDel"]);$i++)
	{
		if($_POST["chkDel"][$i] != "")
		{
			...
			$sel99="SELECT * FROM einfusion WHERE `id`='".$_POST["chkDel"][$i]."'";
...
$u_price=$b_chk_m['u_price'];
	...
	if($res95=mysqli_num_rows($result95)>0 and $mm_qty>0){		
			...
			$strSQL = "UPDATE einfusion set pstatus='Served', puser='$user', pdate='$pdate',rfid='$tfid' ";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			
			
			$ins_query4="update medi_stock set `add_qty`='$m_qty1' where code='$mcode' and add_qty!='0' and location='Pharmacy' and rfid='$tfid' ";
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from einfusion where pmrn= '$pmrn'and eid='$eid' and odate1='$dd' and pstatus='ordered'order by `id` DESC;";
...
$new_qty=$sumr['SUM(add_qty)'];
	  ...
	  <td align="center"colspan="1"><?php echo $row["ndate"]; ?></td>  
...
$dd=date('Y-m-d');
$count=1;
$sel_query="Select * from einfusion where pmrn= '$pmrn' and eid='$eid'and pstatus ='served' and odate1='$dd'order by id asc;";
