...
$query40 = "SELECT COUNT(id) FROM user where uname= '$user' and utype in ('doctor','physio','diet')"; 
...
if($res95=mysqli_num_rows($result95)==0)
{
  ...
  $sql = "insert into icnote (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`inves`,`infusion`,`pnote`,`user`,`status`,`ugroup`,`charge`,`vtype`,`daten`,`dcode`,`ccode`,`ip`,`op`,`app_con`,`ccentre`)
   values ( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$adate1','$inves','$infu','$pnote','$full','Data Updated','Doctor','$charge','$vtype','$daten','$dcode','$code','$ip','$op','$app_con','$ccentre')";
...
<input list="browsers" id="myBrowser" name="infu" required>
...
<td colspan="3"><textarea name="inves" id="porder" class="form-control action" cols="30" rows="10"required></textarea></td>
						...
						<td colspan="2" >
            <input type="text" name="charge" id="charge" required value="" readonly></td>
</tr>
