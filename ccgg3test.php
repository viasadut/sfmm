...
$ins_query="insert into pappnew (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`user`,`yage`) values ('$name', '$pmrn','$pphone','$padd','$dname','$date1','$slot','NOT SEEN','$page','$psex','$fullname','$yage')";
mysqli_query($con,$ins_query) or die(mysql_error());
$update="update test set status='Booked' where `dname`='$dname' and `ddate`='$date1' and `dslot`='$slot'";
mysqli_query($con,$update) or die(mysql_error());

$update1="update patient set bdate='$hdlate' where `pmrn`='$pmrn'";
...
<!-- Form Title -->
      ...
      <input name="pphone" type="text" size="13" value="<?php echo $data["pphone"]; ?>"readonly/>	  
		...
		<input type="text" name="date5" id="datepicker" placeholder="Select Date" size="15" value="<?php echo date('m/d/Y', strtotime($data["bdate"])); ?>">      
...
<input type="hidden" name='hdlate'value="<?php	  if(isset($_POST['load'])==1)
