...
$sel_all=mysqli_query($db,"SELECT SUM(qty) FROM cathhoscharge WHERE pmrn= '$pmrn' and ieid='$ieid'");
...
$sel990=mysqli_query($db,"SELECT * FROM storenew WHERE `ename`='$medi1';");
$result990 = mysqli_fetch_assoc($sel990);
$code=$result990['eid'];
$btype=$result990['etype'];
$price=$result990['price']*$pdos;
...
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM storenew WHERE `ename`='$medi1';");
$result_p = mysqli_fetch_assoc($sel_p);
$dis_pack=$result_p['COUNT(id)'];
...
<select id="pmrn" onchange="GetDetail(this.value)" class="con_charge" list="categoryname" autocomplete="off" name="medi1" >						<option value=''>-Select Items-</option>
...
/*			$sql76 = "select * from `disposable`";
  ...
  */    $sql77 = "select * from `storenew`";
			$res77 = mysqli_query($con, $sql77);
			if(mysqli_num_rows($res77) > 0) {
				while($row77 = mysqli_fetch_object($res77)) {
					echo "<option value='".$row77->ename."'>".$row77->ename."</option>";
				}
			}
			?>  </select>
...
$count=1;
$sel_query="Select * from cathhoscharge where pmrn= '$pmrn' and ieid='$ieid'order by `id` DESC;";
...
{ echo'
				 ...
				 <td align="center" colspan="2"><a href="cathhosdelete1?id='.$row["id"].'&pmrn='.$row['pmrn'].'&dname='.$dname.'&eid='.$eid.'&ieid='.$ieid.'&type='.$type.'&ID='.$id.'">DELETE</a></td>';
