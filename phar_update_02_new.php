...
$sel96="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
if($eqty5 >= $eqty2 and $mqty=='' and $mm_qty>=$eqty2 and $eqty2>0)
	...
	$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`,`pmrn`,`pname`,`rfid`) values
			('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','$ins','OPD','$code_1','$p_mrn','$p_name','$rf')";
...
$sel97b="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97c="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97d="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97e="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97f="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97g="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97h="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$sel97i="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='2nd Fl Pharmacy' order by exdate asc limit 1;";
...
$mcode = $row1['code'];
	  $sum = "SELECT SUM(add_qty) FROM medi_stock where code='$mcode' and location='2nd Fl Pharmacy'" ;
...
<?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row['qty']*$row1['uprice'];?>
	 ...
	 <td align="center" colspan="1"><input type='checkbox' name='update[]' value='<?= $id ?>' checked hidden></td>						
  	   ...
  	   <td align="center"colspan="1"><input type="button" name="edit" value="Serve" id="<?php echo $id; ?>" class="btn btn-info btn-xs edit_data" /></td>  		  	  
	...
	<td colspan="10"align="right" id='gtotal'style="font-weight: bold;font-size:35px;color:red"><?php echo $dd['SUM(tprice)']?></td>
	</tr>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="medi_ins_p?sno=<?php echo $sno;?>"><img src="phar_pic/barcode.png" title="Print Instruction" width="100" height="80" /></a>
 ...
 <div id="add_data_Modal" class="modal fade" tabindex="-1" >  
                ...
                <div class="modal-body">  
                     <form method="post" id="insert_form" name="frmMain2">  
                          ...
                          <input type="hidden" name="id" id="id" /> 
						   ...
						   <input type="hidden" name="sale_no" id="sale_no" value="<?php echo $sno;?>"/>
						  <input type="hidden" name="uuprice" id="uuprice" /> 
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `medi_stock` where add_qty>0 and location='2nd Fl Pharmacy'";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
...
<label for="age"><strong>Generic Name:</strong></label>							
	<textarea name="g_name" id="code7" class="form-control action" cols="1" rows="1"style="font-weight: bold;font-size:22px;color:green"disabled required>


</textarea>
<input type="hidden" name="g_name" id="code" />
...
<label for="age"><strong>Brand Name:</strong></label>						
		...
		<input type="text" name="b_name" id="brand7" required value="" disabled style="font-weight: bold;font-size:22px;color:green"></td>
		<input type="hidden" name="b_name" id="brand" />
		...
		<label for="age"><strong>Expiry Date:</strong></label>						
						...
						<input type="text" name="location" id="location7" required disabled value="" style="font-weight: bold;font-size:22px;color:green"></td>
		
 <input type="hidden" name="location" id="location" />
		
		
		<label for="age"><strong>R.Qty:</strong></label>	
						  <input type="number" name="result5" id="result57" disabled style="width:70px;"> 
		
		
		<label for="age"><strong>G.Qty:</strong></label>	
						  <input type="number" name="gqty" id="gqty7" disabled style="width:70px;"> 
		<input type="hidden" name="gqty" id="gqty" />
		
<label for="age"><strong>A. Qty:</strong></label>								

<input type="number" name="tqty" id="tqty7" required value="" disabled style="font-weight: bold;font-size:16px;color:green;width:90px">
<input type="hidden" name="tqty" id="tqty" />
...
<input type="number" name="sqty" id="sqty"  required value="" style="font-weight: bold;font-size:16px;color:green;width:80px" >
	...
	<input type="text" name="u_price" id="u_price7" disabled value="" style="font-weight: bold;font-size:20px;color:red;width:250px">
  ...
  $('#add_data_Modal').on('shown.bs.modal', function() {
    $('input[name="code"]').focus();
	  ...
	  $('#add_data_Modal').modal({backdrop: 'static', keyboard: false})  
           ...
           $.ajax({  
                url:"medi_dispense_phar_opd.php",  
                method:"POST",  
...
$('#lrfid').val(data.sno);					 
	  ...
	  //var tr = $('#sqty').val() + $('#gqty').val();
           event.preventDefault();  
			...
			var x= Number(document.getElementById("sqty").value);
			var z= Number(document.getElementById("tqty").value);
			var xx= Number(document.getElementById("gqty").value);
			var xy= Number(document.getElementById("result5").value);
			var ox= Number(+x + +xx);
			//var ox= Number(document.getElementById("result5").value);
           ...
           {  
		    ...
		    //else if(ox > z)  
			  // else if(x !='')  
           {  
                //alert(document.getElementById("demo").innerHTML = (z < x));  
				//document.getElementById("demo").innerHTML = (z > x);
				alert("Insufficient Balance1- " + z);  
           }
		   ...
		   */
           ...
           {  
                $.ajax({  
                     url:"new_phar_dispense3_opd_2nd.php",  
                     method:"POST",  
...
//						document.getElementById(
	//						"page").value = myObj[1];
							...
							document.getElementById(
							"prfid7").value = myObj[6];
							
							document.getElementById(
							"uuprice").value = myObj[7];
							
							document.getElementById(
							"uuprice7").value = myObj[7];
...
if(myObj[8]>30){
				...
				xmlhttp.open("GET", "stock_medi_test_2nd.php?pmrn=" + str, true);
