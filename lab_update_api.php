...
setInterval(function(){
	...
	function myFunction() {
		$.ajax({
			url: "view_notification_lab_api.php",
			type: "POST",
			processData:false,
...
<table width="100%" height ="100%" border="1" align="center" background-color="lightgreen" style="border-collapse:collapse;" id="myTable">
 <tr>
      ...
      <th width="20%"><strong>Consultant name </strong>
      <th width="10%"><strong>Ward</strong>   
	        <th width="10%"><strong>Bed</strong>   
			<th width="31%"><strong>Investigation</strong>   
...
$sql1="SELECT * FROM `iinves` WHERE pmrn='$pp' and eid='$ee' and status='Data Updated' and collect='0' and type in ('Lab','LAB','lab') and rstatus!='Cancelled'";
...
$result1 = mysqli_query($con,$sql1);
  ...
  echo "<td style='background-color:#eed7a1;font-size:13px;font-weight:bold'>";
   ...
   $sql2="SELECT * FROM `iinves` WHERE pmrn='$pp' and eid='$ee' and status='Data Updated' and type  in ('Lab','LAB','lab') and collect='0' and rstatus!='Cancelled'";
...
$result2 = mysqli_query($con,$sql2);
  ...
  echo"<form name='frmMain1' action='' method='post'> <input type='checkbox' name='update[]' value='".$iidd."' style='height:22px; width:22px;'>&nbsp;&nbsp;";
  echo$row2['infusion'];
  echo"<br>";
