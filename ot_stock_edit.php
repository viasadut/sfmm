...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('ot')"; 
...
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from medi_stock where add_qty>0 and location='OT MEDICINE STORE'  and status in('Served','Partially Served') ORDER BY g_name asc;";
...
<?php if($cc1==''){echo '
<a target="_blank" href="ot_hoscharge_details.php?rfid='.$row['sno'].'">

'.$rowq["SUM(pdos)"].'';} else {
	echo '
<a target="_blank" href="ot_hoscharge_details1.php?rfid='.$row['sno'].'">
