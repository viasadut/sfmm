...
$query39 = "SELECT * FROM otivisitendo where id= '$id'"; 
...
$query39a = "SELECT * FROM inpatient where pmrn= '$pmrn' and discharge=''"; 
...
$queryd1 = mysqli_query($db,"select * from doctor_code where dcode='$doc_code' and dname like '%SURGERY%'");
$datad1= mysqli_fetch_assoc($queryd1);

$code=(int)$datad1['ccode'];

$adoc_details=$doc.'-OT Charge';
$pname_new_details=$row['vtype'].' - '.$adoc_details;
...
if($api_status==0){
  ...
  if($decoded_response['invoice_no']!='' and $decoded_response['invoice_id']!=''){
    ...
    //header("Location:$url");
     ...
     $api_query = "update otivisitendo set api_status='1', invoice_no='$invoice_no' where id='".$id."'"; 
  $api_result = mysqli_query($con, $api_query) or die(mysqli_error());
   ...
   header("Location:inpatient_api?pmrn=$pmrn&eid=$eid_in");
 }
