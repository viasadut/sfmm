...
<h1 align="center"style="background-color:lightgreen;">EMERGENCY MEDICINE PANEL</h1>
<!-- Form Title -->
...
$strSQL = "Select * from estat where pmrn= '$pmrn' and eid='$eid'and status='Data Updated' order by `otime` asc;";
...
$ii=$row['id'];
$query43 = "SELECT COUNT(pstatus) FROM estat where id= '$ii' and pstatus='Ordered' and status !='Cancel';"; 
...
?>
	  ...
	  <td align="center"colspan="3"><?php echo $row["bed"]; ?></td>  
...
$cc=$row['code'];
$rf=$row['rfid'];
$url8 = "estat_medi_update?pmrn=$pm&eid=$ed&id=$idd&user=$user&code=$cc&rf=$rf"; 
?>
	  
<td colspan="1" align="center"><?php if($ps=='Served' and $row['status1']!='Returned'){echo"<a href='$url8'><strong>UPDATE<strong></a>";} else if($ps!='Served' and $row['status1']!='Returned')
{echo'<input type="button" name="edit_co" value="Use From Stock" id="'.$row['id'].'" class="btn btn-info btn-xs edit_data_co" />';}
else if($ps=='Served' and $row['status1']=='Returned')
{echo'<input type="button" name="edit_co" value="Use From Stock" id="'.$row['id'].'" class="btn btn-info btn-xs edit_data_co" />';}
 ?></a>
...
$count=1;
$sel_query="Select * from estat where pmrn= '$pmrn' and eid='$eid' and status in ('Rupdated','SEEN') order by `otime` desc;";
           ...
           <div class="modal-content">  
                ...
                <div class="modal-body">  
                     <form method="post" name="insert_form7" id="insert_form7">  
						...
						<input list="stock" name="dilu" id="dilu" class="form-control" autocomplete='Off' required>
  <datalist id="stock">

						<option value=''>-Select Medicine</option>
				<?php 
			/*$sql76 = "select * from `medi_stock` where location='AE' and add_qty>0 and status in('Served','Partially Served')";
