...
<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('msuite','doctor')"; 
...
$sel9=mysqli_query($db,"SELECT * FROM msuite WHERE `id`='$id'");
$result9 = mysqli_fetch_assoc($sel9);
$pname=$result9["pname"];


 $url = "promedicine_ms.php?pmrn=$pmrn&dname=$full&id=$id&eid=$eid&type=$type&ieid=$ieid"; 
?>
...
$ins_query1="insert into promedi_ms (`dname`,`pmrn`,`pname`,`medi`,`brand`,`pdos`,`eid`,`date`,`type`,`ieid`,`duration`,`frelation`,`code`) 
values ('$full','$pmrn','$pname','$ii','$brand2','$ii2','$eid','$date1','$type','$ieid','$ii3','$ii4','$code')";
...
$ins_query1="insert into promedi_ms (`dname`,`pmrn`,`pname`,`medi`,`brand`,`pdos`,`eid`,`date`,`type`,`ieid`,`duration`,`frelation`,`code`) 
values ('$full','$pmrn','$pname','$medi1','$brand2','$pdos','$eid','$date1','$type','$ieid','$duration','$pdos1','$code2')";
mysqli_query($con,$ins_query1) or die(mysql_error());
...
$count=1;
						...
						$sel_query="SELECT * FROM promedi_ms where pmrn='$pmrn' and eid='$eid' ORDER BY page_order";
...
while($row = mysqli_fetch_assoc($result)) 
	        ...
	        <td align="left" ><?php echo $row["brand"].' ('.$row["medi"].')'; ?></td>
				  ...
				  <td align="left"><a href="prodelete1_ms?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$dname"; ?>&eid=<?php echo "$eid"; ?>&ID=<?php echo "$id"; ?>">DELETE</a></td>
...
<script>  
 $(document).ready(function(){  
      ...
      $('#insert_form7').on("submit", function(event){  
           ...
           {  
          $.ajax({  
                     url:"edit_opd_procedure1_ms.php",  
                     method:"POST",  
