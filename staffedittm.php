...
$query39 = "SELECT * FROM staff3 where id= '$sid1'"; 
...
$query39u = "SELECT * FROM user where uname= '$sidu'"; 
	 
$result39u = mysqli_query($con, $query39u) or die(mysqli_error());

// Print out result
$datau = mysqli_fetch_array($result39u);
...
$cforward = $_REQUEST['cforward'];
$email = $_REQUEST['email'];
$scon = $_REQUEST['scon'];
$etime= date('d/m/Y H:i:s');
$mrn = $_REQUEST['mrn'];
$c_dosage = $_REQUEST['c_dosage'];
$ipd_bal = $_REQUEST['ipd_bal'];
$opd_bal = $_REQUEST['opd_bal'];



$ins_query1="update staff3 set sname='$name',gender='$psex',dob='$dob',doj='$doj',dept='$dept',subdept='$subdept',desig='$desig',
cno='$phone',padd='$padd',peradd='$peradd',district='$district',cat='$cat',eby='$user',etime='$etime',incharge='$incharge',hos='$hos',bgroup='$bgroup',cfleave='$cforward',sid1='$sid7',cstatus='$scon',email='$email',mrn='$mrn',c_dosage='$c_dosage',ipd_bal='$ipd_bal',opd_bal='$opd_bal' where id='$sid1'";
mysqli_query($con,$ins_query1) or die(mysql_error());


$ins_querya="update tm3 set dept='$dept' where uid='$sida'";
mysqli_query($con,$ins_querya) or die(mysql_error());


$ins_queryar="update user set rfid_card='$rfid' where uname='$sidu'";
mysqli_query($con,$ins_queryar) or die(mysql_error());
...
<input type="text" name="sid7" id="email" class="input-text" placeholder="SID" size="70"value="<?php echo $data['sid1'];?>"required>     
...
<select name="incharge" >
        <option value='<?php echo $data['incharge'];?>'>
		
		<?php 
		
		$query10 = "SELECT * FROM staff3 where sid1= '$inc'"; 
...
<?php 
			$sql = "select * from `staff3` where cat='incharge' and status='Active'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->sid1."'>".$row->sname."</option>";
				}
			}
			?>
...
<select name="hos" >
        <option value='<?php echo $data['hos'];?>'>
		<?php 
		
		$query10 = "SELECT * FROM staff3 where sid1= '$hos2'"; 
...
<?php 
			$sql = "select * from `staff3` where cat='hod' and status='Active'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
					echo "<option value='".$row->sid1."'>".$row->sname."</option>";
...
<label for="age"><strong>Carry Forward Levae Balance:</strong></label>
<input type="text" name="cforward" id="email" class="input-text" placeholder="Carry Forward Leave" size="70"value="<?php echo $data['cfleave'];?>"readonly>     
...
<label for="age"><strong>Staff Confirmation Status:</strong></label>
<select name="scon" >
        
		<option value='<?php echo $data['cstatus'];?>'selected><?php echo $data['cstatus'];?></option>
		<option value='Confirm'>Confirm</option>
		<option value='nonconfirm'>Nonconfirm</option>
...
<label for="age"><strong>Vaccine Status:</strong></label>

<select name="c_dosage" >
        
		<option value='<?php echo $data['c_dosage'];?>'selected><?php echo $data['c_dosage'];?></option>
		<option value='Booster Dosage Taken'>Booster Dosage Taken</option>
		<option value='Two Dosage Taken'>Two Dosage Taken</option>
		<option value='One Dosage Taken'>One Dosage Taken</option>
...
<label for="age"><strong>IPD Amount:</strong></label>
<input type="text" name="ipd_bal" id="mrn" class="input-text" placeholder="MRN" size="70" value="<?php echo $data['ipd_bal'];?>">     


<label for="age"><strong>OPD Amount:</strong></label>
<input type="text" name="opd_bal" id="mrn" class="input-text" placeholder="MRN" size="70" value="<?php echo $data['opd_bal'];?>">     

<label for="age"><strong>RFID:</strong></label>
<input type="text" name="rfid" id="mrn" class="input-text" placeholder="RFID" size="70" value="<?php echo $datau['rfid_card'];?>">     
 ...
 <a target='_blank' href="seducationstaff?sid=<?php echo "$sid1"; ?>"><b>Education Record<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="strainingstaff?sid=<?php echo "$sid1"; ?>"><b>Training Record<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="spromotionstaff?sid=<?php echo "$sid1"; ?>"><b>Promotion Record<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="stransferstaff?sid=<?php echo "$sid1"; ?>"><b>Transfer Record<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="snidstaff?sid=<?php echo "$sid1"; ?>"><b>Upload NID / Passport<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="staff_equipment?sid=<?php echo "$sid1"; ?>"><b>Material Given<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="snidstafftest?sid=<?php echo $data['sid']; ?>"><b>Upload Picture<b></a>&nbsp;&nbsp;&nbsp;&nbsp;<a target='_blank' href="add_dependencies?sid=<?php echo "$sidu"; ?>"><b>Add Dependencies<b></a>
