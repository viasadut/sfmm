...
$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
/*$sel1=mysqli_query($db,"SELECT * FROM disposable WHERE `disname`='$medi1';");
...
$medi1_api = $medi1.'- ENDOSCOPY';

$dcode=$result1["code"];
$tprice=(int)$result1["ipd_charge"]*$pdos;
...
/*$sel990="SELECT * FROM disposable WHERE `disname`='$medi1';";
...
/*if($res990=mysqli_num_rows($result990)==0)
{
  ...
  $query3 = "SELECT * FROM endohoscharge where pmrn= '$pmrn' and eid='$eid' and date='$date1' and medi='$medi1'"; 
	 
  $result3 = mysqli_query($con, $query3);
  
  // Print out result
  
  $query4 = "SELECT * FROM endohoscharge where pmrn= '$pmrn' and eid='$eid' and date='$date1'and medi='$medi1'"; 
...
$sql = "insert into endohoscharge (`pmrn`,`pname`,`medi`,`eid`,`date`,`pdos`,`code`,`price`,`ieid`,`type`,`remarks`,`ip`,`op`,`acct_code`,`ccentre`)
values ('$pmrn','$pname','$ii','$eid','$date1','$pdos','$code','$t_price','$ieid','$sub_type','Package','$ip','$op','$acct_code','$ccentre')";
...
values ('$last_id','CR','$tb_data','$date','$t_price','ENDOSCOPY')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','DR','615100','$date','$t_price','ENDOSCOPY')";
...
$invoice_no=$decoded_response['invoice_id'];
 ...
 //$api_query = "update endohoscharge set api_status='1', invoice_no='$invoice_no' where id='".$last_id."'"; 
//$api_result = mysqli_query($con, $api_query) or die(mysqli_error());
  ...
  //else {
   ...
   $sql = "insert into endohoscharge (`pmrn`,`pname`,`medi`,`eid`,`date`,`pdos`,`code`,`price`,`ieid`,`type`,`ip`,`op`,`acct_code`,`ccentre`)
   values ('$pmrn','$pname','$medi1','$eid','$date1','$pdos','$dcode','$tprice','$ieid','$sub_type','$ip','$op','$acct_code','$ccentre')";
...
$ins_query="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','CR','$tb_data','$date','$tprice','ENDOSCOPY')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','DR','615100','$date','$tprice','ENDOSCOPY')";
mysqli_query($con,$ins_query2) or die(mysql_error());
   ...
   //echo json_encode($data);
    ...
    // $api_result = mysqli_query($con, $api_query) or die(mysqli_error());
     ...
     header("Location:endochargenurse1?pmrn=$pmrn&eid=$eid&ID=$id");
...
$(document).ready(function() {
    $('.con_charge1').select2();
			...
			<td  colspan="5"align="center"><input type="number" name="pdos" class="form-control" placeholder="QTY" required>
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["medi"]; ?></td>
				 ...
				 <td align="center" colspan="2"><a href="endohosdelete?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>&id1=<?php echo "$id"; ?>&pdos=<?php echo $row['pdos']; ?>&reuse=<?php echo $row['reuse']; ?>&admission_no=<?php echo $api_adminssion_no;?>&invoice_no=<?php echo $row['invoice_no'];?>&code=<?php echo $row['code'];?>&price=<?php echo $row['price'];?>">DELETE</a></td>
