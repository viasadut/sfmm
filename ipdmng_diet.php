<?php 
   session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','diet')"; 
...
$query43 = "SELECT COUNT(room1) FROM inpatient where (room1 like '%NICU%' OR room1 like '%PICU%' OR room1 like '%HMD%')and anew BETWEEN '$start' and '$end';"; 
	 
$result43 = mysqli_query($con, $query43) or die(mysqli_error());
$row43 = mysqli_fetch_assoc($result43);


$query45 = "SELECT COUNT(room1) FROM inpatient where room1 like '%PICU%'and anew BETWEEN '$start' and '$end';"; 
...
<h1 align="center">Inpatient Admission Report</h1>
...
<!-- Form Title -->
        <table align="center" class="table table-bordered" id="dynamic_field">  
						...
						<td colspan="2"><label><strong>Select End Date:</strong></label></td>	
							...
							<td colspan="3"><label><strong> Select Bed Type</strong></label></td> 
  ...
  <select id="browsers1115" name="bt"  style="text-transform:uppercase"required>
						...
						<option value='NICU'>NICU/PICU/HMD</option>
...
$sel_query="Select * from inpatient where anew BETWEEN '$start' and '$end' order by anew";}
 else if (($_POST['bt'])=="NICU"){
	 echo "<font color=blue font size=5> Total Record found in the search  -";
echo   $row43['COUNT(room1)'];
echo " ,  From  ";
echo $start;
echo "  To  ";
echo $end;
	 $sel_query="Select * from inpatient where  anew BETWEEN '$start' and '$end' and (room1 LIKE '%NICU%' OR room1 LIKE '%PICU%' OR room1 like '%HMD%') order by anew";
 } 

 /*else if (($_POST['bt'])=="PICU"){
  echo "<font color=blue font size=5> Total Record found in the search  -";
echo   $row45['COUNT(room1)'];
echo " ,  From  ";
echo $start;
echo "  To  ";
echo $end;
  $sel_query="Select * from inpatient where  anew BETWEEN '$start' and '$end' and room1 like '%PICU%' order by anew";
}*/ 
$count=1;
