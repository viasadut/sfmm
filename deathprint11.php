...
$query = mysqli_query($db,"select * from deathn where id='$id'");
$data = mysqli_fetch_array($query);
$i_by=$data['iby'];

$query1 = mysqli_query($db,"select * from staff3 where sname='$i_by'");
...
$this->ln(2);
$this->SetFont('Arial','B',8);
$this->Cell(0,10,'Contact Numbers:  Ambulance:  +880244077029, +8801791987466, Appointments: +880244077030, +8801703788561 (SFMMKPJSH/NSG/MR-17)',0,0,'C');
...
$pdf->Cell('183',6,'DEATH CERTIFICATE',1,1,'C');
...
$pdf->Cell('35',5,$data['ward'],1,0,'L');
$pdf->Cell('15',5,'Bed:',1,0,'L');
$pdf->Cell('35',5,$data['bed'],1,0,'L');

$pdf->Cell('40',5,'Date Of Admission:',1,0,'L');
$pdf->Cell('43',5,$data['adm'],1,1,'L');
//$pdf->Cell('43',5, date('d/m/Y h:i:s a',strtotime($data['adm_new'])),1,1,'L');
//$time=new DateTime('Y-m-d h:i:s a',strtotime($data['patient_receive_at']));
...
$pdf->ln(3);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('50',5,'Date and Time of Death:',0,0,'L');
$pdf->SetFont('Arial' , '' , 10);
...
$pdf->ln(3);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('50',5,'Cause of Death:',0,1,'L');
$pdf->SetFont('Arial' , '' , 10);
$pdf->MultiCell('182' , 5,$data['cdeath'],0,1);
...
$pdf->Cell('70',5,'Name and Signature of certifying Doctor',0,1,'L');
$pdf->Cell('70',5,$data['iby'],0,1,'L');
$pdf->Cell('70',5,$data1['desig'],0,1,'L');
