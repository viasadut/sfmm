...
$query39 = "SELECT * FROM cathhoscharge where id= '$id'"; 
...
$acode=(int)$row['code'];
$aprice=(int)$row['qty'];
$medi=$row['medi'].'- Cathlab'.$row['ctype'];
...
$invoice_no=$decoded_response['invoice_id'];
 ...
 if($decoded_response['invoice_no']!='' and $decoded_response['invoice_id']!=''){
  ...
  $ins_query="UPDATE cathhoscharge SET api_status='1', invoice_no='$invoice_no' WHERE id='$id'";
