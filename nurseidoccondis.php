<?php 
    ...
    if(!isset($_SESSION['sess_username']) || $role!="nurse"){
      	header('Location: /sfmm/login2?err=2');
    }
			...
			try {
				...
				$dbh->query("INSERT INTO `rfid_transaction` (
						...
						'$patient_rfid_v',
							'Receive',
								'$user_id',
									'Patient Release, MRN-.$pmrn',
										'$transaction_by_dept',
											'$transaction_by_dept'
					)");
				$dbh->commit();
				header("location:nurseidoccondis.php");
			} catch (\Throwable $e) {
				$dbh->rollback();
				throw $e;
                        ...
                        <div class="card-body">
							<table>
								...
								<tr>
									...
									<td>Print :</td>
									<td><b><a target='_blank' href="idisreport?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>"><img src="print.png" title="Print Report" width="100" height="60" /></a></b></td>
								</tr>
								</tr><tr>
									<td>Confirm:</td>
									<td><b><a onclick="return confirm_click();" href="ndischarge?eid=<?php echo $eid; ?>&pmrn=<?php echo $pmrn; ?>&room1=<?php echo "$room1"; ?>&room=<?php echo "$room"; ?>&vc=<?php echo "$vc"; ?>&vc1=<?php echo "$vc1"; ?>&ac=<?php echo "$ac"; ?>&user=<?php echo "$user_id"; ?>">Click To Vacant the Bed</a></b></td>
								</tr>
							</table>
                        </div>
                    </div>
