...
$g_name = str_replace("'", "''",$_REQUEST['g_name']);
...
$ins_query1="insert into po_table1 (`code`,`name`,`brand`,`stock`,`o_qty`,`uprice`,`tprice`,`po_ono`,`po_id`,`pid`,`p_remarks`)
 values ('$code','$g_name','$b_name','$stock','$order_qty','$u_price','$t_price','$po_ono','$po_id','$p_id','$p_remarks')";
...
$ins_query1="update po_table1 set `o_qty`='$sum',`tprice`='$t_price_sum',p_remarks='p_remarks' where `po_ono`='$ono4' and code='$code'";
mysqli_query($con,$ins_query1) or die(mysql_error());
...
<!-- Form Title -->
  ...
  if($status=='' || $status=='Rejected' and $user=='1603'){echo'<div style="position: relative;left: 900px;">
...
else if($status=='' || $status=='Rejected' and $user!='1603'){echo'<div style="position: relative;left: 900px;">
...
<?php
	  ...
	  <td colspan="3">
<input type="text" name="b_name" id="brand" required value="" required style="font-weight: bold;font-size:14px;color:green">
</td>
	...
	<td colspan="3">	
			<input type="number" name="order_qty" id="qty" required value=""  style="font-weight: bold;font-size:14px;color:green" step="any"></td>
...
<input type="hidden" name="p_id" id="p_id" value="" style="font-weight: bold;font-size:22px;color:green"required>
		...
		<tr>
		<td colspan="20" >
<textarea  name="p_remarks" id="p_remarks" style="font-weight: bold;font-size:22px;color:green" placeholder="Remarks"></textarea>
</td>
		...
		<?php if($status=='' || $status=='Rejected'){echo'
...
<?php 
	  $id=$row["id"];
	  $user7=$row["user"];
	  $url7 = "delete_po_item_purchase?id=$id&ono=$encryption"; 
	  
	  if($user7==$full and $status=='' || $status=='Rejected'){echo"
