<?php
//require('force_justify.php');
//require('fpdf/fpdf.php');


require('force_justify.php');
$pmrn=$_REQUEST['pmrn'];
$dname=$_REQUEST['adoc'];
$date=$_REQUEST['adate'];
$eid=$_REQUEST['eid'];
$id=$_REQUEST['id'];



$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$query = mysqli_query($db,"select * from inpatient where pmrn='$pmrn' and adoc='$dname' and adate='$date' and eid='$eid'");
$data = mysqli_fetch_array($query);

$query1 = mysqli_query($db,"select * from preadm where id='$id'");
$data1 = mysqli_fetch_array($query1);




//$db = new PDO('mysql:host=localhost;dbname=sfmmkpj','root','');
class myPDF extends FPDF{
function header(){
$this->Image('logo.jpg',15,7);
$this->Image('logo1.jpg',180,7);
$this->SetFont('Arial','B',12);
$this->Cell(190,5,'SHEIKH FAZILATUNESSA MUJIB MEMORIAL',0,0,'C');
$this->Ln(3);
$this->SetFont('Arial','B',12);
$this->Cell(195,10,'KPJ SPECIALIZED HOSPITAL AND NURSING COLLEGE',0,0,'C'); 
$this->ln(5);
$this->SetFont('Arial','B',12);
$this->Cell(190,10,'C/12, Tetuibari, Kashimpur, Gazipur, Bangladesh.',0,0,'C'); 
$this->ln(10);

}
function footer(){
$this->SetY(-8);
$this->SetFont('Arial','B',8);
$this->Cell(0,10,'Page'.$this->PageNo().' /(SFMMKPJ)',0,0,'C');

}




//$this->Ln();
}


$pdf = new myPDF();
$pdf->AliasNbPages();
$pdf->AddPage('P','A4',0);
$pdf->SetFont('Arial' , 'b' , 9);
$pdf->SetLeftMargin('15');
//$pdf->headerTable();
//$pdf->viewTable($db);
$pdf->SetFont('Arial' , 'b' , 16);
$pdf->Cell('183',6,'ADMISSION FORM',1,1,'C');
//$this->SetFont('Arial','B',);
$pdf->ln(1);
$pdf->SetFont('Arial' , '' , 9);
$pdf->Cell('178',5,'Episode:',0,0,'R');
$pdf->Cell('5',5,$data['eid'],0,0,'L');

$pdf->ln(6);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('30',5,'Patient Name:',1,0,'L');
$pdf->Cell('90',5,$data['pname'],1,0,'L');
$pdf->SetFont('Arial' , 'b' , 9);
$pdf->Cell('5');
$pdf->Cell('30',5,'MRN:',1,0,'L');
$pdf->Cell('28',5,$data['pmrn'],1,1,'L');
$pdf->ln(2);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('30',5,'Patient Adress:',1,0,'L');
$pdf->Cell('90',5,$data['padd'],1,0,'L');
$pdf->Cell('5');
$pdf->Cell('30',5,'Gender:',1,0,'L');
$pdf->Cell('28',5,$data['gender'],1,1,'L');
$pdf->ln(2);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('30',5,'Patient Age:',1,0,'L');
$pdf->Cell('20',5,$data['age'],1,0,'L');
$pdf->Cell('2');
$pdf->Cell('30',5,'Admission Date:',1,0,'L');
$pdf->Cell('38',5,$data['adate'],1,0,'L');
$pdf->Cell('5');
$pdf->Cell('30',5,'Ward:',1,0,'L');
$pdf->Cell('28',5,$data['room'],1,1,'L');
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->ln(2);
$pdf->Cell('30',5,'Visitor Card:',1,0,'L');
$pdf->Cell('2');
$pdf->Cell('27',5,$data1['vcard'],1,0,'L');
$pdf->Cell('2');
$pdf->Cell('27',5,$data1['vcard1'],1,0,'L');
$pdf->Cell('2');
$pdf->Cell('33',5,$data1['acard'],1,0,'L');
$pdf->Cell('2');
$pdf->Cell('18',5,'Bed:',1,0,'L');
$pdf->Cell('40',5,$data['room1'],1,1,'L');
$pdf->ln(2);
$pdf->Cell('30',5,'Payment Type:',1,0,'L');
$pdf->Cell(2);
$pdf->Cell('60',5,$data1['pmode'],1,0,'L');
$pdf->Cell(2);
$pdf->Cell('30',5,'Payment By:',1,0,'L');
$pdf->Cell(2);
$pdf->Cell('57',5,$data1['premarks'],1,1,'L');

$pdf->ln(5);
$pdf->SetFont('Arial' , 'b' , 16);
$pdf->Cell('183',6,'FINDINGS ON ADMISSION',1,1,'C');
$pdf->ln(5);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('60',5,'Diagnosis:',1,0,'L');
$pdf->MultiCell('123',5,$data1['diagnosis'],1,1);
$pdf->ln(2);
$pdf->Cell('60',5,'Instruction On Admission:',1,0,'L');
$pdf->MultiCell('123',5,$data1['formo'],1,1);
$pdf->ln(2);
$pdf->Cell('60',5,'Suggested Date Of Admission:',1,0,'L');
$pdf->MultiCell('123',5,$data1['adate'],1,1);

$pdf->ln(2);
$pdf->Cell('60',5,'Plan:',1,0,'L');
$pdf->MultiCell('123',5,$data1['plan'],1,1);

$pdf->ln(2);
$pdf->Cell('60',5,'Probable Date of Discharge:',1,0,'L');
$pdf->MultiCell('123',5,$data1['pdischarge'],1,1);

$pdf->ln(2);
$pdf->Cell('60',5,'Remarks:',1,0,'L');
$pdf->MultiCell('123',5,$data1['remarks'],1,1);

$pdf->ln(5);
$pdf->SetFont('Arial' , 'b' , 16);
$pdf->Cell('183',6,'OTHER DETAILS',1,1,'C');
$pdf->ln(5);
$pdf->SetFont('Arial' , 'b' , 10);
$pdf->Cell('60',5,'Father Name:',1,0,'L');
$pdf->MultiCell('123',5,$data1['fname'],1,1);
$pdf->Cell('60',5,'Mother Name:',1,0,'L');
$pdf->MultiCell('123',5,$data1['mname'],1,1);
$pdf->Cell('60',5,'Permanent Address:',1,0,'L');
$pdf->MultiCell('123',5,$data1['peradd'],1,1);
$pdf->Cell('60',5,'National ID:',1,0,'L');
$pdf->MultiCell('123',5,$data1['nid'],1,1);
$pdf->Cell('60',5,'Occupation:',1,0,'L');
$pdf->MultiCell('123',5,$data1['pocu'],1,1);
$pdf->Cell('60',5,'Monthly:',1,0,'L');
$pdf->MultiCell('123',5,$data1['mincome'],1,1);
$pdf->Cell('60',5,'No Of Spouse:',1,0,'L');
$pdf->MultiCell('123',5,$data1['wife'],1,1);
$pdf->Cell('60',5,'No Of Children:',1,0,'L');
$pdf->MultiCell('123',5,$data1['child'],1,1);
$pdf->Cell('60',5,'Income Source Of Dependent:',1,0,'L');
$pdf->MultiCell('123',5,$data1['isource'],1,1);
$pdf->Cell('60',5,'Land in Favor of Patients (in Acre):',1,0,'L');
$pdf->MultiCell('123',5,$data1['land'],1,1);
$pdf->Cell('60',5,'Service Place:',1,0,'L');
$pdf->MultiCell('123',5,$data1['service'],1,1);
$pdf->Cell('60',5,'Patients possession:',1,0,'L');
$pdf->MultiCell('123',5,$data1['poss'],1,1);
$pdf->Cell('60',5,'Member of any political party:',1,0,'L');
$pdf->MultiCell('123',5,$data1['political'],1,1);



$pdf->ln(20);

$pdf->Cell('35',5,'Patient Party Signature',0,0,'L');
$pdf->Cell('95');
$pdf->Cell('85',5,'Business Office Officer Signature',0,0,'L');
$pdf->ln(40);
$pdf->SetFont('Arial' , 'b' , 8);
$pdf->Cell('182',5,'NOTE: VISITOR CARDS ARE APPLICABLE FOR WARD AND CABIN ONLY. KINDLY RETURN THE CARDS DURING DISCHARGE.',0,0,'L');


$pdf->Output();