...
if(empty($_REQUEST['update']))
{
    ...
    echo '</script>';
			...
			$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$sel1=mysqli_query($db,"SELECT * FROM hits_list1 WHERE `code`='$code_1';");
...
if($eqty5 >= $eqty2 and $mqty=='' and $mm_qty>=$eqty2 and $eqty2>0)
	...
	$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`,`pmrn`,`pname`,`rfid`,`ip`,`op`,`acct_code`,`ccentre`) values
			('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','$ins','OPD','$code_1','$p_mrn','$p_name','$rf','$ip','$op','$acct_code','$ccentre')";
...
<input class="iquantity" name="eqty1_<?= $id ?>" onchange='subTotal()' id="eqty1" type="number" value="0" required <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>>
