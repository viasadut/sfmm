...
$this->Cell('183',6,'PATIENTS BED TRANSFER HISTORY',0,1,'C');
$this->Ln();
}
...
$stmt1 = $db->query("SELECT COUNT(pmrn) as tt FROM newbed where pmrn= '$bt' and adate BETWEEN '$start' and '$end'");
$data1 = $stmt1->Fetch(PDO::FETCH_OBJ);

{

$this->Cell(193,10,$data->pname.'   Have been transfered '.$data1->tt.' Times  '. 'From '.$_REQUEST['date'].' TO '.$_REQUEST['date1'],0,0,'C');
...
$stmt = $db->query("Select * from newbed where pmrn='$bt' and adate BETWEEN '$start' and '$end'");
...
$this->Cell(18,10,$data->bno,1,0,'L');
