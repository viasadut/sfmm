<?php 
    require('db1.php');
/*$sel7="Select COUNT(id) from noti where user in ('all','$user') and sa='0'";

$resu7 = mysqli_query($con,$sel7);
$rw7 = mysqli_fetch_assoc($resu7);
*/

/*$_SESSION['id'] = $rw7['COUNT(id)'];
echo $pid = $_SESSION['id'];

*/

    session_start();

	$role = $_SESSION['sess_userrole'];
	
    $objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");

$cdate=date('Y-m-d');


    $strSQL1 = "select DISTINCT MAX(s_no) from iinves where rdate='$cdate'";
			$objQuery1 = mysqli_query($objConnect,$strSQL1);
			$obj = mysqli_fetch_array($objQuery1);
			$mno=$obj['MAX(s_no)']+1;
			$mno1=$obj['MAX(s_no)'];


			$strSQL1e = "select DISTINCT MAX(s_no) from einves where rdate='$cdate'";
			$objQuery1e = mysqli_query($objConnect,$strSQL1e);
			$obje = mysqli_fetch_array($objQuery1e);
			$mnoe=$obje['MAX(s_no)']+1;
			$mno1e=$obje['MAX(s_no)'];

            /*$strSQL5 = "select COUNT(s_no) from alltest where rdate='$cdate' and s_no=$mno";
			$objQuery5 = mysqli_query($objConnect,$strSQL5);
			$obj5 = mysqli_fetch_array($objQuery5);
			$count_result=$obj5['COUNT(s_no)'];

            
			*/

            $new_bar1=date('myd').'222'.$mno;
			$new_bar2=date('myd').'22'.$mno;
			$new_bar3=date('myd').'22'.$mno;
			$new_bar4=date('myd').$mno;

            $strSQL5 = "select COUNT(barcode1) from iinves where barcode1 in ('$new_bar1','$new_bar2','$new_bar3','$new_bar4')";
			$objQuery5 = mysqli_query($objConnect,$strSQL5);
			$obj5 = mysqli_fetch_array($objQuery5);
			$count_result=$obj5['COUNT(barcode1)'];


			$new_bar1e=date('dmy').'333'.$mnoe;
			$new_bar2e=date('dmy').'33'.$mnoe;
			$new_bar3e=date('dmy').'33'.$mnoe;
			$new_bar4e=date('dmy').$mnoe;

            $strSQL5e = "select COUNT(barcode1) from einves where barcode1 in ('$new_bar1','$new_bar2','$new_bar3','$new_bar4')";
			$objQuery5e = mysqli_query($objConnect,$strSQL5e);
			$obj5e = mysqli_fetch_array($objQuery5e);
			$count_resulte=$obj5e['COUNT(barcode1)'];
			
			$host1 = $_SERVER["REMOTE_ADDR"];
			$host2= explode(".", $host1);
			$ip = substr($host2[3], -1);
			//$host[3]=



	?>

<!DOCTYPE html>

<body>
    
<input type="hidden" name='serial' value="<?php echo $mno;?>"> 
<input type="hidden" name='seriale' value="<?php echo $mnoe;?>"> 
<input type="hidden" name="host" value="<?php echo $ip;?>">


</body>


</html>