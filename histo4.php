...
$sel_query="Select * from histo where status1 !='Cancel' and status='REPORT DONE' ORDER BY id asc;";
...
$sel_query="Select * from alltest where medi in ('HISTOLOGY- SMALL','HISTOLOGY - LARGE','REQUEST FOR HISTOLOGY PARAFFIN','REQUEST FOR HISTOLOGY/CYTOLOGY','SECOND OPINION HISTOPATHOLOGY','HISTOPATHOLOGY-MEDIUM') and status='REPORT DONE' ORDER BY id asc;";
...
$sel_query="Select * from iinves where infusion in ('HISTOLOGY- SMALL','HISTOLOGY - LARGE','REQUEST FOR HISTOLOGY PARAFFIN','REQUEST FOR HISTOLOGY/CYTOLOGY','SECOND OPINION HISTOPATHOLOGY','HISTOPATHOLOGY-MEDIUM') and status='REPORT DONE' ORDER BY id asc;";
...
$sel_query="Select * from proinves where medi in ('HISTOLOGY- SMALL','HISTOLOGY - LARGE','REQUEST FOR HISTOLOGY PARAFFIN','REQUEST FOR HISTOLOGY/CYTOLOGY','SECOND OPINION HISTOPATHOLOGY','HISTOPATHOLOGY-MEDIUM') and status='REPORT DONE' ORDER BY id asc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
      ...
      <td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><?php echo $row["status"];?> </td> 
	       ...
	       <td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><a href="histo4doc?pmrn=<?php echo $row["pmrn"]; ?>&id=<?php echo $row["id"];?>&dname5=<?php echo $row["user"];?>&daten=<?php echo $row["ndate"];?>">UPDATE</a> </td>
		   

<td align="center"><a href="labreceivehisto2?pmrn=<?php echo "$row[pmrn]"; ?>&id=<?php echo "$row[id]"; ?>&eid=<?php echo "$row[eid]"; ?>"><strong>RECEIVE</strong></a></td>	  
