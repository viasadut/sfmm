...
$sel1=mysqli_query($db,"SELECT * FROM hits_list WHERE `code`='$medi6';");
$result1 = mysqli_fetch_assoc($sel1);
$medi1 = str_replace("'", "''",$result1['item_name']);
$medi2 = $result1['item_name'];
$medi1_api = $medi1.' - OT';

$code=(int)$result1["code"];
$tprice=$result1["ipd_charge"]*$pdos;
$u_price=(int)$result1['ipd_charge'];

$btype=$result1['sub_type'];
...
$sel990="SELECT * FROM hits_list WHERE `item_name`='$medi1';";
...
if($res990=mysqli_num_rows($result990)==0)
{
  ...
  $sql = "insert into othoscharge (`pmrn`,`pname`,`medi`,`eid`,`date`,`pdos`,`code`,`ins`,`nuser`,`ip`,`op`,`acct_code`,`ccentre`,`ieid`) values 
  ('$pmrn','$pname','$medi1','$id','$date1','$pdos','$code','$tprice','$user','$ip','$op','$acct_code','$ccentre','$ieid')";
...
<tr>
<td colspan="2" align="center"><input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control" list="browsers2" autocomplete="off" name='medi6' required style="font-weight: bold;font-size:22px;color:green">
  <datalist id="browsers2">
            ...
            $query = "select * from `hits_list` where status='0'";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
            <option value="<?php echo $row['code']; ?>"><?php echo $row['item_name'].','.$row['code']; ?></option>
        <?php } ?>  </datalist></td>
...
{ echo'
				 ...
				 <td align="center" colspan="2"><a href="othosdelete1_new1?id='.$row["id"].'&pmrn='.$pmrn.'&eid='.$id.'&rfid='.$row["rfid"].'&reuse='.$row["reuse"].'&pdos='.$row["pdos"].'&admission_no='.$admission_id.'&invoice_no='.$row['invoice_no'].'&code='.$row['code'].'&price='.$row['ins'].'">DELETE</a></td>';
...
<script>
		...
		function GetDetail(str) {
			if (str.length == 0) {
				...
				xmlhttp.open("GET", "fetch_api_data_ot.php?pmrn=" + str, true);
