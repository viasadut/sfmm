...
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
	...
	for($i=0;$i<count($_POST["chkDel"]);$i++)
	{
		if($_POST["chkDel"][$i] != "")
		{
						...
						$sel99="SELECT * FROM estat WHERE `id`='".$_POST["chkDel"][$i]."'";
...
$m_id=$b_chk_m['id'];
			...
			$strSQL = "UPDATE estat set status1='Returned', ruser='$user', rdate='$pdate' ";
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from estat where pmrn= '$pmrn' and eid='$eid' and status in ('Cancel','Data Updated') and status1 !='Returned' and pstatus='Served' order by `infusion` asc;";
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from estat where pmrn= '$pmrn' and eid='$eid'and status !='Cancel' and status1='Returned' and pstatus='Served' order by `infusion` asc;";
