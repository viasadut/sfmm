...
$update="update opd_appoint1 set status='NOT AVAILABLE',remarks='$reason' where dslot='$select' and date1='$dd' and dname='$name' and status!='Booked'";
...
<!-- Form Title -->
		<h2 align="center">BLOCK DOCTOR'S AVAILABLE DATE &amp; TIME </h2>
...
<input name="load" type="submit" id="load" value="Check Slots">
			...
			<select id='select' name = 'select[]' multiple size=60>   
			  ...
			  if(isset($_POST['load'])==1){
				...
				if($date11==$tday){
			$sql = "select * from opd_appoint1 where dslot>='$ct' and dname='$dname2' and date1='$date11' order by dslot";
