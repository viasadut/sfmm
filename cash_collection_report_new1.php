...
$bt=$_REQUEST["bt"];
//$id=$_REQUEST["id"];
$query43_p = "SELECT SUM(amount_receive) FROM pms_bill where date between '$start' and '$end' and location IN ('OPD-DIS','OTC_Sale','OPD_Medi');"; 
...
echo  $row43_p['SUM(amount_receive)'];
echo " BDT ,  From  ";
echo $start;
echo "  To  ";
echo $end;
...
$sel_query="Select * from pms_bill where date BETWEEN '$start' and '$end' and location IN ('OPD-DIS','OTC_Sale','OPD_Medi')  group by user";
 } 
$count=1;
...
$bill_user=$row['user'];
$bill_date=$row['date'];
$query43 = "SELECT SUM(amount_receive) FROM pms_bill where user= '$bill_user'and date='$bill_date' and dname !='IPD ADVANCE' and location IN ('OPD-DIS','OTC_Sale','OPD_Medi');"; 
	 
$result43 = mysqli_query($con, $query43) or die(mysqli_error());
$row43 = mysqli_fetch_assoc($result43);

$query43_ipd = "SELECT SUM(amount) FROM pms_bill where user= '$bill_user'and date='$bill_date' and dname='IPD ADVANCE' and location IN ('OPD-DIS','OTC_Sale','OPD_Medi');"; 
...
$user_name_d = mysqli_fetch_assoc($user_name_r);
      ...
      <td align="center">
      <a target='_blank' href="cashier_details_phar?user=<?php echo $row['user'];?>&date=<?php echo $bill_date; ?>">
      <?php echo $row43["SUM(amount_receive)"]+$row43_ipd['SUM(amount)']; ?></a>   </td>
	        <td align="center">
