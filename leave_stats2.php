...
<?php 
   session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('doctor','staff','mng','ot','endo','imo','mofficer','nurse','emergency','moopd','lab','rad','diet','physio')"; 
...
<h1 align="center">Departmental Carry Forward Leave Balance</h1>
...
<option ="<?php echo $dept1;?>"><?php echo $dept1;?></option>
<?php if ($dept2!=''){echo'
<option ="'.$dept2.'">'.$dept2.'</option>';}?>
...
$sel_query="Select * from staff3 where dept in ('$dept','$gd1') and status='Active' order by cat asc";
...
<td align="center"><?php if($bl>0){echo $bl;} else {echo '0';}?></td>
	...
	<td> 
	 ...
	 <?php 
	  ...
	  else if($status1=='nonconfirm'){echo '0';}
	 ...
	 if($status1=='Confirm'){echo $sl1s;} 
	  else if($status1!='Confirm'){echo $sl1s_y;}
	  ?>
	  </td>
