...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><h1 style="color:red"><strong>Datewise Pharmacy Report</strong></h1></label></td> </tr>
...
<td colspan="2"><label><strong>Select Start Date:</strong></label></td>
            ...
            <td colspan="2"><label><strong>Select Mode</strong></label></td>	
           ...
           <select name="mode" required>
            ...
            <option value='OPD_DIS'>OPD_DIS</option>
            <option value='OTC'>OTC</option>
            <option value='IPD'>IPD</option>
            <option value='OPD_RETURN'>OPD_RETURN</option>
...
<tr>
      ...
      <td align="center"><strong>Bill By</strong></td>
...
if($mode=='OPD' || $mode=='IPD' || $mode=='OPD_DIS' || $mode=='OTC'){                    
$query = "Select * from phar_sale where adate between '$start' and '$end' and status!='Cancel' and location in ('$mode')";
}
else if($mode=='OPD_RETURN'){                    
    $query = "Select * from phar_sale_return where adate between '$start' and '$end'";
    }
   ...
   <tr>
     <td align="center" ><?php echo $count; ?></td>
      ...
      <td align="center" ><a target='_blank' href="?id=<?php echo $row["acct_code"]; ?>&start=<?php echo $start; ?>&end=<?php echo $end; ?>"><?php echo $row["pmrn"]; ?></a></td>
	  
      
      <td align="center" ><?php echo $row['tprice']; ?></td></a></td>
      
      <td align="center" ><?php echo date('d/m/Y', strtotime($row["adate"])); ?></td>
...
<?php $count++;}
  ...
  $querydr = "select SUM(tprice) FROM phar_sale WHERE adate between '$start' and '$end' and location='$mode' and status!='Cancel'"; 
  $resultdr = mysqli_query($con,$querydr) or die ( mysqli_error());
  $data = mysqli_fetch_assoc($resultdr);	
$queryre = "select SUM(tprice) FROM phar_sale_return WHERE adate between '$start' and '$end'"; 
  $resultre = mysqli_query($con,$queryre) or die ( mysqli_error());
  $data_re = mysqli_fetch_assoc($resultre);	


  

  if($mode=='OPD' || $mode=='IPD' || $mode=='OPD_DIS' || $mode=='OTC'){                    
    $bill=$data['SUM(tprice)'];
      }


      else if($mode=='OPD_RETURN'){                    
        $bill=$data_re['SUM(tprice)'];
          }
  ...
  <td align="center" style="font-size:22px;font-weight:bold;color:red">'.$bill.'</td>
...
<tr>
  <td colspan="20" style="text-align:center; padding:15px;">
    <a href="datewise_phar_sale_pdf.php?stdate='.urlencode($start).'&endate='.urlencode($end).'&mode='.urlencode($mode).'" 
       target="_blank"
       style="display:inline-block;padding:10px 18px;background:#dc3545;color:#fff;text-decoration:none;border-radius:6px;font-weight:bold;">
      Download PDF
    </a>
  </td>
</tr>
