...
//$id =$_GET['id'];
$count=1;
$sel_query="SELECT * FROM medicine where status='Active' and `per_qty` >=(select SUM(add_qty) from medi_stock where medicine.code=medi_stock.code and location='Pharmacy' and add_qty>0) order by mname asc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	  ...
	  <?php
	  $rr=$row['code'];
	  $sel_query2="SELECT SUM(add_qty) FROM medi_stock where code='$rr' and location='Pharmacy'";

$result2 = mysqli_query($con,$sel_query2);
$row2 = mysqli_fetch_assoc($result2);
	  
	  ?>
	  <td align="center" style="font-size:22px;color:red;font-weight:bold;"><?php echo $row2['SUM(add_qty)']; ?></td>
	  <td align="center"style="font-size:22px;color:green;font-weight:bold;"><?php echo $row["per_qty"]; ?></td>
