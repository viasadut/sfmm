...
$pid=$_REQUEST['pid'];
$query23 = "select * FROM idischarge1 WHERE id='$pid'"; 
...
//header("Location: newtest2.php"); 
 ...
 $query_pa1 = mysqli_query($db,"select COUNT(ID) from patient where pmrn='$pmrn' and type in('Staff','Staff Children','Staff Spouse','Consultant','Spouse')");
 ...
 $queryc5 = "SELECT COUNT(id) FROM staff3 where mrn='$pmrn' and status='Active' and cstatus='Confirm'"; 
   ...
   $queryc56 = "SELECT COUNT(id) FROM staff1 where mrn='$pmrn' and astatus='Active'"; 
...
('$pmrn', '$eid', 'OPD_DIS', '$gtotal','$gtotal', '$appdate', '$apptime', '$user', '$ipd', 'OPD_DIS','$mno', '$vehicle1', '$due_remarks','$sno','$host')";
...
('$pmrn', '$eid', 'OPD_DIS', '$gtotal','$gtotal', '$appdate', '$apptime', '$user', '$ipd', 'OPD_DIS','$mno', '$vehicle1', '$due_remarks','$sno','$discount_taka','$host')";
}

else if($discount_type=='percentage'){
$sql = "insert into pms_bill(pmrn,eid,location,amount,amount_receive,date,time,user,remarks,dname,s_no,p_mode,p_remarks,sno,dis_amount,queue) VALUES
('$pmrn', '$eid', 'OPD_DIS', '$gtotal','$gtotal', '$appdate', '$apptime', '$user', '$ipd', 'OPD_DIS','$mno', '$vehicle1', '$due_remarks','$sno',$discount_percentage,'$host')";
}
...
if($eqty2>0)
	...
	
	{
			...
			$strSQL1 = "update idismedi set status='Served',qty='$eqty2',billno='$last_id' where id='".$updateid."'";
	...
	$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`,`pmrn`,`pname`,`rfid`,`billno`,`eid`) values
			('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','$ins','OPD_DIS','$code_1','$p_mrn','$p_name','$rf','$last_id','$eid')";
...
//#1
			...
			else if($eqty5 >= $eqty2 and $mqty=='' and $eqty2>10000)
	{
			...
			$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`,`pmrn`,`pname`,`rfid`,`billno`,`eid`) values
			('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','$ins','OPD-DIS','$code_1','$p_mrn','$p_name','$rf','$last_id','$eid')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery2 = mysqli_query($objConnect,$strSQL2);
...
//$conn->query($sql1);
header("Location: phar_update_new1_dis.php?pmrn=$pmrn&dname=$dname&eid=$eid&sno=$sno");
...
<table id="itemTable" border="1" cellpadding="4" width="1100" style="font-size: 18px;">
<tr>
      <td colspan="1" align="center"><strong>S.No</strong></td>
     
      <td colspan="4" align="center"><strong>Medicine Name</strong></td>
	  ...
	  <td colspan="3" align="center"><strong>Instruction</strong></td>
		  ...
		  <td colspan="5" align="center"><strong>Total price</strong></td>
...
<?php
                    ...
                    $query = "select * from idismedi where pmrn='$pmrn' and eid='$eid' and status in('Active','Served')";
...
<td align="center" colspan="4" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?= $i['medi'].' ('.$i['brand'].')'; ?></td>
    ...
    <td align="center"colspan="3" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $i["pdos"].'<br>'.$i["frelation"].'<br>'.$i["duration"]; ?></td>
    <td align="center" colspan="1" 
	<?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>>
    
	<?= $new_qty ?>
	<input name="eqty2_<?= $id ?>" type="hidden" class="avail" value="<?= $new_qty ?>" readonly <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>>
    </td>

    
    <td align="left" colspan="2" <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>>
    <?= $row1['uprice'] ?>  
	<input type="hidden" class="price" name="eqty3_<?= $id ?>" value="<?= $row1['uprice'] ?>" readonly <?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>>
    </td>
