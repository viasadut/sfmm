...
$sel9=mysqli_query($db,"SELECT * FROM endopapp WHERE `pmrn`='$pmrn' and `eid`='$eid'");
...
('$pmrn','$eid','Endoscopy','$total_bill','$appdate','$apptime','$user','Endoscopy Room','Endoscopy','$mno','$vehicle1','$due_remarks')";
...
//$ins_query1="update endopapp set bby='$user',bstatus='Billed',btime='$dd' where id='$id'";
//mysqli_query($con,$ins_query1) or die(mysql_error());

  
  $sql1 = "update endopapp set doc_charge='$doc_charge',hos_charge='$hos_charge', medi_charge='$medi_charge', total_bill='$total_bill',
  bstatus='Billed',bby='$user',btime='$billtime', billno='$last_id', payment_status='PAID' where id='$id'";
$conn->query($sql1);
//header("Location: endoscopy_bill.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
header("Location:new_bill/new_endo_payment.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
...
('$pmrn','$eid','Endoscopy','$total_bill','$appdate','$apptime','$user','Endoscopy Room','Endoscopy','$mno','$vehicle1','$due_remarks','$taka')";
...
//mysqli_query($con,$ins_query1) or die(mysql_error());
  ...
  $sql1 = "update endopapp set doc_charge='$doc_charge',hos_charge='$hos_charge', medi_charge='$medi_charge', total_bill='$total_bill',
  bstatus='Billed',bby='$user',btime='$billtime', billno='$last_id', payment_status='PAID',discount_type='$discount_type',dis_amount='$taka',dis_payment='$dis_taka' where id='$id'";
$conn->query($sql1);
...
$sql = "insert into pms_bill(pmrn,eid,location,amount,date,time,user,remarks,dname,s_no,p_mode,`p_remarks`,`dis_amount`) VALUES
('$pmrn','$eid','Endoscopy','$total_bill','$appdate','$apptime','$user','Endoscopy Room','Endoscopy','$mno','$vehicle1','$due_remarks','$dis_percentage_amount')";
...
$date=date('Y-m-d');
  $ins_query="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
		values ('$last_id','CR','615100','$date','$dis_percentage_amount','ENDOSCOPY')";
		mysqli_query($con,$ins_query) or die(mysql_error());

		
		$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
		values ('$last_id','DR','221170','$date','$dis_percentage_amount','ENDOSCOPY')";
		mysqli_query($con,$ins_query2) or die(mysql_error());

  
  $sql1 = "update endopapp set doc_charge='$doc_charge',hos_charge='$hos_charge', medi_charge='$medi_charge', total_bill='$total_bill',
  bstatus='Billed',bby='$user',btime='$billtime', billno='$last_id', payment_status='PAID',discount_type='$discount_type',dis_amount='$percentage',dis_payment='$dis_percentage' where id='$id'";
$conn->query($sql1);
//header("Location: endoscopy_bill.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
header("Location:new_bill/new_endo_payment.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
...
{
return confirm("Are you Sure to Confirm the Bill ?");
}
...
$count=1;
$sel_query="Select * from endohoscharge where pmrn= '$pmrn' and eid='$eid'order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="5"><?php echo $row["price"]; ?></td>
      <td align="center"colspan="2"><?php if($row['remarks']!=''){echo $row["remarks"];}
      else {echo "Others";}
      ?></td>
...
$count=1;
$sel_query="Select * from endomedi where pmrn= '$pmrn' and eid='$eid' and status='Active' and pstatus='Given'order by `id` DESC;";
...
{ ?>    <tr>
	...
	<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>List Of Infusion Used</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from endoinfusion where pmrn= '$pmrn' and eid='$eid' and status='Given' and status1='Active'order by `id` DESC;";
...
{ ?>    <tr>
	...
	<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>List Of Special Treatment Done</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from endostret where pmrn= '$pmrn' and eid='$eid' and status1='' and status='Given'order by `id` DESC;";
...
{ ?>    <tr>
	...
	<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Consultant Charges</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from ivisitendo where pmrn= '$pmrn' and eid='$eid' order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
			...
			<td align="center"colspan="5"><?php echo $row["room"]; ?></td>
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Investigation Request</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from alltest where pmrn= '$pmrn' and date1='$yy' and location='Endoscopy' order by `id` DESC;";
...
{ ?>    <tr>
    ...
    <tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Other Charges</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from endo_extra_charge where pmrn= '$pmrn' and eid='$eid'  order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="8"><?php echo $row["medi"]; ?></td>
			<td align="center"colspan="5"><?php echo $row["qty"]; ?></td>
			<td align="center"colspan="5"><?php echo $row["price"]; ?></td>
...
$selected = mysqli_select_db($dbhandle,"sfmmkpjnew") 
  or die("Could not select examples");
	...
	$opd_procedure = "SELECT SUM(price) FROM endohoscharge where pmrn= '$pmrn' and eid='$eid' "; 
...
$opd_procedure_medi = "SELECT SUM(price) FROM endohoscharge1 where pmrn= '$pmrn' and eid='$eid' "; 
...
$opd_procedure_doc = "SELECT SUM(room) FROM ivisitendo where pmrn= '$pmrn' and eid='$eid' "; 
...
$opd_procedure_sum_doc=	$opd_procedure_data_doc['SUM(room)'];



$endo_q_extra = "SELECT SUM(price) FROM endo_extra_charge where pmrn= '$pmrn' and eid='$eid' "; 
	 
$endo_s_extra = mysqli_query($dbhandle,$endo_q_extra) or die(mysql_error());
...
$opd_pro_summary=$opd_procedure_sum+$opd_procedure_sum_medi+$opd_procedure_sum_doc+$endo_r_extra;
...
?>
	...
	<td colspan="6"align="right"><input id='gtotal' name='gtotal' style="font-weight: bold;font-size:35px;color:red" value="<?php echo $opd_pro_summary;?>"readonly></td>

  <td colspan="4"align="right"><a href='endo_extra_charge?pmrn=<?php echo $pmrn;?>&eid=<?php echo $eid;?>'>Add Other Charge</a></td>
...
<input name="taka" type="number" class="style1" id="sdate12" placeholder="Discount In Taka" max="50000" hidden style="font-size:20px;color:red;font-weight:bold;">
...
<tr><td colspan="20"align="left" hidden>
...
<td colspan="5"><a target='_blank' href="endouse.php?pmrn=<?php echo "$pmrn"; ?>&full=<?php echo "$full"; ?>&eid=<?php echo "$eid"; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>	
<td colspan="15"align="right"><button type="submit" name="Submit1">Confirm</button></td>
</tr>
</table>
