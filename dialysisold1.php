...
$sel43="SELECT * FROM diaapp WHERE `dname`='$dname' and `ddate`='$date1' and dslot='$slot' and status='Booked';";
...
$ins_query="insert into diapapp (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`dreffer`) values ('$name', '$pmrn','$pphone','$padd','$dname','$date1','$slot','NOT SEEN','$page','$psex','$tname23')";
mysqli_query($con,$ins_query);
$update="update diaapp set status='Booked' where `ddate`='$date1' and `dslot`='$slot'";
...
?>" size ="57" readonly>
	   ...
	   $doc1= $_REQUEST['doc'];
	   		if(isset($_POST['load'])){
			$sql = "select * from `diaapp` where  `status`='Available'and `ddate`='$date1'order by id asc";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
