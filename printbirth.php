...
$sel_query="Select * from birth where pphone= '$pmrn' and status != 'Waiting Approval';";}
 else{
	 $sel_query="Select * from birth where pmrn= '$pmrn' and status != 'Waiting for Approval';";
 } 
$count=1;	 
...
{ ?>    <tr>
	  ...
	  $pmrn=$row['pmrn'];
	  $eid=$row['eid'];
     if($status!='' and $eid>=1 and $row['i_date']==NULL)
	  {echo
	  ...
	  else if($status=='' and $eid<1 and $row['i_date']==NULL)
	  {
