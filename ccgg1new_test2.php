...
$sel="SELECT * FROM pappnew WHERE `pmrn`='$pmrn' and `dname`='$dname' and adate='$date_nn1' and status!='Cancel';";
...
$ins_query="insert into pappnew (`pname`,`pmrn`,`pphone`,`padd`,`dname`,`adate`,`aslot`,`status`,`page`,`psex`,`user`,`yage`,`bdate`,`dis`,`aatime`,`adate1`,`page1`) values 
('$name', '$pmrn','$pphone','$padd','$dname','$date_nn1','$slot','NOT SEEN','$diff1','$psex','$user','$diff2','$date91','$dis','$aatime','$date_nn','ccgg1new_test2')";
...
?>" required placeholder="YYYY" readonly>		  
	  ...
	  <br><br> 
		...
		<label for="mail"><strong>Appointment Date :</strong></label>
									<p>
									<input type="text" name="date_nn_nn" placeholder="Select Date" size="30" value="" id="datepicker1">
									  
									  <?php if(isset($_POST['load'])==1){
									  $date_new = $_REQUEST['date_nn_nn'];
									  $date_new1=date('Y-m-d', strtotime($date_new));
										  echo'
										  	  
<input type="text" name="date_nn" placeholder="Select Date" size="30" value="'.$date_new1.'" readonly required>
';}
...
$date_new2=date('Y-m-d', strtotime($date_new));
