...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('staff','opdpro')"; 
...
$url = "procedure_return_phar?sno=$sno";
header("Location: $url"); 
...
<td colspan="1" align="center"><label><strong>Return Qty</strong></label></td>
...
<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='code' required style="font-weight: bold;font-size:22px;color:green">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `medi_stock` where add_qty>0 and location in('OPD PROCEDURE ROOM(Gynae)','OPD PROCEDURE ROOM(General)')";
...
$count=1;
//$count=1;
$sel_query="Select * from medi_stock1 where rfid= '$sno' and req_loc in('OPD PROCEDURE ROOM(Gynae)','OPD PROCEDURE ROOM(General)') order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="9"><?php echo $row["g_name"]; ?></td>
