...
$s_no=$row4['COUNT(id)']+1;
	...
	$ono1=$yy.$host.date(ymd).$s_no;

if($user!=''){
		
$ins_query1="insert into po_table (`req_department`,`po_type`,`purchase_department`,`payment_terms`,`expiry_date`,`ex_date`,`d_department`,`creditor_code`,`sup_code`,`subamount`,`percentage_dis`,`amount_discount`,`total_amount`,`issue_date`,`issue_person`,`record_status`,`auth_date`,`auth_person`,`enter_date`,`record_no`,`po_date`,`remarks`,`ono`,`terms`,`com_add`)
values ('$req_department','$po_type','$purchase_department','$payment_terms','$expiry_date','$ex_date','$d_department','$creditor_code','$sup_code','$subamount','$percentage_dis','$amount_discount','$total_amount','$issue_date','$issue_person','$record_status','$auth_date','$auth_person','$enter_date','$record_no','$po_date','$remarks','$ono1','$terms','$com_add')";
...
header("Location: po_prepare1_purchase?ono=$encryption");
...
<table width="95%" height ="100%" border="0" align="center" bgcolor="lightblue" style="border-collapse:collapse;">	
		...
		<select name="po_type" style="font-weight: bold;font-size:14px;color:green" required /> 
...
"jsnew/select2.min.css" />
	   ...
	   <option value='Cash'>Cash</option>
		...
		<tr>				
						...
						<select name="sup_code" class="sup_code" required style="font-weight: bold;font-size:14px;color:green" id="sup_code" onchange="GetDetail(this.value)" required> <option value="">-Select Company-</option>
...
"jsnew/select2.min.css" />					
		...
		<tr>				
						<td colspan="5"><input type="number" name="amount_discount" id="location" required value="0" style="font-weight: bold;font-size:14px;color:green"></td>
		

		<td colspan="5"><input name="percentage_dis" type="number" size="70" style="text-transform:uppercase" value="0"required></td>
		<td colspan="5"></td>
		<td colspan="5"><input name="subamount" type="number" size="70" style="text-transform:uppercase" value="0"required></td>
		...
		<td colspan="5"><input name="issue_date" type="date" size="70" style="text-transform:uppercase" value="<?php echo $date;?>"required  ></td>
		<td colspan="5"></td>
		<td colspan="5"><input name="total_amount" type="number" size="70" style="text-transform:uppercase" value="0"readonly ></td>
  ...
  <tr>
		<td colspan="5"style="font-weight: bold;font-size:14px;color:red" hidden> Authorization Person ID</td>
		<td colspan="5" style="font-weight: bold;font-size:14px;color:red" hidden> Authorization Date</td>
		<td colspan="5" style="font-weight: bold;font-size:14px;color:red" hidden> </td>
		<td colspan="5" style="font-weight: bold;font-size:14px;color:red" hidden> Record Status</td>
		...
		<tr>				
						<td colspan="5" hidden><input type="text" name="auth_person" id="location"  value="<?php echo $user;?>" style="font-weight: bold;font-size:14px;color:green"></td>
		

		<td colspan="5" hidden><input name="auth_date" type="date" size="70" style="text-transform:uppercase" value=""></td>
		<td colspan="5" hidden></td>
		<td colspan="5" hidden><input name="record_status" type="text" size="70" style="text-transform:uppercase" value="Pending" readonly></td>
		  ...
		  <tr>
		<td colspan="5"style="font-weight: bold;font-size:14px;color:red" hidden> Purchase Order Date</td>
		<td colspan="5" style="font-weight: bold;font-size:14px;color:red" hidden>Record Number</td>
		<td colspan="5" style="font-weight: bold;font-size:14px;color:red" hidden> </td>
		<td colspan="5" style="font-weight: bold;font-size:14px;color:red" hidden>Entered date</td>
		...
		<tr>				
						<td colspan="5" hidden><input type="date" name="po_date" id="location" required value="<?php echo $date;?>" style="font-weight: bold;font-size:14px;color:green"></td>
		

		<td colspan="5" hidden><input name="record_no" type="text" size="70" style="text-transform:uppercase" value="0" readonly></td>
		<td colspan="5" hidden></td>
		<td colspan="5" hidden><input name="enter_date" type="date" size="70" style="text-transform:uppercase" value="<?php echo $date;?>"></td>
				...
				<tr><td colspan="20"><textarea name="terms" style="text-transform:uppercase" required rows="15" cols="130">
