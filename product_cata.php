...
if(isset($_POST['submitEquipment'])){
        ...
        if ($file_name !='') {
            ...
            $ext = end($exp);
            $path = "product_cat/".$file_name;
            if(in_array($ext, $allowed_ext)){
                if(move_uploaded_file($file_temp, $path)){
                    $select1="insert into pro_cat (`ename`,`image`)
values('$ename','$cat','$vendor','$from','$image_name')";
$sel1=mysqli_query($con,$select1) or die(mysql_error());
            ...
            try {
                ...
                $dbh->commit();
                header("location:index.php?project_id=$project_id&insertEquipment=1");
            } catch (\Throwable $e) {
                $dbh->rollback();
                throw $e;
...
<script>  
 ...
 <div class="modal-body" >  
                     <form method="post" id="insert_form" name="frmMain2" action="">  
...
<tr>				 
					<td colspan="10"><input type="file" name="image5" class="form-control"></td>
