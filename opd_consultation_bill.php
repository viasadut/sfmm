...
$sel_query="Select * from pms_bill where date='$billdate' and location in ('OPD') order by billno desc";
...
<?php
$bill_user=$row['billno'];
$bill_date=$row['date'];

$user_name_q = "SELECT * FROM pappnew where billno= '$bill_user';"; 
...
$user_name_d = mysqli_fetch_assoc($user_name_r);
      ...
      <td align="center"><?php echo ucwords(strtolower($user_name_d['pname'])); ?></td>
      ...
      if($row['location']=='OPD')
      {echo '
      <a target="_blank" href="bill_sticker_consultation?billno='.$row['billno'].'&pmrn='.$row['pmrn'].'&eid='.$row['eid'].'">Sticker Print</a>';}
