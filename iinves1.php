...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('imo','nurse','ot')"; 
...
{ ?>    <tr>
     ...
     <td align="center"colspan="1"><?php echo $row["pmrn"]; ?></td>
	  ...
	  <td align="center"colspan="2"><?php echo $row["odate"]; ?></td>
	  	  <td colspan="2"><a target='_blank' href="view_ins.php?infu=<?php echo $row['infusion']; ?>">Instructions</a>
			
		  </td>  
		  
		  <td colspan="2">
		  <?php if($row['infusion']=='ECG' || $row['infusion']=='ecg' || $row['infusion']=='Ecg' and $row['upload']==''){echo" 
			<a target='_blank' href='spdupload_new_ipd.php?id=".$row['id']."&&pmrn=".$row['pmrn']."'>Upload ECG Flim</a>";}
			
			else if($row['infusion']=='ECG' || $row['infusion']=='ecg' || $row['infusion']=='Ecg' and $row['upload']!=''){echo" 
				<a target='_blank' href='spdpic/".$row['upload']."?id=".$row['id']."'>View File</a>";}?>
		  </td>  
 <td align="center" colspan="2">
 
 <?php
 $inves_name=$row['infusion'];
 $id3=$row['id'];
 $url = "new_inves_1?id=$id3&pmrn=$pmrn&eid=$episode"; 
 $url5 = "cam_test/OAE_TEST?id=$id3&pmrn=$pmrn&eid=$episode"; 
 $url2 = "delete1iinvesimo?id=$id3&pmrn=$pmrn&eid=$episode"; 
		 if($inves_name=='ABG (ARTERIAL BLOOD GASES)')
	{ 
echo "<a target='_blank' href='$url' style='color: red'><b>Upload Report<b></a>";
	}
	
	else if($inves_name=='OAE HEARING SCREENING TEST')
	{ 
echo "<a target='_blank' href='$url5' style='color: red'><b>Upload Report<b></a>";
	}
	
	else
	{ 
echo "<a target='_blank' href='$url2' style='color: blue'><b><b></a>";
	}
 ?>
...
$count=1;
$sel_query="Select * from einves where pmrn= '$pmrn' and eid='$eeid' and status IN ('RECEIVED','Data Updated')  order by `id` DESC;";
...
echo "<a target='_blank' href='$url'>$infu</a>";
	...
	else if($type=='rad' || $type=='spd' || $type=='spd1')
	{ 
echo $infu;
	}
