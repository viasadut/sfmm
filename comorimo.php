<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('imo','doctor','nurse','gpopd')"; 
...
$query44 = "select * from dcomor1 where mname='$medi'"; 
$result44 = mysqli_query($con, $query44) or die(mysqli_error());



$query43 = "SELECT * FROM allcomor where pmrn= '$pmrn' and medi='$medi' and status!='Cancel';"; 
$result43 = mysqli_query($con, $query43) or die(mysqli_error());
//$row43 = mysqli_fetch_assoc($result43);
...
if($res1=mysqli_num_rows($result44)==0)
{
    ...
    echo 'alert("Unsuccessful !!This Comorbidity is not is the list"); ';
    echo '</script>';
    }
...
else if($res=mysqli_num_rows($result43)>0)
{
       ...
       echo '<script language="javascript">';
    echo 'alert("Unsuccessful !!This Comorbidity is already added in Patient history"); ';
    echo '</script>';
    }
...
$ins_query="insert into allcomor (`dname`,`pmrn`,`pname`,`medi`,`ins`,`date`,`type`) values ('$full', '$pmrn','$pname','$medi','$pins','$date','$type')";
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Comorbities History</strong></label></td> </tr>
<tr><td colspan="10" align="center"><label><strong>Name of Comorbities</strong></label></td> 
...
<tr>
      ...
      <td colspan="2" align="center"><strong>MRN</strong></td>
     	  <td colspan="8" align="center"><strong>Comorbities</strong></td>
...
$count=1;
$sel_query="Select * from allcomor where pmrn= '$pmrn' and status!='Cancel' order by `id` DESC;";
...
<td align="center" colspan="2"><a onclick="return confirm_click();" href="delete_cmor?id=<?php echo $row["id"]; ?>&pmrn=<?php echo $row["pmrn"]; ?>">Delete</a></td>
