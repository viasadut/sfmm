...
<table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;" id="myTable">
	...
	<th width="4%"><strong>S.No</strong></th>
	  ...
	  <th width="10%"><strong>Block Today's Slot</strong></th>
	  <th width="10%"><strong>UnBlock Today's Slot</strong></th>
	  <th width="10%"><strong>Advance Block</strong></th>
	  <th width="10%"><strong>Make Today's Appointment(Old Patient)</strong></th>
	  <th width="10%"><strong>Make Today's Appointment(New Patient)</strong></th>
	  <th width="10%"><strong>Make Advance Appointment(Old Patient)</strong></th>
	  <th width="10%"><strong>Make Advance Appointment(New Patient)</strong></th>
...
while($row = mysqli_fetch_assoc($row1)) { ?>
      <td align="left"><span style='color:red;text-align:center;font-size:20px'><b><?php echo $count; ?></td>
	   ...
	   <?php
	   $dn = $row['dname'];
$query40 = "SELECT * FROM doctor where dname= '$dn'"; 
...
if($row41aa['COUNT(status)']==0 and $row41a['COUNT(status)']==0)
...
$dname2=$row["dname"];
$date2=date('Y-m-d');
$url = "test_appoint4?dname=$dname2&date=$date2"; 
...
$dname2=$row["dname"];
$date2=date('Y-m-d');
$url = "test_appoint7?dname=$dname2&date=$date2"; 
...
$sql = "select * from opd_appoint1 where dname='$dname2' and date1='$date2' and status='iui'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
...
echo "AVAILABLE";}
	...
	echo "<a target='_blank' href='$url'>Manual Block</a>";
}
?>	
</td> 
...
$dname2=$row["dname"];
$date2=date('Y-m-d');
$url = "ccgg1new_test1?dname=$dname2&date=$date2"; 
...
$dname2=$row["dname"];
$date2=date('Y-m-d');
$url = "ccgg1new_test1_call?dname=$dname2&date=$date2"; 
...
$dname2=$row["dname"];
$date2=date('Y-m-d');
$url = "ccgg1new_test2?dname=$dname2&date=$date2"; 


	
echo "<a target='_blank' href='$url'>Make Advance Appointment(Old Patient)</a>";
...
$dname2=$row["dname"];
$date2=date('Y-m-d');
$url = "ccgg1new_test2_call?dname=$dname2&date=$date2"; 


	
echo "<a target='_blank' href='$url'>Make Advance Appointment(New Patient)</a>";
