...
{
return confirm("Are you Sure to Confirm IPD Charges ??");
}
...
<?php if($charge=='')
{echo	'
<td align="left" colspan="20"><font size="4.5" color="#FF000"><b><a onclick="return confirm_click();" href="ipd_charge_confirm?pmrn='.$pmrn.'&eid='.$eid.'">Confirm Charges: </a></td>
';}
else {
	
	echo '<td align="left" colspan="20"><font size="4.5" color="#FF000"><b>confirmed</td>';}
?>
...
$count=1;
$sel_query="Select * from imedi2 where pmrn= '$pmrn' and eid='$eid' and udone !='' group by infusion order by `time` asc;";
...
{ ?>    <tr>
	  ...
	  <?php
						...
						$query4p = mysqli_query($db,"select COUNT(infusion) from imedi2 where pmrn='$pmrn' and eid='$eid' and infusion='$pp_medi' and udone !=''");
						$datap = mysqli_fetch_assoc($query4p);
						$t_qty=$datap['COUNT(infusion)'];

						
						$query4pc = mysqli_query($db,"select SUM(uprice) from imedi2 where pmrn='$pmrn' and eid='$eid' and infusion='$pp_medi' and udone !='' ");
...
$selected = mysqli_select_db($dbhandle,"sfmmkpjnew") 
  or die("Could not select examples");
	  ...
	  $query198ad = "SELECT SUM(uprice) FROM imedi2 where pmrn= '$pmrn' and eid='$eid' and udone !='' and reuse=''"; 
...
$count=1;
$sel_query="Select * from iinves where pmrn= '$pmrn' and eid='$eid' and type='lab' and status='RECEIVED' group by infusion  order by `id` DESC;";
...
$count=1;
$sel_query="Select * from iinves where pmrn= '$pmrn' and eid='$eid' and type='rad' and status='RECEIVED' group by infusion  order by `id` DESC;";
...
$selected = mysqli_select_db($dbhandle,"sfmmkpjnew") 
  or die("Could not select examples");
	  ...
	  $query198af = "SELECT SUM(price) FROM iinves where pmrn= '$pmrn' and eid='$eid' and status='RECEIVED' and type='rad'"; 
...
?>	  
<tr><td colspan="20" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Total Radiology Charge is:<?php echo $test1al;?> (BDT)</strong></td></tr>
...
$count=1;
$sel_query="Select * from iinves where pmrn= '$pmrn' and eid='$episode' and type in('spd','spd1') order by `id`  DESC;";
...
<td align="right"bgcolor="lightgreen" colspan="3"><a target='_blank' href="b_ot_dis_new.php?pmrn=<?php echo $row['pmrn']; ?>&id=<?php echo $row['id']; ?>"><font size="6" color="#FF0000"><strong><?php echo $test1c_doc+$test1c_dis+$test1c_medi+$test1c_amedi+$test1c_ainfu;?></strong></a></td>		
	
<?php $count++; } ?>	


<tr><td colspan="20" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Total Inpatient Charge is:<?php echo $test1c+$test1+$test1as+$test1a1+$test1al+$test1ai+$test1am+$test1c_bed;?> (BDT)</strong></td></tr>	

<tr><td colspan="20" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Grand Total is:<?php echo $test1c+$test1+$test1as+$test1a1+$test1al+$test1ai+$test1am+$test1c_bed+$test1c_doc+$test1c_dis+$test1c_medi+$test1c_amedi+$test1c_ainfu;?> (BDT)</strong></td></tr>	
</table>
</form>
</body>
