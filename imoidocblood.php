...
$order_remarks = $_REQUEST['order_remarks'];
//$pizza  = "piece1 piece2 piece3 piece4 piece5 piece6";
$infu5 = explode(" ", $infu);
$bgroup=$infu5[0]; // piece1
$bgroup1=$infu5[1]; // piece2


$remarks = $_REQUEST['remarks'];
$infu1 = $_REQUEST['infu1'];
$positive = implode(",",$_POST["positive"]);
...
$ins_query="insert into iblood (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`room`,`barcode`,`status`,`otime`,`rdate`,`location`,`positive`,`diagnosis`,`hb`,`reaction_type`,`reaction`,`dot`,`nou`,`hpt`,`obs`,`bgroup`,`bgroup1`,`amount1`,`order_type`,`order_remarks`) values 
( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$odate','$infu','$user','$remarks','$infu1','Data Updated','$otime','$datenew','IPD','$positive','$diagnosis','$hb','$reaction_type','$reaction','$dot','$nou','$hpt','$obs','$bgroup','$bgroup1','$amount','$order_type','$order_remarks')";
...
<tr>
						...
						<td colspan="10"><label><strong>Blood Type:</strong></label></td>
...
<td colspan="5" align="center"><input list="browsers1" name="infu" size=60% class="form-control" value="<?php echo $b_group;?>">
  <datalist id="browsers1">
			...
			<td colspan="10" align="center"><input list="browsers3" name="remarks" size=60% class="form-control">
  <datalist id="browsers3">
          ...
          <td colspan="3" align="center"><input name="amount" type="number" size=60% class="form-control" max="450" required>
 </td>
...
<tr>
<td colspan="4"><label><strong>Hb(%):</strong></label></td>
<td colspan="8"><label><strong>Dianosis:</strong></label></td>
<td colspan="8"><label><strong>Obstetrical History:</strong></label></td>
</tr>
<tr>
	
    <td colspan="4"align="right">
    <input type="text" name="hb" required value="<?php $diagnosis;?>">
    </td>
    ...
    <textarea name="diagnosis" required><?php echo $inves;?></textarea>
    </td>
    ...
    <?php
    if($data['gender']=='F') echo'<textarea name="obs" required></textarea>';?>
    </td>
...
<tr>
<td colspan="20"><label id="l_hpt"><strong>H/O Previous Blood Transfusion:</strong></label></td>
...
<tr>
		<td colspan="20"align="right">
    <select name="hpt" required onkeyup="GetDetail(this.value)" id="hpt">
...
<tr>
    ...
    <input type="number" name="nou" value="" hidden id="nou" placeholder="Unit">
    </td>
	  
    <td colspan="10"align="right">
    <input type="date" name="dot" value="" hidden id='dot' placeholder="Remarks">
    </td>
</tr>
...
<tr>
<td colspan="10"><label id="l_reaction" hidden><strong>Any Reaction Occured During That Transfusion:</strong></label></td>
<td colspan="10"></td>
...
<tr>
		<td colspan="10"align="right">
    <select name="reaction" onkeyup="GetDetail(this.value)" id="reaction" hidden>
    <option value="">--Select--</option>
    <option value="Yes">Yes</option>
    <option value="No">No</option>
		...
		<td colspan="10"align="right">
    <input type="text" name="reaction_type" value="" hidden id="reaction_type" placeholder='Type Of Reaction'>
    </td>
   ...
   <tr>
<td colspan="10">
<label><strong>HBsAg, HIV, HCV, VDRL, Malaria (If Positive Should be mentioned Clearly):</strong></label></td>
   
    <td colspan="10"align="left">

    <select type="text" name="positive[]" id="pbp1" multiple="multiple" class="3col active" required size="80">
...
<tr>
    ...
    <select type="text" name="order_type" id="order_type" required >
...
<tr>
		<td colspan="20"align="right"><textarea  name="order_remarks" placeholder="REMARKS"></textarea></td>
...
<td colspan="1"><a target='_blank' href="bloodrequest_test.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>&id=<?php echo $row['id']; ?>"><img src="lab.png" title="Print Report" width="50" height="40" /></a></td>
      </tr>
    <?php $count++; } ?>
...
$(document).ready(function(){
  $('#hpt').change(function() {
    if( $(this).val() == 'Yes') {
        $('#nou').prop( "disabled", false );
        $('#dot').prop( "disabled", false );
        $('#l_reaction').prop( "disabled", false );
...
$(document).ready(function(){
  $('#reaction').change(function() {
    if( $(this).val() == 'Yes') {
        $('#reaction_type').prop( "disabled", false );
