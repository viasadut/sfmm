...
//lis data
$query_CHOL = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='CHOL' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_CHOL= mysqli_fetch_assoc($query_CHOL);


$query_TGL = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='TGL' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_TGL= mysqli_fetch_assoc($query_TGL);

$query_AHDL = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='AHDL' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_AHDL= mysqli_fetch_assoc($query_AHDL);

$query_ALDL = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='ALDL' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_ALDL= mysqli_fetch_assoc($query_ALDL);
...
$ins_query1="insert into lipid (`pname`,`pmrn`,`pphone`,`psex`,`page`,`tc`,`tri`,`hdl`,`ldl`,`cho`,`uby`,`udate`,`eid`,`iname`,`inid`,`sno`) values 
('$pname','$pmrn','$pphone','$psex','$page','$tc','$tri','$hdl','$ldl','$cho','$user','$adate','$eid','$iname','$id','$sno')";
...
<!-- Form Title -->
      ...
      <input name="tc" type="text" size="70" style="text-transform:uppercase" value="<?php echo round($data_CHOL['machine_result'], 2);?>"required>
	  <label for="age"><strong>Triglycerides:</strong></label>
      <input name="tri" type="text" size="70" style="text-transform:uppercase" value="<?php echo round($data_TGL['machine_result'], 2);?>"required>
	  <label for="age"><strong>HDL Cholesterol:</strong></label>
      <input name="hdl" type="text" size="70" style="text-transform:uppercase" value="<?php echo round($data_AHDL['machine_result'], 2);?>"required>
	  <label for="age"><strong>LDL Cholesterol:</strong></label>
      <input name="ldl" type="text" size="70" style="text-transform:uppercase" value="<?php echo round($data_ALDL['machine_result'], 2);?>"required>
	  <label for="age"><strong>Chol/HDL Chol:</strong></label>
      <input name="cho" type="text" size="70" style="text-transform:uppercase" value="<?php echo round($data_CHOL['machine_result']/$data_AHDL['machine_result'], 2);?>"required>	  
