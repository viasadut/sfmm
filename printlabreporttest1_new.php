...
$tbl .='<table width="100%" border="1">
                                <tr>
                                    <td style="font-family: freesans; font-size: 12px; text-align:left" colspan="6"><b>Name: '.$data7['pname'].'</b> </td>
...
$tbl .='<table><tr><td></td></tr>
                                <tr>
                                    <td style="font-family: freesans; font-size: 18px; text-align:center"><b>LAB INVESTIGATION RECORD (INPATIENT)<br></b></td>
                                </tr>
                            </table>
                            ';
                $query1k = mysqli_query($con,"Select * from einves where pmrn='$pmrn' and eid='$eeid' and type='lab' and status='Received' order by odate1 desc");
                $count=1;
                while ($data11k = mysqli_fetch_array($query1k)) {
                    $tbl .='<table>
                            <tr>
                                 <td style="font-family: freesans; font-size: 12px; text-align:left"><b>'.$count.'. '.$data11k['infusion'].' ('.$data11k['barcode'].' / '.$data11k['ndate'].' )</b>
							...
							if($data11k['result']==''){
								$tbl .='
							<tr>
							<td style="font-family:freesans;font-size: 12px; text-align:left;">Report Pending<br></td>
							
                            </tr>
							';}
					
							else if($data11k['result']!=''){
								$tbl .='
							<tr>
							<td style="font-family:freesans;font-size: 12px; text-align:left;">'.$data11k['result1'].'<br></td>
                            </tr>
							';}
                    $count++;
                }
                $tbl .= '</table>';
								 ...
								 $tbl .='
                            <table width="100%">';
                $query11k = mysqli_query($con,"Select * from iinves where pmrn='$pmrn' and eid='$eid' and type='lab' and status='RECEIVED'order by ndate desc");
                //$count=1;
                while ($data111k = mysqli_fetch_array($query11k)) {
                    $tbl .='
                            <tr>
                                <td style="font-family: freesans; font-size: 12px; text-align:left;"><b>'.$count.'. '.$data111k['infusion'].' ('.$data111k['barcode'].' / '.$data111k['ndate'].' )</b>
							...
							else if($data111k['result']!=''){
								$tbl .='
							<tr>
							<td style="font-family:freesans;font-size: 12px; text-align:left;">'.$data111k['result1'].' <br></td>
                            </tr>
							';}
