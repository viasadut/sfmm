...
$mm='Patient Name:'.$pname.''.'%0a'.'Patient Adress:'.$padd.''.'%0a'.'Patient Age:'.$page.''.'%0a'.'Admission Date:'.$adate.''.'%0a'.'Admission Under:'.$dname.''.'%0a'.'Ward:'.$btype.''.'%0a'.'Bed No:'.$bno.'';	
	...
	/*else if($dname=='Covid Unit'){
		if ($patient_rfid_count==1) {
			...
			echo '</script>';
			//header("location:https://api.whatsapp.com/send?phone='".$doc_phone."'&text='".$mm."'&source=&data=");
		}else{
			echo '<script language="javascript">';
								...
								'$user',
									'MRN-.$pmrn',
										'Billing',
											'Inpatient')";
				mysqli_query($con,$insert_rfid_trans) or die(mysql_error());*/
$message = 'One Patient MRN- '.$pmrn.', Name- '.$pname. ', Bed- '.$bno.', admitted at your supervision';
	...
	//$message = 'MRN- '.$pmrn.', Name- '.$pname. ', Bed- '.$bno.', admitted at your care';
		//		header("location:/sfmm/sms/public/sms?message=$message&phone=$doc_phone");
	 //header("location:https://api.whatsapp.com/send?phone='".$doc_phone."'&text='".$mm."'&source=&data=");
}
?>
