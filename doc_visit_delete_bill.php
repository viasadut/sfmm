...
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM icnote WHERE `id`='$id3';");
$result_p = mysqli_fetch_assoc($sel_p);
$del_id=$result_p['COUNT(id)'];
if($del_id>0){


$query = "DELETE FROM icnote WHERE id=$id3"; 
...
//Setting Other Logic after receving the decoded response 
 ...
 header("Location:imoidocnotedoc_bill.php?pmrn=$pmrn&eid=$eid");
