<?php 
   session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','billin','bill','doctor','imo')"; 
...
<tr><td colspan="20"><?php echo $data["padd"]; ?></td></tr>
	...
	<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Infusion Charges</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from iinfusion where pmrn= '$pmrn' and eid='$eid' and infusion='$medi' and status='implemented' order by `ddate` DESC;";
...
<td align="center"colspan="2"><?php echo $row["odate"]; ?></td>
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
			...
			?>
						...
						<td align="center"colspan="5"><?php echo $u_price; ?></td>
...
$selected = mysqli_select_db($dbhandle,"sfmmkpjnew") 
  or die("Could not select examples");
	...
	$query198 = "SELECT SUM(uprice) FROM iinfusion where pmrn= '$pmrn' and eid='$eid' and infusion='$medi' and status='implemented'"; 
	 
$result198 = mysqli_query($dbhandle,$query198) or die(mysql_error());

// Print out result
$row198 = mysqli_fetch_array($result198);
$test1=	$row198['SUM(uprice)'];



	?>
	
	<td colspan="20" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Total Infusion Charge is:<?php echo $test1;?> (BDT)</strong></td></tr>
