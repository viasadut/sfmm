...
$strSQL = "UPDATE estat set pstatus='Served', puser='$user', p_time='$pdate',rfid='$tfid' ";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);


	
$ins_query4="update medi_stock set `add_qty`='$m_qty1' where code='$mcode' and add_qty>'0' and location='Pharmacy' and id='$mid' ";
...
$strSQL4 = "UPDATE medicine set tqty='$m_qty'";
	...
	$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`,`code`) values
			('$g_name','1','$u_price','$u_price','$user','$adate','$bb_name','$pmrn','$eid','$tfid','Sale','AE','$mcode')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
	...
	$query="update emergency set medinoti='', medinotitime='$pdate' where pmrn='$pmrn' and eid='$eid'";
...
<tr><td align="left" colspan="20"><a target='_blank' href="phar_ae_bar_new.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>">bar_print</a></td></tr>	
<tr><td align="left" colspan="20"><a target='_blank' href="pharinmedireturn_new_ae.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>">Return Medicine</a></td></tr>	
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from estat where pmrn= '$pmrn' and eid='$eid'and status !='Cancel' and ndate='$dd' and pstatus='Ordered' order by `infusion` asc;";
...
$new_qty=$sumr['SUM(add_qty)'];
	...
	<td align="center"colspan="1"><?php echo $row["ortime"]; ?></td>
	  ...
	  <td align="center"colspan="4"><?php echo $row["instruc"]; ?></td>
...
$dd=date('Y-m-d');
$count=1;
$sel_query="Select * from estat where pmrn= '$pmrn' and eid='$eid' and ndate='$dd' and status1 !='Returned' and status in ('Data Updated','Rupdated') order by `id` asc;";
...
<td align="left" colspan="20"><?php if ($row['loc']=='1') {echo '<style="font-size:16px;color:red;font-weight:bold;">'.'Departmental Stock'.'</style>';} else {echo "<style='font-size:16px;color:green;font-weight:bold'>"."Pharmacy Stock"."</style>";}?></td>		
<td align="left" colspan="20"><a target='_blank' href="phar_ae_bar_indu.php?id=<?php echo $row['id']; ?>">bar_print</a></td>
