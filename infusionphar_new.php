...
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
	...
	for($i=0;$i<count($_POST["chkDel"]);$i++)
	{
		if($_POST["chkDel"][$i] != "")
		{
			...
			$sel99="SELECT * FROM iinfusion WHERE `id`='".$_POST["chkDel"][$i]."'";
$result99 = mysqli_query($con,$sel99);
$medi_take=mysqli_fetch_assoc($result99);
$mcode=$medi_take['code'];
$iidd=$medi_take['id'];

$ad1=$medi_take['a_d_c_1'];
$ad2=$medi_take['a_d_c_2'];
			
$runningTime = date('ddii')+$_POST["chkDel"][$i];
...
$sel95a="SELECT * FROM medicine WHERE `code`='$ad1' and status='Active';";
$result95a = mysqli_query($con,$sel95a);
$b_chka=mysqli_fetch_assoc($result95a);
//$brand=$b_chk['brand1'];
$m_qty_a=$b_chka['tqty']-1;

$addi_name=$b_chka['mname'];
$addi_brand=$b_chka['brand1'];


$sel95b="SELECT * FROM medicine WHERE `code`='$ad2' and status='Active';";
$result95b = mysqli_query($con,$sel95b);
$b_chkb=mysqli_fetch_assoc($result95b);
//$brand=$b_chk['brand1'];
$m_qty_b=$b_chkb['tqty']-1;

$addi_name1=$b_chkb['mname'];
$addi_brand1=$b_chkb['brand1'];
...
$sel96b="SELECT * FROM medi_stock WHERE `code`='$ad2' and add_qty>0 and location='Pharmacy' order by id asc limit 1;";
$result96b = mysqli_query($con,$sel96b);
$b_chk_b=mysqli_fetch_assoc($result96b);
$mm_qtyb=$b_chk_b['add_qty'];
$m_qty1b=$b_chk_b['add_qty']-1;
$tfid2=$b_chk_b['rfid'];


$sel96a="SELECT * FROM medi_stock WHERE `code`='$ad1' and add_qty>0 and location='Pharmacy' order by id asc limit 1;";
$result96a = mysqli_query($con,$sel96a);
$b_chk_a=mysqli_fetch_assoc($result96a);
$mm_qtya=$b_chk_a['add_qty'];
$m_qty1a=$b_chk_a['add_qty']-1;
$tfid1=$b_chk_a['rfid'];
			
			
	if($res95=mysqli_num_rows($result95)>0 and $mm_qty>0){		
			
			
			
			
			
			$strSQL = "UPDATE iinfusion set pstatus='Served', puser='$user', pdate='$pdate',rfid='$runningTime',rfid1_1='$tfid' ";
...
$strSQL4 = "UPDATE medicine set tqty='$m_qty'";
		...
		$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`,`iidd`,`code`) values
			...
			if($ad1!=''){
				...
				$ins_query4a="update medi_stock set `add_qty`='$m_qty1a' where code='$ad1' and location='Pharmacy'and rfid='$tfid1' ";
mysqli_query($con,$ins_query4a) or die(mysql_error());

	
$strSQL = "UPDATE iinfusion set rfid1_2='$tfid1' ";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);

$strSQL4a = "UPDATE medicine set tqty='$m_qty_a'";
			$strSQL4a .="WHERE code = '$ad1' ";
			$objQuery4a = mysqli_query($con,$strSQL4a);
			
			$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`,`iidd`,`code`) values
			('$addi_name','1','$u_price','$u_price','$user','$adate','$addi_brand','$pmrn','$eid','$tfid1','Sale','IPD','$iidd','$ad1')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery2 = mysqli_query($con,$strSQL2);
			...
			if($ad2!=''){
				...
				$ins_query4b="update medi_stock set `add_qty`='$m_qty1b' where code='$ad2' and add_qty!='0' and location='Pharmacy' and rfid='$tfid2' ";
mysqli_query($con,$ins_query4b) or die(mysql_error());

	
	$strSQL = "UPDATE iinfusion set rfid1_3='$tfid2' ";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);

$strSQL4b = "UPDATE medicine set tqty='$m_qty_b'";
			$strSQL4b .="WHERE code = '$ad2' ";
			$objQuery4b = mysqli_query($con,$strSQL4b);
			
			
			$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`,`iidd`,`code`) values
			('$addi_name1','1','$u_price','$u_price','$user','$adate','$addi_brand1','$pmrn','$eid','$tfid2','Sale','IPD','$iidd','$ad2')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery2 = mysqli_query($con,$strSQL2);
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Infusion Form</strong></label></td> </tr>
		...
		<td colspan="20"align="right"><font size="4.5" color="#FF0000"><b><label><strong><a href="idocinfusiondateimophar?pmrn=<?php echo $pmrn; ?>&eid=<?php echo $eid; ?>"><font size="4.5" color="#FF0000"><b>(See Datewise Medicine List)<b></a></td>
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from iinfusion where pmrn= '$pmrn'and eid='$eid'and odate='$dd' and status1='Active'and pstatus='Ordered' and status !='implemented' order by `id` DESC;";
...
<tr>
	  ...
	  <td colspan="1" align="center"><strong>Additive-1</strong></td>
	  <td colspan="1" align="center"><strong>Qty</strong></td>
	  <td colspan="1" align="center"><strong>Stock</strong></td>
	  <td colspan="1" align="center"><strong>Additive-2</strong></td>
	  <td colspan="1" align="center"><strong>Qty</strong></td>
	  <td colspan="1" align="center"><strong>Stock</strong></td>
...
$suma = "SELECT SUM(add_qty) FROM medi_stock where code='$mcodea' and location='Pharmacy'" ;
	 
$sum1a = mysqli_query($con, $suma) or die(mysqli_error());
$sumra = mysqli_fetch_assoc($sum1a);
$new_qtya=$sumra['SUM(add_qty)'];
         

$sumb = "SELECT SUM(add_qty) FROM medi_stock where code='$mcodeb' and location='Pharmacy'" ;
	 
$sum1b = mysqli_query($con, $sumb) or die(mysqli_error());
$sumrb = mysqli_fetch_assoc($sum1b);
$new_qtyb=$sumrb['SUM(add_qty)'];
	  ...
	  <td align="center"colspan="1"><?php echo $row["addi"]; ?></td>
	  <td align="center"colspan="1"><?php echo $row["qty1"]; ?></td>
	  <td align="center"colspan="1" <?php if($new_qtya!=0 and $mcodea!=''){echo'style="font-weight: bold;font-size:22px;color:green"';} else if($mcodea==''){echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php if($mcodea!='') {echo $new_qtya;} ?></td>
	  <td align="center"colspan="1"><?php echo $row["add1"]; ?></td>
	  <td align="center"colspan="1"><?php echo $row["qty2"]; ?></td>
	  <td align="center"colspan="1" <?php if($new_qtyb!=0 and $mcodeb!=''){echo'style="font-weight: bold;font-size:22px;color:green"';} else if($mcodea==''){echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php if($mcodeb!='') {echo $new_qtyb;} ?></td>
	  <td align="center"colspan="1"><?php echo $row["infu1"]; ?></td>
  ...
  <?php if ($mcode!='' and $new_qty!=0 and $mcodea!='' and $new_qtya!='' and $mcodeb!='' and $new_qtyb!=''){echo'
    <td align="center"><input type="checkbox" name="chkDel[]" id="chkDel'.$i.'" value="'.$row["id"].'"></td>';}
	
	else if ($mcode!='' and $new_qty!=0 and $mcodea!='' and $new_qtya!=''){echo'
    <td align="center"><input type="checkbox" name="chkDel[]" id="chkDel'.$i.'" value="'.$row["id"].'"></td>';}
	
	else if ($mcode!='' and $new_qty!=0 and $mcodeb!='' and $new_qtyb!=''){echo'
    <td align="center"><input type="checkbox" name="chkDel[]" id="chkDel'.$i.'" value="'.$row["id"].'"></td>';}
	
	else if ($mcode!='' and $new_qty!=0){echo'
    <td align="center"><input type="checkbox" name="chkDel[]" id="chkDel'.$i.'" value="'.$row["id"].'"></td>';}
	else 
	{
...
<table align="center" class="table table-bordered" id="dynamic_field">  
 <tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Served Infusion List</strong></label></td> </tr>
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from iinfusion where pmrn= '$pmrn' and eid='$eid'and pstatus ='served' and odate='$dd' order by `otime` and `infusion` asc;";
...
{ ?>    <tr>
	  ...
	  <td align="center"colspan="3"><?php echo $row["infu1"]; ?></td>
	  <td align="center"colspan="1"><?php echo $row["ddate"]; ?></td>  
...
<td align="left" colspan="20"><a target='_blank' href="phar_bar_print_indu.php?id=<?php echo $row['id']; ?>">bar_print</a></td>
