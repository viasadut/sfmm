...
$url = "cam_test/injury_upload.php?pmrn=$pmrn&dname=$dname&dname1=$dname1"; 
...
{ ?>    <tr>
	  ...
	  <td colspan="3"><input list="browsers10" name="dname" class="form-control" autocomplete="off" size="60%"value=''required>
  <datalist id="browsers10">
			...
			?>
			<?php 
			$sql1 = "select * from `staff3` where status in ('Active','Active1') and dept='Medical Services'";
      ...
      <td colspan="3"><input list="browsers11" name="dname1" class="form-control" autocomplete="off" size="60%"value=''required>
  <datalist id="browsers11">
			...
			if(mysqli_num_rows($res1) > 0) {
				while($row1 = mysqli_fetch_object($res1)) {
					echo "<option value='".$row1->mname."'>".$row1->mname."</option>";
				}
			}
			?>
