...
$query43_dis_t_card = "SELECT SUM(dis_amount) FROM pms_payment where date between '$start' and '$end' and p_mode='Card';"; 
	 
$result43_dis_t_card = mysqli_query($con, $query43_dis_t_card) or die(mysqli_error());
$total_di_card = mysqli_fetch_assoc($result43_dis_t_card);
$total_discount_card=$total_di_card['SUM(dis_amount)'];


$query43_dis_t_bkash = "SELECT SUM(dis_amount) FROM pms_payment where date between '$start' and '$end' and p_mode in ('bKash','Bkash');"; 
	 
$result43_dis_t_bkash = mysqli_query($con, $query43_dis_t_bkash) or die(mysqli_error());
$total_di_bkash = mysqli_fetch_assoc($result43_dis_t_bkash);
$total_discount_bkash=$total_di_bkash['SUM(dis_amount)'];
...
$card_t_net=$card_t-$total_discount_card-$card_t_refund;
...
//$bkash_t_net1=$bkash_data_t['SUM(amount)']+$phar_collection_t_bkash['SUM(amount_receive)']-$phar_collection_dis_t_bkash['SUM(dis_amount)']-$total_discount_bkash;
$bkash_t_net=$bkash_t-$bkash_t_refund-$total_discount_bkash;
...
$expense = "SELECT SUM(total_amount) FROM fund_transfer_master where posting_date between '$start' and '$end';"; 
	 
$expense_result = mysqli_query($con, $expense) or die(mysqli_error());
$expense_data = mysqli_fetch_assoc($expense_result);

$expense_ch = "SELECT SUM(total_amount) FROM fund_transfer_master where posting_date between '$start' and '$end' and account_type='Bank';"; 
	 
$expense_result_ch = mysqli_query($con, $expense_ch) or die(mysqli_error());
$expense_data_ch = mysqli_fetch_assoc($expense_result_ch);


$expense_pt = "SELECT SUM(total_amount) FROM fund_transfer_master where posting_date between '$start' and '$end' and account_type='Petty Cash';"; 
	 
$expense_result_pt = mysqli_query($con, $expense_pt) or die(mysqli_error());
$expense_data_pt = mysqli_fetch_assoc($expense_result_pt);



$opd_inves_dis = "SELECT SUM(dis_amount) FROM pms_bill where date between '$start' and '$end' and location IN ('OPD_inves');"; 
 
$result_opd_inves_dis = mysqli_query($con, $opd_inves_dis) or die(mysqli_error());
$data_opd_inves_dis = mysqli_fetch_assoc($result_opd_inves_dis);
...
<a target="_blank" href="expense_details?account_type=Bank&date='.$start.'&date1='.$end.'">
      
Cheque: '.$expense_data_ch['SUM(total_amount)'].'
</a>


<br />

<a target="_blank" href="expense_details?account_type=Petty Cash&date='.$start.'&date1='.$end.'">
      
Petty Cash: '.$expense_data_pt['SUM(total_amount)'].'
</a>
...
if(isset($_POST['bsearch'])){
   echo'<br />';
   echo '<span style="color:red;font-size:22px; font-weight:bold">Breakdown of the Cllection:</span>';
   echo '
   <table width="100%" height ="100%" border="1" align="center" bgcolor="#FFFF99" style="border-collapse:collapse;">
  
   <tr>
   <th width="10%" style="font-size:18px; color:green; font-weight:bold; text-align:center"><strong>No Of Records</strong></th>  
    <th width="10%" style="font-size:18px; color:green; font-weight:bold; text-align:center"><strong>Location</strong></th>
     <th width="10%" style="font-size:18px; color:green; font-weight:bold; text-align:center"><strong>Total amount</strong></th>
    <th width="10%" style="font-size:18px; color:green; font-weight:bold; text-align:center"><strong>Total Discount</strong></th>
...
$query_group = "SELECT COUNT(location),location,SUM(amount),SUM(dis_amount) FROM pms_payment where date between '$start' and '$end' and refund NOT IN ('1','2') group by location order by location;"; 
$query_result = mysqli_query($con, $query_group) or die(mysqli_error());
while
...
<td style="font-size:18px; color:green; font-weight:bold; text-align:center">

<a target="_blank" href="breakdown_details?location='.$query_data["location"].'&date='.$start.'&date1='.$end.'">
      
'.$query_data['COUNT(location)'].'
</a>
</td>
<td style="font-size:18px; color:green; font-weight:bold;text-align:center">
'.$query_data['location'].'
...
<td style="font-size:18px; color:green; font-weight:bold; text-align:center">
'.$query_data['SUM(amount)'].'
</td>

<td style="font-size:18px; color:green; font-weight:bold;text-align:center">
'.$query_data['SUM(dis_amount)'].'
</td>
</tr>
';
...
$query_group1 = "SELECT COUNT(location),location,SUM(amount_receive), SUM(dis_amount) FROM pms_bill where date between '$start' and '$end' and location IN ('OPD_DIS','OTC_Sale','OPD_Medi')  group by location;"; 
$query_result1 = mysqli_query($con, $query_group1) or die(mysqli_error());
while
($query_data1 = mysqli_fetch_assoc($query_result1))
{
...
<tr>
<td style="font-size:18px; color:green; font-weight:bold; text-align:center">
<a target="_blank" href="breakdown_details_phar?location='.$query_data1["location"].'&date='.$start.'&date1='.$end.'">
      
'.$query_data1['COUNT(location)'].'
</a>

</td>
<td style="font-size:18px; color:green; font-weight:bold; text-align:center">
'.$query_data1['location'].'
</td>

<td style="font-size:18px; color:green; font-weight:bold; text-align:center">
'.$query_data1['SUM(amount_receive)'].'
</td>

<td style="font-size:18px; color:green; font-weight:bold; text-align:center">
'.$query_data1['SUM(dis_amount)'].'
</td>
</tr>
'.$data_opd_inves_dis['SUM(dis_amount)'].'
';
