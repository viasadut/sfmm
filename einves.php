...
$data = mysqli_fetch_assoc($query4);
  ...
  $query4d = mysqli_query($db,"select * from user where uname='$user'");
...
$query159 = mysqli_query($db,"select * from radio2 where id='$infu' and status='Active'");
$data159 = mysqli_fetch_assoc($query159);

$query1590 = mysqli_query($db,"select * from radio where id='$infu' and status='Active'");
$data1590 = mysqli_fetch_assoc($query1590);


$type=$data159["type"];
$code=$data159["code"];
$inves_name=$data159["iname"];
$price=$data159["price"];
...
$sel90="SELECT * FROM radio2 WHERE `id`='$infu' and status='Active';";
...
$ins_query="insert into einves (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`room`,`status`,`type`,`code`,`price`,`odate1`,`subtype`,`link`,`linkv`,`report`,`reportv`,`ndate`,`dname`,`ip`,`op`,`acct_code`,`ccentre`) values 
( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$odate','$inves_name','$user','$remarks','Data Updated','$type','$code','$price','$odate1','$subtype','$link','$linkv','$report','$reportv','$ndate','$d_name','$ip','$op','$acct_code','$ccentre')";
...
<!-- Form Title -->
		...
		<tr><td colspan="20" align="right"><button onClick="goBack()">Back</button></td></tr>
				...
				<select name="d_name"  size="1" required style='font-size:20px;color:red;font-weight:bold;'>
						  
						  <option value='<?php echo $d_na;?>'><?php echo $d_na;?></option>
						  
		<?php 
			$sql = "select infusion from `erefferal` where pmrn='$pmrn' and eid='$eid' order by id asc";
...
$count=1;
$sel_query="Select * from einves where pmrn= '$pmrn' and eid='$eid' and rstatus IN ('RECEIVED','REJECTED')  order by `id` DESC;";
