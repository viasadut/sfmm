...
if (isset($_POST['but_update'])) {
    ...
    try {
        ...
        if ($total > 0) {
            ...
            $ins_queryr = "
                INSERT INTO acct_ap
                (grn, creditor_code, vat, tax, amount, payable, cheque_no, bank, user, add_time, date, pono, grn_time, grn_by, invoice_no)
                VALUES
                ('{$grn}','{$creditor_code}','','','{$total}','{$total}','','','','','',
                 '{$po_ono}','{$add_time}','{$user}','{$invoice_no}')
            ";
            $con->query($ins_queryr);

            $date = date('Y-m-d');

            $con->query("INSERT INTO pms_tb (trans_id, trans_type, acct_code, date, amount, location) VALUES ('{$last_id}','CR','311140','{$date}','{$total}','PO_PHAR')");
            $con->query("INSERT INTO pms_tb (trans_id, trans_type, acct_code, date, amount, location) VALUES ('{$last_id}','DR','713085','{$date}','{$total}','PO_PHAR')");
...
header("Location: ".$_SERVER['PHP_SELF']."?ono=".urlencode($ono_q)."&grn=".urlencode($grn_q));
exit;
