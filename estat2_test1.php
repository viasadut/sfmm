...
$sel990="SELECT * FROM medi_stock WHERE mname='$infu' and status='Active';";
...

{
	$s=0;
	while($infu2>$s) {
  $ins_query="insert into estat (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`room`,`status`,`ndate`,`pstatus`,`alert`,`reuse`,`code`) values 
('$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$odate','$infu','$user','$remarks','Data Updated','$ndate','Ordered','$alert','$reuse','$p_code')";
...
<td colspan="8" align="center"><input list="rr" name="infu" class="form-control" autocomplete="off">
  <datalist id="rr">
				...
				<?php 
			$sql76 = "select distinct code,g_name,b_name from `medi_stock` where location='Pharmacy' and add_qty>0";
			$res76 = mysqli_query($con, $sql76);
			if(mysqli_num_rows($res76) > 0) {
				while($row76 = mysqli_fetch_object($res76)) {
					echo "<option value='".$row76->g_name.'('.$row76->b_name.')'."'>".$row76->g_name.'('.$row76->b_name.')'."</option>";
				}
			}
			?>  </datalist></td>
...
<tr>
     ...
     <td colspan="2" align="center"><strong>Done Date</strong></td>
      <td colspan="2" align="center"><strong>Done Time</strong></td>
	   ...
	   <td colspan="3" align="center"><strong>Dosages</strong></td>
