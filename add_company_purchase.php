...
$query="insert into add_com_product (`company`,`cid`,`product`,`status`,`date`,`mid`,`location`) values 
('$com_name','$cid','$g1','Active','$date','$g2','Purchase')";
...
<label for="age"><strong>Available Product:</strong></label>
 <select class="country"
					multiple="true" 
					style="width: 680px; overflow-y: auto;" name="coun[]">
...
?>
				...
				<?php
	$stmt = $DB_con->prepare("select * from `storenew` where estatus='Active'");
	$stmt->execute();
	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
	{
		?>
        <option value="<?php echo $row['ename'].','.$row['id']; ?>"><?php echo $row['ename']; ?></option>
        <?php
	} 
?>
