...
$sel95="SELECT * FROM purchase_stock3 WHERE `code`='$code1' and location='Store';";
...
if($res95=mysqli_num_rows($result95)==0)
{
    ...
    echo '</script>';
			...
			*/
			...
			$ins_query1="insert into purchase_stock3 (`code`,`g_name`,`b_name`,`add_qty`,`req_qty`,`req_loc`,`status`,`location`,`rfid`,`sno`,`u_price`,`t_price`,`req_by`,`re_time`,`remarks`,`lpdate`,`plevel`,`balance`,`musage`,`p_id`,`hod`,`ptype`,`manual`)
 values ('$code1','$medi','$medi','$tprice','$tprice','$p_mrn1','Served','$p_mrn1','$sno','$runningTime','$uprice','$charge','$user','$re_time','$ins','$lpdate','$plevel','$balance','$musage','$p_id','$hod','$ptype','1')";
mysqli_query($con,$ins_query1) or die(mysql_error());
			
			
$url = "manual_stock_update?sno=$sno";
header("Location: $url"); 
...
$ins_query1="insert into purchase_stock3 (`code`,`g_name`,`b_name`,`add_qty`,`req_qty`,`req_loc`,`status`,`location`,`rfid`,`sno`,`u_price`,`t_price`,`req_by`,`re_time`,`remarks`,`lpdate`,`plevel`,`balance`,`musage`,`p_id`,`hod`,`ptype`,`incharge`,`manual`)
values ('$code1','$medi','$medi','$tprice','$tprice','$p_mrn1','Served','$p_mrn1','$sno','$runningTime','$uprice','$charge','$user','$re_time','$ins','$lpdate','$plevel','$balance','$musage','$p_id','$hod','$ptype','$incharge','1')";
mysqli_query($con,$ins_query1) or die(mysql_error());
...
"jsnew/select2.min.css" />
    ...
    <option value="Stock Items" selected>Stock Items</option>
...
<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='code' required style="font-weight: bold;font-size:22px;color:green">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select distinct code,g_name from `purchase_stock3` where location='store'";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
            <option value="<?php echo $row['code'].','.$dept; ?>"><?php echo $row['g_name']; ?></option>
        <?php } ?>
            </datalist>
...
<td colspan="1" ><input type="number" name="plevel" id="plevel" required value=""  style="font-weight: bold;font-size:22px;color:green" step="any" ></td>
<td colspan="1" ><input type="number" name="balance" id="balance" required  value=""  style="font-weight: bold;font-size:22px;color:green" step="any" ></td>
<td colspan="1" ><input type="number" name="musage" id="musage" required value=""  style="font-weight: bold;font-size:22px;color:green" step="any" ></td>
<td colspan="1" ><input type="text" name="uprice" id="uprice" required value=""  style="font-weight: bold;font-size:22px;color:green" readonly></td>
