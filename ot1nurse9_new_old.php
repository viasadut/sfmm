...
<td colspan="2"><input type="text" name="duration" size="15"value="<?php if(isset($_POST['load'])==1)
{ $duration1 = $_REQUEST['duration'];
echo $duration1;
}
?>" required></td>  		             		
...
if(isset($_POST['load'])){
			$sql = "select * from `privilege` where dname='$dname1' and status='Approved'";
			$res = mysqli_query($con, $sql);
			if(mysqli_num_rows($res) > 0) {
				while($row = mysqli_fetch_object($res)) {
...
<input name="load" class="style1" type="submit" id="load" value="Load Approved Privilege">
