...
$full = $row39['fullname'];
//$test=date('m/d/Y');
//echo $date2=date_create("$test");
//echo $test1=date("dd/mm/yy");
?>
...
$name6= strlen($name2);
$name7= strlen($name3);
$bb=$name1.'/'.$name2.'/'.$name3;

$date=date('d/m/Y');
$date2=date_create($date);
$date22=date_create($bb);
$date21=strtotime('d/m/Y',$date22);
$diff=date_diff($date21,$date2);
echo $diff1= $diff->format("%y Y %m M %d D");
...
echo '<script language="javascript">';
    echo 'alert("No DD Slot Selected!!"); ';
    echo '</script>';
}

else if ($name6 = 2){
echo '<script language="javascript">';
    echo 'alert("No MM Slot Selected!!"); ';
    echo '</script>';
}

else if ($name7 = 4){
echo '<script language="javascript">';
    echo 'alert("No YYYY Slot Selected!!"); ';
    echo '</script>';
}
...
$update1="update patient set bdate='$hd1' where `pmrn`='$pmrn'";
...
<!-- Form Title -->
	  ...
	  <input name="name1" type="text" size="2" value=""placeholder="DD" required >
	  <input name="name2" type="text" size="2" value="" placeholder="MM"required>
	  <input name="name3" type="text" size="3" value=""placeholder="YYYY"required >
      ...
      <input name="pphone" type="text" size="13" value="<?php echo $data["pphone"]; ?>"readonly>	  
							  ...
							  <input type="text" name="date5" id="datepicker" placeholder="Select Date" size="15" value="<?php echo $data["bdate"]; ?>">      

<input name="page" type="text" size="11"value="<?php	  if(isset($_POST['load'])==1)
{ $start = $_REQUEST['date5'];
//echo $doc1;
//$date= date('m/d/Y');
...
<input type="hidden" name='hd'value="<?php	  if(isset($_POST['load'])==1)
{ $hd = $_REQUEST['date5'];
echo $hd;
}
?>" size="57" readonly>
