...
//$tprice=(int)$u_price*$pdos;
$medi1_details=$b_chk_m['g_name'].'-OPRO';
...
$sel9=mysqli_query($db,"SELECT * FROM promediused where pmrn= '$pmrn' and eid='$eid' and code='$code' and rfid='$rfid' and adate='$adate'");
$result9 = mysqli_fetch_assoc($sel9);
$pdos1=(int)$result9['pdos'];
$pdos2=(int)$result9['pdos']+$pdos;
$iid=$result9['id'];
//$tprice2=(int)$pdos2*$u_price;
...
$tprice=(int)$charge_code['uprice']*$pdos;

$tprice2=(int)$charge_code['uprice']*$pdos2;
...
$charge_name_details=$dd1['charge_name'].'-OPRO';
...

{
  ...
  $sql = "insert into promediused (`dname`,`pmrn`,`pname`,`medi`,`brand`,`pdos`,`eid`,`date`,`type`,`ieid`,`rfid`,`code`,`adate`,`price`,`ip`,`op`,`acct_code`,`ccentre`) 
  values ('$full','$pmrn','$pname','$medi1','$bb_name','$pdos','$eid','$date1','$type','$ieid','$rfid','$code','$adate','$tprice','$ip','$op','$acct_code','$ccentre')";
  ...
  $strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`,`code`) values
        ('$g_name','$pdos','$u_price','$tprice','$user','$adate','$bb_name','$pmrn','$eid','$rfid','Sale','Procedure Room','$code')";
...
values ('$last_id','CR','111001','$date','$tprice','OPD PROCEDURE')";
mysqli_query($con,$ins_query) or die(mysql_error());


$ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','DR','615100','$date','$tprice','OPD PROCEDURE')";
...
$sql = "insert into promediused (`dname`,`pmrn`,`pname`,`medi`,`brand`,`pdos`,`eid`,`date`,`type`,`ieid`,`rfid`,`reuse`,`adate`,`code`,`price`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$full','$pmrn','$pname','$charge_name','$bb_name','$pdos','$eid','$date1','$type','$ieid','$rfid','Yes','$adate','$c_code','$new_charge_price','$ip','$op','$acct_code','$ccentre')";
...
$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`,`code`,`reuse`) values
			('$charge_name','$pdos','$charge_price','$new_charge_price','$user','$adate','$bb_name','$pmrn','$eid','$rfid','Sale','Procedure Room','$c_code','Yes')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery2 = mysqli_query($con,$strSQL2);
    ...
    $ins_query="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$last_id','CR','111001','$date','$new_charge_price','OPD PROCEDURE')";
    mysqli_query($con,$ins_query) or die(mysql_error());
    
    
    $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$last_id','DR','615100','$date','$new_charge_price','OPD PROCEDURE')";
    mysqli_query($con,$ins_query2) or die(mysql_error());
...
<td colspan="10" align="center"><input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='medi1' required style="font-weight: bold;font-size:22px;color:green">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `medi_stock` where add_qty>0 and location in('OPD PROCEDURE ROOM(General)','OPD PROCEDURE ROOM(Gynae)')  and status in('Served','Partially Served')";
...
<input list="browsers11" name="pdos" class="form-control" required>
...
<tr>
		<td colspan="20"align="right"><button type="submit" name="Submit1" id='uu'>ADD</button></td>
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["medi"].' ('.$row["brand"].')'; ?></td>
				  ...
				  <td align="center" colspan="2"><a href="prodelete1used_new?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$dname"; ?>&eid=<?php echo "$eid"; ?>&ID=<?php echo "$id"; ?>&rfid=<?php echo $row['rfid']; ?>&pdos=<?php echo $row['pdos']; ?>&reuse=<?php echo $row['reuse']; ?>&admission_no=<?php echo $api_adminssion_no;?>&invoice_no=<?php echo $row['invoice_no'];?>&code=<?php echo $row['code'];?>&price=<?php echo $row['price'];?>">DELETE</a></td>
