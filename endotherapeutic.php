...
$sel990="SELECT * FROM therapeutic WHERE `tname`='$medi1';";
...
if($res990=mysqli_num_rows($result990)==0)
{
    ...
    echo 'alert("Unsuccessful !!! The Name is not in the Database List.. Please contact with IT Department"); ';
    echo '</script>';
    }
else {
$ins_query1="insert into addtherapeutic (`pmrn`,`pname`,`medi`,`eid`,`date`,`ins`,`dname`,`date1`) values ('$pmrn','$pname','$medi1','$eid','$date1','$medi2','$full','$date2')";
...
<tr><td colspan="20" align="center"><label><strong>Select Used Items</strong></label></td> 



</tr>
<tr>
<td colspan="5" align="center"><input list="browsers2" name="medi1"  class="form-control" autocomplete="off" required/>
  <datalist id="browsers2">

						<option value=''>-Select Therapeutic Name</option>
				<?php 
			$sql76 = "select * from `therapeutic`";
			$res76 = mysqli_query($con, $sql76);
			if(mysqli_num_rows($res76) > 0) {
				while($row76 = mysqli_fetch_object($res76)) {
					echo "<option value='".$row76->tname."'>".$row76->tname."</option>";
				}
			}
			?>  </datalist></td>

<td colspan="15" align="center"> <textarea rows="5" name="medi2"></textarea></td>
...
$count=1;
$sel_query="Select * from addtherapeutic where pmrn= '$pmrn' and eid='$eid'order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["medi"]; ?></td>
				 ...
				 <td align="center" colspan="2"><a href="therapeiticdelete?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>&id1=<?php echo "$id"; ?>">DELETE</a></td>
