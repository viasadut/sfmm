...
$sel_query="Select * from research where status='pending'  ORDER BY id asc;";

$result = mysqli_query($con,$sel_query);
//echo   $bt;
echo "Pending Research List For Approval"; 
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	  ...
	  echo substr($row["comments"],0,200); ?>
	  
	  <?php if(substr($row['comments'])<200){
	  echo"
	  
	  
	  <a target='_blank'  href='research_details?id=".$row["id"]."'>More</a>";} ?>
	  
	  </td>
	  	  
	  <td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><a href="research_approval_md?id=<?php echo $row["id"]; ?>"><strong>Approve</a> </td>
	  
	  <td align="center"><a href="research_reject_md?id=<?php echo $row["id"]; ?>"><strong>Reject</strong></a></td>
...
$sel_query="Select * from research where status in('Approved','Reject')  ORDER BY status asc;";

$result = mysqli_query($con,$sel_query);
//echo   $bt;
echo "Approved / Rejected Research List"; 

while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	
	<?php
	$rrt=substr($row['comments'],0,20);
	?>
      <td align="center"><?php echo $count; ?></td>
      <td align="center"><?php echo $row["pname"]; ?></td>
	  ...
	  <td align="Left"><?php echo $rrt; ?>  
	  <td align="Left"><?php echo $row["aby"]; ?>  
	  <td align="Left"><?php echo $row["atime"]; ?>  
<td align="center"><a onclick="return confirm_click2();" href="mmar1?id=<?php echo $row["id"]; ?>"><strong>Cancel</strong></a></td>	  	  
