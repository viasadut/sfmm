...
$sql = "insert into pms_bill(pmrn,eid,location,amount,date,time,user,remarks,dname,s_no,p_mode,p_remarks,receive_amount, outstanding) VALUES
('$pmrn', '$eid', '$ipd', '$payment', '$appdate', '$apptime', '$user', '$ipd', '$ipd','$mno', '$vehicle1','$due_remarks','$receive_amount','$outstanding')";
...
$sh = $db1->prepare("UPDATE hos_discount SET billno=? WHERE pmrn=? and eid=?");
$sh->execute([$last_id, $pmrn, $eid]);
...
$sh = $db1->prepare("UPDATE m_suite SET billno=? WHERE pmrn=? and ieid=?");
$sh->execute([$last_id, $pmrn, $eid]);

$sh = $db1->prepare("UPDATE cath_receive SET billno=? WHERE pmrn=? and ieid=?");
$sh->execute([$last_id, $pmrn, $eid]);



/*$sh = $db1->prepare("UPDATE iinves SET dis_date=?,billno=? WHERE pmrn=? and eid=? and rstatus=?");
  $sh->execute([$dis_date, $billno, $pmrn, $eid, $rstatus]);


  $sh = $db1->prepare("UPDATE icnote SET dis_date=? WHERE pmrn=? and eid=? and ugroup=?");
  $sh->execute([$dnew, $pmrn, $eid, $ugroup]);


  $sh = $db1->prepare("UPDATE otreport SET dis_date=? WHERE pmrn=? and date1 between ? and ? ");
  $sh->execute([$dnew, $pmrn, $adm_date, $dis_date]);
*/
...
implant=?, extra=?, endo=?, opdpro=?, payment_status=?, dis_medi=?, emer_bill=?, cath_bill=?, msuite_bill=?,receive_amount=?,gtotal=? WHERE pmrn=? and eid=?");
$sh->execute([$last_id, $payment, $room_charge, $inves_charge, $disposable_charge, $doc_charge, $pharmacy_charge,
$ot_hos_charge, $ot_doc_charge, $ot_phar_charge, $implant, $extra, $endo, $opdpro, $payment_status, $dis_medi, $emer_all_bill, $cath_bill, $msuite_bill, $receive_amount, $gtotal, $pmrn, $eid]);
...
$total_day=	$row198j_stay['SUM(tdays)']/24;
	  ...
	  <td align="center"colspan="2"><?php if($start_d=='NULL' || $start_d==''){echo 'Charge Not Finalized';} else if($start_d!='' and $ttday<1 and $total_day<1){echo $row["b_charge"];} 
	  else if($start_d!='' and $ttday<1 and $total_day>1){echo $row["charge"];}
	  else if($start_d!='' and $ttday>=1){echo $row["charge"];}?></td>
...
echo $bed_charge_new=	$row198j_stay['b_charge'];
	...
	<td colspan="16" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Total Room Charge is:<?php if($total_day<1){echo $bed_charge_new;} else {echo $test1c_bed;}?> (BDT)</strong></td>
...
<td align="right"bgcolor="lightgreen" colspan="3"><a target="_blank" href="cath_used.php?pmrn='.$pmrn.'&id='.$row['id'].'&eid='.$eid.'&dname='.$adoc.'"><font size="6" color="#FF0000"><strong>'.$opd_cath_summary.'</strong></a></td>
';}
...
if($endo_summary>0)	{echo'
  <td align="right"bgcolor="lightgreen" colspan="3"><a target="_blank" href="endouse.php?pmrn='.$pmrn.'&eid='.$eid.'&full='.$row['dreffer'].'"><font size="6" color="#FF0000"><strong>'.$endo_summary.'</strong></a></td>
';}?>		
...
?>	  
	...
	<td colspan="16" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Total Emergency Charge is:<?php echo $emer_all_bill;?> (BDT)</strong></td>
...
<td align="right"bgcolor="lightgreen" colspan="3"><a target="_blank" href="b_ot_dis_new.php?pmrn='.$pmrn.'&id='.$test1c_doc_ot.'"><font size="6" color="#FF0000"><strong>'.$ot_payment.'</strong></a></td>
  ';}
...
$query198j_extra = "SELECT SUM(price) FROM ipd_extra_charge where pmrn= '$pmrn' and eid='$eid' and medi NOT IN('SERVICE CHARGE')"; 
...
$service_charge = "SELECT SUM(price) FROM ipd_extra_charge where pmrn= '$pmrn' and eid='$eid' and medi IN ('SERVICE CHARGE')"; 
	 
$service_data = mysqli_query($dbhandle,$service_charge) or die(mysqli_error());

$service_result= mysqli_fetch_array($service_data);
$servicecharge=	$service_result['SUM(price)'];


$new_hos_dis=$data['hos1_dis']+$data['lab_dis']+$data['rad_dis']+$data['room_dis'];

$in_new_charge1=$test1c+$test1+$test1as+$test1a1+$test1al+$test1al_rad+$test1ai+$test1am+$test1c_bed+$implant+$extra+$ot_payment+$emer_all_bill+$test1_dis+$opd_pro_summary+$endo_summary+$opd_cath_summary+$servicecharge;
$in_new_charge2=$test1c+$test1+$test1as+$test1a1+$test1al+$test1al_rad+$test1ai+$test1am+$bed_charge_new+$implant+$extra+$ot_payment+$emer_all_bill+$test1_dis+$opd_pro_summary+$endo_summary+$opd_cath_summary+$servicecharge;
...
echo '<tr><td colspan="20" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Service Charge is:'.$servicecharge.' (BDT)</td></tr>';	
}
?>
...
<tr><td colspan="20" align="right"bgcolor="lightgreen"><font size="6" color="#FF0000"><strong>Consultant Discount:<?php echo $data['hos_doc_dis'];?> (BDT)</strong></td></tr>	
...
<input type="hidden" name="gtotal" value="<?php if($total_day<1){echo $in_new_charge2;} else {echo $in_new_charge1;};?>">
<tr>
...
<input type="radio" id="vehicle1" name="vehicle1" value="bKash"id="chkPassport1" onclick="EnableDisableTextBox1(this)" style="height:20px; width:20px; color:red;"><span style="font-size:40px;color:red;font-weight:bold;">Bikash</span>	
