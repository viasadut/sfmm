...
$laba = "SELECT COUNT(id) from alltest where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate' and subtype in('BLOOD BANK') and rejectby=''"; 
$lab_resulta= mysqli_query($con, $laba) or die(mysqli_error());
$lab_rowa = mysqli_fetch_array($lab_resulta);
$lab_ra=$lab_rowa['COUNT(id)'];

$lab1a = "SELECT COUNT(id) from iinves where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate' and subtype in('BLOOD BANK') and rejectby=''"; 
...
$lab2a = "SELECT COUNT(id) from einves where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate'and subtype in('BLOOD BANK') and rejectby=''"; 
...
$lab2a4 = "SELECT COUNT(id) from alltest where type='lab' and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate'and subtype in('BLOOD BANK') and rejectby!=''"; 
...
<body>
    ...
    <br>
    <h1 align="center" style="background-color:lightgreen;text-align:left"><a href='lab_all_opd_bb' style="color:red">OPD (<?php echo $lab_ra;?>)</a>&nbsp;&nbsp;&nbsp;<a href='lab_all_ipd_bb'>IPD(<?php echo $lab_r1a;?>)</a>&nbsp;&nbsp;&nbsp;<a href='lab_all_ae_bb'>AE(<?php echo $lab_r2a;?>)</a></h1>
            ...
            <tbody>
                <?php
                    if($ugroup=='lab' && $status='active'){	
                        ...
                        $test=date('Y-m-d', strtotime('-30 days') );
                        $count=1;
                        $sel_query="Select * from alltest where type in ('lab','LAB','Lab') and rstatus ='RECEIVED' and status ='RECEIVED' and resultstatus='Updated By Technologist' and rdate between '$test' and '$apdate' and subtype in('BLOOD BANK') and rejectby='' order by `resulttime` ASC;";
...
*/
                    ...
                    <td align="center"colspan="1"><a target='_blank' href="allreportdocnew?pmrn=<?php echo $row['pmrn']; ?>"style="color:#FF0000;"><?php echo $row["pmrn"]; ?></a>
                    ...
                    //if($ref1=!'' and $ref2!='' and is_int($ref1) and is_int($ref2)){
                        if($ref1=!'' and $ref2!=''){
			...
			?>
                    ...
                    <td align="center" colspan="1"><a onclick="return confirm_click();"
                        href="labreportconfirmopd_bb?id=<?php echo $row["id"]; ?>">Confirm</a></td>
                    ...
                    <td align="center" colspan="1"><a target='_blank'
                        ...
                        <?php
    if ($row['code'] == '61040652') {
        echo "<br><a target='_blank' href='bcrossreport1?id=".$row['id']."&pmrn=".$row['pmrn']."&eid=".$row['eid']."&sno=O".$row['id']."'>Blood Bag Report</a>";
    }
    ?>
                    </td>
                    <td align="center" colspan="1"><a onclick="return confirm_click1();" href="reject_opd_bb?id=<?php echo $row["id"]; ?>">Reject</a></td>
                </tr>
                <?php $count++;  }}
                    else {
...
<td align="center"colspan="1">
    <a target='_blank' href="<?php echo $row['report']?>?id=<?php echo $row['id']; ?>&pmrn=<?php echo $row['pmrn']; ?>&eid=<?php echo $row['eid']; ?>&sno=<?php echo 'E'.$row['id']; ?>">Report</a>
...
<td align="center" colspan="1"><a onclick="return confirm_click1();" href="reject_opd_bio?id=<?php echo $row["id"]; ?>">Reject</a></td>
      </tr>
