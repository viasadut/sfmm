...
mysqli_query($db,"UPDATE iinves SET resultstatus='Updated By Technologist',resulttime='$adate',resultby='$user',result='$rr',result1='$rr1' WHERE id='$id'");

//echo'<script>alert("Report Confirm Successfull !")</script>';
echo '<script>
    alert("Report Confirm Successful!");
    window.location.href = "labinp?pmrn='.$pmrn.'&eid='.$eid.'";
</script>';
...
$Tdata = mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM iinves WHERE id='$id'"));
$eid=$Tdata['eid'];
$iname=$Tdata['medi'];
...
<html lang="en">
  <head>
    ...
    <title>Hb Electrophoresis Analysis Report IPD - Sheikh Fazilatunnesa Mujib Memorial KPJ Specialized Hospital & Nursing College</title>
    <!-- Bootstrap core CSS -->
  ...
  <body  style="background-color: rgb(144, 238, 144);">
    ...
    <header>
        <nav class="navbar navbar-light bg-light">
          ...
          <h3><b>Hb Electrophoresis Analysis Report IPD</b></h3>
        </nav>
    </header>
    <main role="main" class="container-fluid">
               ...
               <form action="hbea-report-ipd.php" method="POST" enctype="multipart/form-data">
                <?php
                if($Existing_data_check==null){
                ?>
