...
$sel_query="Select * from po_table where po_type!='Pharmacy' and status IN ('','Rejected') ORDER BY id desc;";
...
<?php if($row['status']=='' || $row['status']=='Rejected' and $fullname=='1603'){

   $ono=$row['id'];
echo "<a href=po_prepare_new_purchase_edit?id=$ono>View</a>";
}

if($row['status']=='' and $fullname=='534'){
   $ono=$row['id'];
   echo "<a href=po_prepare_new_purchase_edit?id=$ono>View</a>";
   }
else {
...
<a href="po_print_new?ono=<?php echo $row["ono"]; ?>"></a> 
</td>
