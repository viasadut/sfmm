<?php 
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('bill','mng','staff')"; 
...
$sel9=mysqli_query($db,"SELECT * FROM m_suite WHERE `id`='$id'");
...
('$pmrn','$eid','Maternity Suite','$total_bill','$appdate','$apptime','$user','Maternity Suite','MS_ROOM','$mno','$vehicle1','$due_remarks')";
...
$sql1 = "update m_suite set doc_charge='$doc_charge',hos_charge='$hos_charge', medi_charge='$medi_charge', total_bill='$total_bill',ustatus='Paid',billby='$user',billtime='$billtime', billno='$last_id', payment_status='PAID' where id='$id'";
$conn->query($sql1);
header("Location: opd_ms_room_bill.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
...
$conn->close();
  ...
  ('$pmrn','$eid','Maternity Suite','$total_bill','$appdate','$apptime','$user','Maternity Suite','MS_ROOM','$mno','$vehicle1','$due_remarks','$taka')";
  ...
  $sql1 = "update m_suite set doc_charge='$doc_charge',hos_charge='$hos_charge', medi_charge='$medi_charge', total_bill='$total_bill',ustatus='Paid',billby='$user',billtime='$billtime', billno='$last_id', payment_status='PAID',discount_type='$discount_type',dis_amount='$taka',dis_payment='$dis_taka' where id='$id'";
  $conn->query($sql1);
  header("Location: opd_ms_room_bill.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
  ...
  $conn->close();
    ...
    $sql = "insert into pms_bill(pmrn,eid,location,amount,date,time,user,remarks,dname,s_no,p_mode,`p_remarks`,`dis_amount`) VALUES
    ('$pmrn','$eid','Maternity Suite','$total_bill','$appdate','$apptime','$user','Maternity Suite','MS_ROOM','$mno','$vehicle1','$due_remarks','$dis_percentage_amount')";
    ...
    $sql1 = "update m_suite set doc_charge='$doc_charge',hos_charge='$hos_charge', medi_charge='$medi_charge', total_bill='$total_bill',ustatus='Paid',billby='$user',billtime='$billtime', billno='$last_id', payment_status='PAID',discount_type='$discount_type',dis_amount='$percentage',dis_payment='$dis_percentage' where id='$id'";
    $conn->query($sql1);
    header("Location: opd_ms_room_bill.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");
...
$count=1;
$sel_query="Select * from promediused_ms where pmrn= '$pmrn' and eid='$eid'order by `id` DESC;";
...
$selected = mysqli_select_db($dbhandle,"sfmmkpjnew") 
  or die("Could not select examples");
	...
	$opd_procedure = "SELECT SUM(price) FROM prohoscharge_ms where pmrn= '$pmrn' and eid='$eid' "; 
...
echo $opd_procedure_sum=	$opd_procedure_data['SUM(price)'];

$opd_procedure_medi = "SELECT SUM(price) FROM promediused_ms where pmrn= '$pmrn' and eid='$eid' "; 
...
echo $opd_procedure_sum_medi=	$opd_procedure_data_medi['SUM(price)'];


$opd_procedure_doc = "SELECT SUM(procharge) FROM m_suite where pmrn= '$pmrn' and eid='$eid' "; 
