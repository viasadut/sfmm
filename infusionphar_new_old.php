...
$u_price=$b_chk_m['u_price'];
	...
	if($res95=mysqli_num_rows($result95)>0){		
			...
			$strSQL = "UPDATE iinfusion set pstatus='Served', puser='$user', pdate='$pdate',rfid='$tfid' ";
			$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			
			
			$ins_query4="update medi_stock set `add_qty`='$m_qty1' where code='$mcode' and add_qty!='0' and location='Pharmacy' ";
mysqli_query($con,$ins_query4) or die(mysql_error());

	/*$strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`brand`,`pmrn`,`eid`,`rfid`,`status`,`location`) values
			('$g_name','1','$u_price','$u_price','$user','$adate','$bb_name','$pmrn','$eid','$tfid','Sale','IPD')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from iinfusion where pmrn= '$pmrn'and eid='$eid'and odate='$dd' and status1='Active'and pstatus='Ordered'order by `id` DESC;";
