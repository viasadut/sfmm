...
$query = mysqli_query($db,"select * from injury where pmrn='$pmrn' and eid='$eid'");
...
$pdf->Cell('253',5,'THIS IS TO CERTIFY THAT ',0,1,'C');
$pdf->ln(4);
$pdf->Cell('253',5,strtoupper($data['m1'].' '.$data['pname']),0,1,'C');
$pdf->ln(3);
$pdf->Cell('253',5,strtoupper(' AGE '.'- ' .$data['page'].','.' MRN '.'- ' .$data['pmrn'].'- ' .$data['nid'].'- '.$data['nid1']) ,0,0,'C');
$pdf->ln(8);
$pdf->Cell('253',5,strtoupper('Was Admitted Through - '.$data['staff']),0,1,'C');
$pdf->ln(4);
$pdf->Cell('253',5,strtoupper('ON '. $data['fdate'].' AT '. $data['fdate1']),0,1,'C');
$pdf->ln(4);
$pdf->Cell('253',5,strtoupper('ON ADMISSION I HAD EXAMINED THE PATIENT AND THE FINDINGS AS FOLLOWS'),0,1,'C');

$pdf->ln(4);
$pdf->SetFont('Arial' , '' , 12);
$pdf->MultiCell('253',5,strtoupper($data['ffor']),0,1);
$pdf->SetFont('Arial' , 'BI' , 12);
$pdf->ln(4);
$pdf->Cell('253',5,strtoupper('THE PATIENT WAS DISCHARGED ON- '.$data['ddate']. ' AT '.$data['dtime'].' WITH THE DIAGNOSIS OF -'),0,1,'C');

$pdf->SetFont('Arial' , '' , 12);
$pdf->MultiCell('253',5,strtoupper($data['diagnosis']),0,1);
