...
('$pmrn','$eid','$pname','$padd','$pag','$addate','$pgen','$pp','$adate','$dd','Covid Unit','$bno','$count1','Followup Visit','$sid','$anew','Active','$user','Continuous')";
mysqli_query($con,$ins_query) or die(mysql_error());

}
	}
$update24="update inpatient set adoc='$btype' where pmrn='$pmrn' and eid= '$eid'";
mysqli_query($con,$update24) or die(mysql_error());

$ins_query99="insert into tinpatient (`adoc`,`pname`,`pmrn`,`adate`,`room`,`room1`,`eid`,`tby`,`ttime`) values ('$btype', '$pname','$pmrn','$adate','$psex','$ptemp','$eid','$user','$ttime')";
...
<!-- Form Title -->
        ...
        <fieldset>
			...
			<legend></legend>
            <!-- Name Input -->
			<label for="name"><strong>Select Transfer Doctor Name :</strong></label>
			<p>
			<select name="btype1" class="con_charge" required>
			
  

						<option value=''>-Select Doctor-</option>
				<?php 
			$sql76 = "select * from `doctor` where status='Active' order by dname asc";
...
<input name="psex" type="text" size="15" value="<?php echo $data["room"]; ?>"readonly>
      <input name="ptemp" type="text" size="13" value="<?php echo $data["room1"]; ?>"readonly>	 
		...
		<label for="age"><strong>Current Doctor's Name :</strong></label>
