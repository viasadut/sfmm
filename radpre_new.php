...
{
	echo '<script language="javascript">';
    echo 'alert("Unsuccessful !!! Session Out!!"); ';
    echo '</script>';
...
{
$ins_query_format="insert into rad_report_format (`dname`,`dcode`,`inves`,`add_by`,`add_time`,`status`,`type`,`iname`) 
...
$ins_query="insert into radreport (`dname`,`pmrn`,`pname`,`age`,`gender`,`pphone`,`dreffer`,`report`,`report1`,`type`,`eid`,`status`,`rdate`,`r1date`,`find`,`rdone`,`date2`,`type1`,`time`,`ac_no`,`status1`,`ineid`,`emerid`,`price`,`done_date`,`code`,`critical`,`updated_by`,`up_time`) 
values ('$full', '$pm','$pn','$pa','$ps','$pp','$dreffer','$cdetails','$plaintext','$pd','$count1','SEEN','$date','$date1','$find','$user','$date2','$dname1','$stime','$a_no','Updated By Technologist','$ineid','$emerid','$price','$date_d','$price_code','$critical','$user','$up_time')";
mysqli_query($con,$ins_query);



$update="update radpapp set status='SEEN',done_date='$date_d' where `ID`='$id'";
mysqli_query($con,$update);


$ins_query54="insert into his_report (`Accession_Number`,`Report_Data`,`Report_Date_Time`,`Status`) 
values ('$a_no', '$plaintext','$his_rd','1')";
mysqli_query($con,$ins_query54);

if($location=='OPD'){
  $update="update alltest set resultstatus='Confirmed By Consultant',rdate='$date',resulttime='$add_time',critical='$critical' where `id`='$a_no'";
  mysqli_query($con,$update);
...
if($location=='Inpatient'){
  $update="update iinves set resultstatus='Confirmed By Consultant',rdate='$date',resulttime='$add_time',critical='$critical' where `id`='$new_ano'";
  mysqli_query($con,$update);
...
if($location=='A&E'){
  $update="update einves set resultstatus='Confirmed By Consultant',rdate='$date',resulttime='$add_time',critical='$critical' where `id`='$new_ano'";
  mysqli_query($con,$update);
...
$ins_query="insert into radreport (`dname`,`pmrn`,`pname`,`age`,`gender`,`pphone`,`dreffer`,`report`,`report1`,`type`,`eid`,`status`,`rdate`,`r1date`,`find`,`rdone`,`date2`,`type1`,`time`,`ac_no`,`status1`,`ineid`,`emerid`,`price`,`done_date`,`code`,`critical`,`updated_by`,`up_time`) 
values ('$full', '$pm','$pn','$pa','$ps','$pp','$dreffer','$cdetails','$plaintext','$pd','$count1','SEEN','$date','$date1','$find','$user','$date2','$dname1','$stime','$a_no','Confirmed By Consultant','$ineid','$emerid','$price','$date_d','$price_code','$critical','$user','$up_time')";
mysqli_query($con,$ins_query);
...
$ins_query="insert into radreport (`dname`,`pmrn`,`pname`,`age`,`gender`,`pphone`,`dreffer`,`report`,`report1`,`type`,`eid`,`status`,`rdate`,`r1date`,`find`,`rdone`,`date2`,`type1`,`time`,`ac_no`,`status1`,`ineid`,`emerid`,`price`,`done_date`,`code`,`critical`) 
values ('$full', '$pm','$pn','$pa','$ps','$pp','$dreffer','$cdetails','$plaintext','$pd','$count1','Draft','$date','$date1','$find','$user','$date2','$dname1','$stime','$a_no','Updated By Technologist','$ineid','$emerid','$price','$date_d','$price_code','$critical')";
mysqli_query($con,$ins_query);



$update="update radpapp set status='Draft',done_date='$date_d' where `ID`='$id'";
mysqli_query($con,$update);



/*$ins_query54="insert into his_report (`Accession_Number`,`Report_Data`,`Status`) 
values ('$a_no', '$plaintext','1')";
mysqli_query($con,$ins_query54);
...
$ins_query="insert into radreport (`dname`,`pmrn`,`pname`,`age`,`gender`,`pphone`,`dreffer`,`report`,`report1`,`type`,`eid`,`status`,`rdate`,`r1date`,`find`,`rdone`,`date2`,`type1`,`time`,`ac_no`,`status1`,`ineid`,`emerid`,`price`,`done_date`,`code`,`critical`) 
values ('$full', '$pm','$pn','$pa','$ps','$pp','$dreffer','$cdetails','$plaintext','$pd','$count1','Draft','$date','$date1','$find','$user','$date2','$dname1','$stime','$a_no','Confirmed By Consultant','$ineid','$emerid','$price','$date_d','$price_code','$critical')";
mysqli_query($con,$ins_query);
...
<td colspan="20">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query_1 = "select * from `rad_report_format` where status='Active' and dcode='$fullname'";
            $result_1 = mysqli_query($con, $query_1);


            $query_2 = "select * from `rad_report_format` where status='Active' and dcode in ('$fullname','1444')";
...
if($fullname!=1527){
            ...
            echo '<option value="'.$row_1['type'].'">'.$row_1['type'].'</option>';
        }
      }


      else if($fullname==1527){
        while($row_2 = mysqli_fetch_array($result_2)) {
    
        echo '<option value="'.$row_2['type'].'">'.$row_2['type'].'</option>';
    }
  }
    ...
    function EnableDisableTextBox(chkPassport) {
        ...
        if (!txtPassportNumber.disabled) {
            txtPassportNumber.focus();
	 		...
	 		<td colspan="10">
	 <input type="checkbox" id="vehicle1" name="vehicle1" value="Bike" id="chkPassport" onclick="EnableDisableTextBox(this)" style="height:20px; width:20px; color:red;">				 
...
<tr>
    ...
    <td colspan="5"><button type="submit" name="Submit_d" style="background-color:pink">Save As Draft</button></td>
...
//var variables = "pmrn=str&string=$pd";
				...
				xmlhttp.open("GET", "pro_radio.php?pmrn=" + str + "&porder=<?php echo $pd;?>", true);
