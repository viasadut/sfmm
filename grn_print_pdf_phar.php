...
$row43=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM medi_stock WHERE sno='$id' and grn='$grn'"));
//$grn_date=$row43['re_date'];

$po_no=$row43['p_id'];

$row433=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM po_table WHERE ono='$po_no'"));

$row44=mysqli_fetch_assoc(mysqli_query($db,"SELECT COUNT(id) FROM medi_stock WHERE sno='$id' and grn='$grn'"));
$acct_discount=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM acct_ap WHERE grn='$grn'"));
...
$strSQL = "Select * from medi_stock where sno='$id' and grn='$grn' order by `id` asc;";
...
$strSQL1 = "Select * from medi_stock where sno='$id' and grn='$grn' order by `id` desc;";
$objQuery1 = mysqli_query($objConnect,$strSQL1) or die ("Error Query [".$strSQL."]");
$data=mysqli_fetch_assoc($objQuery1);
$uname=$data['add_by'];

$strSQL2 = "Select SUM(t_price) from medi_stock where sno='$id' and grn='$grn' and b_remarks='' order by `id` desc;";
$objQuery2 = mysqli_query($objConnect,$strSQL2) or die ("Error Query [".$strSQL."]");
$data2=mysqli_fetch_assoc($objQuery2);


$user_receive_q=mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM user WHERE uname='$uname'"));
?>
<html lang="en">
...
<br>
		...
		<div style="height:800px;border:1px solid;margin-left:30px;margin-right:20px;">
        <table width="100%">
    ...
    <br>
        <table width="100%">
              ...
              <tr>
                        ...
                        <td style="font-family: freesans; text-align:center;font-size:12px;" width="14%"><b>Batch No.</b>
                        </td>

                        <td style="font-family: freesans; text-align:center;font-size:12px;" width="14%"><b>Expire Date.</b>
                        </td>
...
<tr>
                        ...
                        if($row['b_remarks']==''){echo '<td style="font-family: freesans; text-align:left;font-size:12px;" width="50%">'.$row['g_name'].' ('. $row['b_name'].')'.'
                        </td>';}
                        
                        else if($row['b_remarks']!=''){echo '<td style="font-family: freesans; text-align:left;font-size:12px;" width="50%">'.$row['g_name'].' ('. $row['b_name'].')'.'('.$row['b_remarks'].')
                          </td>';}
                        
                        ?>

                        
                        <td style="font-family: freesans; text-align:center;font-size:12px;" width="14%"><?php echo $row['batch_no'];?>
                        </td>

                        <td style="font-family: freesans; text-align:center;font-size:12px;" width="14%"><?php echo $row['exdate'];?>
                        </td>
...
<tr>
                ...
                <td style="font-family: freesans; text-align:right;font-size:14px;font-weight:bold" width="20%"><?php 
                if($acct_discount['discount']>0){

                  echo $acct_discount['discount'];
                }
                else {
                echo $row433['amount_discount'];
                }
                ?>
                </td>
</tr>
...
<table width="100%">
                ...
                <td style="font-family: freesans; text-align:left;font-size:10px;" width="50%"><b>Received By:</b><?php echo $user_receive_q['fullname'];?>
                </td>
                <td style="font-family: freesans; text-align:right;font-size:10px;" width="50%"><b>Checked By:</b><?php echo $data['check_by'];?>
                </td>
