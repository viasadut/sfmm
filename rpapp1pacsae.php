...
if (!isset($_SESSION['sess_username']) || 
    !in_array($_SESSION['sess_userrole'], ['staff','rad'])) {
...
$rdate = date('Y-m-d');
$app_date = !empty($date1) ? date('Ymd', strtotime($date1)) : '';

// ================= FETCH DATA =================
$row339 = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM einves WHERE id='$id'"));
$tname  = $row339['infusion'] ?? '';
$link   = $row339['link'] ?? '';
$a_id   = 'E'.($row339['id'] ?? '');
$pname  = $row339['pname'] ?? '';
$price  = (int)($row339['price'] ?? 0);
$pcode  = $row339['code'] ?? '';
$ddname = $row339['dname'] ?? '';

$row39 = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM patient WHERE pmrn='$pmrn'"));
$age1  = !empty($row39['bdate']) ? date('Ymd', strtotime($row39['bdate'])) : '';
$psex  = $row39['psex'] ?? '';

$row40 = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM emergency WHERE pmrn='$pmrn' AND eid='$eid'"));
$api_adminssion_no = (int)($row40['OUT_ADMISSION_NO_PK'] ?? 0);

/* ================= DOCTOR ================= */
$doctor_name = "Emergency Medical Officer";
$doctor_code = "emo";
/* ================= AE TITLE MAP ================= */
$ae_map = [
  'MR'=>'NODENAME',
...
if(isset($_REQUEST['Submit'])){
    ...
    // ================= VALIDATION =================
    if(empty($slot)){
        echo "<script>alert('No Appointment Slot Selected!!');</script>";
    } else {

        $check = mysqli_query($con,"SELECT * FROM radpapp 
            WHERE pmrn='$pmrn' AND tname='$tname' 
            AND adate='$date1' 
            AND status NOT IN ('CANCEL','NOT SEEN')");

        if(mysqli_num_rows($check) > 0){
            echo "<script>alert('Patient already has appointment!');</script>";
        } else {
          ...
          $ins_querypac = "INSERT INTO his_order (
              ...
              '$app_date','$slot',
              '$eid','$api_adminssion_no','ER',
              '1','$ae_title'
          )";

            mysqli_query($con, $ins_querypac) or die(mysqli_error($con));

            // ================= UPDATE STATUS =================
            mysqli_query($con,"UPDATE einves 
                SET rby='$user', rtime='$btime', rstatus='RECEIVED', status='DONE', rdate='$rdate' 
                WHERE id='$id'");
            ...
            (pname, pmrn, pphone, dname, status, psex, dreffer, tname, btime, a_no, link, adate, aslot, location, emerid, adate1, price) 
            VALUES 
            ('$pname','$pmrn','$pphone','$dname','NOT SEEN','$psex','$ref_doc','$tname','$btime','$a_id','$link','$date1','$slot','A&E','$eid','$rdate','$price')");

            // ================= ACCOUNTING =================
            $tb_q = mysqli_query($con,"SELECT * FROM acct_master_new WHERE item_code='$pcode'");
            ...
            VALUES 
            ('$id','CR','$tb_data','$date','$price','AE_INVES_CHARGE_RAD')");

            mysqli_query($con,"INSERT INTO pms_tb 
            (trans_id, trans_type, acct_code, date, amount, location) 
            VALUES 
            ('$id','DR','111999','$date','$price','AE_INVES_CHARGE_RAD')");

            // ================= SUCCESS =================
            header("Location: opd_sticker.php?a_no=$a_id");
            exit;
        }
    }
}
