...
$data = mysqli_fetch_assoc($query4);
      if($_POST["employee_id3"] != '')  {
		...
		if($roption=='Back To Stock'){
        
      
           $query = "update bcross1 set status='Available' WHERE bagno = '$bagno'";  
		   mysqli_query($connect,$query) or die(mysql_error());
           $message = 'Data Updated';  
		   
		   $query1 = "update iblood set return_received_remarks='$return_remarks',return_received_by='$user',return_receive_time='$dtime',bbstatus='$roption' WHERE id = '".$_POST["employee_id3"]."'";  
		...
		else if($roption=='Discard'){
        
      
            $query = "update bcross1 set status='Discard' WHERE bagno = '$bagno'";  
            mysqli_query($connect,$query) or die(mysql_error());
            $message = 'Data Updated';  
