...
$query4 = mysqli_query($db,"select * from alltest where pmrn='$pmrn' and location='OUTSIDE'");
...
$query43 = "SELECT COUNT(pmrn) FROM alltest where pmrn= '$pmrn' and location='OUTSIDE';"; 
...
$sel99="SELECT * FROM doctor1 WHERE `dname`='$dname';";
$result99 = mysqli_query($con,$sel99);


if($res99=mysqli_num_rows($result99)==0)
{
 	
       echo '<script language="javascript">';
    echo 'alert("Unsuccessful !!! Doctor Name is not in the Database List.. Please contact with Concern Department"); ';
    echo '</script>';
    }
...
$ins_query="insert into alltest (`dname`,`pmrn`,`pname`,`medi`,`ins`,`date`,`type`,`price`,`code`,`link`,`date1`,`location`,`eid`) values ('$dname', '$pmrn','$pname','$medi','$pins','$date','$type','$price','$code','$link','$date77','OUTSIDE','$count1')";
...
<!-- Form Title -->
	...
	<input list="tname45" name="dname" size=75% class="form-control" value=""autocomplete="off">
...
$count=1;
$sel_query="Select * from alltest where pmrn= '$pmrn' and location='OUTSIDE' and date1='$sdate'order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["medi"]; ?></td>
				  ...
				  <td align="center" colspan="2"><a href="deleterad?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>">DELETE</a></td>
