...
$count=1;
$sel_query="Select * from dleave where uname='$fullname' and sdate between '$d1' and '$d2' and edate between '$d1' and '$d2' order by id desc";
//$start=$row["aadate"];
...
echo "<a target='_blank' href='$url'><img src='print.png' title='Print Report' width='150' height='60'/></a>";
	  ...
	  $id2=$row["id"];
		...
		$url = "leaveapprovereuser?id=$id2&user=$user"; 
		if($dr=='Approval Pending' and $incharge=='')
		{
echo "<a target='_blank' href='$url'>Reject</a>";
		}
		
			else if($dr=='Forwarded to Incharge' and $incharge!='')
		{
echo "<a target='_blank' href='$url'>Reject</a>";
		}
	  ?>
