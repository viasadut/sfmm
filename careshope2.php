...
$ndate = date('Y-m-d');
$infu = $_REQUEST['infu'];
$infu_api = $_REQUEST['infu'].' - CareShope';
$remarks = (int)$_REQUEST['remarks'];

$query159 = mysqli_query($db,"select * from storenew where ename='$infu' and estatus='Active'");
$data159 = mysqli_fetch_assoc($query159);
//$type=$data159["type"];
$code=(int)$data159["eid"];
$price=(int)$data159["price"];
...
$sel90="SELECT * FROM storenew WHERE `ename`='$infu' and estatus='Active';";
...
$sql = "insert into careshope1 (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`room`,`status`,`ordate`,`code`,`price`,`rstatus`,`ndate`) 
values ( '$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$odate','$infu','$user','$remarks','Data Updated','$ordate','$code','$price','Ordered','$ndate')";
...
//echo json_encode($data);
  ...
  $invoice_no=$decoded_response['invoice_id'];
   ...
   if($decoded_response['invoice_no']!='' and $decoded_response['invoice_id']!=''){
    ...
    $api_query = "update careshope1 set api_status='1', invoice_no='$invoice_no' where id='".$last_id."'"; 
...
<tr><td colspan="15" align="center"bgcolor="lightgreen"><label><strong>Careshope Item Request Form</strong></label></td> </tr>
<tr><td colspan="12" align="center"bgcolor="lightblue"><label><strong>Name Of the Item</strong></label></td>
...
<td colspan="12" align="center"><input list="browsers1" name="infu" size=60% class="form-control" autocomplete="off">
  <datalist id="browsers1">
			...
			?>  </datalist></td>
			<td colspan="8" align="center"><input type="number" name="remarks" value="" /></td>
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Careshope Item Request Form</strong></label></td> </tr>
...
$count=1;
$sel_query="Select * from careshope1 where pmrn= '$pmrn' and eid='$episode'and rstatus ='Ordered' order by `id` DESC;";
...
{ ?>    <tr>
 ...
 <td align="center" colspan="2"><a href="careshopedelete_nurse?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>&invoice_no=<?php echo $row['invoice_no']; ?>&admission_no=<?php echo $api_adminssion_no; ?>&code=<?php echo $row['code']; ?>&pdos=<?php echo $row['room']; ?>">DELETE</a></td>  	  
	...
	<tr><td colspan="22" align="center"bgcolor="lightblue"><label><strong>RECEIVED Item</strong></label></td> </tr>	
...
<tr>
      ...
      <td colspan="2" align="center"><strong>Remarks</strong></td>   
      <td colspan="4" align="center"><strong>Status</strong></td>
	  <td colspan="2" align="center"><strong>Provide Time</strong></td>
       	  <td colspan="2" align="center"><strong>Provided By</strong></td>
...
//$count=1;
$sel_query="Select * from careshope1 where pmrn= '$pmrn' and eid='$eid' and rstatus IN ('Provided','Not Provided')  order by `id` DESC;";
