...
$sql = "insert into prohoscharge_ms (`dname`,`pmrn`,`pname`,`medi`,`eid`,`date`,`pdos`,`type`,`ieid`,`code`,`qty`,`ins`,`price`,`ip`,`op`,`acct_code`,`ccentre`) 
values ('$full','$pmrn','$pname','$medi1','$eid','$date1','$pdos','$type','$ieid','$dcode','$price','$btype','$tprice','$ip','$op','$acct_code','$ccentre')";
...
*/
if ($conn->query($sql) === TRUE and $type=='Inpatient') {
...
echo json_encode($data);
  ...
  $invoice_no=$decoded_response['invoice_id'];
   ...
   if($decoded_response['invoice_no']!='' and $decoded_response['invoice_id']!=''){
    ...
    $api_query = "update prohoscharge_ms set api_status='1', invoice_no='$invoice_no' where id='".$last_id."'"; 
...
<td colspan="15" align="center">
      ...
      <select class="con_charge21"
                    name="medi1" id="con_charge1" onchange="GetDetail(this.value)" required>
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["medi"]; ?></td>
				 ...
				 <td align="center" colspan="2"><a href="prohosdelete1_ms?id=<?php echo $row["id"]; ?>&pmrn=<?php echo "$pmrn"; ?>&dname=<?php echo "$dname"; ?>&eid=<?php echo "$eid"; ?>&ID=<?php echo "$id"; ?>&ieid=<?php echo "$ieid"; ?>&type=<?php echo "$type"; ?>&pdos=<?php echo $row["pdos"];?>&admission_no=<?php echo $api_adminssion_no;?>&invoice_no=<?php echo $row['invoice_no'];?>&code=<?php echo $row['code'];?>">DELETE</a></td>
