...
else if($res=mysqli_num_rows($result99)>0)
{
	...
	$update33="update pappnew set `aslot`='$slot', `bill`='$bill',`billby`='$user',`pmrn`='$pmrn', `pname`='$name', `padd`='$padd',`billtime`='$billtime',`payment`='$payment',`ptype`='$ptype',`adate1`='$aad' where `ID`='$id'";
mysqli_query($con,$update33);

//$update="update test set status='Booked' where `dname`='$dname' and `ddate`='$date1' and `dslot`='$slot'";
//mysqli_query($con,$update) or die(mysql_error());

$update87="update opd_appoint1 set status='Booked' where `dname`='$9' and `ddate`='$date11' and `dslot`='$slot'";
...
<!-- Form Title -->
      ...
      <?php if($data1['remarks']!=''){echo'<strong><SPAN STYLE="font-size:18.0pt;color:red;">PAYMENT DUE</span> </strong>';}?>
    </label>
