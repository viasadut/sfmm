...
//$id =$_GET['id'];
$count=1;
$sel_query="Select SUM(add_qty),SUM(s_qty),SUM(given_qty),g_name,b_name,code from medi_stock where location='Pharmacy_ipd'  and status in('Served','Partially Served') group by code ORDER BY code asc;";
...
$sd=date('Y-m-01');
$ed=date('Y-m-31');
$queryq = "SELECT SUM(qty) from phar_sale where code='".$cc."' and adate between '$sd' and '$ed' and location in ('IPD','DISCHARGE') and status!='Cancel'"; 
...
<?php if($cc1==''){echo '
<a target="_blank" href="ipd_hoscharge_details.php?code='.$row['code'].'">
