...
$queryd1 = mysqli_query($db,"select * from doctor_code where dcode='$dcode' and dname like '%CONSUL%'");
...
else if($res=mysqli_num_rows($result99)>0)
{
	...
	$update33="update pappnew set `aslot`='$slot', `bill`='$bill',`billby`='$user',`pmrn`='$pmrn', `pname`='$name', `padd`='$padd',`billtime`='$billtime',`payment`='$payment',`ptype`='$ptype',`adate1`='$aad',`dcode`='$dcode',`ccode`='$code4',`ip`='$ip',`op`='$op',`app_con`='$app_con',`ccentre`='$ccentre' where `ID`='$id'";
mysqli_query($con,$update33);

//$update="update test set status='Booked' where `dname`='$dname' and `ddate`='$date1' and `dslot`='$slot'";
//mysqli_query($con,$update) or die(mysql_error());

$update87="update opd_appoint1 set status='Booked' where `dname`='$dname' and `ddate`='$date11' and `dslot`='$slot'";
...
$ins_query1="insert into patient (`pname`,`pmrn`,`pphone`,`padd`,`psex`,`bdate`,`dis`,`type`) values 
('$name', '$pmrn','$pphone','$padd','$psex','$date91','$dis','$ptype')";
mysqli_query($con,$ins_query1);

$update33="update pappnew set `aslot`='$slot', `bill`='$bill',`billby`='$user',`pmrn`='$pmrn', `pname`='$name', `padd`='$padd',`billtime`='$billtime',`payment`='$payment',`ptype`='$ptype',`dcode`='$dcode',`ccode`='$code4',`ip`='$ip',`op`='$op',`app_con`='$app_con',`ccentre`='$ccentre' where `ID`='$id'";
mysqli_query($con,$update33);
...
<!-- Form Title -->
		...
		<label for="mail"><strong>Appointment Date :</strong></label>
			...
			<select name="slot"required> <option value=''>-Select Slot-</option>
