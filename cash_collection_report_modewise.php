...
echo $row43_p3['SUM(amount)'] + $row43_p['SUM(amount_receive)'];
echo
' BDT </span>';
echo " ,  From  ";
echo $start;
...
$bill_user=$row['user'];
$bill_date=$row['date'];
$query43 = "SELECT SUM(amount_receive) FROM pms_bill where user= '$bill_user'and date='$bill_date' and location IN ('OPD-DIS','OTC_Sale','OPD_Medi');"; 
	 
$result43 = mysqli_query($con, $query43) or die(mysqli_error());
$phar_collection = mysqli_fetch_assoc($result43);


$query43_o = "SELECT SUM(amount) FROM pms_payment where user='$bill_user'and date='$bill_date' and refund!='1';"; 
	 
$result43_o = mysqli_query($con, $query43_o) or die(mysqli_error());
$temp_opd_collection_all = mysqli_fetch_assoc($result43_o);


$query43_dis = "SELECT SUM(dis_amount) FROM pms_payment where user= '$bill_user'and date='$bill_date' and refund!='1';"; 
	 
$result43_dis = mysqli_query($con, $query43_dis) or die(mysqli_error());
$opd_collection_dis = mysqli_fetch_assoc($result43_dis);

$query43_refund = "SELECT SUM(amount) FROM pms_payment where user= '$bill_user'and date='$bill_date' and refund='1';"; 
...
$opd_collection=$phar_collection['SUM(amount_receive)']+$temp_opd_collection_all['SUM(amount)']-$opd_collection_dis['SUM(dis_amount)']-$opd_collection_refund['SUM(amount)'];
