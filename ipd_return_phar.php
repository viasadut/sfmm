...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('staff','nurse')"; 
...
$sel95="SELECT * FROM medi_stock WHERE `sno`='$code' and location='$loc';";
...
if($tprice>$aqty)
{
    ...
    echo 'alert("Unsuccessful !!! Return Quantity is Greater Than Available Quantity"); ';
    echo '</script>';
    }
			...
			*/
			...
			$ins_query1="insert into medi_stock1 (`code`,`g_name`,`b_name`,`add_qty`,`req_loc`,`status`,`location`,`rfid`,`sno`,`batch_no`,`add_by`,`add_time`)
 values ('$cc','$medi','$brand','$tprice','$loc','Returned','Pharmacy','$sno','$code','$batch_no','$user','$add_time')";
mysqli_query($con,$ins_query1) or die(mysql_error());
			
			
			$strSQL = "update medi_stock set add_qty='$n_qty' where sno='$code' and location='$loc'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			
$url = "ipd_return_phar?sno=$sno";
header("Location: $url"); 
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><h1 style="color:red"><strong>Serial No - <?php echo $sno;?></strong></h1></label></td> </tr>
...
<td colspan="3" align="center"><label><strong>Code</strong></label></td> 

<td colspan="12" align="center"><label><strong>Medicine Name</strong></label></td> 
<td colspan="1" align="center"><label><strong>Location</strong></label></td> 
<td colspan="1" align="center"><label><strong>Available Qty</strong></label></td> 

<td colspan="1" align="center"><label><strong>Unit Price</strong></label></td>

<td colspan="1" align="center"><label><strong>Request Qty</strong></label></td>
<td colspan="1" align="center"><label><strong>Total Price</strong></label></td></tr>
<tr>
<td colspan="3">
...
<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='code' required style="font-weight: bold;font-size:22px;color:green">
	...
	<option value=''>-Select-</option>
            ...
            require('db1.php');
            $uname = '';
            $query = "select * from `medi_stock` where add_qty>0 and location in ('ICU','5AB Medicine stock','5C Medicine stock','6CD Medicine stock','6AB Medicine stock','5AB emergency trolley','5CD emergency trolley','HMD','NICU','Maternity Suite')";
            $result = mysqli_query($con, $query);
            while($row = mysqli_fetch_array($result)) {
        ?>
            <option value="<?php echo $row['sno']; ?>"><?php echo $row['g_name']; ?></option>
        <?php } ?>
...
<td colspan="12" align="center"><textarea name="medi" id="code" class="form-control action" cols="30" rows="5"style="font-weight: bold;font-size:22px;color:green"readonly required>
							...
							<td colspan="1"><textarea name="loc" id="loc" class="form-control action" cols="30" rows="5"style="font-weight: bold;font-size:22px;color:green"readonly required>
...
<td colspan="1" ><input type="text" name="uprice" id="uprice" required value="" readonly style="font-weight: bold;font-size:22px;color:green"></td>
						
						
						<td colspan="1" ><input type="text" name="tprice" id="qty" required value="" style="font-weight: bold;font-size:22px;color:green"></td>
						
							
						
						
						<td colspan="1" ><input type="text" name="charge" id="tprice" readonly value="" style="font-weight: bold;font-size:22px;color:green"required></td>
...
<tr>
      ...
      <td colspan="9" align="center"><strong>Medi</strong></td>
...
$count=1;
//$count=1;
$sel_query="Select * from medi_stock where rfid= '$sno' and req_loc in ('ICU','5AB Medicine stock','5C Medicine stock','6CD Medicine stock','6AB Medicine stock','5AB emergency trolley','5CD emergency trolley','HMD','NICU','Maternity Suite') order by `id` DESC;";
...
{ ?>    <tr>
      ...
      <td align="center"colspan=""><?php echo $row["g_name"]; ?></td>
	  
      
	  <td align="center"colspan="3"><?php echo $row["add_qty"]; ?></td>
...
//						document.getElementById(
	//						"page").value = myObj[1];
				...
				xmlhttp.open("GET", "phar_out1_return.php?pmrn=" + str, true);
