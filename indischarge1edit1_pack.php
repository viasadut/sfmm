...
$query40 = "SELECT * FROM idischarge1_pack where pmrn= '$pmrn' and eid='$eid'"; 
...
$sel90="SELECT * FROM idischarge1_pack WHERE `pmrn`='$pmrn' and eid='$eid';";
...
$query = "SELECT * from package_screening where pmrn='$pmrn' and eid='$eid'"; 
...
if($res90=mysqli_num_rows($result90)==0)
{
    ...
    echo 'alert("Unsuccessful !!! No Discharge Note written.. Pls Wrtite The Discharge Note from Prepare Discharge Note option..."); ';
    echo '</script>';
...
$update="update idischarge1_pack set dname='$dname', emo='$full', ddate='$date1', discharge='$discharge',ddia='$ddia', surgery='$surgery', ill='$ill', other='$other', plan='$plan', dinves='$dinves',refer='$refer' where `eid`='$eid' and pmrn='$pmrn'";
...
$url = "indischarge1edit_pack?pmrn=$pmrn&eid=$eid" ;
header("Location:$url");
}
}
?>
...
<tr><td colspan="20"><input list=dis name="discharge" placeholder="-Select Discharge Type-" class="form-control" value="<?php echo $row40['discharge'];?>"required>
					<datalist id="dis">	
...
<tr><td colspan="20"><textarea id="exampleTextarea2" name="dinves" rows="5" placeholder="Advise on Discharge">

<?php echo $row40['dinves'];?>
...
<tr><td colspan="20"><label for="age"><strong>Advise On Discharge: (Can Write Both in Bangla & English)</strong></label>

<input type="text" id="pmrn" onkeyup="GetDetail(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='vtype' placeholder='Use Template Discharge Plan & Follow Up Plan' style="color:green;font-size:22px; font-weight:bold">
<datalist id="categoryname">
	<option value=''>-Select-</option>
            ...
            $query_1 = "select * from `dis_template` where add_by='$ddname'";
...
<tr><td colspan="20"><textarea id="exampleTextarea2" name="other" rows="5" placeholder="Advise on Discharge">


<?php echo $row40['other'];?>
</textarea></td>  </tr>	
<script>
...
<tr><td colspan="20"><textarea id="exampleTextarea22" name="plan" rows="5" placeholder="Follow Up Plan">
<?php echo $row40['plan'];?>
...
//						document.getElementById(
	//						"page").value = myObj[1];
							...
							document.getElementById("exampleTextarea22").value = myObj[1];
							
							//document.getElementById("pd").value = myObj[2];
							
							
							CKEDITOR.instances["exampleTextarea22"].setData(myObj[1]);
...
//var variables = "pmrn=str&string=$pd";
				...
				xmlhttp.open("GET", "pro_radio_test1.php?pmrn=" + str + "&porder=<?php echo $ddname;?>", true);
