...
session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('mng','doctor','qc','mrd','covid','call','covid1','bill','lab')"; 
...
$count=1;
if($dt==$adate)
{$sel_query="Select * from covidopd where ssent='$adate' and status='collected' order by sid asc;";}

else if($dt==$rd)
{$sel_query="Select * from covidopd where ssent='$rd'and status='collected'  order by sid asc;";}

else if($dt==$rd2)
{$sel_query="Select * from covidopd where ssent='$rd2' and status='collected'order by sid asc;";}

else if($dt==$rd3)
{$sel_query="Select * from covidopd where ssent='$rd3' and status='collected'order by sid asc;";}

else if($dt==$rd4)
{$sel_query="Select * from covidopd where ssent='$rd4'and status='collected' order by sid asc;";}


else if($dt==$rd5)
{$sel_query="Select * from covidopd where ssent='$rd5' and status='collected'order by sid asc;";}


else if($dt==$rd6)
{$sel_query="Select * from covidopd where ssent='$rd6'and status='collected' order by sid asc;";}
...
<td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><a target='_blank' href="covidopdprint1?id=<?php echo $row["id"]; ?>"><img src="print.png" title="Print Report" width="50" height="25" /></a> </td>
