...
$query198 = "SELECT SUM(amount) FROM advance_bill where pmrn='$pmrn'and eid='$eid' and billno='' and dname='IPD ADVANCE'"; 
...
$query198_r = "SELECT SUM(amount) FROM advance_bill where pmrn='$pmrn'and eid='$eid' and billno='' and dname='Refund'"; 
	 
$result198_r = mysqli_query($dbhandle,$query198_r) or die(mysql_error());

// Print out result
$row198_r = mysqli_fetch_array($result198_r);
$test1_r=	$row198_r['SUM(amount)'];

$new_advance=$test1-$test1_r;
//echo $test1;


$count = "SELECT COUNT(id) FROM advance_bill where pmrn='$pmrn'and eid='$eid' and billno='' and dname='IPD ADVANCE'"; 
	 
$count_res = mysqli_query($dbhandle,$count) or die(mysql_error());

// Print out result
$count_data = mysqli_fetch_array($count_res);
...
$item = $_REQUEST['item'];
$r_remarks = $_REQUEST['r_remarks'];
$date = date('m/d/Y');


$price1 =$test1+$data['advance'];
$price =$_REQUEST["price"];
$price2 =-$_REQUEST["price"];
$vehicle1 =$_REQUEST["vehicle1"];
$p_remarks =$_REQUEST["p_remarks"];



$query198 = "SELECT SUM(amount) FROM advance_bill where pmrn='$pmrn'and eid='$eid' and dname='IPD ADVANCE'"; 
...
$query198_r = "SELECT SUM(amount) FROM advance_bill where pmrn='$pmrn'and eid='$eid' and dname='Refund'"; 
...
$new_advance=$price+$test1-$test1_r;
$new_advance1=$test1-$price-$test1_r;
...
if($item=='Advance Payment'){
  ...
  $sql1 = "insert into pms_bill (pmrn,eid,location,amount,date,time,user,remarks,dname,s_no,p_mode,p_remarks) values
		('$pmrn', '$eid', '$ipd', '$price', '$appdate', '$apptime', '$user', '$ipd', '$ipd1', '$mno', '$vehicle1','$p_remarks')";
...
$sh = $db1->prepare("insert into pms_payment(pmrn,eid,location,amount,date,time,user,billno1,p_mode) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
$sh->execute([$pmrn, $eid, $ipd1, $price, $appdate, $apptime, $user, $last_id,$vehicle1]);


$sh = $db1->prepare("insert into advance_bill(pmrn,eid,location,amount,date,time,user,remarks,dname,r_remarks,billno) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$sh->execute([$pmrn, $eid, $ipd, $price, $appdate, $apptime, $user, $item, $ipd1, $r_remarks, $last_id]);

$sh = $db1->prepare("UPDATE inpatient SET advance=? WHERE pmrn=? and eid=?");
$sh->execute([$new_advance, $pmrn, $eid]);
...
$ins_query="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$last_id','CR','111999','$date','$price','IPD ADVANCE')";
  ...
  values ('$last_id','DR','619210','$date','$price','IPD ADVANCE')";
  ...
  {
  $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$last_id','DR','619230','$date','$price','IPD ADVANCE')";
  mysqli_query($con,$ins_query2) or die(mysql_error());
  }
  ...
  else if($vehicle1=='Cheque')
  {
  $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$last_id','DR','619220','$date','$price','IPD ADVANCE')";
  mysqli_query($con,$ins_query2) or die(mysql_error());
  }


header("Location: ipd_advance_bill.php?pmrn=$pmrn&eid=$eid");
}
}
...
$row45=mysqli_fetch_assoc(mysqli_query($db,"SELECT SUM(amount) FROM advance_bill WHERE location='IPD' and pmrn='$pmrn' and eid='$eid' and dname='IPD ADVANCE'"));

$row455=mysqli_fetch_assoc(mysqli_query($db,"SELECT SUM(amount) FROM advance_bill WHERE location='IPD'and pmrn='$pmrn' and eid='$eid' and dname='Refund'"));
//echo $row45['SUM(price)'];

$new_bill=$row455['SUM(amount)']+$price;
$old_bill=$row45['SUM(amount)'];

$test1=$row45['SUM(amount)'];


  /*$query198 = "SELECT SUM(amount) FROM advance_bill where pmrn='$pmrn'and eid='$eid'"; 
...
$sql1 = "insert into pms_bill (pmrn,eid,location,amount,date,time,user,remarks,dname,s_no,p_mode,p_remarks) values
('$pmrn', '$eid', '$ipd', '$price', '$appdate', '$apptime', '$user', '$ipd', '$refund', '$mno', '$vehicle1','$p_remarks')";
...
//$sql1 = "insert into refund_bill (amount,refund_by,refund_time,billno,r_amount,location,date,p_mode,p_remarks,pmrn,eid,b_remarks) values
//('$test1','$user','$refund_time','','$price','IPD','$r_date','Cash','$r_remarks','$pmrn','$eid','$r_remarks')";

if($vehicle1=='Cash'){
$sh = $db1->prepare("insert into refund_bill(amount,refund_by,refund_time,billno,r_amount,location,date,p_mode,pmrn,eid,b_remarks) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$sh->execute([$price, $user, $refund_time, $last_id, $price, $ipd, $r_date, $vehicle1, $pmrn, $eid, $r_remarks]);

$sh = $db1->prepare("insert into pms_payment(pmrn,eid,location,amount,date,time,user,billno1,p_mode,refund) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
$sh->execute([$pmrn, $eid, $ipd1, $price, $appdate, $apptime, $user, $last_id,$vehicle1,$r_s]);

}


else if($vehicle1!='Cash'){
  $sh = $db1->prepare("insert into refund_bill(amount,refund_by,refund_time,billno,r_amount,location,date,p_mode,p_remarks,pmrn,eid,b_remarks) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  $sh->execute([$price, $user, $refund_time, $last_id, $price, $ipd, $r_date, $vehicle1, $p_remarks, $pmrn, $eid, $r_remarks]);
...
$sh->execute([$pmrn, $eid, $ipd1, $price, $appdate, $apptime, $user, $last_id,$vehicle1,$r_s]);
  ...
  $sh = $db1->prepare("insert into advance_bill(pmrn,eid,location,amount,date,time,user,remarks,dname,r_remarks,billno) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  $sh->execute([$pmrn, $eid, $ipd, $price, $appdate, $apptime, $user, $item, $item, $r_remarks,$last_id]);

  $sh = $db1->prepare("UPDATE inpatient SET advance=? WHERE pmrn=? and eid=?");
$sh->execute([$new_advance1, $pmrn, $eid]);
...
if($db1->commit()){
  ...
  $date=date('Y-m-d');
  $ins_query="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$last_id','DR','111999','$date','$price','IPD REFUND')";
    mysqli_query($con,$ins_query) or die(mysql_error());
    ...
    {
    $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$last_id','CR','619210','$date','$price','IPD REFUND')";
    mysqli_query($con,$ins_query2) or die(mysql_error());
    }
    ...
    {
    $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$last_id','CR','619230','$date','$price','IPD REFUND')";
    mysqli_query($con,$ins_query2) or die(mysql_error());
    }
  ...
  {
  $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
  values ('$last_id','CR','619220','$date','$price','IPD REFUND')";
  mysqli_query($con,$ins_query2) or die(mysql_error());
  }
...
echo '<script language="javascript">';
    echo 'alert("Refund Amount cannot be greater than Deposit Amount!!!"); ';
    echo '</script>';
//  $db1->commit();
  ...
  $db1->rollBack();
	...
	$sql3 = "update pms_bill set eid='', amount='0', error='Network Problem' where billno='$last_id'";
  $conn->query($sql3);
  
  echo '<script language="javascript">';
      echo 'alert("Falied333 !!"); ';
      echo '</script>';
}	
...
$query44 = mysqli_query($db,"select COUNT(id) from advance_bill where pmrn='$pmrn' and eid='$eid' and billno='' and location='IPD'");
...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>IPD Deposit / Refund Form</strong></label></td> </tr>
<tr><td colspan="10" align="center"><label><strong>Type</strong></label></td> 

<td colspan="10" align="center"><label><strong>Amount</strong></label></td> 
</tr>
<tr>
<td colspan="10" align="center"><select name="item" class="con_charge" id="pmrn" onchange="GetDetail(this.value)" required>
...
<tr>
<td colspan="20" align="left"><textarea required name="r_remarks"></textarea>
...
<input type="radio" id="vehicle1" name="vehicle1" value="Cheque"id="chkPassport3" onclick="EnableDisableTextBox3(this)" style="height:20px; width:20px; color:red;"><span style="font-size:40px;color:red;font-weight:bold;">Cheque</span>				 
...
<tr>
    ...
    <td colspan="20"align="right"><a target="_blank" 
    href="ipd_bill_paper_advance?pmrn=<?php echo $pmrn; ?>&eid=<?php echo $eid; ?>">Print</a></td>  	  
...
<tr>
      ...
      <td colspan="2" align="center"><strong>MRN</strong></td>
     	  <td colspan="10" align="center"><strong>Amount</strong></td>
...
{ ?>    <tr>
      ...
      <td align="center"colspan="2"><?php echo $row["pmrn"]; ?></td>
	        <td align="center"colspan="10"><?php echo $row["amount"]; ?></td>
			      <td align="center"colspan="3"><?php echo $row["dname"]; ?></td>
				  
          <td align="center"colspan="3"><?php echo $row["time"]; ?></td>
		...
		$url = "?pmrn=$pmrn&id=$id1&ID=$id&dname=$dname"; 
		   
		   
		
	if($rstatus!='RECEIVED')
	{ 
echo "<td align='center' colspan='2' style='background-color:lightblue;'><a href='$url'><b></b></a></td>";
	}
...
<script type="text/javascript">
  ...
  function EnableDisableTextBox3(chkPassport3) {
   ...
   var txtPassportNumber66 = document.getElementById("sdate21");
   txtPassportNumber66.disabled = chkPassport3.checked ? false : true;
   if (!txtPassportNumber66.disabled) {
       txtPassportNumber66.focus();
   }
}
</script>
