...
<?php if($tt=='192.168.100.252:8081'){echo'
...
else if($tt!='192.168.100.252:8081'){echo'
<form action="http://182.160.124.36/Launch_Viewer.asp?" method="post" id="tt">
<input type="hidden" name="PatientID" value="'.$pmrn.'"></input>
<input type="hidden" name="Username" value="hisuser"></input>
<input type="hidden" name="Password" value="hisuser"></input>
<input type="submit" name="Submit90" value="PACS VIEW"align="right"></input>
...
{ ?>    <tr>
      ...
      <td align="center"><a target='_blank' href="all_report_indu_all?pmrn=<?php echo $row["pmrn"]; ?>&test=<?php echo $row["medi"]; ?>"><?php echo $row["medi"]; ?></a>
...
$count=1;
$sel_query="Select * from iinves where pmrn= '$pmrn' and pstatus=''order by id desc;";
...
{ ?>    <tr>
	  ...
	  <td align="center"><a target='_blank' href="all_report_indu_all?pmrn=<?php echo $row["pmrn"]; ?>&test=<?php echo $row["infusion"]; ?>"><?php echo $row["infusion"]; ?></a></td>
...
<td>
	  ...
	  $url = "spd_report?pmrn=$pp&id=$ee&ac_no=$dd"; 
...
$qe = mysqli_fetch_array($rr1);
	  ...
	  $url = "endo_report?pmrn=$pp&eid=$ee"; 
...
{ ?>    <tr>
	  ...
	  <td align="left" ><a target='_blank' href="opdbill_mng?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>"></a></td>
	  
	  <td colspan="5"><a target='_blank' href="procedure_report.php?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>"><img src="print.png" title="Print Report" width="150" height="60" /></a></td>						
      </tr>
