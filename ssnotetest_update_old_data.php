...
$sel95="SELECT COUNT(id) FROM privilege WHERE `pname`='$proname' and status in ('Approved','Waiting For CFO Approval') and dname='$full';";
...
$ins_query="insert into otreport (`pmrn`,`eid`,`pname`,`otdate`,`sname`,`sreport`,`ottype`,`date1`,`charge`,`plevel`,`md_approve`) values
( '$pmrn','$id','$proname','$otdate','$full','$sreport','$ottype','$date1','$charge','$plevel','Pending')";
mysqli_query($con,$ins_query) or die(mysql_error());
...
$update90= "update ot set dnamestatus='Written' where `id`='$id'";

mysqli_query($con,$update90);


if(mysqli_query($con,$ins_query7)==true){
echo '<script language="javascript">';
...
$ins_query="insert into otreport (`pmrn`,`eid`,`pname`,`otdate`,`sname`,`sreport`,`ottype`,`date1`,`charge`,`plevel`) 
values ( '$pmrn','$id','$proname','$otdate','$full','$sreport','$ottype','$date1','$charge1','$plevel')";
mysqli_query($con,$ins_query) or die(mysql_error());
...
<input list="browsers1" name="plevel" size=60% class="form-control" autocomplete="off" id="pmrn" onkeyup="GetDetail(this.value)">
...
<tr><td colspan="20" id="uu3">
	...
	<input type="text" id="pmrn" onkeyup="GetDetail1(this.value)" class="form-control action" list="categoryname" autocomplete="off" name='proname' placeholder='Select Procedure Name' style="color:green;font-size:22px; font-weight:bold">
...
//var variables = "pmrn=str&string=$pd";
				...
				// xhttp.open("GET", "filename", true);
				xmlhttp.open("GET", "ot_pull.php?pmrn=" + str + "&porder=<?php echo $full;?>"+ "&mrn=<?php echo $pmrn;?>", true);
