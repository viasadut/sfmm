...
try {
 ...
 $sh = $db2->prepare("UPDATE iinves SET dis_date=? WHERE pmrn=? and eid=? and rstatus=?");
  ...
  /*$sh = $db->prepare("UPDATE dis_request SET disstatus=?, bstatustime=?, buser=? WHERE pmrn=? and eid=?");
  $sh->execute([$r_s, $adate1, $user, $pmrn, $eid]);

  $sh = $db->prepare("UPDATE preadm SET dnew=? WHERE pmrn=? and eid=?");
  $sh->execute([$dnew, $pmrn, $eid]);

  $sh = $db->prepare("UPDATE inpatient SET disstatus=?, bstatustime=?,buser=?, billdate=?, damount=?, dtype=?, dnew=? WHERE id=?");
