...
$url = "promedicineused_new1.php?pmrn=$pmrn&eid=$eid&dname=$dname&id=$id";	 

$db = mysqli_connect('localhost','root','Godiloveu16');
mysqli_select_db($db,'sfmmkpjnew');
$sel_p=mysqli_query($db,"SELECT COUNT(id) FROM promediused WHERE `id`='$id';");
...
$query = "DELETE FROM promediused WHERE id='$id'"; 
...
$date=date('Y-m-d');
      $tb_q = mysqli_query($db,"select * from acct_master_new where item_code='$aa_code'");
    ...
    values ('$last_id','DR','111001','$date','$price','OPD PROCEDURE')";
    mysqli_query($con,$ins_query) or die(mysql_error());
    
    
    $ins_query2="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
    values ('$last_id','CR','615100','$date','$price','OPD PROCEDURE')";
...
header("Location:promedicineused_new1.php?pmrn=$pmrn&eid=$eid&dname=$dname&id=$id");
