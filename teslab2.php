<?php 
   session_start();
	...
	$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('lab','doctor','imo','nurse','mng')"; 
...
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from bcross1 where result='Compatible' and status='available'group by btype;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
      ...
      <td align="center"><?php echo $count; ?></td>
		  	  	  ...
		  	  	  <td align="center"><a href="teslab3?btype=<?php echo $row["btype"]; ?>"><?php echo $row["btype"];?></a></td> 
	  
<?php
$bb=$row['btype'];
$edate=date('Y-m-d');
$querymax2 = "SELECT count(btype) FROM bcross1 where btype='$bb' and result='Compatible'and status='available' and edate>='$edate'"; 
$resultmax2 = mysqli_query($con, $querymax2) or die(mysqli_error());
$rowmax2= mysqli_fetch_array($resultmax2);
$max2=$rowmax2['count(btype)'];
