...
<?php
    $ipmrn=$row['pmrn'];
    $in_p = "SELECT * FROM inpatient where pmrn= '$ipmrn' and discharge=''"; 
	 
      $in_re = mysqli_query($con, $in_p) or die(mysqli_error());
      
      // Print out result
      $in_data = mysqli_fetch_array($in_re);

echo      $in_rfid=$in_data['rfid'];
?>
...
?>
		...
		//$url = "otpatientreceive?pmrn=$pp&id=$rid"; 
          $url = "ot_rfid_scan?pmrn=$pp&id=$rid"; 
...
$date11=date_create($f2);
	...
	{ 
echo "<td align='center' style='background-color:lightblue;'><a target='_blank' href='$url'>RECEIVE</a>
<input type='button' name='".$in_rfid."' value='".$in_rfid."' id='".$row['id']."' class='btn btn-info btn-xs edit_data1'>
	...
	{ 
echo "<td align='center' style='background-color:lightgreen;'>Waiting For Finance Clearance
<input type='button' name='".$in_rfid."' value='".$in_rfid."' id='".$row['id']."' class='btn btn-info btn-xs edit_data1'>
	...
	{ 
echo "<td align='center' style='background-color:orange;'>Finance Clearance Done but OT Time Not Set
<input type='button' name='".$in_rfid."' value='".$in_rfid."' id='".$row['id']."' class='btn btn-info btn-xs edit_data1'>
</td>";
	}
...
$sel_query="Select * from ot where status='Received' and room2 ='' and room3 ='' and date5 between '$test' and '$test1'  ORDER BY date5 desc;";
...
<div id="dataModal" class="modal fade">  
 ...
 <label>Request OT Date</label>                          
                          
                         						 
						  <input type="text" name="pbp1" id="pbp1" placeholder="Select Date" size="15" required>
						  <label>Set New OT Date</label>                          
						  <input type="date" name="pbp2" id="pbp2" placeholder="Select Date" size="15" required>
						  
                          <label>Approx. OT Duration</label>  
                          
						 <input type="text" name="pbp" id="pbp" class="form-control"readonly>
      ...
      <div class="modal-dialog">  
           <div class="modal-content">  
                ...
                <div class="modal-body">  
                     ...
                     <form method="post" id="insert_form1" name="frmMain3">  
					 
                     <label>Patient Name</label>  
                          <input type="text" name="ppluse1" id="ppluse1" class="form-control" size="15" readonly style="font-weight: bold;font-size:18px;color:red;">  
                          <label>Patient MRN</label>  
                          <input type="text" name="pmrn1" id="pmrn1" class="form-control" size="15" readonly style="font-weight: bold;font-size:18px;color:red;">  
                          <label>RFID</label>  
                          <input type="text" name="srfid" id="srfid" class="form-control" size="15" required style="font-weight: bold;font-size:18px;color:green;">  
                         
                          <input type="hidden" name="pmrn11" id="pmrn11" class="form-control" size="15" >  
					...
					<br>	  
                                ...
                                &nbsp;&nbsp;						<input type="submit" name="insert" id="insert451" value="Submit" class="btn btn-success" style="font-weight: bold;font-size:18px;color:red;width:80px">  
						</strong>&nbsp;&nbsp;&nbsp;&nbsp;
						<button type="button" class="btn btn-default" name="close" id="close" data-dismiss="modal" style="font-weight: bold;font-size:18px;color:red;width:80px">Close</button>  
						</label>
...
<script>  
 $(document).ready(function(){  
  ...
  $('#add_data_Modal1').on('shown.bs.modal', function() {
    $('input[name="srfid"]').focus();
  });
});
      $(document).on('click', '.edit_data1', function(){  
          $('#add_data_Modal1').modal({backdrop: 'static', keyboard: false})  
           var employee_id1 = $(this).attr("id");  
           var rfid = $(this).attr("name");  
           $.ajax({  
                url:"ot_patient_new.php",  
                method:"POST",  
                ...
                success:function(data){  
					...
					$('#peid1').val(data.proce); 
					 $('#temp1').val(data.dname); 
                     $('#pbp1').val(data.duration2); 
                     $('#id1').val(data.id); 
					 $('#pbp11').val(data.otdate); 
					 $('#pbp21').val(pbp2); 
      ...
      $('#insert_form1').on("submit", function(event){  
           ...
           var x= Number(document.getElementById("pmrn1").value);
           var y= Number(document.getElementById("id1").value);
           if($('#pmrn1').val() == "")  
           {  
                alert("MRN is required");  
           }  
           ...
           if($('#srfid').val() != $('#pmrn11').val())  
           {  
                alert("RFID Not Matched");  
           }  
           
           else  
           {  
                /*$.ajax({  
                     url:"update_ot_date0000.php",  
                     method:"POST",  
                ...
                //window.location.href = 'otpatientreceive?pmrn="+x+"&id="+11519+"';
                //lat="+elemA+"&lon="+elemB+"&setLatLon=Set";
                window.location.href = "otpatientreceive?pmrn="+x+"&id="+y+"";

                //otpatientreceive?pmrn=372423&id=11519
