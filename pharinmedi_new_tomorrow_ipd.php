...
*/
//include("auth.php"); 
//echo $nn=date('Y-m-d',strtotime("+1 days"));
require('db1.php');
$days_ago=date('Y-m-d');
$ex_date = date('Y-m-d', strtotime('+8 days', strtotime($days_ago)));

$dd2= date('m/d/Y',strtotime("+1 days")); 
...
$runningTime = $_POST["chkDel"][$count];
			$bar_code = date('iYds');			
...
$reuse=$medi_take['reuse'];
			...
			$sel95="SELECT * FROM medicine WHERE `code`='$mcode' order by id desc;";
$result95 = mysqli_query($con,$sel95);
$b_chk=mysqli_fetch_assoc($result95);
$brand=$b_chk['brand1'];
$m_qty=$b_chk['tqty']-1;
$u_price=$b_chk['uprice'];
...
//$sel96="SELECT * FROM medi_stock WHERE `code`='$mcode' and add_qty>0 and location='Pharmacy_ipd' and `exdate`>'$ex_date' order by id asc limit 1;";
$sel96="SELECT * FROM medi_stock WHERE `code`='$mcode' and add_qty>0 and location='Pharmacy_ipd' order by id asc limit 1;";
...
$strSQL = "UPDATE imedi3 set pstatus='Served', puser='$user', pdate='$pdate',rfid='$runningTime',pno='$pno',rfid1='$tfid',p2='1'";
			$strSQL .="WHERE id = '".$iidd."' and pstatus!='served' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
	
	
	
$ins_query4="update medi_stock set `add_qty`='$m_qty1' where code='$mcode' and location='Pharmacy_ipd' and id='$medi_id'";
...
$strSQL = "UPDATE imedi3 set pstatus='Served', puser='$user', pdate='$pdate',rfid='$runningTime',pno='$pno',rfid1='$tfid',p2='1' ";
			$strSQL .="WHERE pmrn ='$pmrn' and eid ='$eid' and code ='$mcode' and odate='$dd2'  and pstatus!='served'";
			$objQuery = mysqli_query($objConnect,$strSQL);
...
<tr><td align="left" colspan="20"><a target='_blank' href="pha_medi_bar_tomorrow.php?pmrn=<?php echo "$pmrn"; ?>&eid=<?php echo "$eid"; ?>">bar_print</a></td></tr>	
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($objConnect,"sfmmkpjnew");
$strSQL = "Select * from imedi3 where pmrn= '$pmrn' and eid='$eid'and status !='Cancel' and odate='$dd2' and pstatus='Ordered' and status1 not in ('SEEN','implemented') and reuse='' order by `time` and `infusion` asc;";
$objQuery = mysqli_query($objConnect,$strSQL) or die ("Error Query [".$strSQL."]");

$strSQLa = "Select * from imedi3 where pmrn= '$pmrn' and eid='$eid'and status !='Cancel' and odate='$dd2' and pstatus='Ordered' and status1 not in ('SEEN','implemented') and reuse='Reuse' group by infusion;";
...
?>
  ...
  <td align="center" colspan="1"><?php echo $count; ?></td>
	...
	<td align="center"colspan="1"><?php echo $row["ortime"]; ?></td>
	  ...
	  <?php
	  $mcode = $row['code'];
	  //$sum = "SELECT SUM(add_qty) FROM medi_stock where code='$mcode' and location='Pharmacy_ipd' and `exdate`>'$ex_date'" ;
	  $sum = "SELECT SUM(add_qty) FROM medi_stock where code='$mcode' and location='Pharmacy_ipd'" ;
...
?>
  ...
  <td align="center" colspan="1"><?php echo $count; ?></td>
	...
	<td align="center"colspan="1"><?php echo $row1["ortime"]; ?></td>
	  ...
	  <?php
	  $mcode1 = $row1['code'];
	  $sum1 = "SELECT SUM(add_qty) FROM medi_stock where code='$mcode1' and location='Pharmacy_ipd' and `exdate`>'$ex_date'" ;
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from imedi3 where pmrn= '$pmrn' and eid='$eid'and pstatus ='served' and odate='$dd2' and status1 not in('SEEN','implemented')order by `time` and `infusion` asc;";
...
<td align="left" colspan="20"><a target='_blank' href="phar_bar_print_indu1_tomorrow.php?id=<?php echo $row['id']; ?>">bar_print</a></td>
