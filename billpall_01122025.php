...
<?php 
  ...
  if(!isset($_SESSION['sess_username']) || $role!="billin"){
    //if(!isset($_SESSION['sess_username']) || $role!="billin" || $tt !='192.168.101.251:8081'){
...
header("Location: ae_bill_paper.php?adate1=$adate1&pmrn=$pmrn&dname=$dname&billno=$last_id&eid=$eid");  
...
$count=1;
$sel_query="Select * from estat where pmrn= '$pmrn' and eid='$eid' and status='Rupdated' order by `id` DESC;";
...
$medicine = "SELECT SUM(uprice) FROM estat where pmrn='$pmrn' and eid='$eid' and status='Rupdated'"; 
...
$count=1;
$sel_query="Select * from einves where pmrn= '$pmrn' and eid='$eid' and status in ('RECEIVED','SEEN') order by `id` DESC;";
...
$inves = "SELECT SUM(price) FROM einves where pmrn='$pmrn' and eid='$eid' and status in ('RECEIVED','SEEN')"; 
...
$queryae_extra = "SELECT SUM(price) FROM ae_extra_charge where pmrn='$pmrn'and eid='$eid' "; 
...
$total_dis=$data['hos_discount']+$data['hos1_dis']+$data['lab_dis']+$data['rad_dis'];
