...
$queryc = "SELECT COUNT(utype) FROM user where '$role' in ('pharmacy02')"; 
...
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from medi_stock where add_qty>0 and location='2nd Fl Pharmacy'  and status='Served' ORDER BY g_name asc;";
...
$queryqq = "SELECT SUM(qty) from phar_sale where rfid='".$rrid."' and reuse='' and status='Finished' and adate between '$sd' and '$ed'"; 
$resultqq = mysqli_query($con, $queryqq) or die ( mysqli_error());
$rowqq = mysqli_fetch_assoc($resultqq);
...
<td align="center"style="font-size:18px;font-weight:bold;color:green;"><?php echo $row["rfid"]; ?></td>
