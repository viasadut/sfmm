<?php 
    ...
    if(!isset($_SESSION['sess_username']) || $role!="cafe"){
...
$ins_query="insert into iidiet (`pmrn`,`eid`,`pname`,`padd`,`page`,`padmission`,`pgender`,`pphone`,`odate`,`infusion`,`user`,`dtime`,`status`) values ('$pmrn','$eid','$pname','$padd','$page','$adm','$psex','$pphone','$odate','$infu','$user','$remarks','Served')";
...
$dd=date('Y-m-d');
$count=1;
$sel_query="Select * from iidiet where pmrn= '$pmrn' and eid='$eid'and status1 !='Cancel' and status='Diet Ordered'and odate='$dd'order by `rtime` and `infusion` asc;";
...
<td colspan="2" align="center"><a onclick="return confirm_click();" href="dietconfirm?pmrn=<?php echo "$row[pmrn]"; ?>&id=<?php echo "$row[id]"; ?>&eid=<?php echo "$row[eid]"; ?>&user=<?php echo "$user"; ?>"><strong>Served</strong></a></td>	  
