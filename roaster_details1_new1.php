...
$url = "roaster_details1_new1?id=$id&id1=$id1";
            ...
            $ins_query="Update roaster_2 set a_status='Pending',a_send='$fullname',a_send_time='$time' where location='$c_location' and date between '$id' and '$id3'";
...
if($r_cor=='yes')
{
$sel_query="Select * from staff3 where status ='Active' and c_location='$c_location' and c_location!='' order by sname asc";
...
while($row = mysqli_fetch_assoc($rown)) { ?>
      ...
      <?php 
	  $ss_id=$row['sid'];
	  $query333 = "SELECT COUNT(id) FROM roaster_2 where date between '$id' and '$id3' and mor='$ss_id'"; 
	 
$result333 = mysqli_query($con, $query333) or die(mysqli_error());

// Print out result
$row777 = mysqli_fetch_array($result333);
$rcount=$row777['COUNT(id)'];
	  ...
	  if($rcount>'0'){echo'
	   ...
	   {
		echo'
	  <td align="center"><a href="roaster_details1_new?sid='.$row['sid'].'&id='.$id.'&id1='.$id1.'">'.$row["sname"].'</a></td>
	 ';  
...
';
...
else if($n2==0 and $a_status=='' and $edit=='')
	...
	else if($n2==0 and $estatus=='' and $edit=='' and $a_status=='Pending')
	...
	
	{
		...
		echo '<input type="button" name="'.$dd.'"  value="A" id="'.$row['sid'].'" class="btn btn-info btn-xs edit_data11">
	...
	
	{
		...
		<s style="color:red">'.$row1['emor'].'-'.$row1['location'].'</s><br>
		<input type="button" name="'.$dd.'"  value="A" id="'.$row['sid'].'" class="btn btn-info btn-xs edit_data11">
...
else if($n2==0 and $estatus!='A' and $edit=='' and $a_status!='Pending')
...
<?php $count++;}?>
	...
	<td>
<?php if($a_status=='Pending' and $edit==''){echo'
<input type="submit" name="s_approval" id="s_approval" value="Send For Approval" class="btn btn-success" />';}


else if($a_status=='' and $edit==''){echo'
<input type="submit" name="s_approval" id="s_approval" value="Not Prepared Yet" class="btn btn-success" disabled/>';}


else if($a_status=='approved' and $edit=='') {echo '<input type="submit" name="s_approval" id="s_approval" value="Roster Already Approved By Consultant" class="btn btn-success" disabled/>';}
      ...
      <div class="modal-dialog">  
		  ...
		  <label>Duty Shift</label>  
			...
			<option value='Delete'>Delete</option>			
                          ...
                          <input type="hidden" name="a_status1" id="a_status1" />  
...
<script>  
 ...
 <div id="add_data_Modal11" class="modal fade">  
      <div class="modal-dialog">  
           <div class="modal-content">  
                <div class="modal-header">  
                     <button type="button" class="close" data-dismiss="modal">&times;</button>  
                     <h4 class="modal-title"align='center'>Update Roster Duty9</h4>  
                </div>  
                <div class="modal-body">  
                     <form method="post" id="insert_form22" name="frmMain222">  
                          <label>Staff ID</label>  
                          <input type="text" name="pmrn11" id="pmrn11" class="form-control" size="15" readonly>  
						   
						   
						   <label>Date</label>  
						  
                          <input type="text" class="form-control" name="date1" id="date1" readonly></td>
						  
						  
		  		   <label>Duty Location</label>  
						  <select type="text" name="pbp111" id="pbp111" class="form-control" required>
		  ...
		  <label>Duty Shift</label>  
						  <select type="text" name="pbp311" id="pbp311" class="form-control" required>
			...
			?>
                          ...
                          <input type="hidden" name="employee_id22" id="employee_id22" />  
						  
						       
							   <input type="submit" name="insert" id="insert44" value="Insert" class="btn btn-success" />  
 ...
 $(document).ready(function(){  
      $('#add').click(function(){  
           $('#insert').val("Insert");  
           $('#insert_form22')[0].reset();  
      });  
      $(document).on('click', '.edit_data11', function(){  
           var employee_id22 = $(this).attr("id");  
		  var employee_id33 = $(this).attr("name");  
		   
		   
           $.ajax({  
                url:"roaster_2_21_e.php",  
                method:"POST",  
                data:{employee_id22:employee_id22},  
				
                dataType:"json",  
                success:function(data){  
                     $('#pmrn11').val(data.sid);  
                     
					 $('#pbp111').val(data.c_location); 
					 $('#pbp311').val(data.emor); 
					 $('#date1').val(employee_id33); 
					 
					 
					  
                     
					 
                     $('#employee_id22').val(data.id);  
                     $('#insert44').val("Add");  
                     $('#add_data_Modal11').modal('show');  
                }  
      ...
      $('#insert_form22').on("submit", function(event){  
           event.preventDefault();  
           if($('#pmrn11').val() == "")  
           {  
                alert("MRN is required");  
           ...
           {  
                $.ajax({  
                     url:"roaster3_31_e.php",  
                     method:"POST",  
                     data:$('#insert_form22').serialize(),  
                     beforeSend:function(){  
                          $('#insert').val("Inserting");  
                     },  
                     success:function(data){  
                          $('#insert_form22')[0].reset();  
                          $('#add_data_Modal11').modal('hide');  
                          $('#employee_table').html(data);  
