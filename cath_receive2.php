...
$sel_query="Select * from cath_receive where pdate= '$date' and ustatus='' order by ll asc ;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	  ...
	  <td align="Left"><?php echo $row["proname"]; ?>  
	       ...
	       <td align="center" style="font:Verdana, Arial, Helvetica, sans-serif large" style="font-weight:bold"><a href="cath_receive_update?pmrn=<?php echo $row["pmrn"]; ?>&id=<?php echo $row["id"];?>">UPDATE</a> </td>
		   <td align="center"><a onclick="return confirm_click();" href="cathcancel?id=<?php echo $row["id"]; ?>&cby=<?php echo $fullname; ?>"> Cancel</a></td>
...
$sel_query="Select * from cath_receive where date1 between '$test' and '$date' and ustatus='Updated' and type='OPD' order by ll asc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
	  ...
	  <td align="Left"><?php echo $row["proname"]; ?>  
	       ...
	       <td align="left" ><a target='_blank' href="cathhoscharge?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>&type=<?php echo $row["type"];?>&ieid=<?php echo $row["ieid"];?>">Add Hospital Charges</a></td>
		   <td align="left" ><a target='_blank' href="cath_medi_use?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>&type=<?php echo $row["type"];?>&ieid=<?php echo $row["ieid"];?>">Add Used Medication</a></td>
       <td align="left" ><a target='_blank' href="spd_final_report_cath?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>&type=<?php echo $row["type"];?>&ieid=<?php echo $row["ieid"];?>">Report</a></td>
		   		   <td colspan="5"><a target='_blank' href="proused.php?pmrn=<?php echo $row["pmrn"]; ?>&dname=<?php echo $row["dname"]; ?>&eid=<?php echo $row["eid"]; ?>&id=<?php echo $row["id"]; ?>"><img src="print.png" title="Print Report" width="50" height="30" /></a></td>						
...
$data59 = mysqli_fetch_assoc($query4);
  ...
  $noti=$data59['charge_confirm'];
              
              
              if($noti=='')
{echo	'<td align="left" colspan="20"><font size="4.5" color="#FF000"><b><a onclick="return confirm_click();" href="spd_charge_confirm?pmrn='.$pmrn.'&id='.$id.'">Confirm Charges</a></td>
';}
else {
	
	echo '<td align="left" colspan="20"><font size="4.5" color="#FF000"><b>Charge Confirmed</td>';}
?>
