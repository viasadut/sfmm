...
$mcode = $row1['code'];
	  $sum = "SELECT SUM(add_qty) FROM medi_stock where code='$code_medi' and location='Pharmacy'" ;
...
<?php if($new_qty>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row['qty']*$row1['uprice'];?>
	 ...
	 <?php
	                ...
	                $queryg = "select COUNT(id) from medi_stock2 where sno='$pno' and code='$code_medi'";
                    $resultg = mysqli_query($con,$queryg);
					$rowg = mysqli_fetch_array($resultg);
     if($rowg['COUNT(id)']>0){
	 echo'
	 
	 <a target="_blank" href="department_bar_transfer_new_bill?sno='.$pno.'"><img src="phar_pic/barcode.png" title="Print Instruction" width="20" height="20" /></a>';}
	 ?>
	 </td>						
  ...
  $('#add_data_Modal').on('shown.bs.modal', function() {
    $('input[name="code"]').focus();
	  ...
	  $('#add_data_Modal').modal({backdrop: 'static', keyboard: false})  
           ...
           $.ajax({  
                url:"medi_dispense_phar_opd_new.php",  
                method:"POST",  
...
$('#lrfid').val(data.sno);					 
	  ...
	  //var tr = $('#sqty').val() + $('#gqty').val();
           ...
           {  
                $.ajax({  
                     url:"new_phar_dispense3_opd_new.php",  
                     method:"POST",  
