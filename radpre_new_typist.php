...
try {
  ...
  $sh = $db->prepare("insert into his_report(Accession_Number,Report_Data,Report_Date_Time,Status) VALUES(?, ?, ?, ?)");
  $sh->execute([$a_no, $plaintext, $his_rd, $qqt]);
  ...
  if($_REQUEST['critical']=='critical'){
  $sh = $db->prepare("insert into radreport (dname,pmrn,pname,age,gender,pphone,dreffer,report,report1,type,eid,status,rdate,r1date,find,rdone,date2,type1,time,ac_no,status1,ineid,emerid,price,done_date,code,critical,updated_by,up_time) 
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  $sh->execute([$select, $pm, $pn, $pa, $ps, $pp, $dreffer, $cdetails, $plaintext, $pd, $count1, $ss, $date, $date1, $find1, $user, $date2, $dname1, $stime, $a_no, $up, $ineid, $emerid, $price, $date_d, $price_code, $critical, $user, $up_time]);
  }
  else {
    
      $sh = $db->prepare("insert into radreport (dname,pmrn,pname,age,gender,pphone,dreffer,report,report1,type,eid,status,rdate,r1date,find,rdone,date2,type1,time,ac_no,status1,ineid,emerid,price,done_date,code,updated_by,up_time) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      $sh->execute([$select, $pm, $pn, $pa, $ps, $pp, $dreffer, $cdetails, $plaintext, $pd, $count1, $ss, $date, $date1, $find1, $user, $date2, $dname1, $stime, $a_no, $up, $ineid, $emerid, $price, $date_d, $price_code, $user, $up_time]);
...
<td colspan="20"><select name="select" required>
	  ...
	  <?php 
			$sql_1 = "select * from `doctor` where status='Active' and tt='radio' and type='res'";
