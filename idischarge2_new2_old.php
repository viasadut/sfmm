<?php 
	...
	if(isset($_POST['Submit'])){
		...
		try {
			...
			$dbh->query("UPDATE `inpatient` SET `disstatus`='Discharge Bill Confirmed', `bstatustime`='$adate1', `buser`='$user', `billdate`='$date1', `damount`='$damount', `dtype`='$dtype',`dnew`='$dnew' WHERE `id`='$id'");
			$dbh->query("UPDATE `preadm` SET `dnew`='$dnew' WHERE `pmrn`='$pmrn' AND `eid`='$eid'");
			$dbh->query("UPDATE `dis_request` SET `disstatus`='Discharge Bill Confirmed', `bstatustime`='$adate1', `buser`='$user' WHERE `pmrn`='$pmrn' and `eid`='$eid'");
			$dbh->commit();
			echo '<script language="javascript">';
			echo 'alert("Success !!Patient Discharge Success "); ';
			echo '</script>';
		} catch (\Throwable $e) {
			$dbh->rollback();
			throw $e;
			echo '<script language="javascript">';
			echo 'alert("NOT Success !!Patient Discharge NOT Success. '.$e.' "); ';
			echo '</script>';
		}
	}
?>
...
<body>
	...
	<form action="" method="post">
		<h1 align="center"style="background-color:lightgreen;">INPATIENT DISCHARGE</h1>
        <table align="center" class="table table-bordered" id="dynamic_field">  
			...
			<tr>
				<td colspan="20" align="center"bgcolor="lightgreen"><label><strong>Discharge Request</strong></label></td>
			</tr>
