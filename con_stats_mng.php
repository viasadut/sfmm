...
$query39 = "SELECT * FROM user where fullname= '$bt'"; 
	 
$result39 = mysqli_query($con, $query39) or die(mysqli_error());

// Print out result
$row39 = mysqli_fetch_array($result39);
$fullname = $row39['uname'];
...
$query48l1 = "SELECT COUNT(pmrn) FROM iinves where ndate between '$date77' and '$date77e' and conby='$fullname' and type='lab';"; 
...
$query43i = "SELECT SUM(charge) FROM icnote where daten between '$date77' and '$date77e' and user='$bt' and ugroup='Doctor';"; 
	 
$result43i = mysqli_query($con, $query43i) or die(mysqli_error());
$row43i = mysqli_fetch_assoc($result43i);

$query43o = "SELECT SUM(charge) FROM otreport where date1 between '$date77' and '$date77e' and sname='$bt';"; 
...
$query48loc = "SELECT COUNT(id) FROM alltest where date1 between '$date77' and '$date77e' and dname='$full';"; 
	 
$result48loc = mysqli_query($con, $query48loc) or die(mysqli_error());
$row48loc = mysqli_fetch_assoc($result48loc);


$query43eoc = "SELECT SUM(price) FROM alltest where date1 between '$date77' and '$date77e' and dname='$full';"; 
	 
$result43eoc = mysqli_query($con, $query43eoc) or die(mysqli_error());
$row43eoc = mysqli_fetch_assoc($result43eoc);
...
echo "<br>";
echo "Investigation Prescribed-  ";	 
echo $row48eoc['COUNT(id)'];

echo "<br>";
echo "Investigation Prescribed-  ";	 
echo $row43eoc['COUNT(price)'];


echo "<br>";
echo "Grand Total-  ";	 
echo $row43p['SUM(payment)'] + $row43i['SUM(charge)'] + $row43o['SUM(charge)'] + $row43pp['SUM(procharge)'] + $row43e['SUM(room)'] + $row43ecg['COUNT(ron)']*100 + $row43ecg1['COUNT(ron)']*1000+ $row43ecg2['COUNT(ron)']*1500+ $row43ecg3['COUNT(ron)'];
echo " BDT";	 
