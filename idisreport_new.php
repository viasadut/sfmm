...
$pdf->Cell('60',5,strtoupper($data['pname']),1,0,'L');
...
$sel_query=mysqli_query($db,"Select * from allcomor where pmrn= '$pmrn' and status!='Cancel'");

		$pdf->SetFont('Arial' , 'b' , 12);
$pdf->Cell('182',5,'Comorbidities:',0,1,'L');
...
$pdf->MultiCell('182' , 5,$row['medi'].' '.$row['ins'],0,1);
	
}

if($res91=mysqli_num_rows($sel_query)==0)
	
	{
		$pdf->Cell('182',5,'NONE',0,1,'L');
	//	$pdf->ln(3);
...
$sel_query=mysqli_query($db,"Select * from allergy where pmrn= '$pmrn' and status!='Cancel'");

	

		$pdf->SetFont('Arial' , 'b' , 12);
$pdf->Cell('182',5,'Allergies:',0,1,'L');
...
$pdf->ln(3);
$query1 = mysqli_query($db,"select * from idismedi where pmrn='$pmrn'and eid='$eid' and status in('Active','Served') ORDER BY page_order");
$count=1;
...
$pdf->Cell('7' , 5,$count.'.',0,0);
$pdf->MultiCell('182' , 5,$data1['brand'].' ('.$data1['medi'].')',0,1);
$pdf->SetFont('Arial' , '' , 12);
$pdf->Cell('7' , 5,'',0,0);
$pdf->MultiCell('182' , 5,$data1['pdos'].','.$data1['duration'].','.$data1['frelation'],0,1);
	$pdf->ln(1);
