...
$query = mysqli_query($db,"select * from mcertificate where pmrn='$pmrn' and eid='$eid' and ct='unfit'");
...
$this->ln(2);
$this->SetFont('Arial','B',8);
$this->Cell(0,10,'Contact Numbers:  Ambulance:  +880244077029, +8801791987466, Appointments: +880244077030, +8801703788561 (SFMMKPJSH/OPD/MR-03)',0,0,'C');
...
$pdf = new myPDF('P','mm',array(100,150));
//$pdf->AliasNbPages();
...
$pdf->Cell('253',10,'MEDICAL CERTIFICATE',1,1,'C');
...
$pdf->Cell('48',5,$data['idate1'],0,0,'L');
...
$pdf->ln(8);
$pdf->Cell('253',5,strtoupper($data['m2'].' ' .'IS UNFIT FOR THE PROPER PERFORMANCE OF'.' '. $data['m3'].' ' .$data['ffor'].' '.'FOR'. ' '.$data['tdays'].' '.'DAYS'),0,1,'C');
$pdf->ln(8);
$pdf->Cell('253',5,strtoupper('ON / FROM -'.' ' .$data['fdate1'].' '.'TO'. ' '.$data['tdate1']),0,1,'C');

$pdf->ln(6);
$pdf->Cell('253',12,strtoupper($data['m2'].' '.'SHOULD RETURN FOR RE-EXAMINATION ON'.'  '.$data['rdate1']),0,1,'C');
