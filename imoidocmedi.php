...
$sel_re="SELECT * FROM medicine WHERE `mname`='$infu' and status='Active' and pre IN('Tablet','Vaginal Suppository','VT','Suppository','Soft Capsule','Sachet','Capsule','Injection','Infusion','Laxative Saline','Nebuliser Suspension','Inhalation  Solution','Rectal Solution','Solution','')";
$result_re = mysqli_query($con,$sel_re);

$sel_dis="SELECT COUNT(id) FROM imedi3 WHERE `pmrn`='$pmrn' and `eid`='$eid' and`infusion`='$infu' and reuse='Reuse' and discard !='discarded' and pstatus='Served' and status1!='Returned';";
$result_dis = mysqli_query($con,$sel_dis);
$row_dis = mysqli_fetch_assoc($result_dis);
$dis_count=$row_dis['COUNT(id)'];

/*$sel_dis1="SELECT COUNT(id) FROM imedi3 WHERE `pmrn`='$pmrn' and `eid`='$eid' and`infusion`='$infu' and reuse='Reuse' and discard !='discarded' and status!='Cancel' and pstatus='Served';";
$result_dis = mysqli_query($con,$sel_dis1);
$row_dis1 = mysqli_fetch_assoc($result_dis1);
$dis_count1=$row_dis1['COUNT(id)'];
*/
...
if($res990=mysqli_num_rows($result990)==0)
{
    ...
    echo '</script>';
        ...
        { 
			...
			//{	
echo '<script language="javascript">';
    echo 'alert("One Medication is already active.. pls discard the medicine to place a new order!!"); ';
...
else if($res_re=mysqli_num_rows($result_re)<=0 and $hh=='')
        { 
            // Retrieving each selected option 
            //foreach ($_POST['infu1'] as $infu1)  
            //print "You selected $subject<br/>"; 
...
$ins_query6="insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`ndate`,`uprice`,`code`,`reuse`,`discard`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$infu','$infu1','$ddate','$alert','$user','$datem1','Active','$root','Rupdated','Ordered','$ortime','$datem','$uprice','$p_code','Reuse','New','$pre','$stest','$ip','$op','$acct_code','$ccentre')";
...
else if($res_re=mysqli_num_rows($result_re)<=0 and $hh=='Single')
        { 
            // Retrieving each selected option 
            //foreach ($_POST['infu1'] as $infu1)  
            //print "You selected $subject<br/>"; 
...
$ins_query6="insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`ndate`,`uprice`,`code`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$infu','$infu1','$ddate','$alert','$user','$datem1','Active','$root','Rupdated','Ordered','$ortime','$datem','$uprice','$p_code','$pre','$stest','$ip','$op','$acct_code','$ccentre')";
mysqli_query($con,$ins_query6);
...
else if($res_re=mysqli_num_rows($result_re)>0 and $hh!='Reuse')
        { 
            // Retrieving each selected option 
            //foreach ($_POST['infu1'] as $infu1)  
            //print "You selected $subject<br/>"; 
...
$sel_re="SELECT * FROM medicine WHERE `mname`='$ii' and status='Active' and pre IN('Tablet','Vaginal Suppository','VT','Suppository','Soft Capsule','Sachet','Capsule','Injection','Infusion','Laxative Saline','Nebuliser Suspension','Inhalation  Solution','Rectal Solution','Solution','');";
$result_re = mysqli_query($con,$sel_re);

$sel_dis="SELECT COUNT(id) FROM imedi3 WHERE `pmrn`='$pmrn' and `eid`='$eid' and`infusion`='$ii' and reuse='Reuse' and discard !='discarded';";
$result_dis = mysqli_query($con,$sel_dis);
$row_dis = mysqli_fetch_assoc($result_dis);
$dis_count=$row_dis['COUNT(id)'];
...
$ins_query6="insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`ndate`,`uprice`,`code`,`reuse`,`discard`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$ii','$infu1','$ddate','$alert','$user','$datem1','Active','$root','Rupdated','Ordered','$ortime','$datem','$uprice2','$p_code','Reuse','New','$pre','$stest','$ip','$op','$acct_code','$ccentre')";
mysqli_query($con,$ins_query6);
...
$ins_query6="insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`ndate`,`uprice`,`code`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$ii','$infu1','$ddate','$alert','$user','$datem1','Active','$root','Rupdated','Ordered','$ortime','$datem','$uprice2','$p_code','$pre','$stest','$ip','$op','$acct_code','$ccentre','$ip','$op','$acct_code','$ccentre')";
mysqli_query($con,$ins_query6);
...
$ccentre=$result1["ccentre"];
			...
			$sel95 = "SELECT * from medicine where mname='$infu' and pre in('Tablet','Vaginal Suppository','VT','Suppository','Soft Capsule','Sachet','Capsule','Injection','Infusion','Laxative Saline','Nebuliser Suspension','Inhalation  Solution','Rectal Solution','Solution','') and status='Active'"; 
...
echo '<script language="javascript">';
    ...
    echo '</script>';
			...
			{
			$strSQL = "insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`uprice`,`code`,`ndate`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$infu','$time','$instruc','$alert','$user','$odate','Active','$root','Rupdated','Ordered','$ortime','$uprice1','$p_code','$ndate3','$pre','$stest','$ip','$op','$acct_code','$ccentre')";
			...
			else if($row95=mysqli_num_rows($result95)<=0 and $discard!='Discarded')
			
			
			{
			$strSQL = "insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`reuse`,`uprice`,`code`,`ndate`,`status_select`,`rfid`,`rfid1`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$infu','$time','$instruc','$alert','$user','$odate','Active','$root','Rupdated','Served','$ortime','Reuse','0','$p_code','$ndate3','selected','$rf','$rf1','$pre','$stest','$ip','$op','$acct_code','$ccentre')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
			...
			{
			$strSQL = "insert into imedi3 (`pmrn`,`dname`,`eid`,`infusion`,`time`,`instruc`,`alert`,`orderby`,`odate`,`status`,`root`,`status1`,`pstatus`,`ortime`,`reuse`,`uprice`,`code`,`ndate`,`discard`,`pre`,`stest`,`ip`,`op`,`acct_code`,`ccentre`) values 
('$pmrn','$dname','$eid','$infu','$time','$instruc','$alert','$user','$odate','Active','$root','Rupdated','Ordered','$ortime','Reuse','0','$p_code','$ndate3','New','$pre','$stest','$ip','$op','$acct_code','$ccentre')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery = mysqli_query($objConnect,$strSQL);
...
<td colspan="14" align="center"><input list="rr" name="infu" class="form-control" autocomplete="off" required>
  <datalist id="rr">
			...
			?>
				...
				<?php 
			$sql76 = "select * from `medicine` where status='Active' and pcat !='EPI Vaccine'";
			$res76 = mysqli_query($con, $sql76);
			if(mysqli_num_rows($res76) > 0) {
				while($row76 = mysqli_fetch_object($res76)) {
					echo "<option value='".$row76->mname."'>".$row76->mname.'('.$row76->pre.')'."</option>";
				}
			}
			?>  </datalist></td>
</tr>
...
<td colspan="15" align="center"><label><strong>Instruction</strong></label></td>

<td colspan="3" align="center"><label><strong>Change Medicine Type(if Require)</strong></label></td>
</tr>
<tr>
<td colspan="3" align="center"><input list="rr10" name="root" class="form-control" required>
  <datalist id="rr10">
			  ...
			  <td colspan="15" align="center"><textarea name="ddate"  value="" /></textarea></td>

		 <td colspan="3">
			  <select list="rr1" name="hh" id=""  class="form-control">
...
$strSQL = "Select * from imedi3 where pmrn= '$pmrn' and eid='$eid'and status !='Cancel' and odate='$dd' and status1 in ('RUPDATED','Returned') order by `time` and `infusion` asc;";
...
<tr>
	  ...
	  <?php
if($row['retrive_by']=='') {echo 

	'<td align="center"colspan="4"> <a href="addpharmediimodetails?pmrn='.$pmrn.'&eid='.$eid.'&infu='.$row['infusion'].'">'.$row["infusion"].'</a>
	  
	  
	  <input type="button" name="edit_co" value="E" id="'.$row['id'].'" class="btn btn-info btn-xs edit_data_co" />
	...
	'<td align="center"colspan="4" style="background-color:pink"> <a href="addpharmediimodetails?pmrn='.$pmrn.'&eid='.$eid.'&infu='.$row['infusion'].'">'.$row["infusion"].'</a>
...
$objConnect = mysqli_connect("localhost","root","Godiloveu16") or die("Error Connect to Database");
$objDB = mysqli_select_db($con,"sfmmkpjnew");
$strSQL = "Select * from imedi3 where pmrn= '$pmrn' and eid='$eid'and status !='Cancel' and odate='$dd' and status1 IN('SEEN','implemented')order by `time` and `infusion` asc;";
$objQuery = mysqli_query($con,$strSQL) or die ("Error Query [".$strSQL."]");
//$sel_query="Select * from pmedi where pmrn= '$pmrn' and eid='$eid1'order by `id` DESC;";
						...
						<option value='23:00'>23:00</option>
						  ...
						  <input type="hidden" name="alert" id="alert" /> 
						   <input type="hidden" name="pre" id="pre" /> 
                                 <input type="hidden" name="stest" id="stest" /> 
                ...
                <div class="modal-body">  
                     <form method="post" name="insert_form7" id="insert_form7">  
                         <label>Patient MRN</label>  
                          ...
                          <label>Dilution</label>  
                          <input type="text" name="result1" id="result1" class="form-control" readonly>  
