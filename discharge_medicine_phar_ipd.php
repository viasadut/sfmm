...
$sale_date=date('Y-m-d');
  ...
  $date=date('Y-m-d');
	...
	values ('$last_id','CR','111001','$date','$gtotal','DIS PHARMACY')";
	mysqli_query($con,$ins_query) or die(mysql_error());

	
$ins_query7="insert into pms_tb (`trans_id`,`trans_type`,`acct_code`,`date`,`amount`,`location`) 
values ('$last_id','DR','111999','$date','$gtotal','DIS PHARMACY')";
...
$sel96="SELECT * FROM medi_stock WHERE `code`='$code_1' and add_qty>0 and location='Pharmacy_ipd' order by exdate asc limit 1;";
...
if($eqty2>0)
	
	{
			...
			$strSQL1 = "update idismedi set status='Served',qty='$eqty2',billno='$billno' where id='".$updateid."'";
            ...
            $strSQL2 = "insert into phar_sale(`medi`,`qty`,`uprice`,`tprice`,`aby`,`adate`,`sno`,`brand`,`ins`,`location`,`code`,`pmrn`,`pname`,`rfid`,`eid`,`billno`) values
			('$medi_1','$eqty2','$p_price','$u_price','$user','$adate','$sno','$brand','$ins','Discharge','$code_1','$p_mrn','$p_name','$rf','$eid','$billno')";
...
$qq = mysqli_query($db,"select SUM(tprice) from phar_sale where pmrn='$pmrn' and eid='$eid' and location='OPD' and billno='$billno'");
			$dd = mysqli_fetch_assoc($qq);
...
//$conn->query($sql1);
//header("Location: phar_update_bill3_new_07.php?pmrn=$pmrn&dname=$dname&eid=$eid&sno=$sno");
...
<select hidden name="discount_type" value="" class="style1" id="pmrn" onchange="GetDetail(this.value)" width="20px;">
 ...
 <script>
  $("input").on("change", function() {
   // var ret = parseInt($("#field1").val()) - parseInt($("#field2").val())
	...
	<tr >
<td colspan="10" ><input hidden type="radio" id="vehicle1" name="vehicle1" value="Cash"  id="chkPassport"onclick="EnableDisableTextBox(this)"  style="height:20px; width:20px; color:red;"checked>
<input hidden type="radio" id="vehicle1" name="vehicle1" value="bkash"id="chkPassport1" onclick="EnableDisableTextBox1(this)" style="height:20px; width:20px; color:red;">
<input hidden type="radio" id="vehicle1" name="vehicle1" value="Card"id="chkPassport2" onclick="EnableDisableTextBox2(this)" style="height:20px; width:20px; color:red;">
