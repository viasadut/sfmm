...
//$id =$_GET['id'];
$count=1;
$sel_query="Select * from storenew where estatus='Active' ORDER BY id desc;";
...
while($row = mysqli_fetch_assoc($result)) { ?>
    <tr>
      ...
      <td align="center"><a target='_blank' href="all_product_list5_purchase?id=<?php echo $row['id'];?>"><?php echo $row["ename"]; ?></a></td>
...
?>
               ...
               <select class="selectpicker" multiple data-live-search="true" name="test[]">
               <?php
	$stmt = $DB_con->prepare("select * from `medicine` where status='Active'");
	$stmt->execute();
	while($row=$stmt->fetch(PDO::FETCH_ASSOC))
