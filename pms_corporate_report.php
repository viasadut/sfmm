...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><h1 style="color:red"><strong>Corporate Activity Report</strong></h1></label></td> </tr>
...
<td colspan="2"><label><strong>Select Patient MRN:</strong></label></td>
                     ...
                     <select class="js-example-basic-single" name="stdate" required>

						<option value=''>-Select Corporate-</option>
				<?php 
			$sql76 = "select * from `corporate`";
			$res76 = mysqli_query($con, $sql76);
			
			while($row76 = mysqli_fetch_object($res76)) {
                echo "<option value='".$row76->code."'>".$row76->c_name."</option>";
            }
...
<tr>
      ...
      <td align="center"><strong>Total Bill Amount</strong></td>
     	  ...
     	  <td align="center"><strong>Outstanding Amount</strong></td>
...
';
                    ...
                    $query = "Select * from pms_bill where cor_name='$pmrn' and amount>(amount_receive+dis_amount) and dname !='IPD ADVANCE' and amount!='0' order by date desc";
   ...
   <tr>
     <td align="center" ><?php echo $count; ?></td>
      ...
      <td align="center" ><a target='_blank' href="report_tb_details?id=<?php echo $data22["acct_name"]; ?>&start=<?php echo $start; ?>&end=<?php echo $end; ?>"><?php echo $row["amount"]; ?></a></td>
      <td align="center" ><?php echo $row['amount']- ($row['amount_receive']+$row['dis_amount']); ?></td>
...
<?php $count++;}
  ...
  $querydr = "select SUM(amount) FROM pms_bill WHERE cor_name='$pmrn' and amount>(amount_receive+dis_amount) and dname !='IPD ADVANCE' and amount!='0' order by date desc"; 
...
$bill=$data['SUM(amount)'];
 ...
 $querycr = "select SUM(amount_receive) FROM pms_bill WHERE cor_name='$pmrn' and amount>(amount_receive+dis_amount) and dname !='IPD ADVANCE' and amount!='0' order by date desc"; 
 $resultcr = mysqli_query($con,$querycr) or die ( mysqli_error());
 $data1 = mysqli_fetch_assoc($resultcr);	

$receive=$data1['SUM(amount_receive)'];

$querycr1 = "select SUM(dis_amount) FROM pms_bill WHERE cor_name='$pmrn' and amount>(amount_receive+dis_amount) and dname !='IPD ADVANCE' and amount!='0' order by date desc"; 
 $resultcr1 = mysqli_query($con,$querycr1) or die ( mysqli_error());
 $data2 = mysqli_fetch_assoc($resultcr1);	

$discount=$data2['SUM(dis_amount)'];

$outstanding=$bill-$receive-$discount;
  ...
  <td align="center" style="font-size:22px;font-weight:bold;color:red">'.$outstanding.'</td>
