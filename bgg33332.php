...
else if($res=mysqli_num_rows($result99)>0)
{
	...
	$update33="update pappnew set `aslot`='$slot', `bill`='$bill',`billby`='$user',`pmrn`='$pmrn', `pname`='$name', `padd`='$padd',`billtime`='$billtime',`payment`='$payment' where `ID`='$id'";
...
<input name="payment" type="text" size="65" style="text-transform:uppercase; color:red" value="<?php if($diff1_b<=3 and $data['pmrn']!=0){echo "0";} else if($diff1_b >3 and $diff1_b<=10 and $data['pmrn']!=0){echo $v2;}else if($diff1_b>10 and $data['pmrn']!=0){echo $v1;}else if($data['pmrn']==0){echo '600';}?>" readonly>
