...
$u_price=$eqty2*$p_price;
	...
	$bn=$chk_row['brand'];
		...
		$qq_update = mysqli_query($db,"select * from medi_stock where code='".$code_1."' and location='Pharmacy' and add_qty!='0' order by id asc limit 1");
		$dd_update = mysqli_fetch_assoc($qq_update);
		$p_m_qty=$dd_update['add_qty']-$eqty2;
		$p_m_id=$dd_update['id'];
		$exdate=$dd_update['exdate'];
		$batchno=$dd_update['batch_no'];
		$uprice=$dd_update['u_price'];
		$tprice=$dd_update['u_price']*$eqty2;
...
$add_time=date('Y-m-d H:i:s');		
			...
			else if($eqty5 == 0 and $mqty=='')
	{
			...
			$strSQL1 = "update phar_sale set status='Served',qty='$eqty2',sno='$bar_code' where id='".$updateid."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery1 = mysqli_query($objConnect,$strSQL1);

			
			$strSQL_p = "update medi_stock set add_qty='$p_m_qty' where id='".$p_m_id."'";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
			$objQuery_p = mysqli_query($objConnect,$strSQL_p);
			
			
			$strSQL22 = "insert into medi_stock(`code`,`g_name`,`location`,`b_name`,`add_qty`,`exdate`,`batch_no`,`rfid`,`u_price`,`t_price`,`add_by`,`add_time`,`sno`) values
			('$code_1','$medi_1`','$lc','$bn','$eqty2','$exdate','$batchno','$runningTime','$uprice','$tprice','$user','$add_time','$bar_code')";
			//$strSQL .="WHERE id = '".$_POST["chkDel"][$i]."' ";
...
/*echo '<script language="javascript">';
    ...
    echo '</script>';
	*/
	$url = "phar_transfer_barcode?sno=$sno" ;
header("Location:$url");
...
<tr>
	  ...
	  <td colspan="7" align="center"><strong>Instruction</strong></td>
	    ...
	    <?php 
                    $query = "select * from phar_sale where sno='$sno' and s_type='Transfer' and pmrn='$pmrn' and status='Pending'";
...
$query1 = "select * from medicine where mname='$medi' and status='Active'";
     ...
     <td align="center" colspan="1" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $count; ?></td>
	  ...
	  <td align="center"colspan="7" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["pdos"].'<br>'.$row["frelation"].'<br>'.$row["duration"]; ?></td>
						...
						<td align="center"colspan="1" <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>><?php echo $row["req_qty"]; ?></td>
...
<input name="eqty1_<?= $id ?>" type="text" value="0" id="eqty1" required <?php if($row1['tqty']>0){echo'style="font-weight: bold;font-size:22px;color:green"';} else{echo'style="font-weight: bold;font-size:22px;color:red"';}?>></td>
