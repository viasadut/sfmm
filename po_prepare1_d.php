...
if (isset($_POST['but_update'])) {
    ...
    try {
        ...
        $stmtSelect = mysqli_prepare($con, "SELECT * FROM purchase_stock3 WHERE id=? FOR UPDATE");
        if (!$stmtSelect) throw new Exception("Prepare select failed: " . mysqli_error($con));

        $stmtInsert = mysqli_prepare($con, "
            INSERT INTO po_table1
              (code, name, brand, stock, o_qty, uprice, tprice, po_ono, po_id, pid)
            VALUES
              (?,?,?,?,?,?,?,?,?,?)
        ");
        if (!$stmtInsert) throw new Exception("Prepare insert failed: " . mysqli_error($con));

        $stmtUpdateStock = mysqli_prepare($con, "
            UPDATE purchase_stock3
            SET po_status='Done', po_id=?, ono=?
            WHERE rfid=? AND req_loc=?
        ");
        if (!$stmtUpdateStock) throw new Exception("Prepare update purchase_stock3 failed: " . mysqli_error($con));

        $stmtUpdateGallery = mysqli_prepare($con, "
            UPDATE po_gallery
            SET rfid=?, pmrn=?, eid=?
            WHERE rfid=?
        ");
        if (!$stmtUpdateGallery) throw new Exception("Prepare update po_gallery failed: " . mysqli_error($con));

        $stmtUpdateComparison = mysqli_prepare($con, "
            UPDATE po_comparison
            SET rfid=?, pmrn=?, eid=?
            WHERE rfid=?
        ");
        if (!$stmtUpdateComparison) throw new Exception("Prepare update po_comparison failed: " . mysqli_error($con));

        // ---------- Insert selected rows into po_table1 ----------
        foreach ($_POST['update'] as $updateid) {

            $updateid = (int)$updateid;
            if ($updateid <= 0) continue;

            mysqli_stmt_bind_param($stmtSelect, "i", $updateid);
            mysqli_stmt_execute($stmtSelect);

            $resSel = mysqli_stmt_get_result($stmtSelect);
            $dd1 = mysqli_fetch_assoc($resSel);
            if (!$dd1) throw new Exception("Row not found in purchase_stock3 for id=$updateid");

            // Read values
            $code      = (string)$dd1['code'];
            $g_name    = (string)$dd1['g_name'];
            $b_name    = (string)$dd1['b_name'];
            $stock     = (float)$dd1['balance'];
            $order_qty = (float)$dd1['req_qty'];
            $u_price   = (float)$dd1['u_price'];
            $t_price   = (float)$dd1['t_price'];
            $p_id      = (string)$dd1['p_id'];
            ...
            if (($re_data['po_status'] ?? '') != 'Done') {
                ...
                mysqli_stmt_bind_param(
                    ...
                    $t_price,   // d
                    $po_ono,    // s
                    $po_id,     // s (or i if numeric)
                    $p_id    // i? depends on your DB - keeping as s below if not int
                 
                );

                // If your pid is integer, change the bind to ... "sssddddsiss" and cast $p_id = (int)$dd1['p_id'];
                // For now, to match above "sssddddssis", $p_id MUST be integer:
                // If pid is NOT integer, use this instead:
                // mysqli_stmt_bind_param($stmtInsert, "sssddddsssss", $code,$name,$brand,$stock,$order_qty,$u_price,$t_price,$po_ono,$po_id,$p_id,$b_name);

                mysqli_stmt_execute($stmtInsert);
            }
        }

        // ---------- Update purchase_stock3 for this rfid+req_loc ----------
        mysqli_stmt_bind_param($stmtUpdateStock, "isss", $po_id, $ono4, $rfid, $req_loc);
        mysqli_stmt_execute($stmtUpdateStock);

        // ---------- Update po_gallery ----------
        mysqli_stmt_bind_param($stmtUpdateGallery, "ssis", $rfid, $ono4, $po_id, $rfid);
        mysqli_stmt_execute($stmtUpdateGallery);

        // ---------- Update po_comparison ----------
        mysqli_stmt_bind_param($stmtUpdateComparison, "ssis", $rfid, $ono4, $po_id, $rfid);
        mysqli_stmt_execute($stmtUpdateComparison);

        mysqli_commit($con);

        // NOTE: Your redirect uses $encryption which is not set in your posted code.
        // Keeping your original URL style but using ono4 directly:
        header("Location: po_prepare1_d?ono=" . urlencode($ono4) . "&rfid=" . urlencode($rfid) . "&req_loc=" . urlencode($req_loc));
        exit;

    } catch (Exception $e) {
        mysqli_rollback($con);
        echo "Transaction Failed: " . h($e->getMessage());
        exit;
    }
}
