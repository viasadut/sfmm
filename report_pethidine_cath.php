...
$query = mysqli_query($db,"select * from cathmediused where id='$id'");
$data = mysqli_fetch_array($query);


$phar_name=$data['m_received'];
$patient_pmrn=$data['pmrn'];
$patient_eid=$data['ieid'];
$nurse_name=$data['cath_user'];
$charge_date=$data['adate'];

$query_pa = mysqli_query($db,"select * from cath_receive where pmrn='$patient_pmrn' and date1='$charge_date' order by id desc");
$data_patient = mysqli_fetch_array($query_pa);
//$mo_name=$data_patient['nanes'];
$page=$data_patient['page'];

$queryn = mysqli_query($db,"select * from patient where pmrn='$patient_pmrn'");
...
$pdf->Cell('5',5,$data_mo['psex'],1,0,'L');
$pdf->Cell('10',5,'AGE:',1,0,'L');
$pdf->Cell('28',5,$page,1,1,'L');
$pdf->Cell('30',5,'Admission Date:',1,0,'L');
$pdf->Cell('38',5,$data_patient['date1'],1,0,'L');
$pdf->Cell('13',5,'Date:',1,0,'L');
$pdf->Cell('38',5,date('d/m/Y H:i:s',strtotime($data['cath_time'])),1,0,'L');
$pdf->Cell('13',5,'Area:',1,0,'L');
$pdf->Cell('49',5,'Cathlab',1,1,'L');
...
$pdf->Cell('141', 5,$mo_name,1,1,'L');

$pdf->ln(3);

$pdf->Cell('120',5,'Item Name:',1,0,'L');
$pdf->Cell('61',5,'Quantity:',1,1,'L');
$pdf->Cell('120',5,$data['medi'],1,0,'L');

$pdf->Cell('61',5,$data['pdos'],1,1,'L');
