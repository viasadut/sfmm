...
//lis data
$query_URCA = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='URCA' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_URCA= mysqli_fetch_assoc($query_URCA);


$query_CRE2 = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='CRE2' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_CRE2= mysqli_fetch_assoc($query_CRE2);

$query_TP = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='TP' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_TP= mysqli_fetch_assoc($query_TP);

$query_CA = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='CA' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_CA= mysqli_fetch_assoc($query_CA);


$query_PHOS = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='PHOS' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
$data_PHOS= mysqli_fetch_assoc($query_PHOS);


$query_ALB = mysqli_query($db,"select * from lab_machine_response where LAB_CODE='$lis_code' and machine_ATTRIB='ALB' and MACHINE_CODE='D200' order by response_no_pk desc limit 1");
...
$rr='Uric Acid :'.$uacid."<br />".'Creatinine:'.$creatinine."<br />".'Urea:'.$urea."<br />".'Sodium:'.$sodium."<br />".'Potassium:'.$potassium."<br />".'Chloride:'.$chloride."<br />".'Total Protein:'.$tprotein."<br />".'Albumin:'.$albumin."<br />".'Albumin /Globulin(A:G) Ratio:'.$ah."<br />".'Serum Calcium :'.$scal."<br />".'Serum Inorganic Phosphate:'.$po4."<br />".'eGFR:'.$egfr;

$rr1='Uric Acid :'.$uacid.' '.'umol/L'."<br />".'Creatinine:'.$creatinine.' '.'mg/dL'."<br />".'Urea:'.$urea.' '.'mmol/L'."<br />".'Sodium:'.$sodium.' '.'mmol/L'."<br />".'Potassium:'.$potassium.' '.'mmol/L'."<br />".'Chloride:'.$chloride.' '.'mmol/L'."<br />".'Total Protein:'.$tprotein.' '.'g/L'."<br />".'Albumin:'.$albumin.' '.'g/L'."<br />".'Albumin /Globulin(A:G) Ratio:'.$ah.' '.''."<br />".'Serum Calcium :'.$scal.' '.'mmol/L'."<br />".'Serum Inorganic Phosphate:'.$po4.' '.'mmol/L'."<br />".'eGFR:'.$egfr.' '.'mL/min/1.7m2';
...
$ins_query1="insert into renal (`pname`,`pmrn`,`psex`,`page`,`uacid`,`creatinine`,`urea`,`sodium`,`potassium`,`chloride`,`tprotein`,`albumin`,`uby`,`udate`,`eid`,`iname`,`inid`,`sno`,`ah`,`scal`,`po4`,`egfr`) values 
('$pname','$pmrn','$psex','$page','$uacid','$creatinine','$urea','$sodium','$potassium','$chloride','$tprotein','$albumin','$user','$adate','$eid','$iname','$id','$sno','$ah','$scal','$po4','$egfr')";
...
<!-- Form Title -->
	 ...
	 <label for="age"><strong>Patient's Name :</strong></label>
      ...
      <input name="urea" type="text" size="70" style="text-transform:uppercase" value=""required>
	  ...
	  <label for="age"><strong>Albumin:</strong></label>
      <input name="albumin" type="text" size="70" style="text-transform:uppercase" value="<?php echo $data_ALB['machine_result'];?>"required>
	  
	  	  <label for="age"><strong>Albumin /Globulin(A:G) Ratio:</strong></label>
      <input name="ah" type="text" size="70" style="text-transform:uppercase" value=""required>
