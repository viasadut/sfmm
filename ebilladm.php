...
function callAPI1($url1, $data1, $token1 = null) {
    $ch1 = curl_init($url1);
    ...
    if ($response1 === false) {
        throw new Exception("Curl error: " . curl_error($ch1));
    }
    ...
    if ($httpCode1 !== 200) {
        throw new Exception("HTTP Error: $httpCode1 | Response: $response1");
    }
...
// ---------- SMS FUNCTION----------
function sendSMS($receiver1, $message1) {

    $API_BASE_URL1 = "https://api.mobireach.com.bd/";

    //Get Token
    $tokenResponse1 = callAPI1($API_BASE_URL1 . "auth/tokens", [
        "username" => "sfmc",
        "password" => "Ada@si@2022"
    ]);

    if (empty($tokenResponse1['token'])) {
        return [
            'success' => false,
            'error'   => 'Token not received'
        ];
    ...
    //Normalize receiver
    if (!is_array($receiver1)) {
        $receiver1 = [$receiver1];
    }

    //Send SMS
    return callAPI1(
        $API_BASE_URL1 . "sms/send",
        [
            "sender"       => "8801810008062",
            "receiver"     => $receiver1,
            "content"      => $message1,
