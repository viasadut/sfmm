...
<tr><td colspan="20" align="center"bgcolor="lightgreen"><label><strong>EDIT RECEIVED SAMPLE</strong></label></td> </tr>	
...
$dd=date('m/d/Y');
$count=1;
$sel_query="Select * from endolab where pmrn= '$pmrn' and eid='$eid' and date='$dd'and status IN ('RECEIVED','REJECTED') and type='lab'  order by `id` DESC;";
...
{ ?>    <tr>
     ...
     <td align="center"colspan="1"><?php echo $row["pmrn"]; ?></td>
	  ...
	  <td align="center"colspan="4"><?php echo $row["medi"]; ?></td>
			...
			<td align="center"colspan="2"<?php if($row['status']== "REJECTED"): ?> style="background-color:RED;"<?php else: ?> style="background-color:lightblue;" <?php endif ; ?>>
        <?php echo $row['status'];?></td>
        	  	  <td align="center"colspan="2"><?php echo $row["rcomments"]; ?></td>
	  
	  	  <td align="center"colspan="2"><?php echo $row["luser"]; ?></td>
		<td colspan="1" align="center"><a href="labedit2endo?pmrn=<?php echo "$row[pmrn]"; ?>&id=<?php echo "$row[id]"; ?>&eid=<?php echo "$row[eid]"; ?>"><strong>EDIT</strong></a></td>	  
